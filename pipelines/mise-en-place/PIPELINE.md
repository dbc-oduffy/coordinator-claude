...
**Default execution vehicle: ONE background Workflow for the whole run.** It carries the Phase-2e cross-plan DAG across every wave — executors, verifiers, and the per-wave commit phase alike — not one Workflow per wave and never a hand-typed sequence of dispatch calls. Every `agent()` call on an UNPINNED `subagent_type` passes `model: 'sonnet'` explicitly — a `coordinator:*` agent pins its own tier in frontmatter and the engine-plane pin guard refuses any `model:` passed alongside it, so omit the parameter there rather than overriding it (`docs/wiki/coordinator-tripwires/a-model-pin-is-a-vehicle-choice-not-a-parameter.md`); width ceiling ≤5 write-capable executors per barrier; a `/handoff` may still be elected at a wave boundary.
...
Freeze the run's diff before routing, so the capping ceremony inherits a materialized artifact instead of re-deriving a range across a shared branch: `<claude-klabauter-bin>/freeze-review-diff.py --range "<start_sha>..HEAD" --slice-id "mise-<run-id>"` (resolve `<claude-klabauter-bin>` via the `resolve-claude-klabauter-bin` snippet). Take `<start_sha>` from the inventory record's frontmatter (§ Phase 1). It writes TWO files per
slice into `state/review-trail/diffs/` — `<slice-id>.diff` and `<slice-id>.head.sha` — and that
tree is gitignored by design, so absence from the commit is expected and is not a missing freeze.
Say so in the run record rather than leaving it inferred. Tripwire:
...
**Baton disposition (whenever Phase 0a ran):** batons advance automatically. A baton whose governing plan is stamped `implemented`, or whose workstream concludes terminally-positive, is flipped terminal by the engine — there is no manual flip step, and no verb to run. A baton still advertising live work after either event is a **defect**: report it rather than hand-correcting it. Anything unstarted → `pickup-assemble drop <path>`. Anything genuinely mid-stream → a successor via `/handoff`. Archiving is the boot sweep's job. State each disposition in the tail summary.
...
This is a ratified engine property, not a recipe convenience — `resolve_lineage` returns a single `output_path`, so N→1 fan-in is the only cardinality it can express, and fan-out was considered and **retired** by engine ruling (the engine repo, `state/roadmap/sedge-2026-08-06/COORDINATOR-RESOLUTIONS.md` § Resolution 1). The reasoning binds the sentence: the baton is the unit of **work-continuity**, one per session, and deliverable close-out lives in an off-artifact ledger rather than on the succession edge. Deliverable survival is therefore guaranteed by that ledger, never by successor multiplicity — which is why one successor is correct rather than merely tolerable.
