...
headings and content). A nugget must NOT be classified `[ALREADY_CAPTURED]` on the basis of
a `~/.claude/**/memory/*.md` (or any `~/.claude` path) capture pointer — that is machine-local,
RAG-invisible, and NOT durable capture; "already captured" here means present in an IN-REPO
home (`docs/decisions/`, `docs/wiki/`, `state/cross-repo-commitments/`, or a canonical
plan/spec). If a nugget's only capture is a memory pointer, treat it as NOT captured — carry
it downstream, do not classify `[ALREADY_CAPTURED]` (tripwire: `MEMORY-NOT-DURABLE-CAPTURE`).
Mark individual files in single-file form, or group them:
...
**Archived handoffs** (`archive/handoffs/*.md`): **never an input.** Handoffs are not a knowledge source and carry no distill fate. If a handoff path reaches your batch, that is a batching error — report it and scan nothing from it.
