...
description: "Archive superseded handoffs (chain-aware) and PM-directed handoffs from state/handoffs/ to archive/handoffs/. This pipeline's Step 2 is the lineage-predecessor residual backstop — it archives a predecessor only when an active successor still references it (predecessor: frontmatter or Continuing from), never a general terminal sweep. Consumed-handoff archival at large is owned by /workday-complete's reap-orphaned-in-flight-handoffs + handoff-housekeeping pair (DR-384), which drains the whole corpus every ceremony run, and bin/sweep-terminal-handoffs.py on demand (/workday-start Step 1.47); the boot sweep that used to fire this class was killed and sweep-boot.py now dispatches no op. /workstream-complete Step 2.7 and /handoff chain-archival (fallback-only) also contribute. Deployment-axis archival (deployment_state: shipped) is handled by bin/sweep-shipped-handoffs.py invoked from /workday-start Step 1.47."
version: 3.2.0
---
...
- **`/workday-complete`'s reap-orphaned-in-flight-handoffs + handoff-housekeeping pair** (DR-384): drains the whole terminal-handoff corpus every ceremony run, including a `consumed` predecessor once it has a live succession child — named via `predecessor:` / `additional_predecessors:` on a live handoff. `forked_from` children are exempt: a spinoff is a cadet branch and does not retire its origin. This is now the primary archiver for the common case; this pipeline's Step 2 covers only the residue it doesn't reach. The boot sweep that used to fire this class (`fleet.archive_completed_handoffs` via `session.boot_sweep`) was killed; `sweep-boot.py` now carries `never dispatches an op` as a negative spec, pinned by `test_sweep_boot.py`.
- **`/handoff` chain-archival — fallback only** (`skills/handoff/SKILL.md` Step 1, chain-archival paragraph): `/handoff` presumes the ceremony-run drain wins and does not eagerly archive by default; the guarded `housekeeping.cycle --exclude` native call (direct `cc_invoke.route_mutation`, formerly `coordinator-handoff-archive.sh --exclude`) runs only on an install whose engine seam is absent, where the ceremony-run drain has no native route.
