...
- **`/handoff` chain-archival — fallback only** (`skills/handoff/SKILL.md` Step 1, chain-archival paragraph): `/handoff` presumes the boot sweep wins and does not eagerly archive by default; the guarded `housekeeping.cycle --exclude` native call (direct `cc_invoke.route_mutation`, formerly `coordinator-handoff-archive.sh --exclude`) runs only on an install whose engine seam is absent, where the boot sweep has no native route.
