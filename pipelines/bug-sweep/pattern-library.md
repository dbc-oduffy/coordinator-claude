...
Detail companion to `coordinator/skills/bug-sweep/SKILL.md`. Pattern catalog, cost profile, and failure-mode matrix.
...
| Pattern library misses project-specific bugs | Phase 0 reads `state/lessons/` for known gotchas. |
| Executor fix uses hallucinated API | Phase 3.5 docs-checker catches it before commit; reverted finding goes to backlog. |
| example-game-repo-docs server unavailable during Track C | Docs-checker marks claims UNVERIFIED, sweep continues; report notes degraded API verification. |
