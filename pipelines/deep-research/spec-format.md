...
# ── Output Schema ─────────────────────────────────
output_schema:
  reference: <path to full schema definition file, if external — resolved relative to the spec file's own directory, not cwd or repo root, so a spec is portable across POSIX and Windows checkouts without an anchor assumption>
  key_fields:
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `reference` | string | No | Path to external full schema definition, resolved relative to the spec file's own directory (portable across POSIX/Windows; not cwd- or repo-root-relative). When present, the driver validates synthesizer output against it (see `structured-driver.md` Step 6) in addition to `key_fields`. |
