...
7. Message the sweep: SendMessage(to: "[SWEEP_NAME]", message: "DONE: [TOPIC_LETTER] findings written to [SCRATCH_DIR]/[TOPIC_LETTER]-claims.json and [TOPIC_LETTER]-summary.md") — the sweep merges your claims, and the EM emits the merged result to the durable docs/research/{run-stem}.claims.json
...
complete and **before teardown** (auto, on session exit). You will NOT receive a relay request from a Team-2
