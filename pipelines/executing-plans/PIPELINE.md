...
**Core principle:** Batch execution with checkpoints for architect review. If running in a Coordinator session, prefer executor-driven dispatch (`/delegate-execution`) for better review integration.
...
`TaskCreate` absent from this session's surface (`ToolSearch("select:TaskCreate")` returns nothing) → fall back to `coordinator-tasks-mirror` for the same flight-recorder role; do not assume either state without checking. When Task* is unavailable, dispatch the phases in order, waiting on each completion notification — that is the ordering a `blockedBy` chain would otherwise express.
