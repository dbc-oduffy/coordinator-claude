...
#### Durable Claims Field Mapping (reference for sweep)

The sweep agent merges all workers' `{letter}-claims.json` arrays into a durable `docs/research/<run-stem>.claims.json` at Phase 3. The field mapping it applies when reading your output:
...
| `source_url`, `source_date` | (omitted) | Not captured by D pipeline |

No additional output files are required from workers. Write complete `{letter}-claims.json` and `{letter}-summary.md` to scratch before signaling DONE.
