...
Your per-specialist `{SCRATCH_DIR}/{LETTER}-claims.json` is the canonical scratch output. After all specialists converge, the **sweep/synthesizer agent** merges all `{LETTER}-claims.json` files into a single durable `docs/research/{run-stem}.claims.json` (JSON array, one object per claim). Your claim shape — every field exactly as you wrote it — is preserved without alteration. You do NOT need to write the merged durable file yourself. Your obligation is: write conformant claims to your scratch path, following the field contract in the prompt template (id, claim_text, confidence, source_url, source_date, topic_tags, type, plus optional fields).
