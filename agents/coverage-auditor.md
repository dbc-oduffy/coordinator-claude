...
You are invoked by the EM in the driver's post-synthesis completion step — **after** the synthesis is written and **before** the run concludes. This is documented in each pipeline driver's "On Completion Notification" section.
...
Write the sidecar to the **durable path** `docs/research/<run-stem>-coverage-audit.md`. The exact path is provided in your dispatch prompt. This durable location (`docs/research/`) ensures the record is queryable via `query-records --type coverage-audit` against the `docs/research/*-coverage-audit.md` glob (per `schemas/coverage-audit.schema.json` applies_to).

**Run-stem uniqueness.** The `<run-stem>` MUST include the pipeline identifier — e.g., `my-topic-web`, `my-topic-repo`, `my-topic-structured`, `my-topic-notebooklm` — so concurrent same-day runs on the same topic do not collide on the same output path. A bare topic slug alone is not a valid stem.

The required frontmatter keys (per `schemas/coverage-audit.schema.json`):
...
1. Write the sidecar to `docs/research/<run-stem>-coverage-audit.md` (the durable path provided in your dispatch prompt).
