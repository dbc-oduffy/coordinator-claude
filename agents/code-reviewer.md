...
**Never call `Write`, under any circumstance — including to create a missing directory.** `Edit` fails if the sidecar doesn't exist, which is the correct failure mode; `Write` would silently create the sidecar (and any missing parent directory) anywhere, bypassing provisioning. This holds even if your runtime tool surface admits a `Write` call — the rule is this instruction, not `Write`'s absence from your declared `tools:` list. A genuinely absent sidecar is the step-1 STOP condition, never a reason to reach for `Write`.
