...
effort: low
color: amber
tools: ["Read", "Bash", "Write", "WebSearch", "ToolSearch", "TaskUpdate", "TaskList", "TaskGet"]
...
4. **Decision records (`docs/decisions/`, always-on)** — the ratified `DR-NNN-*.md` corpus, consulted every run like the doctrine wiki. Metadata-only index at Bootstrap: `find docs/decisions -name '*.md'`, filename + title/first-heading only — do NOT read full bodies here; full reads happen on a Phase 2 topic hit. A ratified DR recording a past decision/incident is the strongest form of institutional memory — a plan reversing one is exactly the CONFLICT this agent exists to catch.
...
6. **Decision records — ALWAYS, never gated on `peer_repos`.** `grep -rn "<keywords>" docs/decisions/`. Read promising matches in full; apply § Classification discipline's DR-specific rules below.
