...
#   1. Nested working-repo layout (scripts live under plugins/deep-research/)
...
# "chain step 4 of 5" banner appears in stdout.
#
# Tests are intentionally RED until Wave 2 lands scripts/setup.{sh,ps1}.
# "chain step 4 of 5" is the exact banner string asserted; C2 must produce that output.
#
...
# ---------------------------------------------------------------------------
# Locate this file and the DR plugin root
# ---------------------------------------------------------------------------
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
DR_ROOT="$(cd "${SCRIPT_DIR}/.." && pwd)"

# Working-repo root (where this meta-repo lives) — used for nested-layout run
WR_DR_SCRIPTS="${DR_ROOT}/scripts/setup.sh"

BANNER_NEEDLE="chain step 4 of 5"
PASS=0
FAIL=0
ERRORS=()
...
_assert_banner() {
    ...
    if output=$("$@" 2>/dev/null); then
        if echo "${output}" | grep -qF "${BANNER_NEEDLE}"; then
            ...
            _fail "${label}: stdout did not contain '${BANNER_NEEDLE}'. Output: ${output}"
        fi
    else
        local exit_code=$?
        _fail "${label}: command exited ${exit_code}. (setup.sh may not exist yet — tests are RED until Wave 2)"
    fi
}
...
#
# Invoke scripts/setup.sh --check from the DR plugin root.
# The --check flag is the read-only extension declared in C2; it must:
#   - exit 0
#   - emit the "chain step 4 of 5" banner to stdout
# ---------------------------------------------------------------------------
echo "=== Test 1: Nested working-repo layout ==="
if [[ ! -f "${WR_DR_SCRIPTS}" ]]; then
    _fail "Nested layout: ${WR_DR_SCRIPTS} does not exist. C2 must land first."
else
    (
        cd "${DR_ROOT}"
...
# publish repo (X:\deep-research-claude\).
#
...
# no plugins/deep-research/ prefix.
# ---------------------------------------------------------------------------
echo ""
...
for subtree in docs scripts skills; do
    if [[ -d "${DR_ROOT}/${subtree}" ]]; then
        cp -r "${DR_ROOT}/${subtree}" "${FLAT_LAYOUT}/${subtree}"
    else
        _fail "Flat layout setup: ${DR_ROOT}/${subtree} does not exist. C1-C4 must land first."
        COPY_FAILED=1
    fi
done
...
# constraint from plan §7 C2 and the Staff Engineer Finding 7.
# ---------------------------------------------------------------------------
echo ""
echo "=== Test 3: PowerShell mirror (conditional on pwsh availability) ==="

WR_DR_PS1="${DR_ROOT}/scripts/setup.ps1"
...
if command -v pwsh >/dev/null 2>&1; then
    ...
    # Nested layout
    if [[ ! -f "${WR_DR_PS1}" ]]; then
        _fail "Nested layout ps1: ${WR_DR_PS1} does not exist. C2 must land first."
    else
        (
            cd "${DR_ROOT}"
