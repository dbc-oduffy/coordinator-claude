@echo off
setlocal
REM Windows launcher for mise-prep-gate.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "mise-prep-gate.ps1" sibling of
REM     this file over this "mise-prep-gate.cmd" file itself -- if both exist, `mise-prep-gate`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "mise-prep-gate.py"
...
echo [mise-prep-gate] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [mise-prep-gate] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0mise-prep-gate.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0mise-prep-gate.py" %*
exit /b %ERRORLEVEL%
