# session-reachability-cli.ps1 — python-direct Windows launcher for session-reachability-cli.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "session-reachability-cli.ps1" file over
#     any co-located "session-reachability-cli.cmd" sibling -- `session-reachability-cli` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "session-reachability-cli.py"
...
$_entry = Join-Path $_here 'session-reachability-cli.py'
...
[Console]::Error.WriteLine('[session-reachability-cli] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[session-reachability-cli] Install Python: https://www.python.org/downloads/windows/')
exit 127
