@echo off
setlocal
REM Windows launcher for learn-lessons-roots.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "learn-lessons-roots.ps1" sibling of
REM     this file over this "learn-lessons-roots.cmd" file itself -- if both exist, `learn-lessons-roots`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "learn-lessons-roots.py"
...
echo [learn-lessons-roots] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [learn-lessons-roots] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0learn-lessons-roots.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0learn-lessons-roots.py" %*
exit /b %ERRORLEVEL%
