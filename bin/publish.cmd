@echo off
setlocal
REM Windows launcher for publish.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "publish.ps1" sibling of
REM     this file over this "publish.cmd" file itself -- if both exist, `publish`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "publish.py"
...
echo [publish] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [publish] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
