# group-em-watch-cli.ps1 — python-direct Windows launcher for group-em-watch-cli.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "group-em-watch-cli.ps1" file over
#     any co-located "group-em-watch-cli.cmd" sibling -- `group-em-watch-cli` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "group-em-watch-cli.py"
...
$_entry = Join-Path $_here 'group-em-watch-cli.py'
...
[Console]::Error.WriteLine('[group-em-watch-cli] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[group-em-watch-cli] Install Python: https://www.python.org/downloads/windows/')
exit 127
