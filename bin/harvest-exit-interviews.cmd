@echo off
setlocal
REM Windows launcher for harvest-exit-interviews.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "harvest-exit-interviews.ps1" sibling of
REM     this file over this "harvest-exit-interviews.cmd" file itself -- if both exist, `harvest-exit-interviews`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "harvest-exit-interviews.py"
...
echo [harvest-exit-interviews] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [harvest-exit-interviews] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0harvest-exit-interviews.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0harvest-exit-interviews.py" %*
exit /b %ERRORLEVEL%
