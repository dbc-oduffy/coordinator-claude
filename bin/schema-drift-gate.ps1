# schema-drift-gate.ps1 — python-direct Windows launcher for schema-drift-gate.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "schema-drift-gate.ps1" file over
#     any co-located "schema-drift-gate.cmd" sibling -- `schema-drift-gate` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "schema-drift-gate.py"
...
$_entry = Join-Path $_here 'schema-drift-gate.py'
...
[Console]::Error.WriteLine('[schema-drift-gate] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[schema-drift-gate] Install Python: https://www.python.org/downloads/windows/')
exit 127
