@echo off
setlocal
REM Windows launcher for record-platform-outcome.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "record-platform-outcome.ps1" sibling of
REM     this file over this "record-platform-outcome.cmd" file itself -- if both exist, `record-platform-outcome`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "record-platform-outcome.py"
...
echo [record-platform-outcome] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [record-platform-outcome] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0record-platform-outcome.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0record-platform-outcome.py" %*
exit /b %ERRORLEVEL%
