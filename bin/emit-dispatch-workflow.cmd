@echo off
setlocal
REM Windows launcher for emit-dispatch-workflow.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "emit-dispatch-workflow.ps1" sibling of
REM     this file over this "emit-dispatch-workflow.cmd" file itself -- if both exist, `emit-dispatch-workflow`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "emit-dispatch-workflow.py"
...
echo [emit-dispatch-workflow] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [emit-dispatch-workflow] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0emit-dispatch-workflow.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0emit-dispatch-workflow.py" %*
exit /b %ERRORLEVEL%
