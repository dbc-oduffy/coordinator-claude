@echo off
setlocal
REM Windows launcher for session-liveness-cli.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "session-liveness-cli.ps1" sibling of
REM     this file over this "session-liveness-cli.cmd" file itself -- if both exist, `session-liveness-cli`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "session-liveness-cli.py"
...
echo [session-liveness-cli] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [session-liveness-cli] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0session-liveness-cli.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0session-liveness-cli.py" %*
exit /b %ERRORLEVEL%
