@echo off
setlocal
REM Windows launcher for execute-plan-assemble.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "execute-plan-assemble.ps1" sibling of
REM     this file over this "execute-plan-assemble.cmd" file itself -- if both exist, `execute-plan-assemble`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "execute-plan-assemble.py"
...
echo [execute-plan-assemble] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [execute-plan-assemble] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0execute-plan-assemble.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0execute-plan-assemble.py" %*
exit /b %ERRORLEVEL%
