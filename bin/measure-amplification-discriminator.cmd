@echo off
setlocal
REM Windows launcher for measure-amplification-discriminator.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "measure-amplification-discriminator.ps1" sibling of
REM     this file over this "measure-amplification-discriminator.cmd" file itself -- if both exist, `measure-amplification-discriminator`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "measure-amplification-discriminator.py"
...
echo [measure-amplification-discriminator] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [measure-amplification-discriminator] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0measure-amplification-discriminator.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0measure-amplification-discriminator.py" %*
exit /b %ERRORLEVEL%
