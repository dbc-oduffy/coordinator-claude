@echo off
setlocal
REM Windows launcher for migrate-improvement-queue-project.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "migrate-improvement-queue-project.ps1" sibling of
REM     this file over this "migrate-improvement-queue-project.cmd" file itself -- if both exist, `migrate-improvement-queue-project`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "migrate-improvement-queue-project.py"
...
echo [migrate-improvement-queue-project] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [migrate-improvement-queue-project] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127

:run_baked
"%_py%" "%~dp0migrate-improvement-queue-project.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0migrate-improvement-queue-project.py" %*
exit /b %ERRORLEVEL%
