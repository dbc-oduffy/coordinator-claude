# coordinator-safe-commit.ps1 — python-direct Windows launcher for coordinator-safe-commit.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "coordinator-safe-commit.ps1" file over
#     any co-located "coordinator-safe-commit.cmd" sibling -- `coordinator-safe-commit` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "coordinator-safe-commit.py"
...
$_entry = Join-Path $_here 'coordinator-safe-commit.py'
...
[Console]::Error.WriteLine('[coordinator-safe-commit] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[coordinator-safe-commit] Install Python: https://www.python.org/downloads/windows/')
exit 127
