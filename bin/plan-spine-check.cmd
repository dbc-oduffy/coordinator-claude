@echo off
setlocal
REM Windows launcher for plan-spine-check.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "plan-spine-check.ps1" sibling of
REM     this file over this "plan-spine-check.cmd" file itself -- if both exist, `plan-spine-check`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "plan-spine-check.py"
...
echo [plan-spine-check] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [plan-spine-check] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0plan-spine-check.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0plan-spine-check.py" %*
exit /b %ERRORLEVEL%
