@echo off
setlocal
REM Windows launcher for emit-artifact-shape-contract.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "emit-artifact-shape-contract.ps1" sibling of
REM     this file over this "emit-artifact-shape-contract.cmd" file itself -- if both exist, `emit-artifact-shape-contract`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "emit-artifact-shape-contract.py"
...
echo [emit-artifact-shape-contract] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [emit-artifact-shape-contract] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0emit-artifact-shape-contract.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0emit-artifact-shape-contract.py" %*
exit /b %ERRORLEVEL%
