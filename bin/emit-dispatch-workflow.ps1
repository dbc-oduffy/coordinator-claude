# emit-dispatch-workflow.ps1 — python-direct Windows launcher for emit-dispatch-workflow.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "emit-dispatch-workflow.ps1" file over
#     any co-located "emit-dispatch-workflow.cmd" sibling -- `emit-dispatch-workflow` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "emit-dispatch-workflow.py"
...
$_entry = Join-Path $_here 'emit-dispatch-workflow.py'
...
[Console]::Error.WriteLine('[emit-dispatch-workflow] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[emit-dispatch-workflow] Install Python: https://www.python.org/downloads/windows/')
exit 127
