# check-anchor-freshness.ps1 — python-direct Windows launcher for check-anchor-freshness.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "check-anchor-freshness.ps1" file over
#     any co-located "check-anchor-freshness.cmd" sibling -- `check-anchor-freshness` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "check-anchor-freshness.py"
...
$_entry = Join-Path $_here 'check-anchor-freshness.py'
...
[Console]::Error.WriteLine('[check-anchor-freshness] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[check-anchor-freshness] Install Python: https://www.python.org/downloads/windows/')
exit 127
