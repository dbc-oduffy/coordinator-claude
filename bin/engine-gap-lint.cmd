@echo off
setlocal
REM Windows launcher for engine-gap-lint.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "engine-gap-lint.ps1" sibling of
REM     this file over this "engine-gap-lint.cmd" file itself -- if both exist, `engine-gap-lint`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "engine-gap-lint.py"
...
echo [engine-gap-lint] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [engine-gap-lint] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0engine-gap-lint.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0engine-gap-lint.py" %*
exit /b %ERRORLEVEL%
