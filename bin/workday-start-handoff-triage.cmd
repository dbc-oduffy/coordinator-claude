@echo off
setlocal
REM Windows launcher for workday-start-handoff-triage.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "workday-start-handoff-triage.ps1" sibling of
REM     this file over this "workday-start-handoff-triage.cmd" file itself -- if both exist, `workday-start-handoff-triage`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "workday-start-handoff-triage.py"
...
echo [workday-start-handoff-triage] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [workday-start-handoff-triage] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0workday-start-handoff-triage.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0workday-start-handoff-triage.py" %*
exit /b %ERRORLEVEL%
