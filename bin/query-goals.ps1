# query-goals.ps1 — python-direct Windows launcher for query-goals.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "query-goals.ps1" file over
#     any co-located "query-goals.cmd" sibling -- `query-goals` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "query-goals.py"
...
$_entry = Join-Path $_here 'query-goals.py'
...
[Console]::Error.WriteLine('[query-goals] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[query-goals] Install Python: https://www.python.org/downloads/windows/')
exit 127
