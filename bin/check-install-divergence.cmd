@echo off
setlocal
REM Windows launcher for check-install-divergence.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "check-install-divergence.ps1" sibling of
REM     this file over this "check-install-divergence.cmd" file itself -- if both exist, `check-install-divergence`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "check-install-divergence.py"
...
echo [check-install-divergence] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [check-install-divergence] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0check-install-divergence.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0check-install-divergence.py" %*
exit /b %ERRORLEVEL%
