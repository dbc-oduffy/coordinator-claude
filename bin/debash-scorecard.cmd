@echo off
setlocal
REM Windows launcher for debash-scorecard.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "debash-scorecard.ps1" sibling of
REM     this file over this "debash-scorecard.cmd" file itself -- if both exist, `debash-scorecard`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "debash-scorecard.py"
...
echo [debash-scorecard] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [debash-scorecard] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0debash-scorecard.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0debash-scorecard.py" %*
exit /b %ERRORLEVEL%
