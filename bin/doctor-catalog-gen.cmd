@echo off
setlocal
REM Windows launcher for doctor-catalog-gen.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "doctor-catalog-gen.ps1" sibling of
REM     this file over this "doctor-catalog-gen.cmd" file itself -- if both exist, `doctor-catalog-gen`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "doctor-catalog-gen.py"
...
echo [doctor-catalog-gen] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [doctor-catalog-gen] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0doctor-catalog-gen.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0doctor-catalog-gen.py" %*
exit /b %ERRORLEVEL%
