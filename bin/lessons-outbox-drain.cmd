@echo off
setlocal
REM Windows launcher for lessons-outbox-drain.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "lessons-outbox-drain.ps1" sibling of
REM     this file over this "lessons-outbox-drain.cmd" file itself -- if both exist, `lessons-outbox-drain`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "lessons-outbox-drain.py"
...
echo [lessons-outbox-drain] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [lessons-outbox-drain] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0lessons-outbox-drain.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0lessons-outbox-drain.py" %*
exit /b %ERRORLEVEL%
