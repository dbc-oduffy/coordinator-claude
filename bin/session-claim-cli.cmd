@echo off
setlocal
REM Windows launcher for session-claim-cli.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "session-claim-cli.ps1" sibling of
REM     this file over this "session-claim-cli.cmd" file itself -- if both exist, `session-claim-cli`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "session-claim-cli.py"
...
echo [session-claim-cli] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [session-claim-cli] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0session-claim-cli.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0session-claim-cli.py" %*
exit /b %ERRORLEVEL%
