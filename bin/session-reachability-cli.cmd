@echo off
setlocal
REM Windows launcher for session-reachability-cli.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "session-reachability-cli.ps1" sibling of
REM     this file over this "session-reachability-cli.cmd" file itself -- if both exist, `session-reachability-cli`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "session-reachability-cli.py"
...
echo [session-reachability-cli] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [session-reachability-cli] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0session-reachability-cli.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0session-reachability-cli.py" %*
exit /b %ERRORLEVEL%
