@echo off
setlocal
REM Windows launcher for derive-session-hierarchy.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "derive-session-hierarchy.ps1" sibling of
REM     this file over this "derive-session-hierarchy.cmd" file itself -- if both exist, `derive-session-hierarchy`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "derive-session-hierarchy.py"
...
echo [derive-session-hierarchy] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [derive-session-hierarchy] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0derive-session-hierarchy.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0derive-session-hierarchy.py" %*
exit /b %ERRORLEVEL%
