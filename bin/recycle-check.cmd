@echo off
setlocal
REM Windows launcher for recycle-check.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "recycle-check.ps1" sibling of
REM     this file over this "recycle-check.cmd" file itself -- if both exist, `recycle-check`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "recycle-check.py"
...
echo [recycle-check] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [recycle-check] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0recycle-check.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0recycle-check.py" %*
exit /b %ERRORLEVEL%
