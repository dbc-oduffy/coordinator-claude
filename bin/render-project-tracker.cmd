@echo off
setlocal
REM Windows launcher for render-project-tracker.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "render-project-tracker.ps1" sibling of
REM     this file over this "render-project-tracker.cmd" file itself -- if both exist, `render-project-tracker`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "render-project-tracker.py"
...
echo [render-project-tracker] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [render-project-tracker] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127

:run_baked
"%_py%" "%~dp0render-project-tracker.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0render-project-tracker.py" %*
exit /b %ERRORLEVEL%
