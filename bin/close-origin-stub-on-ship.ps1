# close-origin-stub-on-ship.ps1 — python-direct Windows launcher for close-origin-stub-on-ship.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "close-origin-stub-on-ship.ps1" file over
#     any co-located "close-origin-stub-on-ship.cmd" sibling -- `close-origin-stub-on-ship` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "close-origin-stub-on-ship.py"
...
$_entry = Join-Path $_here 'close-origin-stub-on-ship.py'
...
[Console]::Error.WriteLine('[close-origin-stub-on-ship] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[close-origin-stub-on-ship] Install Python: https://www.python.org/downloads/windows/')
exit 127
