@echo off
setlocal
REM Windows launcher for gen-claude-doe-launcher.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "gen-claude-doe-launcher.ps1" sibling of
REM     this file over this "gen-claude-doe-launcher.cmd" file itself -- if both exist, `gen-claude-doe-launcher`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "gen-claude-doe-launcher.py"
...
echo [gen-claude-doe-launcher] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [gen-claude-doe-launcher] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0gen-claude-doe-launcher.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0gen-claude-doe-launcher.py" %*
exit /b %ERRORLEVEL%
