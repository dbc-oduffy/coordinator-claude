# verify-snippet-sync.ps1 — python-direct Windows launcher for verify-snippet-sync.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "verify-snippet-sync.ps1" file over
#     any co-located "verify-snippet-sync.cmd" sibling -- `verify-snippet-sync` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "verify-snippet-sync.py"
...
$_entry = Join-Path $_here 'verify-snippet-sync.py'
...
[Console]::Error.WriteLine('[verify-snippet-sync] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[verify-snippet-sync] Install Python: https://www.python.org/downloads/windows/')
exit 127
