@echo off
setlocal
REM Windows launcher for waste-signal.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "waste-signal.ps1" sibling of
REM     this file over this "waste-signal.cmd" file itself -- if both exist, `waste-signal`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "waste-signal.py"
...
echo [waste-signal] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [waste-signal] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0waste-signal.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0waste-signal.py" %*
exit /b %ERRORLEVEL%
