# check-no-monolith-completion-append.ps1 — python-direct Windows launcher for check-no-monolith-completion-append.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "check-no-monolith-completion-append.ps1" file over
#     any co-located "check-no-monolith-completion-append.cmd" sibling -- `check-no-monolith-completion-append` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "check-no-monolith-completion-append.py"
...
$_entry = Join-Path $_here 'check-no-monolith-completion-append.py'
...
[Console]::Error.WriteLine('[check-no-monolith-completion-append] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[check-no-monolith-completion-append] Install Python: https://www.python.org/downloads/windows/')
exit 127
