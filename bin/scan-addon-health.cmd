@echo off
setlocal
REM Windows launcher for scan-addon-health.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "scan-addon-health.ps1" sibling of
REM     this file over this "scan-addon-health.cmd" file itself -- if both exist, `scan-addon-health`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "scan-addon-health.py"
...
echo [scan-addon-health] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [scan-addon-health] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0scan-addon-health.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0scan-addon-health.py" %*
exit /b %ERRORLEVEL%
