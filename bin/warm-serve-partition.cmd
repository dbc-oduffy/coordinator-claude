@echo off
setlocal
REM Windows launcher for warm-serve-partition.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "warm-serve-partition.ps1" sibling of
REM     this file over this "warm-serve-partition.cmd" file itself -- if both exist, `warm-serve-partition`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "warm-serve-partition.py"
...
echo [warm-serve-partition] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [warm-serve-partition] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0warm-serve-partition.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0warm-serve-partition.py" %*
exit /b %ERRORLEVEL%
