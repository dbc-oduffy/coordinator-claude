@echo off
setlocal
REM Windows launcher for ensure-vscode-readonly.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "ensure-vscode-readonly.ps1" sibling of
REM     this file over this "ensure-vscode-readonly.cmd" file itself -- if both exist, `ensure-vscode-readonly`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "ensure-vscode-readonly.py"
...
echo [ensure-vscode-readonly] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [ensure-vscode-readonly] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0ensure-vscode-readonly.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0ensure-vscode-readonly.py" %*
exit /b %ERRORLEVEL%
