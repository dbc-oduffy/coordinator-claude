@echo off
setlocal
REM Windows launcher for migrate-central-improvement-queue.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "migrate-central-improvement-queue.ps1" sibling of
REM     this file over this "migrate-central-improvement-queue.cmd" file itself -- if both exist, `migrate-central-improvement-queue`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "migrate-central-improvement-queue.py"
...
echo [migrate-central-improvement-queue] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [migrate-central-improvement-queue] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127

:run_baked
"%_py%" "%~dp0migrate-central-improvement-queue.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0migrate-central-improvement-queue.py" %*
exit /b %ERRORLEVEL%
