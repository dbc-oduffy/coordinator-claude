# record-platform-outcome.ps1 — python-direct Windows launcher for record-platform-outcome.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "record-platform-outcome.ps1" file over
#     any co-located "record-platform-outcome.cmd" sibling -- `record-platform-outcome` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "record-platform-outcome.py"
...
$_entry = Join-Path $_here 'record-platform-outcome.py'
...
[Console]::Error.WriteLine('[record-platform-outcome] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[record-platform-outcome] Install Python: https://www.python.org/downloads/windows/')
exit 127
