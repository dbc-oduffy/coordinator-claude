@echo off
setlocal
REM Windows launcher for check-launch-shape.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "check-launch-shape.ps1" sibling of
REM     this file over this "check-launch-shape.cmd" file itself -- if both exist, `check-launch-shape`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "check-launch-shape.py"
...
echo [check-launch-shape] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [check-launch-shape] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0check-launch-shape.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0check-launch-shape.py" %*
exit /b %ERRORLEVEL%
