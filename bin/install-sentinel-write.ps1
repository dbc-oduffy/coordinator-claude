# install-sentinel-write.ps1 — python-direct Windows launcher for install-sentinel-write.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "install-sentinel-write.ps1" file over
#     any co-located "install-sentinel-write.cmd" sibling -- `install-sentinel-write` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "install-sentinel-write.py"
...
$_entry = Join-Path $_here 'install-sentinel-write.py'
...
[Console]::Error.WriteLine('[install-sentinel-write] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[install-sentinel-write] Install Python: https://www.python.org/downloads/windows/')
exit 127
