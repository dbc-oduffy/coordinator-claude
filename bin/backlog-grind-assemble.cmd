@echo off
setlocal
REM Windows launcher for backlog-grind-assemble.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "backlog-grind-assemble.ps1" sibling of
REM     this file over this "backlog-grind-assemble.cmd" file itself -- if both exist, `backlog-grind-assemble`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "backlog-grind-assemble.py"
...
echo [backlog-grind-assemble] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [backlog-grind-assemble] Install Python: https://www.python.org/downloads/windows/ 1>&2
...
:run_baked
"%_py%" "%~dp0backlog-grind-assemble.py" %*
2>nul rd /s /q "%_LAUNCHER_RAW_CMDLINE_DIR%"
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0backlog-grind-assemble.py" %*
2>nul rd /s /q "%_LAUNCHER_RAW_CMDLINE_DIR%"
exit /b %ERRORLEVEL%
