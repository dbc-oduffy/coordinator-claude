@echo off
setlocal
REM Windows launcher for check-forwarder-drift.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "check-forwarder-drift.ps1" sibling of
REM     this file over this "check-forwarder-drift.cmd" file itself -- if both exist, `check-forwarder-drift`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "check-forwarder-drift.py"
...
echo [check-forwarder-drift] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [check-forwarder-drift] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0check-forwarder-drift.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0check-forwarder-drift.py" %*
exit /b %ERRORLEVEL%
