@echo off
setlocal
REM Windows launcher for archive-session-scope.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "archive-session-scope.ps1" sibling of
REM     this file over this "archive-session-scope.cmd" file itself -- if both exist, `archive-session-scope`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "archive-session-scope.py"
...
echo [archive-session-scope] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [archive-session-scope] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0archive-session-scope.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0archive-session-scope.py" %*
exit /b %ERRORLEVEL%
