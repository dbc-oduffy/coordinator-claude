# group-em-watch.ps1 — python-direct Windows launcher for group-em-watch.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "group-em-watch.ps1" file over
#     any co-located "group-em-watch.cmd" sibling -- `group-em-watch` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "group-em-watch.py"
...
$_entry = Join-Path $_here 'group-em-watch.py'
...
[Console]::Error.WriteLine('[group-em-watch] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[group-em-watch] Install Python: https://www.python.org/downloads/windows/')
exit 127
