@echo off
setlocal
REM Windows launcher for verify-subagent-sandbox-preamble-sync.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "verify-subagent-sandbox-preamble-sync.ps1" sibling of
REM     this file over this "verify-subagent-sandbox-preamble-sync.cmd" file itself -- if both exist, `verify-subagent-sandbox-preamble-sync`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "verify-subagent-sandbox-preamble-sync.py"
...
echo [verify-subagent-sandbox-preamble-sync] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [verify-subagent-sandbox-preamble-sync] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0verify-subagent-sandbox-preamble-sync.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0verify-subagent-sandbox-preamble-sync.py" %*
exit /b %ERRORLEVEL%
