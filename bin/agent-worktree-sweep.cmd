@echo off
setlocal
REM Windows launcher for agent-worktree-sweep.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "agent-worktree-sweep.ps1" sibling of
REM     this file over this "agent-worktree-sweep.cmd" file itself -- if both exist, `agent-worktree-sweep`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "agent-worktree-sweep.py"
...
echo [agent-worktree-sweep] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [agent-worktree-sweep] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0agent-worktree-sweep.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0agent-worktree-sweep.py" %*
exit /b %ERRORLEVEL%
