...
#                                          bypasses the CLAUDE_HOME append + F14 guard
...
#
# Two input modes:
#   COORDINATOR_CURRENCY_REPO_ROOT (explicit override) — used verbatim as repo_root,
#     no /.claude append, no F14 guard. Callers that already know the repo_root
#     directly (detect-onboarding-offer, sentinel P-13) pass it here. Mirrors the
#     COORDINATOR_CURRENCY_PLUGIN_ROOT override above.
#   CLAUDE_HOME (default) — treated as a $HOME-substitute; /.claude is appended and
#     the F14 guard rejects a value already ending in /.claude (corpus contract:
#     test-claude-home-contract.sh).
if [[ -n "${COORDINATOR_CURRENCY_REPO_ROOT:-}" ]]; then
  _repo_root="${COORDINATOR_CURRENCY_REPO_ROOT}"
else
  # Review: code-reviewer F14 — CLAUDE_HOME is a $HOME substitute (e.g. /home/user), NOT
  # the .claude directory itself. This script always appends /.claude. An operator whose
  # CLAUDE_HOME still points at the old .claude-suffixed path (legacy convention) would
  # produce a doubled path (/home/user/.claude/.claude), silently probing the wrong place.
  # Guard mirrors ensure-coordinator-venv.sh:70.
  if [[ -n "${CLAUDE_HOME:-}" && "${CLAUDE_HOME%/}" == */.claude ]]; then
    echo "probe-onboarding-currency: FATAL: CLAUDE_HOME='${CLAUDE_HOME}' ends in '/.claude'." >&2
    echo "  CLAUDE_HOME is a \$HOME substitute, NOT the .claude directory itself." >&2
    echo "  Remediation: set CLAUDE_HOME to the PARENT of .claude" >&2
    echo "  (e.g. export CLAUDE_HOME=\$HOME  or  export CLAUDE_HOME=/home/user)" >&2
    exit 1
  fi
