@echo off
setlocal
REM Windows launcher for coordinator-precommit-settings-tracking-check — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "coordinator-precommit-settings-tracking-check.ps1" sibling of
REM     this file over this "coordinator-precommit-settings-tracking-check.cmd" file itself -- if both exist, `coordinator-precommit-settings-tracking-check`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "coordinator-precommit-settings-tracking-check"
...
echo [coordinator-precommit-settings-tracking-check] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [coordinator-precommit-settings-tracking-check] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0coordinator-precommit-settings-tracking-check" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0coordinator-precommit-settings-tracking-check" %*
exit /b %ERRORLEVEL%
