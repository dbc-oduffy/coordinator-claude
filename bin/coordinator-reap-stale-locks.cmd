@echo off
setlocal
REM Windows launcher for coordinator-reap-stale-locks.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "coordinator-reap-stale-locks.ps1" sibling of
REM     this file over this "coordinator-reap-stale-locks.cmd" file itself -- if both exist, `coordinator-reap-stale-locks`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "coordinator-reap-stale-locks.py"
...
echo [coordinator-reap-stale-locks] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [coordinator-reap-stale-locks] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0coordinator-reap-stale-locks.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0coordinator-reap-stale-locks.py" %*
exit /b %ERRORLEVEL%
