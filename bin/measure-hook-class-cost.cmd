@echo off
setlocal
REM Windows launcher for measure-hook-class-cost.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "measure-hook-class-cost.ps1" sibling of
REM     this file over this "measure-hook-class-cost.cmd" file itself -- if both exist, `measure-hook-class-cost`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "measure-hook-class-cost.py"
...
echo [measure-hook-class-cost] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [measure-hook-class-cost] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0measure-hook-class-cost.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0measure-hook-class-cost.py" %*
exit /b %ERRORLEVEL%
