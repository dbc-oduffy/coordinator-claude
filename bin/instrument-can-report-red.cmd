@echo off
setlocal
REM Windows launcher for instrument-can-report-red.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "instrument-can-report-red.ps1" sibling of
REM     this file over this "instrument-can-report-red.cmd" file itself -- if both exist, `instrument-can-report-red`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "instrument-can-report-red.py"
...
echo [instrument-can-report-red] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [instrument-can-report-red] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0instrument-can-report-red.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0instrument-can-report-red.py" %*
exit /b %ERRORLEVEL%
