# group-em-nomination.ps1 — python-direct Windows launcher for group-em-nomination.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "group-em-nomination.ps1" file over
#     any co-located "group-em-nomination.cmd" sibling -- `group-em-nomination` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "group-em-nomination.py"
...
$_entry = Join-Path $_here 'group-em-nomination.py'
...
[Console]::Error.WriteLine('[group-em-nomination] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[group-em-nomination] Install Python: https://www.python.org/downloads/windows/')
exit 127
