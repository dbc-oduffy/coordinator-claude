# scoped-git-commit.ps1 — python-direct Windows launcher for scoped-git-commit (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "scoped-git-commit.ps1" file over
#     any co-located "scoped-git-commit.cmd" sibling -- `scoped-git-commit` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "scoped-git-commit"
...
$_entry = Join-Path $_here 'scoped-git-commit'
...
[Console]::Error.WriteLine('[scoped-git-commit] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[scoped-git-commit] Install Python: https://www.python.org/downloads/windows/')
exit 127
