@echo off
setlocal
REM Windows launcher for workweek-start-goal-and-priorities.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "workweek-start-goal-and-priorities.ps1" sibling of
REM     this file over this "workweek-start-goal-and-priorities.cmd" file itself -- if both exist, `workweek-start-goal-and-priorities`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "workweek-start-goal-and-priorities.py"
...
echo [workweek-start-goal-and-priorities] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [workweek-start-goal-and-priorities] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0workweek-start-goal-and-priorities.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0workweek-start-goal-and-priorities.py" %*
exit /b %ERRORLEVEL%
