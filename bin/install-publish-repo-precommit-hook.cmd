@echo off
setlocal
REM Windows launcher for install-publish-repo-precommit-hook.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "install-publish-repo-precommit-hook.ps1" sibling of
REM     this file over this "install-publish-repo-precommit-hook.cmd" file itself -- if both exist, `install-publish-repo-precommit-hook`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "install-publish-repo-precommit-hook.py"
...
echo [install-publish-repo-precommit-hook] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [install-publish-repo-precommit-hook] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0install-publish-repo-precommit-hook.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0install-publish-repo-precommit-hook.py" %*
exit /b %ERRORLEVEL%
