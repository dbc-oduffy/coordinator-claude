@echo off
setlocal
REM Windows launcher for fan-out-integrator.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "fan-out-integrator.ps1" sibling of
REM     this file over this "fan-out-integrator.cmd" file itself -- if both exist, `fan-out-integrator`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "fan-out-integrator.py"
...
echo [fan-out-integrator] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [fan-out-integrator] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0fan-out-integrator.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0fan-out-integrator.py" %*
exit /b %ERRORLEVEL%
