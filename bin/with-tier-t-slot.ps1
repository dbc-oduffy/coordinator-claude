# with-tier-t-slot.ps1 — python-direct Windows launcher for with-tier-t-slot (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "with-tier-t-slot.ps1" file over
#     any co-located "with-tier-t-slot.cmd" sibling -- `with-tier-t-slot` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "with-tier-t-slot"
...
$_entry = Join-Path $_here 'with-tier-t-slot'
...
[Console]::Error.WriteLine('[with-tier-t-slot] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[with-tier-t-slot] Install Python: https://www.python.org/downloads/windows/')
exit 127
