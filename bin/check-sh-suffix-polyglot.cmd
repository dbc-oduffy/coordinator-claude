@echo off
setlocal
REM Windows launcher for check-sh-suffix-polyglot.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "check-sh-suffix-polyglot.ps1" sibling of
REM     this file over this "check-sh-suffix-polyglot.cmd" file itself -- if both exist, `check-sh-suffix-polyglot`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "check-sh-suffix-polyglot.py"
...
echo [check-sh-suffix-polyglot] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [check-sh-suffix-polyglot] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0check-sh-suffix-polyglot.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0check-sh-suffix-polyglot.py" %*
exit /b %ERRORLEVEL%
