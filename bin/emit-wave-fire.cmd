@echo off
setlocal
REM Windows launcher for emit-wave-fire.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "emit-wave-fire.ps1" sibling of
REM     this file over this "emit-wave-fire.cmd" file itself -- if both exist, `emit-wave-fire`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "emit-wave-fire.py"
...
echo [emit-wave-fire] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [emit-wave-fire] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0emit-wave-fire.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0emit-wave-fire.py" %*
exit /b %ERRORLEVEL%
