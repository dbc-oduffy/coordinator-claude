@echo off
setlocal
REM Windows launcher for check-persona-slug-leak.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "check-persona-slug-leak.ps1" sibling of
REM     this file over this "check-persona-slug-leak.cmd" file itself -- if both exist, `check-persona-slug-leak`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "check-persona-slug-leak.py"
...
echo [check-persona-slug-leak] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [check-persona-slug-leak] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0check-persona-slug-leak.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0check-persona-slug-leak.py" %*
exit /b %ERRORLEVEL%
