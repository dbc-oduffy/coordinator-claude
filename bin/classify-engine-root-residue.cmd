@echo off
setlocal
REM Windows launcher for classify-engine-root-residue.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "classify-engine-root-residue.ps1" sibling of
REM     this file over this "classify-engine-root-residue.cmd" file itself -- if both exist, `classify-engine-root-residue`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "classify-engine-root-residue.py"
...
echo [classify-engine-root-residue] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [classify-engine-root-residue] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0classify-engine-root-residue.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0classify-engine-root-residue.py" %*
exit /b %ERRORLEVEL%
