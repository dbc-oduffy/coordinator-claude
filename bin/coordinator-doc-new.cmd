@echo off
setlocal
REM Windows launcher for coordinator-doc-new.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "coordinator-doc-new.ps1" sibling of
REM     this file over this "coordinator-doc-new.cmd" file itself -- if both exist, `coordinator-doc-new`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "coordinator-doc-new.py"
...
echo [coordinator-doc-new] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [coordinator-doc-new] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0coordinator-doc-new.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0coordinator-doc-new.py" %*
exit /b %ERRORLEVEL%
