@echo off
setlocal
REM Windows launcher for coordinator-ensure-post-commit-hook — python-direct (NO bash re-exec).
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "coordinator-ensure-post-commit-hook"
...
:run_baked
"%_py%" "%~dp0coordinator-ensure-post-commit-hook" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0coordinator-ensure-post-commit-hook" %*
exit /b %ERRORLEVEL%
