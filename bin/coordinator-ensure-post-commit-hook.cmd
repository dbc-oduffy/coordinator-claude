@echo off
setlocal
REM Windows launcher for coordinator-ensure-post-commit-hook.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "coordinator-ensure-post-commit-hook.ps1" sibling of
REM     this file over this "coordinator-ensure-post-commit-hook.cmd" file itself -- if both exist, `coordinator-ensure-post-commit-hook`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "coordinator-ensure-post-commit-hook.py"
...
echo [coordinator-ensure-post-commit-hook] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [coordinator-ensure-post-commit-hook] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0coordinator-ensure-post-commit-hook.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0coordinator-ensure-post-commit-hook.py" %*
exit /b %ERRORLEVEL%
