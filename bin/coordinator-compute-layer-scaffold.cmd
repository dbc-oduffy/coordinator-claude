@echo off
setlocal
REM Windows launcher for coordinator-compute-layer-scaffold.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "coordinator-compute-layer-scaffold.ps1" sibling of
REM     this file over this "coordinator-compute-layer-scaffold.cmd" file itself -- if both exist, `coordinator-compute-layer-scaffold`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "coordinator-compute-layer-scaffold.py"
...
echo [coordinator-compute-layer-scaffold] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [coordinator-compute-layer-scaffold] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0coordinator-compute-layer-scaffold.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0coordinator-compute-layer-scaffold.py" %*
exit /b %ERRORLEVEL%
