@echo off
setlocal
REM Windows launcher for distill-sidecar-sweep.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "distill-sidecar-sweep.ps1" sibling of
REM     this file over this "distill-sidecar-sweep.cmd" file itself -- if both exist, `distill-sidecar-sweep`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "distill-sidecar-sweep.py"
...
echo [distill-sidecar-sweep] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [distill-sidecar-sweep] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0distill-sidecar-sweep.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0distill-sidecar-sweep.py" %*
exit /b %ERRORLEVEL%
