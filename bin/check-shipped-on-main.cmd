@echo off
setlocal
REM Windows launcher for check-shipped-on-main.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "check-shipped-on-main.ps1" sibling of
REM     this file over this "check-shipped-on-main.cmd" file itself -- if both exist, `check-shipped-on-main`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "check-shipped-on-main.py"
...
echo [check-shipped-on-main] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [check-shipped-on-main] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0check-shipped-on-main.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0check-shipped-on-main.py" %*
exit /b %ERRORLEVEL%
