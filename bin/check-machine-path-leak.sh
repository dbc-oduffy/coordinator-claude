...
for arg in "$@"; do
    case "$arg" in
        --staged)
            ...
            ;;
        -h|--help)
            sed -n '2,/^# Negative-spec/p' "$0" | grep '^#' | sed 's/^#[[:space:]]\{0,1\}//' || true
            exit 0
            ;;
        -*)
...
# X:\, E:\, and /Users/<other>/ are intentional cross-machine catalog content.
#
...
# machine's $HOME. X:\, E:\, and /Users/<other>/ are intentional cross-machine
...
_check_working_repos_yaml() {
    ...
    if [[ "$pyyaml_ok" == "yes" ]]; then
        ...
        # Foreign-machine paths (X:\, /Users/other/) are skipped — intentional cross-machine catalog.
