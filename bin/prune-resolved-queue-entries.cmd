@echo off
setlocal
REM Windows launcher for prune-resolved-queue-entries.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "prune-resolved-queue-entries.ps1" sibling of
REM     this file over this "prune-resolved-queue-entries.cmd" file itself -- if both exist, `prune-resolved-queue-entries`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "prune-resolved-queue-entries.py"
...
echo [prune-resolved-queue-entries] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [prune-resolved-queue-entries] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0prune-resolved-queue-entries.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0prune-resolved-queue-entries.py" %*
exit /b %ERRORLEVEL%
