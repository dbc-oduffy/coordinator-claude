# query-routine-signals.ps1 — python-direct Windows launcher for query-routine-signals.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "query-routine-signals.ps1" file over
#     any co-located "query-routine-signals.cmd" sibling -- `query-routine-signals` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "query-routine-signals.py"
...
$_entry = Join-Path $_here 'query-routine-signals.py'
...
[Console]::Error.WriteLine('[query-routine-signals] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[query-routine-signals] Install Python: https://www.python.org/downloads/windows/')
exit 127
