@echo off
setlocal
REM Windows launcher for host-gpu-probe.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "host-gpu-probe.ps1" sibling of
REM     this file over this "host-gpu-probe.cmd" file itself -- if both exist, `host-gpu-probe`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "host-gpu-probe.py"
...
echo [host-gpu-probe] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [host-gpu-probe] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0host-gpu-probe.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0host-gpu-probe.py" %*
exit /b %ERRORLEVEL%
