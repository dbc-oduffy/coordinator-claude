@echo off
setlocal
REM Windows launcher for emit-effective-delivery.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "emit-effective-delivery.ps1" sibling of
REM     this file over this "emit-effective-delivery.cmd" file itself -- if both exist, `emit-effective-delivery`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "emit-effective-delivery.py"
...
echo [emit-effective-delivery] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [emit-effective-delivery] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0emit-effective-delivery.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0emit-effective-delivery.py" %*
exit /b %ERRORLEVEL%
