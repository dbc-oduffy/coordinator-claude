# expired-plan-gates.ps1 — python-direct Windows launcher for expired-plan-gates.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "expired-plan-gates.ps1" file over
#     any co-located "expired-plan-gates.cmd" sibling -- `expired-plan-gates` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "expired-plan-gates.py"
...
$_entry = Join-Path $_here 'expired-plan-gates.py'
...
[Console]::Error.WriteLine('[expired-plan-gates] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[expired-plan-gates] Install Python: https://www.python.org/downloads/windows/')
exit 127
