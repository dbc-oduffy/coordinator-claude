# coordinator-compute-layer-scaffold.ps1 — python-direct Windows launcher for coordinator-compute-layer-scaffold.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "coordinator-compute-layer-scaffold.ps1" file over
#     any co-located "coordinator-compute-layer-scaffold.cmd" sibling -- `coordinator-compute-layer-scaffold` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "coordinator-compute-layer-scaffold.py"
...
$_entry = Join-Path $_here 'coordinator-compute-layer-scaffold.py'
...
[Console]::Error.WriteLine('[coordinator-compute-layer-scaffold] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[coordinator-compute-layer-scaffold] Install Python: https://www.python.org/downloads/windows/')
exit 127
