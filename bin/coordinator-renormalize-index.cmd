@echo off
setlocal
REM Windows launcher for coordinator-renormalize-index.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "coordinator-renormalize-index.ps1" sibling of
REM     this file over this "coordinator-renormalize-index.cmd" file itself -- if both exist, `coordinator-renormalize-index`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "coordinator-renormalize-index.py"
...
echo [coordinator-renormalize-index] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [coordinator-renormalize-index] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0coordinator-renormalize-index.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0coordinator-renormalize-index.py" %*
exit /b %ERRORLEVEL%
