...
if [[ -f "$_py3_exe_posix" ]]; then
  if diff -q "$_py_exe_posix" "$_py3_exe_posix" >/dev/null 2>&1; then
    exit 0  # already valid — idempotent, nothing to do
  fi
  # Falls through: stale shim detected; will re-shim after CHECK_ONLY gate.
fi

# At this point python3.exe is absent or stale — a shim WILL be installed.
# Honor CHECK_ONLY: report what would happen and exit without mutating.
if [[ -n "${CHECK_ONLY:-}" ]]; then
  echo "python3-exe-shim: would install python3.exe shim (check-only)"
  exit 0
fi
