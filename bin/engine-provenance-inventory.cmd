@echo off
setlocal
REM Windows launcher for engine-provenance-inventory.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "engine-provenance-inventory.ps1" sibling of
REM     this file over this "engine-provenance-inventory.cmd" file itself -- if both exist, `engine-provenance-inventory`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "engine-provenance-inventory.py"
...
echo [engine-provenance-inventory] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [engine-provenance-inventory] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0engine-provenance-inventory.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0engine-provenance-inventory.py" %*
exit /b %ERRORLEVEL%
