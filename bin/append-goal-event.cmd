@echo off
setlocal
REM Windows launcher for append-goal-event.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "append-goal-event.ps1" sibling of
REM     this file over this "append-goal-event.cmd" file itself -- if both exist, `append-goal-event`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "append-goal-event.py"
...
echo [append-goal-event] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [append-goal-event] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0append-goal-event.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0append-goal-event.py" %*
exit /b %ERRORLEVEL%
