@echo off
setlocal
REM Windows launcher for central-run-due.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "central-run-due.ps1" sibling of
REM     this file over this "central-run-due.cmd" file itself -- if both exist, `central-run-due`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "central-run-due.py"
...
echo [central-run-due] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [central-run-due] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0central-run-due.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0central-run-due.py" %*
exit /b %ERRORLEVEL%
