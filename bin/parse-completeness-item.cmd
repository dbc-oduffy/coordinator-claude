@echo off
setlocal
REM Windows launcher for parse-completeness-item.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "parse-completeness-item.ps1" sibling of
REM     this file over this "parse-completeness-item.cmd" file itself -- if both exist, `parse-completeness-item`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "parse-completeness-item.py"
...
echo [parse-completeness-item] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [parse-completeness-item] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0parse-completeness-item.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0parse-completeness-item.py" %*
exit /b %ERRORLEVEL%
