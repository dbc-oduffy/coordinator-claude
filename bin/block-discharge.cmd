@echo off
setlocal
REM Windows launcher for block-discharge.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "block-discharge.ps1" sibling of
REM     this file over this "block-discharge.cmd" file itself -- if both exist, `block-discharge`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "block-discharge.py"
...
echo [block-discharge] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [block-discharge] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0block-discharge.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0block-discharge.py" %*
exit /b %ERRORLEVEL%
