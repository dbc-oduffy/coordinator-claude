...
def _make_mock_machine_local_keys_and_get(tmpdir: str, key_paths: dict) -> str:
    ...
    sibling helper of the same name in cross-repo-memo.test.py.
    """
...
def test_draft_unknown_receiver_rejected() -> None:
    """draft with a typo'd/unknown --to exits 2, writes NO file, points at --list-receivers.

    Reproduces the confirmed bug: draft used to accept an unknown receiver silently
    (exit 0, file written) and the error surfaced only much later at `send`.
    """
    name = "test_draft_unknown_receiver_rejected"
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # A registered sibling (project-rag) so 'did you mean' has a candidate.
        mock_impl = _make_mock_machine_local_keys_and_get(
            tmpdir, {"repos.project_rag": "/work/project-rag"}
        )
        env = {"MACHINE_LOCAL_IMPL": mock_impl, "CLAUDE_HOME": claude_home}

        result = _run_dispatcher_in_repo(
            sender_repo,
            ["draft", "unknown-recv", "--to", "project-rag-em-PLACEHOLDER", "--title", "x"],
            env=env,
        )

        if result.returncode != 2:
            fail_test(name, f"unknown receiver should exit 2, got {result.returncode}. stderr: {result.stderr!r}")
            return

        outbox_path = os.path.join(sender_repo, "state", "memo-outbox", "unknown-recv.md")
        if os.path.isfile(outbox_path):
            fail_test(name, f"unknown receiver must not create an outbox file: {outbox_path}")
            return

        combined = (result.stdout + result.stderr).lower()
        if "list-receivers" not in combined:
            fail_test(name, f"error should point at --list-receivers. output: {result.stderr!r}")
            return
        # design-as-offers: the close registered sibling should be suggested.
        if "project-rag-em" not in (result.stdout + result.stderr):
            fail_test(name, f"error should suggest the close match 'project-rag-em'. output: {result.stderr!r}")
            return

        pass_test(name)


def test_draft_publish_target_receiver_rejected() -> None:
    """draft to a publish-target mirror exits 1, writes NO file, names the owner.

    Mirrors the send/legacy publish-target guard — the mirror is an outward OSS
    distribution target, not an EM working tree.
    """
    name = "test_draft_publish_target_receiver_rejected"
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        result = _run_dispatcher_in_repo(
            sender_repo,
            ["draft", "pt-draft", "--to", "coordinator-claude-em", "--title", "x"],
            env=env,
        )

        if result.returncode != 1:
            fail_test(name, f"publish-target receiver should exit 1, got {result.returncode}. stderr: {result.stderr!r}")
            return

        outbox_path = os.path.join(sender_repo, "state", "memo-outbox", "pt-draft.md")
        if os.path.isfile(outbox_path):
            fail_test(name, f"publish-target receiver must not create an outbox file: {outbox_path}")
            return

        if "claude-central-em" not in result.stderr:
            fail_test(name, f"rejection should name the owner (claude-central-em). output: {result.stderr!r}")
            return

        pass_test(name)


def test_draft_unresolved_receiver_warns_but_creates() -> None:
    """draft to a registered-but-unresolved receiver key warns, still creates the draft.

    A present-but-unresolved key is portable: the draft may `send` from a machine
    where the key resolves. So draft warns (non-fatal) rather than blocking.
    """
    name = "test_draft_unresolved_receiver_warns_but_creates"
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # repos.example-sim-repo is listed by `keys` but maps to None → present-but-unresolved.
        mock_impl = _make_mock_machine_local_keys_and_get(
            tmpdir, {"repos.example-sim-repo": None}
        )
        env = {"MACHINE_LOCAL_IMPL": mock_impl, "CLAUDE_HOME": claude_home}

        result = _run_dispatcher_in_repo(
            sender_repo,
            ["draft", "unresolved-recv", "--to", "example-sim-repo-em", "--title", "x"],
            env=env,
        )

        if result.returncode != 0:
            fail_test(name, f"unresolved (but registered) receiver should exit 0, got {result.returncode}. stderr: {result.stderr!r}")
            return

        outbox_path = os.path.join(sender_repo, "state", "memo-outbox", "unresolved-recv.md")
        if not os.path.isfile(outbox_path):
            fail_test(name, f"unresolved-but-registered receiver should still create the draft: {outbox_path}")
            return

        if "warning" not in result.stderr.lower():
            fail_test(name, f"unresolved receiver should emit a WARNING on stderr. output: {result.stderr!r}")
            return
...
def test_draft_resolved_sibling_receiver_ok() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        result = _run_dispatcher_in_repo(
            sender_repo,
            ["draft", "sibling-recv", "--to", "project-rag-em", "--title", "x"],
            env=env,
        )

        if result.returncode != 0:
            fail_test(name, f"resolved sibling receiver should exit 0, got {result.returncode}. stderr: {result.stderr!r}")
            return

        outbox_path = os.path.join(sender_repo, "state", "memo-outbox", "sibling-recv.md")
        if not os.path.isfile(outbox_path):
            fail_test(name, f"resolved sibling receiver should create the draft: {outbox_path}")
            return
...
def test_send_consumes_outbox() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if not os.path.isfile(receiver_inbox):
            ...
            if not candidates:
                fail_test(name, (
                    f"receiver-side file not found at {receiver_inbox} (and no *-{topic}.md in inbox). "
...
def test_send_preserves_supersedes() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if not os.path.isfile(receiver_inbox):
            ...
            if not candidates:
                fail_test(name, f"receiver-side file not found at {receiver_inbox} (and no *-{topic}.md in inbox)")
                return
            receiver_inbox = candidates[0]
...
def main() -> int:
    ...
    tests = [
        ...
        # Draft-time --to validation (receiver caught at draft, not deferred to send)
        test_draft_unknown_receiver_rejected,
        test_draft_publish_target_receiver_rejected,
        test_draft_unresolved_receiver_warns_but_creates,
        test_draft_resolved_sibling_receiver_ok,
