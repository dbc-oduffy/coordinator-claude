# spawn-census.ps1 — python-direct Windows launcher for spawn-census (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "spawn-census.ps1" file over
#     any co-located "spawn-census.cmd" sibling -- `spawn-census` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "spawn-census"
...
$_entry = Join-Path $_here 'spawn-census'
...
[Console]::Error.WriteLine('[spawn-census] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[spawn-census] Install Python: https://www.python.org/downloads/windows/')
exit 127
