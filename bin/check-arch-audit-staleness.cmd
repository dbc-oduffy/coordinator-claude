@echo off
setlocal
REM Windows launcher for check-arch-audit-staleness.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "check-arch-audit-staleness.ps1" sibling of
REM     this file over this "check-arch-audit-staleness.cmd" file itself -- if both exist, `check-arch-audit-staleness`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "check-arch-audit-staleness.py"
...
echo [check-arch-audit-staleness] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [check-arch-audit-staleness] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0check-arch-audit-staleness.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0check-arch-audit-staleness.py" %*
exit /b %ERRORLEVEL%
