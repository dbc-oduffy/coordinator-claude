...
Negative-spec:
  ...
  - Does NOT read the engine in-process. `canonical_body_sha` is transcribed below (ten lines of
    ...
    `coordinator/tests/test_arrival_aggregate_rollup.py`, which may skip, where this may not.
...
sibling `hooks/scripts/repo_root.py` import, which was itself already cwd-walking (never
`__file__`-derived) — same behavior, engine-native seam. `mise-prep-entry.py`
(`coordinator/bin/mise-prep-entry.py`, landed at W2-C9 of this same plan) names this exact module
as the seam it will retarget once this chunk ships; that retarget is a later chunk's `writes:`,
not this row's.
"""
...
def split_frontmatter(file_text: str) -> dict:
    ...
    for index in range(1, len(lines)):
        if _BODY_DELIMITER_RE.match(lines[index]):
            ...
            try:
                loaded = yaml.load(
                    "\n".join(lines[1:index]), Loader=getattr(yaml, "CSafeLoader", yaml.SafeLoader)
                )
...
def _default_repo_root() -> Path:
    ...
    This CLI can be invoked against any consumer repo, not only claude-klabauter's own tree, so its
    default repo root must resolve from the caller's cwd (§ Path resolution "session repo" class),
    never from this module's own `__file__` location.
    """
    ...
    if resolved is None:
        raise RollupError("not inside a git work tree and no --repo-root given")
