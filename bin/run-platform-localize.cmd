@echo off
setlocal
REM Windows launcher for run-platform-localize.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "run-platform-localize.ps1" sibling of
REM     this file over this "run-platform-localize.cmd" file itself -- if both exist, `run-platform-localize`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "run-platform-localize.py"
...
echo [run-platform-localize] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [run-platform-localize] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0run-platform-localize.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0run-platform-localize.py" %*
exit /b %ERRORLEVEL%
