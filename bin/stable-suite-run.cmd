@echo off
setlocal
REM Windows launcher for stable-suite-run.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "stable-suite-run.ps1" sibling of
REM     this file over this "stable-suite-run.cmd" file itself -- if both exist, `stable-suite-run`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "stable-suite-run.py"
...
echo [stable-suite-run] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [stable-suite-run] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0stable-suite-run.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0stable-suite-run.py" %*
exit /b %ERRORLEVEL%
