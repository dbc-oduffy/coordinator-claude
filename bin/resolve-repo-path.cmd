@echo off
setlocal
REM Windows launcher for resolve-repo-path.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "resolve-repo-path.ps1" sibling of
REM     this file over this "resolve-repo-path.cmd" file itself -- if both exist, `resolve-repo-path`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "resolve-repo-path.py"
...
echo [resolve-repo-path] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [resolve-repo-path] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0resolve-repo-path.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0resolve-repo-path.py" %*
exit /b %ERRORLEVEL%
