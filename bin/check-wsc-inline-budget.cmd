@echo off
setlocal
REM Windows launcher for check-wsc-inline-budget.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "check-wsc-inline-budget.ps1" sibling of
REM     this file over this "check-wsc-inline-budget.cmd" file itself -- if both exist, `check-wsc-inline-budget`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "check-wsc-inline-budget.py"
...
echo [check-wsc-inline-budget] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [check-wsc-inline-budget] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0check-wsc-inline-budget.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0check-wsc-inline-budget.py" %*
exit /b %ERRORLEVEL%
