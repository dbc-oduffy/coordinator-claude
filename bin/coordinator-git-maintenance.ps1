# coordinator-git-maintenance.ps1 — python-direct Windows launcher for coordinator-git-maintenance.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "coordinator-git-maintenance.ps1" file over
#     any co-located "coordinator-git-maintenance.cmd" sibling -- `coordinator-git-maintenance` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "coordinator-git-maintenance.py"
...
$_entry = Join-Path $_here 'coordinator-git-maintenance.py'
...
[Console]::Error.WriteLine('[coordinator-git-maintenance] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[coordinator-git-maintenance] Install Python: https://www.python.org/downloads/windows/')
exit 127
