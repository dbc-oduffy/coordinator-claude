@echo off
setlocal
REM Windows launcher for check-citation-integrity.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "check-citation-integrity.ps1" sibling of
REM     this file over this "check-citation-integrity.cmd" file itself -- if both exist, `check-citation-integrity`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "check-citation-integrity.py"
...
echo [check-citation-integrity] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [check-citation-integrity] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0check-citation-integrity.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0check-citation-integrity.py" %*
exit /b %ERRORLEVEL%
