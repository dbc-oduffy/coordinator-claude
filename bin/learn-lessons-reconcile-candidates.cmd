@echo off
setlocal
REM Windows launcher for learn-lessons-reconcile-candidates.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "learn-lessons-reconcile-candidates.ps1" sibling of
REM     this file over this "learn-lessons-reconcile-candidates.cmd" file itself -- if both exist, `learn-lessons-reconcile-candidates`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "learn-lessons-reconcile-candidates.py"
...
echo [learn-lessons-reconcile-candidates] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [learn-lessons-reconcile-candidates] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0learn-lessons-reconcile-candidates.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0learn-lessons-reconcile-candidates.py" %*
exit /b %ERRORLEVEL%
