@echo off
setlocal
REM Windows launcher for queue-triage.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "queue-triage.ps1" sibling of
REM     this file over this "queue-triage.cmd" file itself -- if both exist, `queue-triage`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "queue-triage.py"
...
echo [queue-triage] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [queue-triage] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0queue-triage.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0queue-triage.py" %*
exit /b %ERRORLEVEL%
