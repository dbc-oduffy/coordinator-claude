@echo off
setlocal
REM Windows launcher for set-goal-kr-status.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "set-goal-kr-status.ps1" sibling of
REM     this file over this "set-goal-kr-status.cmd" file itself -- if both exist, `set-goal-kr-status`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "set-goal-kr-status.py"
...
echo [set-goal-kr-status] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [set-goal-kr-status] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0set-goal-kr-status.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0set-goal-kr-status.py" %*
exit /b %ERRORLEVEL%
