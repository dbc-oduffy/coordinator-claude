@echo off
setlocal
REM Windows launcher for handoff-stamp-phase.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "handoff-stamp-phase.ps1" sibling of
REM     this file over this "handoff-stamp-phase.cmd" file itself -- if both exist, `handoff-stamp-phase`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "handoff-stamp-phase.py"
...
echo [handoff-stamp-phase] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [handoff-stamp-phase] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0handoff-stamp-phase.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0handoff-stamp-phase.py" %*
exit /b %ERRORLEVEL%
