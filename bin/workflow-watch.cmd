@echo off
setlocal
REM Windows launcher for workflow-watch.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "workflow-watch.ps1" sibling of
REM     this file over this "workflow-watch.cmd" file itself -- if both exist, `workflow-watch`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "workflow-watch.py"
...
echo [workflow-watch] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [workflow-watch] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0workflow-watch.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0workflow-watch.py" %*
exit /b %ERRORLEVEL%
