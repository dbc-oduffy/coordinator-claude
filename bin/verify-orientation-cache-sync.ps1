# verify-orientation-cache-sync.ps1 — python-direct Windows launcher for verify-orientation-cache-sync.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "verify-orientation-cache-sync.ps1" file over
#     any co-located "verify-orientation-cache-sync.cmd" sibling -- `verify-orientation-cache-sync` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "verify-orientation-cache-sync.py"
...
$_entry = Join-Path $_here 'verify-orientation-cache-sync.py'
...
[Console]::Error.WriteLine('[verify-orientation-cache-sync] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[verify-orientation-cache-sync] Install Python: https://www.python.org/downloads/windows/')
exit 127
