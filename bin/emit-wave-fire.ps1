# emit-wave-fire.ps1 — python-direct Windows launcher for emit-wave-fire.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "emit-wave-fire.ps1" file over
#     any co-located "emit-wave-fire.cmd" sibling -- `emit-wave-fire` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "emit-wave-fire.py"
...
$_entry = Join-Path $_here 'emit-wave-fire.py'
...
[Console]::Error.WriteLine('[emit-wave-fire] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[emit-wave-fire] Install Python: https://www.python.org/downloads/windows/')
exit 127
