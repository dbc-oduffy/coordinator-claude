@echo off
setlocal
REM Windows launcher for approvability-calibration.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "approvability-calibration.ps1" sibling of
REM     this file over this "approvability-calibration.cmd" file itself -- if both exist, `approvability-calibration`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "approvability-calibration.py"
...
echo [approvability-calibration] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [approvability-calibration] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0approvability-calibration.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0approvability-calibration.py" %*
exit /b %ERRORLEVEL%
