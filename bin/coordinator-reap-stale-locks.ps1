# coordinator-reap-stale-locks.ps1 — python-direct Windows launcher for coordinator-reap-stale-locks.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "coordinator-reap-stale-locks.ps1" file over
#     any co-located "coordinator-reap-stale-locks.cmd" sibling -- `coordinator-reap-stale-locks` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "coordinator-reap-stale-locks.py"
...
$_entry = Join-Path $_here 'coordinator-reap-stale-locks.py'
...
[Console]::Error.WriteLine('[coordinator-reap-stale-locks] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[coordinator-reap-stale-locks] Install Python: https://www.python.org/downloads/windows/')
exit 127
