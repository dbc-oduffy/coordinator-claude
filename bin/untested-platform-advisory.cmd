@echo off
setlocal
REM Windows launcher for untested-platform-advisory.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "untested-platform-advisory.ps1" sibling of
REM     this file over this "untested-platform-advisory.cmd" file itself -- if both exist, `untested-platform-advisory`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "untested-platform-advisory.py"
...
echo [untested-platform-advisory] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [untested-platform-advisory] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0untested-platform-advisory.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0untested-platform-advisory.py" %*
exit /b %ERRORLEVEL%
