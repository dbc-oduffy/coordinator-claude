@echo off
setlocal
REM Windows launcher for check-projectragignore-fleet-drift.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "check-projectragignore-fleet-drift.ps1" sibling of
REM     this file over this "check-projectragignore-fleet-drift.cmd" file itself -- if both exist, `check-projectragignore-fleet-drift`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "check-projectragignore-fleet-drift.py"
...
echo [check-projectragignore-fleet-drift] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [check-projectragignore-fleet-drift] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0check-projectragignore-fleet-drift.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0check-projectragignore-fleet-drift.py" %*
exit /b %ERRORLEVEL%
