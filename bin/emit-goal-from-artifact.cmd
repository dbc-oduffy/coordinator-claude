@echo off
setlocal
REM Windows launcher for emit-goal-from-artifact.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "emit-goal-from-artifact.ps1" sibling of
REM     this file over this "emit-goal-from-artifact.cmd" file itself -- if both exist, `emit-goal-from-artifact`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "emit-goal-from-artifact.py"
...
echo [emit-goal-from-artifact] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [emit-goal-from-artifact] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0emit-goal-from-artifact.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0emit-goal-from-artifact.py" %*
exit /b %ERRORLEVEL%
