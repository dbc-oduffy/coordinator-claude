@echo off
setlocal
REM Windows launcher for verify-coverage.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "verify-coverage.ps1" sibling of
REM     this file over this "verify-coverage.cmd" file itself -- if both exist, `verify-coverage`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "verify-coverage.py"
...
echo [verify-coverage] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [verify-coverage] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0verify-coverage.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0verify-coverage.py" %*
exit /b %ERRORLEVEL%
