@echo off
setlocal
REM Windows launcher for query-roadmap-serve.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "query-roadmap-serve.ps1" sibling of
REM     this file over this "query-roadmap-serve.cmd" file itself -- if both exist, `query-roadmap-serve`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "query-roadmap-serve.py"
...
echo [query-roadmap-serve] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [query-roadmap-serve] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0query-roadmap-serve.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0query-roadmap-serve.py" %*
exit /b %ERRORLEVEL%
