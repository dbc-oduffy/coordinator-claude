@echo off
setlocal
REM Windows launcher for publish-allowlist-generate.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "publish-allowlist-generate.ps1" sibling of
REM     this file over this "publish-allowlist-generate.cmd" file itself -- if both exist, `publish-allowlist-generate`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "publish-allowlist-generate.py"
...
echo [publish-allowlist-generate] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [publish-allowlist-generate] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0publish-allowlist-generate.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0publish-allowlist-generate.py" %*
exit /b %ERRORLEVEL%
