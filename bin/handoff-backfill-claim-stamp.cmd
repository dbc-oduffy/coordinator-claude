@echo off
setlocal
REM Windows launcher for handoff-backfill-claim-stamp.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "handoff-backfill-claim-stamp.ps1" sibling of
REM     this file over this "handoff-backfill-claim-stamp.cmd" file itself -- if both exist, `handoff-backfill-claim-stamp`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "handoff-backfill-claim-stamp.py"
...
echo [handoff-backfill-claim-stamp] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [handoff-backfill-claim-stamp] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0handoff-backfill-claim-stamp.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0handoff-backfill-claim-stamp.py" %*
exit /b %ERRORLEVEL%
