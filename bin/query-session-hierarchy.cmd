@echo off
setlocal
REM Windows launcher for query-session-hierarchy.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "query-session-hierarchy.ps1" sibling of
REM     this file over this "query-session-hierarchy.cmd" file itself -- if both exist, `query-session-hierarchy`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "query-session-hierarchy.py"
...
echo [query-session-hierarchy] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [query-session-hierarchy] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0query-session-hierarchy.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0query-session-hierarchy.py" %*
exit /b %ERRORLEVEL%
