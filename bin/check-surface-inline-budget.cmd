@echo off
setlocal
REM Windows launcher for check-surface-inline-budget.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "check-surface-inline-budget.ps1" sibling of
REM     this file over this "check-surface-inline-budget.cmd" file itself -- if both exist, `check-surface-inline-budget`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "check-surface-inline-budget.py"
...
echo [check-surface-inline-budget] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [check-surface-inline-budget] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0check-surface-inline-budget.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0check-surface-inline-budget.py" %*
exit /b %ERRORLEVEL%
