@echo off
setlocal
REM Windows launcher for write-workday-start-marker.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "write-workday-start-marker.ps1" sibling of
REM     this file over this "write-workday-start-marker.cmd" file itself -- if both exist, `write-workday-start-marker`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "write-workday-start-marker.py"
...
echo [write-workday-start-marker] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [write-workday-start-marker] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0write-workday-start-marker.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0write-workday-start-marker.py" %*
exit /b %ERRORLEVEL%
