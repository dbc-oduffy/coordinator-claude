@echo off
setlocal
REM Windows launcher for validate-install-contract.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "validate-install-contract.ps1" sibling of
REM     this file over this "validate-install-contract.cmd" file itself -- if both exist, `validate-install-contract`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "validate-install-contract.py"
...
echo [validate-install-contract] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [validate-install-contract] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0validate-install-contract.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0validate-install-contract.py" %*
exit /b %ERRORLEVEL%
