# classify-env-var-callers.ps1 — python-direct Windows launcher for classify-env-var-callers.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "classify-env-var-callers.ps1" file over
#     any co-located "classify-env-var-callers.cmd" sibling -- `classify-env-var-callers` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "classify-env-var-callers.py"
...
$_entry = Join-Path $_here 'classify-env-var-callers.py'
...
[Console]::Error.WriteLine('[classify-env-var-callers] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[classify-env-var-callers] Install Python: https://www.python.org/downloads/windows/')
exit 127
