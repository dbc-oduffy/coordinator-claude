# sweep-installed-hook-name-refs.ps1 — python-direct Windows launcher for sweep-installed-hook-name-refs.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "sweep-installed-hook-name-refs.ps1" file over
#     any co-located "sweep-installed-hook-name-refs.cmd" sibling -- `sweep-installed-hook-name-refs` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "sweep-installed-hook-name-refs.py"
...
$_entry = Join-Path $_here 'sweep-installed-hook-name-refs.py'
...
[Console]::Error.WriteLine('[sweep-installed-hook-name-refs] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[sweep-installed-hook-name-refs] Install Python: https://www.python.org/downloads/windows/')
exit 127
