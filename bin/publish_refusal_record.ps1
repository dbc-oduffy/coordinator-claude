# publish_refusal_record.ps1 — python-direct Windows launcher for publish_refusal_record.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "publish_refusal_record.ps1" file over
#     any co-located "publish_refusal_record.cmd" sibling -- `publish_refusal_record` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "publish_refusal_record.py"
...
$_entry = Join-Path $_here 'publish_refusal_record.py'
...
[Console]::Error.WriteLine('[publish_refusal_record] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[publish_refusal_record] Install Python: https://www.python.org/downloads/windows/')
exit 127
