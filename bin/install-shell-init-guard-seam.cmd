@echo off
setlocal
REM Windows launcher for install-shell-init-guard-seam.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "install-shell-init-guard-seam.ps1" sibling of
REM     this file over this "install-shell-init-guard-seam.cmd" file itself -- if both exist, `install-shell-init-guard-seam`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "install-shell-init-guard-seam.py"
...
echo [install-shell-init-guard-seam] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [install-shell-init-guard-seam] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0install-shell-init-guard-seam.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0install-shell-init-guard-seam.py" %*
exit /b %ERRORLEVEL%
