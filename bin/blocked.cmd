@echo off
setlocal
REM Windows launcher for blocked.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "blocked.ps1" sibling of
REM     this file over this "blocked.cmd" file itself -- if both exist, `blocked`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "blocked.py"
...
echo [blocked] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [blocked] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
