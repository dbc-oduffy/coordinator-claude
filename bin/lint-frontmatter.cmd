@echo off
setlocal
REM Windows launcher for lint-frontmatter.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "lint-frontmatter.ps1" sibling of
REM     this file over this "lint-frontmatter.cmd" file itself -- if both exist, `lint-frontmatter`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "lint-frontmatter.py"
...
echo [lint-frontmatter] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [lint-frontmatter] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0lint-frontmatter.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0lint-frontmatter.py" %*
exit /b %ERRORLEVEL%
