# mise-prep-run.ps1 — python-direct Windows launcher for mise-prep-run.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "mise-prep-run.ps1" file over
#     any co-located "mise-prep-run.cmd" sibling -- `mise-prep-run` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "mise-prep-run.py"
...
$_entry = Join-Path $_here 'mise-prep-run.py'
...
[Console]::Error.WriteLine('[mise-prep-run] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[mise-prep-run] Install Python: https://www.python.org/downloads/windows/')
exit 127
