@echo off
setlocal
REM Windows launcher for workday-start-reconcile-sweep.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "workday-start-reconcile-sweep.ps1" sibling of
REM     this file over this "workday-start-reconcile-sweep.cmd" file itself -- if both exist, `workday-start-reconcile-sweep`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "workday-start-reconcile-sweep.py"
...
echo [workday-start-reconcile-sweep] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [workday-start-reconcile-sweep] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0workday-start-reconcile-sweep.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0workday-start-reconcile-sweep.py" %*
exit /b %ERRORLEVEL%
