# emit-cockpit-snapshot.ps1 — python-direct Windows launcher for emit-cockpit-snapshot.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "emit-cockpit-snapshot.ps1" file over
#     any co-located "emit-cockpit-snapshot.cmd" sibling -- `emit-cockpit-snapshot` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "emit-cockpit-snapshot.py"
...
$_entry = Join-Path $_here 'emit-cockpit-snapshot.py'
...
[Console]::Error.WriteLine('[emit-cockpit-snapshot] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[emit-cockpit-snapshot] Install Python: https://www.python.org/downloads/windows/')
exit 127
