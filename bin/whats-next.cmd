@echo off
setlocal
REM Windows launcher for whats-next.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "whats-next.ps1" sibling of
REM     this file over this "whats-next.cmd" file itself -- if both exist, `whats-next`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "whats-next.py"
...
echo [whats-next] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [whats-next] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0whats-next.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0whats-next.py" %*
exit /b %ERRORLEVEL%
