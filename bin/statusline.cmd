@echo off
setlocal
REM Windows launcher for statusline.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "statusline.ps1" sibling of
REM     this file over this "statusline.cmd" file itself -- if both exist, `statusline`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "statusline.py"
...
echo [statusline] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [statusline] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0statusline.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0statusline.py" %*
exit /b %ERRORLEVEL%
