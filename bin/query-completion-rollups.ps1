# query-completion-rollups.ps1 — python-direct Windows launcher for query-completion-rollups.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "query-completion-rollups.ps1" file over
#     any co-located "query-completion-rollups.cmd" sibling -- `query-completion-rollups` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "query-completion-rollups.py"
...
$_entry = Join-Path $_here 'query-completion-rollups.py'
...
[Console]::Error.WriteLine('[query-completion-rollups] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[query-completion-rollups] Install Python: https://www.python.org/downloads/windows/')
exit 127
