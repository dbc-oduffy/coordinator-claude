@echo off
setlocal
REM Windows launcher for migrate-lessons-md-to-yaml.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "migrate-lessons-md-to-yaml.ps1" sibling of
REM     this file over this "migrate-lessons-md-to-yaml.cmd" file itself -- if both exist, `migrate-lessons-md-to-yaml`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "migrate-lessons-md-to-yaml.py"
...
echo [migrate-lessons-md-to-yaml] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [migrate-lessons-md-to-yaml] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127

:run_baked
"%_py%" "%~dp0migrate-lessons-md-to-yaml.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0migrate-lessons-md-to-yaml.py" %*
exit /b %ERRORLEVEL%
