...
def test_schema_missing_fails_loud() -> None:
    """Test 7 — COORDINATOR_SCHEMAS_DIR pointing at an empty dir causes fail-loud exit.

    Verifies that coordinator-lesson-promote exits non-zero with a stderr message
    naming the schema problem when the lessons-outbox.yaml schema cannot be loaded.
    Uses COORDINATOR_SCHEMAS_DIR env var (honored by schema-cli.js) to redirect
    schema lookup to a temp dir that does not contain lessons-outbox.yaml.
    """
    name = "Test 7 — COORDINATOR_SCHEMAS_DIR missing schema → exit non-zero, stderr names schema error"
    with tempfile.TemporaryDirectory() as tmpdir:
        empty_schemas_dir = os.path.join(tmpdir, "schemas")
        os.makedirs(empty_schemas_dir)
        outbox = os.path.join(tmpdir, "state", "lessons-outbox")
        result = _run_cli(
            [
                "--title", "some title",
    ...
    if result.returncode == 0:
        fail_test(name, "expected non-zero exit when schema is missing; got 0")
        return
    combined = result.stdout + result.stderr
    if "schema" not in combined.lower():
        fail_test(name, f"error output does not mention 'schema'. stderr: {result.stderr!r}")
        return
    pass_test(name)
...
def main() -> int:
    ...
    test_schema_missing_fails_loud()
