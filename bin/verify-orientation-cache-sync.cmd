@echo off
setlocal
REM Windows launcher for verify-orientation-cache-sync.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "verify-orientation-cache-sync.ps1" sibling of
REM     this file over this "verify-orientation-cache-sync.cmd" file itself -- if both exist, `verify-orientation-cache-sync`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "verify-orientation-cache-sync.py"
...
echo [verify-orientation-cache-sync] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [verify-orientation-cache-sync] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0verify-orientation-cache-sync.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0verify-orientation-cache-sync.py" %*
exit /b %ERRORLEVEL%
