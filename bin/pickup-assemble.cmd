@echo off
setlocal
REM Windows launcher for pickup-assemble.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "pickup-assemble.ps1" sibling of
REM     this file over this "pickup-assemble.cmd" file itself -- if both exist, `pickup-assemble`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "pickup-assemble.py"
...
echo [pickup-assemble] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [pickup-assemble] Install Python: https://www.python.org/downloads/windows/ 1>&2
...
:run_baked
"%_py%" "%~dp0pickup-assemble.py" %*
2>nul rd /s /q "%_LAUNCHER_RAW_CMDLINE_DIR%"
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0pickup-assemble.py" %*
2>nul rd /s /q "%_LAUNCHER_RAW_CMDLINE_DIR%"
exit /b %ERRORLEVEL%
