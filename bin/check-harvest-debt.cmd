@echo off
setlocal
REM Windows launcher for check-harvest-debt.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "check-harvest-debt.ps1" sibling of
REM     this file over this "check-harvest-debt.cmd" file itself -- if both exist, `check-harvest-debt`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "check-harvest-debt.py"
...
echo [check-harvest-debt] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [check-harvest-debt] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0check-harvest-debt.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0check-harvest-debt.py" %*
exit /b %ERRORLEVEL%
