@echo off
setlocal
REM Windows launcher for coordinator-auto-push.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "coordinator-auto-push.ps1" sibling of
REM     this file over this "coordinator-auto-push.cmd" file itself -- if both exist, `coordinator-auto-push`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "coordinator-auto-push.py"
...
echo [coordinator-auto-push] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [coordinator-auto-push] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0coordinator-auto-push.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0coordinator-auto-push.py" %*
exit /b %ERRORLEVEL%
