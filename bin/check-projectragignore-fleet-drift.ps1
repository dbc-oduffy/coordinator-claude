# check-projectragignore-fleet-drift.ps1 — python-direct Windows launcher for check-projectragignore-fleet-drift.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "check-projectragignore-fleet-drift.ps1" file over
#     any co-located "check-projectragignore-fleet-drift.cmd" sibling -- `check-projectragignore-fleet-drift` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "check-projectragignore-fleet-drift.py"
...
$_entry = Join-Path $_here 'check-projectragignore-fleet-drift.py'
...
[Console]::Error.WriteLine('[check-projectragignore-fleet-drift] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[check-projectragignore-fleet-drift] Install Python: https://www.python.org/downloads/windows/')
exit 127
