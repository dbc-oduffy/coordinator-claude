# query-record-history.ps1 — python-direct Windows launcher for query-record-history.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "query-record-history.ps1" file over
#     any co-located "query-record-history.cmd" sibling -- `query-record-history` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "query-record-history.py"
...
$_entry = Join-Path $_here 'query-record-history.py'
...
[Console]::Error.WriteLine('[query-record-history] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[query-record-history] Install Python: https://www.python.org/downloads/windows/')
exit 127
