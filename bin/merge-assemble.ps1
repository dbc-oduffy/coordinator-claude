# merge-assemble.ps1 — python-direct Windows launcher for merge-assemble.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "merge-assemble.ps1" file over
#     any co-located "merge-assemble.cmd" sibling -- `merge-assemble` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "merge-assemble.py"
...
$_entry = Join-Path $_here 'merge-assemble.py'
...
[Console]::Error.WriteLine('[merge-assemble] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[merge-assemble] Install Python: https://www.python.org/downloads/windows/')
exit 127
