@echo off
setlocal
REM Windows launcher for mise-prep-run.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "mise-prep-run.ps1" sibling of
REM     this file over this "mise-prep-run.cmd" file itself -- if both exist, `mise-prep-run`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "mise-prep-run.py"
...
echo [mise-prep-run] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [mise-prep-run] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0mise-prep-run.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0mise-prep-run.py" %*
exit /b %ERRORLEVEL%
