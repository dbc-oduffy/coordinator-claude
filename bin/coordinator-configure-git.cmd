@echo off
setlocal
REM Windows launcher for coordinator-configure-git.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "coordinator-configure-git.ps1" sibling of
REM     this file over this "coordinator-configure-git.cmd" file itself -- if both exist, `coordinator-configure-git`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "coordinator-configure-git.py"
...
echo [coordinator-configure-git] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [coordinator-configure-git] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0coordinator-configure-git.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0coordinator-configure-git.py" %*
exit /b %ERRORLEVEL%
