@echo off
setlocal
REM Windows launcher for migrate-improvement-queue-universals.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "migrate-improvement-queue-universals.ps1" sibling of
REM     this file over this "migrate-improvement-queue-universals.cmd" file itself -- if both exist, `migrate-improvement-queue-universals`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "migrate-improvement-queue-universals.py"
...
echo [migrate-improvement-queue-universals] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [migrate-improvement-queue-universals] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127

:run_baked
"%_py%" "%~dp0migrate-improvement-queue-universals.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0migrate-improvement-queue-universals.py" %*
exit /b %ERRORLEVEL%
