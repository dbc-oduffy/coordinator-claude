# coordinator-doc-new.ps1 — python-direct Windows launcher for coordinator-doc-new.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "coordinator-doc-new.ps1" file over
#     any co-located "coordinator-doc-new.cmd" sibling -- `coordinator-doc-new` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "coordinator-doc-new.py"
...
$_entry = Join-Path $_here 'coordinator-doc-new.py'
...
[Console]::Error.WriteLine('[coordinator-doc-new] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[coordinator-doc-new] Install Python: https://www.python.org/downloads/windows/')
exit 127
