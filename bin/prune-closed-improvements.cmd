@echo off
setlocal
REM Windows launcher for prune-closed-improvements.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "prune-closed-improvements.ps1" sibling of
REM     this file over this "prune-closed-improvements.cmd" file itself -- if both exist, `prune-closed-improvements`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "prune-closed-improvements.py"
...
echo [prune-closed-improvements] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [prune-closed-improvements] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0prune-closed-improvements.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0prune-closed-improvements.py" %*
exit /b %ERRORLEVEL%
