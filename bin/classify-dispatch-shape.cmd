@echo off
setlocal
REM Windows launcher for classify-dispatch-shape.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "classify-dispatch-shape.ps1" sibling of
REM     this file over this "classify-dispatch-shape.cmd" file itself -- if both exist, `classify-dispatch-shape`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "classify-dispatch-shape.py"
...
echo [classify-dispatch-shape] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [classify-dispatch-shape] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0classify-dispatch-shape.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0classify-dispatch-shape.py" %*
exit /b %ERRORLEVEL%
