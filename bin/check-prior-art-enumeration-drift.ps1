# check-prior-art-enumeration-drift.ps1 — python-direct Windows launcher for check-prior-art-enumeration-drift.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "check-prior-art-enumeration-drift.ps1" file over
#     any co-located "check-prior-art-enumeration-drift.cmd" sibling -- `check-prior-art-enumeration-drift` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "check-prior-art-enumeration-drift.py"
...
$_entry = Join-Path $_here 'check-prior-art-enumeration-drift.py'
...
[Console]::Error.WriteLine('[check-prior-art-enumeration-drift] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[check-prior-art-enumeration-drift] Install Python: https://www.python.org/downloads/windows/')
exit 127
