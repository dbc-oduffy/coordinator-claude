@echo off
setlocal
REM Windows launcher for coordinator-safe-name.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "coordinator-safe-name.ps1" sibling of
REM     this file over this "coordinator-safe-name.cmd" file itself -- if both exist, `coordinator-safe-name`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "coordinator-safe-name.py"
...
echo [coordinator-safe-name] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [coordinator-safe-name] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0coordinator-safe-name.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0coordinator-safe-name.py" %*
exit /b %ERRORLEVEL%
