# verify-coverage.ps1 — python-direct Windows launcher for verify-coverage.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "verify-coverage.ps1" file over
#     any co-located "verify-coverage.cmd" sibling -- `verify-coverage` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "verify-coverage.py"
...
$_entry = Join-Path $_here 'verify-coverage.py'
...
[Console]::Error.WriteLine('[verify-coverage] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[verify-coverage] Install Python: https://www.python.org/downloads/windows/')
exit 127
