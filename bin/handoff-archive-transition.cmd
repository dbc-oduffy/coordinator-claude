@echo off
setlocal
REM Windows launcher for handoff-archive-transition.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "handoff-archive-transition.ps1" sibling of
REM     this file over this "handoff-archive-transition.cmd" file itself -- if both exist, `handoff-archive-transition`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "handoff-archive-transition.py"
...
echo [handoff-archive-transition] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [handoff-archive-transition] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0handoff-archive-transition.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0handoff-archive-transition.py" %*
exit /b %ERRORLEVEL%
