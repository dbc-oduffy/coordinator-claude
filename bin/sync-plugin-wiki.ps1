# sync-plugin-wiki.ps1 — python-direct Windows launcher for sync-plugin-wiki.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "sync-plugin-wiki.ps1" file over
#     any co-located "sync-plugin-wiki.cmd" sibling -- `sync-plugin-wiki` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "sync-plugin-wiki.py"
...
$_entry = Join-Path $_here 'sync-plugin-wiki.py'
...
[Console]::Error.WriteLine('[sync-plugin-wiki] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[sync-plugin-wiki] Install Python: https://www.python.org/downloads/windows/')
exit 127
