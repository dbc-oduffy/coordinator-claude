@echo off
setlocal
REM Windows launcher for close-out-and-stamp.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "close-out-and-stamp.ps1" sibling of
REM     this file over this "close-out-and-stamp.cmd" file itself -- if both exist, `close-out-and-stamp`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "close-out-and-stamp.py"
...
echo [close-out-and-stamp] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [close-out-and-stamp] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0close-out-and-stamp.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0close-out-and-stamp.py" %*
exit /b %ERRORLEVEL%
