@echo off
setlocal
REM Windows launcher for list-week-changelog.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "list-week-changelog.ps1" sibling of
REM     this file over this "list-week-changelog.cmd" file itself -- if both exist, `list-week-changelog`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "list-week-changelog.py"
...
echo [list-week-changelog] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [list-week-changelog] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0list-week-changelog.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0list-week-changelog.py" %*
exit /b %ERRORLEVEL%
