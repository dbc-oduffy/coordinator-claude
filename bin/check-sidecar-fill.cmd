@echo off
setlocal
REM Windows launcher for check-sidecar-fill.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "check-sidecar-fill.ps1" sibling of
REM     this file over this "check-sidecar-fill.cmd" file itself -- if both exist, `check-sidecar-fill`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "check-sidecar-fill.py"
...
echo [check-sidecar-fill] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [check-sidecar-fill] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0check-sidecar-fill.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0check-sidecar-fill.py" %*
exit /b %ERRORLEVEL%
