@echo off
setlocal
REM Windows launcher for group-em-watch-cli.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "group-em-watch-cli.ps1" sibling of
REM     this file over this "group-em-watch-cli.cmd" file itself -- if both exist, `group-em-watch-cli`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "group-em-watch-cli.py"
...
echo [group-em-watch-cli] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [group-em-watch-cli] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0group-em-watch-cli.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0group-em-watch-cli.py" %*
exit /b %ERRORLEVEL%
