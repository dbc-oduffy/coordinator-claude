@echo off
setlocal
REM Windows launcher for handoff-reconcile-close-terminal.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "handoff-reconcile-close-terminal.ps1" sibling of
REM     this file over this "handoff-reconcile-close-terminal.cmd" file itself -- if both exist, `handoff-reconcile-close-terminal`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "handoff-reconcile-close-terminal.py"
...
echo [handoff-reconcile-close-terminal] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [handoff-reconcile-close-terminal] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0handoff-reconcile-close-terminal.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0handoff-reconcile-close-terminal.py" %*
exit /b %ERRORLEVEL%
