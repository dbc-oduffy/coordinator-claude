@echo off
setlocal
REM Windows launcher for workday-complete-args-and-validate.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "workday-complete-args-and-validate.ps1" sibling of
REM     this file over this "workday-complete-args-and-validate.cmd" file itself -- if both exist, `workday-complete-args-and-validate`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "workday-complete-args-and-validate.py"
...
echo [workday-complete-args-and-validate] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [workday-complete-args-and-validate] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0workday-complete-args-and-validate.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0workday-complete-args-and-validate.py" %*
exit /b %ERRORLEVEL%
