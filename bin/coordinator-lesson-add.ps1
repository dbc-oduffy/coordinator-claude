# coordinator-lesson-add.ps1 — python-direct Windows launcher for coordinator-lesson-add.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "coordinator-lesson-add.ps1" file over
#     any co-located "coordinator-lesson-add.cmd" sibling -- `coordinator-lesson-add` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "coordinator-lesson-add.py"
...
$_entry = Join-Path $_here 'coordinator-lesson-add.py'
...
[Console]::Error.WriteLine('[coordinator-lesson-add] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[coordinator-lesson-add] Install Python: https://www.python.org/downloads/windows/')
exit 127
