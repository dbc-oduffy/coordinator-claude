@echo off
setlocal
REM Windows launcher for verify-doe-root-seam-sync.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "verify-doe-root-seam-sync.ps1" sibling of
REM     this file over this "verify-doe-root-seam-sync.cmd" file itself -- if both exist, `verify-doe-root-seam-sync`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "verify-doe-root-seam-sync.py"
...
echo [verify-doe-root-seam-sync] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [verify-doe-root-seam-sync] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0verify-doe-root-seam-sync.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0verify-doe-root-seam-sync.py" %*
exit /b %ERRORLEVEL%
