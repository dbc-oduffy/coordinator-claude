@echo off
setlocal
REM Windows launcher for orphan-branch-sweep.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "orphan-branch-sweep.ps1" sibling of
REM     this file over this "orphan-branch-sweep.cmd" file itself -- if both exist, `orphan-branch-sweep`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "orphan-branch-sweep.py"
...
echo [orphan-branch-sweep] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [orphan-branch-sweep] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0orphan-branch-sweep.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0orphan-branch-sweep.py" %*
exit /b %ERRORLEVEL%
