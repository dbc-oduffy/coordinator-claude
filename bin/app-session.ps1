# app-session.ps1 — python-direct Windows launcher for app-session.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "app-session.ps1" file over
#     any co-located "app-session.cmd" sibling -- `app-session` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "app-session.py"
...
$_entry = Join-Path $_here 'app-session.py'
...
[Console]::Error.WriteLine('[app-session] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[app-session] Install Python: https://www.python.org/downloads/windows/')
exit 127
