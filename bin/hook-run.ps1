# hook-run.ps1 — python-direct Windows launcher for hook-run.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "hook-run.ps1" file over
#     any co-located "hook-run.cmd" sibling -- `hook-run` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "hook-run.py"
...
$_entry = Join-Path $_here 'hook-run.py'
...
[Console]::Error.WriteLine('[hook-run] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[hook-run] Install Python: https://www.python.org/downloads/windows/')
exit 127
