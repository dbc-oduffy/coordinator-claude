@echo off
setlocal
REM Windows launcher for machine-local — python-direct (NO bash re-exec).
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "machine-local"
...
echo [machine-local] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [machine-local] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127

:run_baked
"%_py%" "%~dp0machine-local" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0machine-local" %*
exit /b %ERRORLEVEL%
