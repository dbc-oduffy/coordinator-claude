# quick-wrap-assemble.ps1 — python-direct Windows launcher for quick-wrap-assemble.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "quick-wrap-assemble.ps1" file over
#     any co-located "quick-wrap-assemble.cmd" sibling -- `quick-wrap-assemble` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "quick-wrap-assemble.py"
...
$_entry = Join-Path $_here 'quick-wrap-assemble.py'
...
[Console]::Error.WriteLine('[quick-wrap-assemble] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[quick-wrap-assemble] Install Python: https://www.python.org/downloads/windows/')
exit 127
