@echo off
setlocal
REM Windows launcher for generate-exec-summary.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "generate-exec-summary.ps1" sibling of
REM     this file over this "generate-exec-summary.cmd" file itself -- if both exist, `generate-exec-summary`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "generate-exec-summary.py"
...
echo [generate-exec-summary] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [generate-exec-summary] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0generate-exec-summary.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0generate-exec-summary.py" %*
exit /b %ERRORLEVEL%
