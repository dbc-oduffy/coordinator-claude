@echo off
setlocal
REM Windows launcher for workweek-complete-doc-verify.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "workweek-complete-doc-verify.ps1" sibling of
REM     this file over this "workweek-complete-doc-verify.cmd" file itself -- if both exist, `workweek-complete-doc-verify`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "workweek-complete-doc-verify.py"
...
echo [workweek-complete-doc-verify] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [workweek-complete-doc-verify] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0workweek-complete-doc-verify.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0workweek-complete-doc-verify.py" %*
exit /b %ERRORLEVEL%
