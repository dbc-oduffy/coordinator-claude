@echo off
setlocal
REM Windows launcher for refresh-roadmap-callout.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "refresh-roadmap-callout.ps1" sibling of
REM     this file over this "refresh-roadmap-callout.cmd" file itself -- if both exist, `refresh-roadmap-callout`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "refresh-roadmap-callout.py"
...
echo [refresh-roadmap-callout] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [refresh-roadmap-callout] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0refresh-roadmap-callout.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0refresh-roadmap-callout.py" %*
exit /b %ERRORLEVEL%
