@echo off
setlocal
REM Windows launcher for generate-repomap.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "generate-repomap.ps1" sibling of
REM     this file over this "generate-repomap.cmd" file itself -- if both exist, `generate-repomap`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "generate-repomap.py"
...
echo [generate-repomap] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [generate-repomap] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0generate-repomap.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0generate-repomap.py" %*
exit /b %ERRORLEVEL%
