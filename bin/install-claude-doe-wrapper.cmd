@echo off
setlocal
REM Windows launcher for install-claude-doe-wrapper.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "install-claude-doe-wrapper.ps1" sibling of
REM     this file over this "install-claude-doe-wrapper.cmd" file itself -- if both exist, `install-claude-doe-wrapper`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "install-claude-doe-wrapper.py"
...
echo [install-claude-doe-wrapper] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [install-claude-doe-wrapper] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0install-claude-doe-wrapper.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0install-claude-doe-wrapper.py" %*
exit /b %ERRORLEVEL%
