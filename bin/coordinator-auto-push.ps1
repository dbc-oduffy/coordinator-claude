# coordinator-auto-push.ps1 — python-direct Windows launcher for coordinator-auto-push.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "coordinator-auto-push.ps1" file over
#     any co-located "coordinator-auto-push.cmd" sibling -- `coordinator-auto-push` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "coordinator-auto-push.py"
...
$_entry = Join-Path $_here 'coordinator-auto-push.py'
...
[Console]::Error.WriteLine('[coordinator-auto-push] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[coordinator-auto-push] Install Python: https://www.python.org/downloads/windows/')
exit 127
