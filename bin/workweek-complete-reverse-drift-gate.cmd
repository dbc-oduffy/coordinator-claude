@echo off
setlocal
REM Windows launcher for workweek-complete-reverse-drift-gate.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "workweek-complete-reverse-drift-gate.ps1" sibling of
REM     this file over this "workweek-complete-reverse-drift-gate.cmd" file itself -- if both exist, `workweek-complete-reverse-drift-gate`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "workweek-complete-reverse-drift-gate.py"
...
echo [workweek-complete-reverse-drift-gate] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [workweek-complete-reverse-drift-gate] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0workweek-complete-reverse-drift-gate.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0workweek-complete-reverse-drift-gate.py" %*
exit /b %ERRORLEVEL%
