@echo off
setlocal
REM Windows launcher for break_glass.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "break_glass.ps1" sibling of
REM     this file over this "break_glass.cmd" file itself -- if both exist, `break_glass`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "break_glass.py"
...
echo [break_glass] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [break_glass] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0break_glass.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0break_glass.py" %*
exit /b %ERRORLEVEL%
