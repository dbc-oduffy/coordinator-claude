# aggregate-rollup.ps1 — python-direct Windows launcher for aggregate-rollup.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "aggregate-rollup.ps1" file over
#     any co-located "aggregate-rollup.cmd" sibling -- `aggregate-rollup` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "aggregate-rollup.py"
...
$_entry = Join-Path $_here 'aggregate-rollup.py'
...
[Console]::Error.WriteLine('[aggregate-rollup] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[aggregate-rollup] Install Python: https://www.python.org/downloads/windows/')
exit 127
