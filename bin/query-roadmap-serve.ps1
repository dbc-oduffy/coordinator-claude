# query-roadmap-serve.ps1 — python-direct Windows launcher for query-roadmap-serve.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "query-roadmap-serve.ps1" file over
#     any co-located "query-roadmap-serve.cmd" sibling -- `query-roadmap-serve` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "query-roadmap-serve.py"
...
$_entry = Join-Path $_here 'query-roadmap-serve.py'
...
[Console]::Error.WriteLine('[query-roadmap-serve] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[query-roadmap-serve] Install Python: https://www.python.org/downloads/windows/')
exit 127
