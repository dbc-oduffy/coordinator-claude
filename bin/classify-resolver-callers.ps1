# classify-resolver-callers.ps1 — python-direct Windows launcher for classify-resolver-callers.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "classify-resolver-callers.ps1" file over
#     any co-located "classify-resolver-callers.cmd" sibling -- `classify-resolver-callers` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "classify-resolver-callers.py"
...
$_entry = Join-Path $_here 'classify-resolver-callers.py'
...
[Console]::Error.WriteLine('[classify-resolver-callers] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[classify-resolver-callers] Install Python: https://www.python.org/downloads/windows/')
exit 127
