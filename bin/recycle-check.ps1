# recycle-check.ps1 — python-direct Windows launcher for recycle-check.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "recycle-check.ps1" file over
#     any co-located "recycle-check.cmd" sibling -- `recycle-check` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "recycle-check.py"
...
$_entry = Join-Path $_here 'recycle-check.py'
...
[Console]::Error.WriteLine('[recycle-check] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[recycle-check] Install Python: https://www.python.org/downloads/windows/')
exit 127
