@echo off
setlocal
REM Windows launcher for check-plugin-drift.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "check-plugin-drift.ps1" sibling of
REM     this file over this "check-plugin-drift.cmd" file itself -- if both exist, `check-plugin-drift`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "check-plugin-drift.py"
...
echo [check-plugin-drift] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [check-plugin-drift] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0check-plugin-drift.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0check-plugin-drift.py" %*
exit /b %ERRORLEVEL%
