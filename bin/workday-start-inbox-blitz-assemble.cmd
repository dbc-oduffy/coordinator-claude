@echo off
setlocal
REM Windows launcher for workday-start-inbox-blitz-assemble.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "workday-start-inbox-blitz-assemble.ps1" sibling of
REM     this file over this "workday-start-inbox-blitz-assemble.cmd" file itself -- if both exist, `workday-start-inbox-blitz-assemble`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "workday-start-inbox-blitz-assemble.py"
...
echo [workday-start-inbox-blitz-assemble] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [workday-start-inbox-blitz-assemble] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0workday-start-inbox-blitz-assemble.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0workday-start-inbox-blitz-assemble.py" %*
exit /b %ERRORLEVEL%
