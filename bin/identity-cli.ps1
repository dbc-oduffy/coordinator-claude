# identity-cli.ps1 — python-direct Windows launcher for identity-cli.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "identity-cli.ps1" file over
#     any co-located "identity-cli.cmd" sibling -- `identity-cli` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "identity-cli.py"
...
$_entry = Join-Path $_here 'identity-cli.py'
...
[Console]::Error.WriteLine('[identity-cli] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[identity-cli] Install Python: https://www.python.org/downloads/windows/')
exit 127
