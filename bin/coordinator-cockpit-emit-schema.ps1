# coordinator-cockpit-emit-schema.ps1 — python-direct Windows launcher for coordinator-cockpit-emit-schema.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "coordinator-cockpit-emit-schema.ps1" file over
#     any co-located "coordinator-cockpit-emit-schema.cmd" sibling -- `coordinator-cockpit-emit-schema` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "coordinator-cockpit-emit-schema.py"
...
$_entry = Join-Path $_here 'coordinator-cockpit-emit-schema.py'
...
[Console]::Error.WriteLine('[coordinator-cockpit-emit-schema] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[coordinator-cockpit-emit-schema] Install Python: https://www.python.org/downloads/windows/')
exit 127
