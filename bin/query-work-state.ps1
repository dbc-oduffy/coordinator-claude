# query-work-state.ps1 — python-direct Windows launcher for query-work-state.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "query-work-state.ps1" file over
#     any co-located "query-work-state.cmd" sibling -- `query-work-state` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "query-work-state.py"
...
$_entry = Join-Path $_here 'query-work-state.py'
...
[Console]::Error.WriteLine('[query-work-state] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[query-work-state] Install Python: https://www.python.org/downloads/windows/')
exit 127
