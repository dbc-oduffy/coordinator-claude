@echo off
setlocal
REM Windows launcher for workday-complete-step9-append-changelog.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "workday-complete-step9-append-changelog.ps1" sibling of
REM     this file over this "workday-complete-step9-append-changelog.cmd" file itself -- if both exist, `workday-complete-step9-append-changelog`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "workday-complete-step9-append-changelog.py"
...
echo [workday-complete-step9-append-changelog] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [workday-complete-step9-append-changelog] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0workday-complete-step9-append-changelog.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0workday-complete-step9-append-changelog.py" %*
exit /b %ERRORLEVEL%
