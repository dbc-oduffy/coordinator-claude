@echo off
setlocal
REM Windows launcher for migrate-completion-log-legacy.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "migrate-completion-log-legacy.ps1" sibling of
REM     this file over this "migrate-completion-log-legacy.cmd" file itself -- if both exist, `migrate-completion-log-legacy`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "migrate-completion-log-legacy.py"
...
echo [migrate-completion-log-legacy] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [migrate-completion-log-legacy] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127

:run_baked
"%_py%" "%~dp0migrate-completion-log-legacy.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0migrate-completion-log-legacy.py" %*
exit /b %ERRORLEVEL%
