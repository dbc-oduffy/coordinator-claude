@echo off
setlocal
REM Windows launcher for coordinator-queue-close — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "coordinator-queue-close.ps1" sibling of
REM     this file over this "coordinator-queue-close.cmd" file itself -- if both exist, `coordinator-queue-close`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "coordinator-queue-close"
...
echo [coordinator-queue-close] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [coordinator-queue-close] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0coordinator-queue-close" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0coordinator-queue-close" %*
exit /b %ERRORLEVEL%
