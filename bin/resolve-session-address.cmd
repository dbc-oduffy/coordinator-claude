@echo off
setlocal
REM Windows launcher for resolve-session-address.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "resolve-session-address.ps1" sibling of
REM     this file over this "resolve-session-address.cmd" file itself -- if both exist, `resolve-session-address`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "resolve-session-address.py"
...
echo [resolve-session-address] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [resolve-session-address] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0resolve-session-address.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0resolve-session-address.py" %*
exit /b %ERRORLEVEL%
