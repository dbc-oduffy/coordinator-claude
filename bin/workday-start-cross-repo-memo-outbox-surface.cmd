@echo off
setlocal
REM Windows launcher for workday-start-cross-repo-memo-outbox-surface.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "workday-start-cross-repo-memo-outbox-surface.ps1" sibling of
REM     this file over this "workday-start-cross-repo-memo-outbox-surface.cmd" file itself -- if both exist, `workday-start-cross-repo-memo-outbox-surface`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "workday-start-cross-repo-memo-outbox-surface.py"
...
echo [workday-start-cross-repo-memo-outbox-surface] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [workday-start-cross-repo-memo-outbox-surface] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0workday-start-cross-repo-memo-outbox-surface.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0workday-start-cross-repo-memo-outbox-surface.py" %*
exit /b %ERRORLEVEL%
