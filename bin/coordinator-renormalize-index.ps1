# coordinator-renormalize-index.ps1 — python-direct Windows launcher for coordinator-renormalize-index.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "coordinator-renormalize-index.ps1" file over
#     any co-located "coordinator-renormalize-index.cmd" sibling -- `coordinator-renormalize-index` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "coordinator-renormalize-index.py"
...
$_entry = Join-Path $_here 'coordinator-renormalize-index.py'
...
[Console]::Error.WriteLine('[coordinator-renormalize-index] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[coordinator-renormalize-index] Install Python: https://www.python.org/downloads/windows/')
exit 127
