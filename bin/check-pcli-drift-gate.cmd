@echo off
setlocal
REM Windows launcher for check-pcli-drift-gate.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "check-pcli-drift-gate.ps1" sibling of
REM     this file over this "check-pcli-drift-gate.cmd" file itself -- if both exist, `check-pcli-drift-gate`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "check-pcli-drift-gate.py"
...
echo [check-pcli-drift-gate] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [check-pcli-drift-gate] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0check-pcli-drift-gate.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0check-pcli-drift-gate.py" %*
exit /b %ERRORLEVEL%
