@echo off
setlocal
REM Windows launcher for workday-complete-reconcile.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "workday-complete-reconcile.ps1" sibling of
REM     this file over this "workday-complete-reconcile.cmd" file itself -- if both exist, `workday-complete-reconcile`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "workday-complete-reconcile.py"
...
echo [workday-complete-reconcile] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [workday-complete-reconcile] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0workday-complete-reconcile.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0workday-complete-reconcile.py" %*
exit /b %ERRORLEVEL%
