# validate-code-comparison-record.ps1 — python-direct Windows launcher for validate-code-comparison-record.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "validate-code-comparison-record.ps1" file over
#     any co-located "validate-code-comparison-record.cmd" sibling -- `validate-code-comparison-record` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "validate-code-comparison-record.py"
...
$_entry = Join-Path $_here 'validate-code-comparison-record.py'
...
[Console]::Error.WriteLine('[validate-code-comparison-record] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[validate-code-comparison-record] Install Python: https://www.python.org/downloads/windows/')
exit 127
