@echo off
setlocal
REM Windows launcher for audit-enabled-plugins.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "audit-enabled-plugins.ps1" sibling of
REM     this file over this "audit-enabled-plugins.cmd" file itself -- if both exist, `audit-enabled-plugins`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "audit-enabled-plugins.py"
...
echo [audit-enabled-plugins] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [audit-enabled-plugins] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0audit-enabled-plugins.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0audit-enabled-plugins.py" %*
exit /b %ERRORLEVEL%
