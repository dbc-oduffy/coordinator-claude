# migrate-cross-repo-layout.ps1 — python-direct Windows launcher for migrate-cross-repo-layout.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "migrate-cross-repo-layout.ps1" file over
#     any co-located "migrate-cross-repo-layout.cmd" sibling -- `migrate-cross-repo-layout` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "migrate-cross-repo-layout.py"
...
$_entry = Join-Path $_here 'migrate-cross-repo-layout.py'
...
[Console]::Error.WriteLine('[migrate-cross-repo-layout] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[migrate-cross-repo-layout] Install Python: https://www.python.org/downloads/windows/')
exit 127
