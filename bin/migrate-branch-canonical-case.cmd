@echo off
setlocal
REM Windows launcher for migrate-branch-canonical-case.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "migrate-branch-canonical-case.ps1" sibling of
REM     this file over this "migrate-branch-canonical-case.cmd" file itself -- if both exist, `migrate-branch-canonical-case`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "migrate-branch-canonical-case.py"
...
echo [migrate-branch-canonical-case] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [migrate-branch-canonical-case] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127

:run_baked
"%_py%" "%~dp0migrate-branch-canonical-case.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0migrate-branch-canonical-case.py" %*
exit /b %ERRORLEVEL%
