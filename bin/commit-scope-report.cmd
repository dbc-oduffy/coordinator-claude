@echo off
setlocal
REM Windows launcher for commit-scope-report.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "commit-scope-report.ps1" sibling of
REM     this file over this "commit-scope-report.cmd" file itself -- if both exist, `commit-scope-report`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "commit-scope-report.py"
...
echo [commit-scope-report] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [commit-scope-report] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0commit-scope-report.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0commit-scope-report.py" %*
exit /b %ERRORLEVEL%
