@echo off
setlocal
REM Windows launcher for coordinator-workflow-scaffold.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "coordinator-workflow-scaffold.ps1" sibling of
REM     this file over this "coordinator-workflow-scaffold.cmd" file itself -- if both exist, `coordinator-workflow-scaffold`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "coordinator-workflow-scaffold.py"
...
echo [coordinator-workflow-scaffold] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [coordinator-workflow-scaffold] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0coordinator-workflow-scaffold.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0coordinator-workflow-scaffold.py" %*
exit /b %ERRORLEVEL%
