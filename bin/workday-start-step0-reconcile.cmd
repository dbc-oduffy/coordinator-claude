@echo off
setlocal
REM Windows launcher for workday-start-step0-reconcile.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "workday-start-step0-reconcile.ps1" sibling of
REM     this file over this "workday-start-step0-reconcile.cmd" file itself -- if both exist, `workday-start-step0-reconcile`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "workday-start-step0-reconcile.py"
...
echo [workday-start-step0-reconcile] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [workday-start-step0-reconcile] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0workday-start-step0-reconcile.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0workday-start-step0-reconcile.py" %*
exit /b %ERRORLEVEL%
