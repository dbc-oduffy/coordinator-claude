@echo off
setlocal
REM Windows launcher for autonomous-verb.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "autonomous-verb.ps1" sibling of
REM     this file over this "autonomous-verb.cmd" file itself -- if both exist, `autonomous-verb`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "autonomous-verb.py"
...
echo [autonomous-verb] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [autonomous-verb] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0autonomous-verb.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0autonomous-verb.py" %*
exit /b %ERRORLEVEL%
