@echo off
setlocal
REM Windows launcher for memo-outbox-tracking-guard.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "memo-outbox-tracking-guard.ps1" sibling of
REM     this file over this "memo-outbox-tracking-guard.cmd" file itself -- if both exist, `memo-outbox-tracking-guard`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "memo-outbox-tracking-guard.py"
...
echo [memo-outbox-tracking-guard] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [memo-outbox-tracking-guard] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0memo-outbox-tracking-guard.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0memo-outbox-tracking-guard.py" %*
exit /b %ERRORLEVEL%
