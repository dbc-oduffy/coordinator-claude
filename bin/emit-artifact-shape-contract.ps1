# emit-artifact-shape-contract.ps1 — python-direct Windows launcher for emit-artifact-shape-contract.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "emit-artifact-shape-contract.ps1" file over
#     any co-located "emit-artifact-shape-contract.cmd" sibling -- `emit-artifact-shape-contract` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "emit-artifact-shape-contract.py"
...
$_entry = Join-Path $_here 'emit-artifact-shape-contract.py'
...
[Console]::Error.WriteLine('[emit-artifact-shape-contract] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[emit-artifact-shape-contract] Install Python: https://www.python.org/downloads/windows/')
exit 127
