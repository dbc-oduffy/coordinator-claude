@echo off
setlocal
REM Windows launcher for install-sentinel-write.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "install-sentinel-write.ps1" sibling of
REM     this file over this "install-sentinel-write.cmd" file itself -- if both exist, `install-sentinel-write`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "install-sentinel-write.py"
...
echo [install-sentinel-write] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [install-sentinel-write] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0install-sentinel-write.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0install-sentinel-write.py" %*
exit /b %ERRORLEVEL%
