# cmd-autorun-guard.ps1 — python-direct Windows launcher for cmd-autorun-guard.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "cmd-autorun-guard.ps1" file over
#     any co-located "cmd-autorun-guard.cmd" sibling -- `cmd-autorun-guard` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "cmd-autorun-guard.py"
...
$_entry = Join-Path $_here 'cmd-autorun-guard.py'
...
[Console]::Error.WriteLine('[cmd-autorun-guard] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[cmd-autorun-guard] Install Python: https://www.python.org/downloads/windows/')
exit 127
