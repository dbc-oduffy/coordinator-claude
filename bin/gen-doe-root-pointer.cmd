@echo off
setlocal
REM Windows launcher for gen-doe-root-pointer.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "gen-doe-root-pointer.ps1" sibling of
REM     this file over this "gen-doe-root-pointer.cmd" file itself -- if both exist, `gen-doe-root-pointer`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "gen-doe-root-pointer.py"
...
echo [gen-doe-root-pointer] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [gen-doe-root-pointer] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0gen-doe-root-pointer.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0gen-doe-root-pointer.py" %*
exit /b %ERRORLEVEL%
