@echo off
setlocal
REM Windows launcher for generate-tested-platforms.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "generate-tested-platforms.ps1" sibling of
REM     this file over this "generate-tested-platforms.cmd" file itself -- if both exist, `generate-tested-platforms`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "generate-tested-platforms.py"
...
echo [generate-tested-platforms] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [generate-tested-platforms] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0generate-tested-platforms.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0generate-tested-platforms.py" %*
exit /b %ERRORLEVEL%
