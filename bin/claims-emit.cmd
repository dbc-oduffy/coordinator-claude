@echo off
setlocal
REM Windows launcher for claims-emit.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "claims-emit.ps1" sibling of
REM     this file over this "claims-emit.cmd" file itself -- if both exist, `claims-emit`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "claims-emit.py"
...
echo [claims-emit] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [claims-emit] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0claims-emit.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0claims-emit.py" %*
exit /b %ERRORLEVEL%
