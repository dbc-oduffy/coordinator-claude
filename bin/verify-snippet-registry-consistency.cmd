@echo off
setlocal
REM Windows launcher for verify-snippet-registry-consistency.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "verify-snippet-registry-consistency.ps1" sibling of
REM     this file over this "verify-snippet-registry-consistency.cmd" file itself -- if both exist, `verify-snippet-registry-consistency`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "verify-snippet-registry-consistency.py"
...
echo [verify-snippet-registry-consistency] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [verify-snippet-registry-consistency] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0verify-snippet-registry-consistency.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0verify-snippet-registry-consistency.py" %*
exit /b %ERRORLEVEL%
