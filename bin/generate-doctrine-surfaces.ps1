# generate-doctrine-surfaces.ps1 — python-direct Windows launcher for generate-doctrine-surfaces.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "generate-doctrine-surfaces.ps1" file over
#     any co-located "generate-doctrine-surfaces.cmd" sibling -- `generate-doctrine-surfaces` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "generate-doctrine-surfaces.py"
...
$_entry = Join-Path $_here 'generate-doctrine-surfaces.py'
...
[Console]::Error.WriteLine('[generate-doctrine-surfaces] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[generate-doctrine-surfaces] Install Python: https://www.python.org/downloads/windows/')
exit 127
