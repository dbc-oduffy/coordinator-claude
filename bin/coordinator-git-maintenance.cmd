@echo off
setlocal
REM Windows launcher for coordinator-git-maintenance.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "coordinator-git-maintenance.ps1" sibling of
REM     this file over this "coordinator-git-maintenance.cmd" file itself -- if both exist, `coordinator-git-maintenance`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "coordinator-git-maintenance.py"
...
echo [coordinator-git-maintenance] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [coordinator-git-maintenance] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0coordinator-git-maintenance.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0coordinator-git-maintenance.py" %*
exit /b %ERRORLEVEL%
