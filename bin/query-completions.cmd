@echo off
setlocal
REM Windows launcher for query-completions.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "query-completions.ps1" sibling of
REM     this file over this "query-completions.cmd" file itself -- if both exist, `query-completions`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "query-completions.py"
...
echo [query-completions] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [query-completions] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0query-completions.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0query-completions.py" %*
exit /b %ERRORLEVEL%
