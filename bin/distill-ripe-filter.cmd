@echo off
setlocal
REM Windows launcher for distill-ripe-filter.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "distill-ripe-filter.ps1" sibling of
REM     this file over this "distill-ripe-filter.cmd" file itself -- if both exist, `distill-ripe-filter`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "distill-ripe-filter.py"
...
echo [distill-ripe-filter] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [distill-ripe-filter] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0distill-ripe-filter.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0distill-ripe-filter.py" %*
exit /b %ERRORLEVEL%
