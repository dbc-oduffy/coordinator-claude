...
# Source the settings-home seam for machine-local dir resolution (C2b reader repoint).
# Provides _coordinator_settings_home() used by _ML_DIR below.
# Spec backlink: docs/plans/2026-07-06-durable-substrate-to-settings-home.md § C2b
# shellcheck source=../lib/settings-home.sh
source "${_SCRIPT_DIR}/../lib/settings-home.sh" || {
  echo "[coordinator-doctor] ERROR: failed to source settings-home.sh" >&2
  exit 2
}
...
# Capture original CLAUDE_HOME before reassignment so the P-18 subprocess sees the
# HOME-substitute value, not the .claude-suffixed one we set below.
# Empty string is intentional when CLAUDE_HOME is unset: the sub-script uses
# ${CLAUDE_HOME:-$HOME}/.claude which resolves to HOME when CLAUDE_HOME is empty
# (bash :-  substitutes when null OR unset, so empty string triggers the fallback).
...
# Settings-home bin dir — the primary resolver location after C4 relocation.
# BIN_DIR ($CLAUDE_HOME/bin) is the retained compat forwarder location (phase-2 gated removal).
# Spec backlink: docs/plans/2026-07-06-durable-substrate-to-settings-home.md § C6
_SH="$(_coordinator_settings_home)"
_SH_BIN="${_SH}/bin"
...
# note_advisory: surfaces an informational note that is EXPLICITLY EXCLUDED from
# RED/AMBER verdict computation. Use for advisory-only probes (e.g. P-19 release
# currency) where the condition is informational, never a health failure.
note_advisory() { advisory_notes+=("$1: $2"); }

# Resolve the machine-local directory via the settings-home seam (C2b reader repoint).
# MACHINE_LOCAL_REGISTRY_DIR is rung-1 (direct dir override); otherwise derive from
# _coordinator_settings_home() (settings-home.sh, already sourced above).
# P-1 uses _ML_DIR for the dir existence check; C6 will repoint P-2..Pn's paths.
# Spec backlink: docs/plans/2026-07-06-durable-substrate-to-settings-home.md § C2b
if [[ -n "${MACHINE_LOCAL_REGISTRY_DIR:-}" ]]; then
  _ML_DIR="$MACHINE_LOCAL_REGISTRY_DIR"
else
  _ML_DIR="$(_coordinator_settings_home)/machine-local"
fi

# Resolve machine-local CLI once (P-3 and P-4 both use it).
# Settings-home bin is the primary post-C4 location; BIN_DIR is the retained compat forwarder.
ml_cmd=""
if [[ -x "$_SH_BIN/machine-local" ]]; then
  ml_cmd="$_SH_BIN/machine-local"
elif [[ -x "$BIN_DIR/machine-local" ]]; then
...
# Settings-home bin is the primary post-C4 location; BIN_DIR is the retained compat forwarder.
ch_cmd=""
if [[ -x "$_SH_BIN/claude-home" ]]; then
  ch_cmd="$_SH_BIN/claude-home"
elif [[ -x "$BIN_DIR/claude-home" ]]; then
...
# --- P-1: machine-local/ directory exists (resolved via settings-home seam) ----------
# Path is _ML_DIR (settings-home-based, or MACHINE_LOCAL_REGISTRY_DIR override) not the
# legacy $CLAUDE_HOME/machine-local literal. C6 will repoint P-2..Pn's paths.
...
# Resolved via _ML_DIR (settings-home-based, or MACHINE_LOCAL_REGISTRY_DIR override) not the
# legacy $CLAUDE_HOME/machine-local literal. Spec backlink: § C6.
if is_active "P-2"; then
  if [[ -f "$_ML_DIR/registry.toml" ]]; then
    if ! DOCTOR_REG="$_ML_DIR/registry.toml" "$PY" -c "
import os, pathlib
try:
    import tomllib
except ImportError:
...
# Primary location is $_SH/bin (settings-home); compat forwarder at ~/.claude/bin is retained.
if is_active "P-4"; then
  if [[ -n "$ml_cmd" ]]; then
    if ! "$ml_cmd" keys >/dev/null 2>&1; then
      note_red "P-4" "machine-local CLI failed — registry.toml unparseable; re-run /coordinator:install Phase 3 (primary resolver: $_SH/bin; compat forwarder also at ~/.claude/bin during transition)"
    fi
  else
    note_red "P-4" "machine-local CLI not found on PATH — re-run /coordinator:install Phase 3 (on macOS, $_SH/bin is reached via the coordinator/bin forwarder, not PATH directly; compat forwarder also at ~/.claude/bin)"
  fi
fi
...
if is_active "P-5"; then
  if ! _whoami_importable; then
    note_red "P-5" "coordinator_whoami not importable under $_PY_IDENT — venv is now at $_SH/.coordinator-venv/ (run bin/ensure-coordinator-venv.sh to rebuild); editable installs are per-interpreter — install under THIS interpreter, or set COORDINATOR_PYTHON to the one that has it"
  fi
fi
...
# Check settings-home bin (primary post-C4) then compat ~/.claude/bin forwarder.
if is_active "P-9"; then
  _p9_cmd=""
  if [[ -x "$_SH_BIN/verify-ue-overrides.sh" ]]; then
    _p9_cmd="$_SH_BIN/verify-ue-overrides.sh"
  elif [[ -x "$BIN_DIR/verify-ue-overrides.sh" ]]; then
    _p9_cmd="$BIN_DIR/verify-ue-overrides.sh"
  fi
  if [[ -n "$_p9_cmd" ]]; then
    if ! "$_p9_cmd" >/dev/null 2>&1; then
      note_amber "P-9" "verify-ue-overrides.sh emitted remediation — check repos.example_game_workbench_repo"
    fi
  fi
fi
...
if is_active "P-10"; then
  if [[ -n "$ch_cmd" ]]; then
    ...
    note_red "P-10" "claude-home resolver not found — re-run /coordinator:install Phase 3 (on macOS, $_SH/bin is the primary resolver location reached via the coordinator/bin forwarder; compat forwarder also at ~/.claude/bin during transition)"
  fi
fi
...
if is_active "P-11"; then
  ...
  if [[ -x "$_p11_script" ]]; then
    if ! bash "$_p11_script" >/dev/null 2>&1; then
      note_amber "P-11" "templates/setup drift detected — run verify-templates-setup-sync.sh (no flags, inspect-only) to inspect; recover manually with cp coordinator/templates/setup/<file> ~/.claude/setup/<file>"
    fi
  fi
fi
...
if is_active "P-13"; then
  ...
  if [[ -x "$_p13_script" ]]; then
    # $CLAUDE_HOME here is already the .claude-suffixed meta-repo root (reassigned
    # above at line 194); pass it as the explicit repo_root override to bypass the
    # probe's F14 guard (which rejects a CLAUDE_HOME already ending in /.claude).
    _p13_out="$(COORDINATOR_CURRENCY_REPO_ROOT="$CLAUDE_HOME" \
...
# --- P-19: release currency advisory (git-tag anchor vs installed version) -------
# spec-backlink: docs/plans/2026-06-30-chain-review-coverage-dag-consumer.md § P-19
# Advisory-only: uses note_advisory (NOT note_amber / note_red) so the 3-color
# verdict is NEVER affected regardless of probe result. triage=false in the manifest
# ensures the selector never includes P-19 in bare/--triage runs (network I/O is
# unacceptable in the always-on cheap first-pass). --cluster currency / --probe P-19
# / --full only.
#
# Reuses lib/release-currency.sh exclusively (no duplicate currency mechanism).
# Checks coordinator-claude; per-plugin action hints match the boot hook's
# _action_hint_for() for consistency.
#
# Result mapping:
#   source_is_live  → silent GREEN (authoring machine / registry-matched git clone)
#   current         → silent GREEN
#   offline         → silent inconclusive skip (NEVER contributes RED)
#   behind / differs / behind-clone → note_advisory (informational; verdict unchanged)
if is_active "P-19"; then
  _p19_release_lib="${_SCRIPT_DIR}/../lib/release-currency.sh"
  _p19_lib_ok=0
  if [[ -f "$_p19_release_lib" ]]; then
    # shellcheck source=../lib/release-currency.sh
    if source "$_p19_release_lib" 2>/dev/null; then
      _p19_lib_ok=1
    fi
  fi

  if [[ "$_p19_lib_ok" -eq 1 ]]; then
    # Install roots: mirror the boot hook's PLUGIN_ROOT derivation.
    # Sentinel is at coordinator/bin/; coordinator install root is coordinator/.
    _p19_coordinator_root="$(cd "${_SCRIPT_DIR}/.." && pwd)"

    # Per-plugin action hint — mirrors boot hook's _action_hint_for() for consistency.
    _p19_action_hint() {
      case "$1" in
    ...
    for _p19_entry in \
        "coordinator|dbc-oduffy/coordinator-claude|${_p19_coordinator_root}"; do
      IFS='|' read -r _p19_plugin _p19_repo _p19_root <<< "$_p19_entry"
      _p19_result="$(release_currency_probe "$_p19_plugin" "$_p19_repo" "$_p19_root" 2>/dev/null)" || _p19_result="offline"
      [[ -z "$_p19_result" ]] && _p19_result="offline"
      case "$_p19_result" in
        current|source_is_live)
          : # silent GREEN
          ;;
        offline)
          : # silent inconclusive skip — NEVER RED
          ;;
        behind-clone*)
          _p19_n="$(printf '%s' "$_p19_result"   | awk '{print $2}')"
          _p19_ref="$(printf '%s' "$_p19_result" | awk '{print $3}')"
          # Review: code-reviewer — F1: mirror boot hook behind-clone wording exactly (git pull + install path)
          note_advisory "P-19" "${_p19_plugin} clone is ${_p19_n} commits behind ${_p19_ref} — git pull in ${_p19_root} to update"
          ;;
        behind*)
          _p19_from="$(printf '%s' "$_p19_result" | awk '{print $2}')"
          _p19_to="$(printf '%s' "$_p19_result"   | awk '{print $3}')"
          note_advisory "P-19" "${_p19_plugin} ${_p19_from}→${_p19_to} behind latest release — $(_p19_action_hint "$_p19_plugin")"
          ;;
        differs*)
          _p19_to="$(printf '%s' "$_p19_result" | awk '{print $2}')"
          note_advisory "P-19" "${_p19_plugin} differs from latest release (${_p19_to}) — $(_p19_action_hint "$_p19_plugin")"
          ;;
        *)
          : # unknown result — silent (treat as offline)
          ;;
      esac
    done
  else
    # Review: code-reviewer — F3: lib-absent is intentionally a visible advisory (broken/partial-install
    # diagnostic), distinct from the offline→silent mapping. Do NOT silence this path.
    note_advisory "P-19" "release-currency lib absent or failed to source — release currency check skipped"
  fi
fi
...
if [[ "$MODE" == "full" ]]; then
  ...
  if ! RED_PROBES_CSV="$(IFS=,; echo "${red_probes[*]:-}")" \
       ...
       ADVISORY_NOTES_JSON="$(printf '%s\n' "${advisory_notes[@]:-}" | "$PY" -c "import json,sys; print(json.dumps([l for l in sys.stdin.read().splitlines() if l]))" 2>/dev/null || echo '[]')" \
       RAN_AT="$ran_at" \
       VERDICT="$verdict" \
       HINT="$hint" \
...
# advisory_notes are informational only — explicitly NOT counted in error/amber.
# Review: code-reviewer — F4: JSON-serialized (was newline-join) to handle arbitrary note content.
advisory = json.loads(os.environ.get('ADVISORY_NOTES_JSON', '[]'))
try:
...
" 2>/dev/null; then
  ...
  # Surface advisory notes (informational; labelled INFO so they are distinct from
  # RED/AMBER probe failures — advisory notes never drive a verdict change).
  if [[ "${#advisory_notes[@]}" -gt 0 ]]; then
    echo "[coordinator-doctor] INFO (advisory — verdict unchanged):"
    for _adv in "${advisory_notes[@]}"; do
      echo "  [INFO] $_adv"
    done
  fi
  ...
  # Advisory notes are informational only — labelled INFO, never affect verdict.
  if [[ "${#advisory_notes[@]}" -gt 0 ]]; then
    echo "  Advisory (informational — verdict unchanged):"
    for _adv in "${advisory_notes[@]}"; do
      echo "    [INFO] $_adv"
    done
  fi
