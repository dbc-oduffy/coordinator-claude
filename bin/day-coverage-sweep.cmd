@echo off
setlocal
REM Windows launcher for day-coverage-sweep.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "day-coverage-sweep.ps1" sibling of
REM     this file over this "day-coverage-sweep.cmd" file itself -- if both exist, `day-coverage-sweep`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "day-coverage-sweep.py"
...
echo [day-coverage-sweep] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [day-coverage-sweep] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0day-coverage-sweep.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0day-coverage-sweep.py" %*
exit /b %ERRORLEVEL%
