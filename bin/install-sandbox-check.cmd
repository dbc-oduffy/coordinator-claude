@echo off
setlocal
REM Windows launcher for install-sandbox-check.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "install-sandbox-check.ps1" sibling of
REM     this file over this "install-sandbox-check.cmd" file itself -- if both exist, `install-sandbox-check`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "install-sandbox-check.py"
...
echo [install-sandbox-check] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [install-sandbox-check] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0install-sandbox-check.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0install-sandbox-check.py" %*
exit /b %ERRORLEVEL%
