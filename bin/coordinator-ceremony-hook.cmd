@echo off
setlocal
REM Windows launcher for coordinator-ceremony-hook.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "coordinator-ceremony-hook.ps1" sibling of
REM     this file over this "coordinator-ceremony-hook.cmd" file itself -- if both exist, `coordinator-ceremony-hook`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "coordinator-ceremony-hook.py"
...
echo [coordinator-ceremony-hook] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [coordinator-ceremony-hook] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0coordinator-ceremony-hook.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0coordinator-ceremony-hook.py" %*
exit /b %ERRORLEVEL%
