@echo off
setlocal
REM Windows launcher for query-handoff-columns.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "query-handoff-columns.ps1" sibling of
REM     this file over this "query-handoff-columns.cmd" file itself -- if both exist, `query-handoff-columns`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "query-handoff-columns.py"
...
echo [query-handoff-columns] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [query-handoff-columns] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0query-handoff-columns.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0query-handoff-columns.py" %*
exit /b %ERRORLEVEL%
