@echo off
setlocal
REM Windows launcher for percolate-sweep-scope-probe.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "percolate-sweep-scope-probe.ps1" sibling of
REM     this file over this "percolate-sweep-scope-probe.cmd" file itself -- if both exist, `percolate-sweep-scope-probe`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "percolate-sweep-scope-probe.py"
...
echo [percolate-sweep-scope-probe] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [percolate-sweep-scope-probe] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0percolate-sweep-scope-probe.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0percolate-sweep-scope-probe.py" %*
exit /b %ERRORLEVEL%
