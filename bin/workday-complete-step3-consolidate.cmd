@echo off
setlocal
REM Windows launcher for workday-complete-step3-consolidate.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "workday-complete-step3-consolidate.ps1" sibling of
REM     this file over this "workday-complete-step3-consolidate.cmd" file itself -- if both exist, `workday-complete-step3-consolidate`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "workday-complete-step3-consolidate.py"
...
echo [workday-complete-step3-consolidate] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [workday-complete-step3-consolidate] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0workday-complete-step3-consolidate.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0workday-complete-step3-consolidate.py" %*
exit /b %ERRORLEVEL%
