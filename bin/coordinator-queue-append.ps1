# coordinator-queue-append.ps1 — python-direct Windows launcher for coordinator-queue-append.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "coordinator-queue-append.ps1" file over
#     any co-located "coordinator-queue-append.cmd" sibling -- `coordinator-queue-append` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "coordinator-queue-append.py"
...
$_entry = Join-Path $_here 'coordinator-queue-append.py'
...
[Console]::Error.WriteLine('[coordinator-queue-append] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[coordinator-queue-append] Install Python: https://www.python.org/downloads/windows/')
exit 127
