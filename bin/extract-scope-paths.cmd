@echo off
setlocal
REM Windows launcher for extract-scope-paths.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "extract-scope-paths.ps1" sibling of
REM     this file over this "extract-scope-paths.cmd" file itself -- if both exist, `extract-scope-paths`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "extract-scope-paths.py"
...
echo [extract-scope-paths] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [extract-scope-paths] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0extract-scope-paths.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0extract-scope-paths.py" %*
exit /b %ERRORLEVEL%
