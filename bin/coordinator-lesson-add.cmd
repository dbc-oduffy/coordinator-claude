@echo off
setlocal
REM Windows launcher for coordinator-lesson-add.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "coordinator-lesson-add.ps1" sibling of
REM     this file over this "coordinator-lesson-add.cmd" file itself -- if both exist, `coordinator-lesson-add`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "coordinator-lesson-add.py"
...
echo [coordinator-lesson-add] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [coordinator-lesson-add] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0coordinator-lesson-add.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0coordinator-lesson-add.py" %*
exit /b %ERRORLEVEL%
