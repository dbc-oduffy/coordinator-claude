@echo off
setlocal
REM Windows launcher for check-doctrine-citations.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "check-doctrine-citations.ps1" sibling of
REM     this file over this "check-doctrine-citations.cmd" file itself -- if both exist, `check-doctrine-citations`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "check-doctrine-citations.py"
...
echo [check-doctrine-citations] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [check-doctrine-citations] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0check-doctrine-citations.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0check-doctrine-citations.py" %*
exit /b %ERRORLEVEL%
