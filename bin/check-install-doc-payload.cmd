@echo off
setlocal
REM Windows launcher for check-install-doc-payload.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "check-install-doc-payload.ps1" sibling of
REM     this file over this "check-install-doc-payload.cmd" file itself -- if both exist, `check-install-doc-payload`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "check-install-doc-payload.py"
...
echo [check-install-doc-payload] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [check-install-doc-payload] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0check-install-doc-payload.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0check-install-doc-payload.py" %*
exit /b %ERRORLEVEL%
