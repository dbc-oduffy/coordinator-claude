@echo off
setlocal
REM Windows launcher for advance-tracker-status.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "advance-tracker-status.ps1" sibling of
REM     this file over this "advance-tracker-status.cmd" file itself -- if both exist, `advance-tracker-status`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "advance-tracker-status.py"
...
echo [advance-tracker-status] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [advance-tracker-status] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0advance-tracker-status.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0advance-tracker-status.py" %*
exit /b %ERRORLEVEL%
