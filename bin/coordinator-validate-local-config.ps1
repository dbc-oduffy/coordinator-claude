# coordinator-validate-local-config.ps1 — python-direct Windows launcher for coordinator-validate-local-config.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "coordinator-validate-local-config.ps1" file over
#     any co-located "coordinator-validate-local-config.cmd" sibling -- `coordinator-validate-local-config` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "coordinator-validate-local-config.py"
...
$_entry = Join-Path $_here 'coordinator-validate-local-config.py'
...
[Console]::Error.WriteLine('[coordinator-validate-local-config] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[coordinator-validate-local-config] Install Python: https://www.python.org/downloads/windows/')
exit 127
