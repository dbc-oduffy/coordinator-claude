@echo off
setlocal
REM Windows launcher for coordinator-fold-execution-record.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "coordinator-fold-execution-record.ps1" sibling of
REM     this file over this "coordinator-fold-execution-record.cmd" file itself -- if both exist, `coordinator-fold-execution-record`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "coordinator-fold-execution-record.py"
...
echo [coordinator-fold-execution-record] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [coordinator-fold-execution-record] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0coordinator-fold-execution-record.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0coordinator-fold-execution-record.py" %*
exit /b %ERRORLEVEL%
