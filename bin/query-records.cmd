@echo off
setlocal
REM Windows launcher for query-records.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "query-records.ps1" sibling of
REM     this file over this "query-records.cmd" file itself -- if both exist, `query-records`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "query-records.py"
...
echo [query-records] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [query-records] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0query-records.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0query-records.py" %*
exit /b %ERRORLEVEL%
