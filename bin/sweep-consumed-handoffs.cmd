@echo off
setlocal
REM Windows launcher for sweep-consumed-handoffs.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "sweep-consumed-handoffs.ps1" sibling of
REM     this file over this "sweep-consumed-handoffs.cmd" file itself -- if both exist, `sweep-consumed-handoffs`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "sweep-consumed-handoffs.py"
...
echo [sweep-consumed-handoffs] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [sweep-consumed-handoffs] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0sweep-consumed-handoffs.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0sweep-consumed-handoffs.py" %*
exit /b %ERRORLEVEL%
