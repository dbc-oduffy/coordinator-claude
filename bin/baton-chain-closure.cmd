@echo off
setlocal
REM Windows launcher for baton-chain-closure.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "baton-chain-closure.ps1" sibling of
REM     this file over this "baton-chain-closure.cmd" file itself -- if both exist, `baton-chain-closure`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "baton-chain-closure.py"
...
echo [baton-chain-closure] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [baton-chain-closure] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0baton-chain-closure.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0baton-chain-closure.py" %*
exit /b %ERRORLEVEL%
