@echo off
setlocal
REM Windows launcher for check-multi-event-hook-hardcoded-event.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "check-multi-event-hook-hardcoded-event.ps1" sibling of
REM     this file over this "check-multi-event-hook-hardcoded-event.cmd" file itself -- if both exist, `check-multi-event-hook-hardcoded-event`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "check-multi-event-hook-hardcoded-event.py"
...
echo [check-multi-event-hook-hardcoded-event] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [check-multi-event-hook-hardcoded-event] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0check-multi-event-hook-hardcoded-event.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0check-multi-event-hook-hardcoded-event.py" %*
exit /b %ERRORLEVEL%
