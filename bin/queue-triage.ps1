# queue-triage.ps1 — python-direct Windows launcher for queue-triage.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "queue-triage.ps1" file over
#     any co-located "queue-triage.cmd" sibling -- `queue-triage` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "queue-triage.py"
...
$_entry = Join-Path $_here 'queue-triage.py'
...
[Console]::Error.WriteLine('[queue-triage] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[queue-triage] Install Python: https://www.python.org/downloads/windows/')
exit 127
