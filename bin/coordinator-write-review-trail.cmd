@echo off
setlocal
REM Windows launcher for coordinator-write-review-trail.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "coordinator-write-review-trail.ps1" sibling of
REM     this file over this "coordinator-write-review-trail.cmd" file itself -- if both exist, `coordinator-write-review-trail`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "coordinator-write-review-trail.py"
...
echo [coordinator-write-review-trail] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [coordinator-write-review-trail] Install Python: https://www.python.org/downloads/windows/ 1>&2
...
:run_baked
"%_py%" "%~dp0coordinator-write-review-trail.py" %*
2>nul rd /s /q "%_LAUNCHER_RAW_CMDLINE_DIR%"
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0coordinator-write-review-trail.py" %*
2>nul rd /s /q "%_LAUNCHER_RAW_CMDLINE_DIR%"
exit /b %ERRORLEVEL%
