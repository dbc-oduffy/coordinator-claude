# check-version-consistency.ps1 — python-direct Windows launcher for check-version-consistency.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "check-version-consistency.ps1" file over
#     any co-located "check-version-consistency.cmd" sibling -- `check-version-consistency` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "check-version-consistency.py"
...
$_entry = Join-Path $_here 'check-version-consistency.py'
...
[Console]::Error.WriteLine('[check-version-consistency] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[check-version-consistency] Install Python: https://www.python.org/downloads/windows/')
exit 127
