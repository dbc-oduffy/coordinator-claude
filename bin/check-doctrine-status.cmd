@echo off
setlocal
REM Windows launcher for check-doctrine-status.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "check-doctrine-status.ps1" sibling of
REM     this file over this "check-doctrine-status.cmd" file itself -- if both exist, `check-doctrine-status`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "check-doctrine-status.py"
...
echo [check-doctrine-status] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [check-doctrine-status] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0check-doctrine-status.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0check-doctrine-status.py" %*
exit /b %ERRORLEVEL%
