@echo off
setlocal
REM Windows launcher for wait-for-count.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "wait-for-count.ps1" sibling of
REM     this file over this "wait-for-count.cmd" file itself -- if both exist, `wait-for-count`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "wait-for-count.py"
...
echo [wait-for-count] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [wait-for-count] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0wait-for-count.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0wait-for-count.py" %*
exit /b %ERRORLEVEL%
