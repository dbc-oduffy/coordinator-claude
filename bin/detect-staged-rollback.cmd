@echo off
setlocal
REM Windows launcher for detect-staged-rollback.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "detect-staged-rollback.ps1" sibling of
REM     this file over this "detect-staged-rollback.cmd" file itself -- if both exist, `detect-staged-rollback`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "detect-staged-rollback.py"
...
echo [detect-staged-rollback] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [detect-staged-rollback] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0detect-staged-rollback.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0detect-staged-rollback.py" %*
exit /b %ERRORLEVEL%
