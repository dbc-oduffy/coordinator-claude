@echo off
setlocal
REM Windows launcher for plan-capture-persist.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "plan-capture-persist.ps1" sibling of
REM     this file over this "plan-capture-persist.cmd" file itself -- if both exist, `plan-capture-persist`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "plan-capture-persist.py"
...
echo [plan-capture-persist] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [plan-capture-persist] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0plan-capture-persist.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0plan-capture-persist.py" %*
exit /b %ERRORLEVEL%
