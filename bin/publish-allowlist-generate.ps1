# publish-allowlist-generate.ps1 — python-direct Windows launcher for publish-allowlist-generate.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "publish-allowlist-generate.ps1" file over
#     any co-located "publish-allowlist-generate.cmd" sibling -- `publish-allowlist-generate` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "publish-allowlist-generate.py"
...
$_entry = Join-Path $_here 'publish-allowlist-generate.py'
...
[Console]::Error.WriteLine('[publish-allowlist-generate] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[publish-allowlist-generate] Install Python: https://www.python.org/downloads/windows/')
exit 127
