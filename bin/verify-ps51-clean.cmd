@echo off
setlocal
REM Windows launcher for verify-ps51-clean.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "verify-ps51-clean.ps1" sibling of
REM     this file over this "verify-ps51-clean.cmd" file itself -- if both exist, `verify-ps51-clean`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "verify-ps51-clean.py"
...
echo [verify-ps51-clean] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [verify-ps51-clean] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0verify-ps51-clean.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0verify-ps51-clean.py" %*
exit /b %ERRORLEVEL%
