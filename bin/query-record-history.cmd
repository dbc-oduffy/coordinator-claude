@echo off
setlocal
REM Windows launcher for query-record-history.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "query-record-history.ps1" sibling of
REM     this file over this "query-record-history.cmd" file itself -- if both exist, `query-record-history`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "query-record-history.py"
...
echo [query-record-history] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [query-record-history] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0query-record-history.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0query-record-history.py" %*
exit /b %ERRORLEVEL%
