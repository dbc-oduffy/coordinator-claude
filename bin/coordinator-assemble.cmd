@echo off
setlocal
REM Windows launcher for coordinator-assemble.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "coordinator-assemble.ps1" sibling of
REM     this file over this "coordinator-assemble.cmd" file itself -- if both exist, `coordinator-assemble`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "coordinator-assemble.py"
...
echo [coordinator-assemble] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [coordinator-assemble] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0coordinator-assemble.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0coordinator-assemble.py" %*
exit /b %ERRORLEVEL%
