# check-settings-env.ps1 — python-direct Windows launcher for check-settings-env.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "check-settings-env.ps1" file over
#     any co-located "check-settings-env.cmd" sibling -- `check-settings-env` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "check-settings-env.py"
...
$_entry = Join-Path $_here 'check-settings-env.py'
...
[Console]::Error.WriteLine('[check-settings-env] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[check-settings-env] Install Python: https://www.python.org/downloads/windows/')
exit 127
