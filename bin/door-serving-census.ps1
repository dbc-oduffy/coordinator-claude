# door-serving-census.ps1 — python-direct Windows launcher for door-serving-census.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "door-serving-census.ps1" file over
#     any co-located "door-serving-census.cmd" sibling -- `door-serving-census` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "door-serving-census.py"
...
$_entry = Join-Path $_here 'door-serving-census.py'
...
[Console]::Error.WriteLine('[door-serving-census] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[door-serving-census] Install Python: https://www.python.org/downloads/windows/')
exit 127
