@echo off
setlocal
REM Windows launcher for workday-complete-assemble.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "workday-complete-assemble.ps1" sibling of
REM     this file over this "workday-complete-assemble.cmd" file itself -- if both exist, `workday-complete-assemble`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "workday-complete-assemble.py"
...
echo [workday-complete-assemble] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [workday-complete-assemble] Install Python: https://www.python.org/downloads/windows/ 1>&2
...
:run_baked
"%_py%" "%~dp0workday-complete-assemble.py" %*
2>nul rd /s /q "%_LAUNCHER_RAW_CMDLINE_DIR%"
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0workday-complete-assemble.py" %*
2>nul rd /s /q "%_LAUNCHER_RAW_CMDLINE_DIR%"
exit /b %ERRORLEVEL%
