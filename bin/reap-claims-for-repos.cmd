@echo off
setlocal
REM Windows launcher for reap-claims-for-repos.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "reap-claims-for-repos.ps1" sibling of
REM     this file over this "reap-claims-for-repos.cmd" file itself -- if both exist, `reap-claims-for-repos`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "reap-claims-for-repos.py"
...
echo [reap-claims-for-repos] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [reap-claims-for-repos] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0reap-claims-for-repos.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0reap-claims-for-repos.py" %*
exit /b %ERRORLEVEL%
