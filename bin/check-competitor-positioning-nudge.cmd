@echo off
setlocal
REM Windows launcher for check-competitor-positioning-nudge.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "check-competitor-positioning-nudge.ps1" sibling of
REM     this file over this "check-competitor-positioning-nudge.cmd" file itself -- if both exist, `check-competitor-positioning-nudge`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "check-competitor-positioning-nudge.py"
...
echo [check-competitor-positioning-nudge] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [check-competitor-positioning-nudge] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0check-competitor-positioning-nudge.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0check-competitor-positioning-nudge.py" %*
exit /b %ERRORLEVEL%
