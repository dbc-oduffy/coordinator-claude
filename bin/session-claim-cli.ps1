# session-claim-cli.ps1 — python-direct Windows launcher for session-claim-cli.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "session-claim-cli.ps1" file over
#     any co-located "session-claim-cli.cmd" sibling -- `session-claim-cli` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "session-claim-cli.py"
...
$_entry = Join-Path $_here 'session-claim-cli.py'
...
[Console]::Error.WriteLine('[session-claim-cli] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[session-claim-cli] Install Python: https://www.python.org/downloads/windows/')
exit 127
