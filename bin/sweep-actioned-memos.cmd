@echo off
setlocal
REM Windows launcher for sweep-actioned-memos.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "sweep-actioned-memos.ps1" sibling of
REM     this file over this "sweep-actioned-memos.cmd" file itself -- if both exist, `sweep-actioned-memos`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "sweep-actioned-memos.py"
...
echo [sweep-actioned-memos] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [sweep-actioned-memos] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0sweep-actioned-memos.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0sweep-actioned-memos.py" %*
exit /b %ERRORLEVEL%
