# coordinator-ensure-post-commit-hook.ps1 — python-direct Windows launcher for coordinator-ensure-post-commit-hook.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "coordinator-ensure-post-commit-hook.ps1" file over
#     any co-located "coordinator-ensure-post-commit-hook.cmd" sibling -- `coordinator-ensure-post-commit-hook` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "coordinator-ensure-post-commit-hook.py"
...
$_entry = Join-Path $_here 'coordinator-ensure-post-commit-hook.py'
...
[Console]::Error.WriteLine('[coordinator-ensure-post-commit-hook] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[coordinator-ensure-post-commit-hook] Install Python: https://www.python.org/downloads/windows/')
exit 127
