# check-launch-shape.ps1 — python-direct Windows launcher for check-launch-shape.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "check-launch-shape.ps1" file over
#     any co-located "check-launch-shape.cmd" sibling -- `check-launch-shape` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "check-launch-shape.py"
...
$_entry = Join-Path $_here 'check-launch-shape.py'
...
[Console]::Error.WriteLine('[check-launch-shape] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[check-launch-shape] Install Python: https://www.python.org/downloads/windows/')
exit 127
