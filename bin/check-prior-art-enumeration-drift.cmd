@echo off
setlocal
REM Windows launcher for check-prior-art-enumeration-drift.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "check-prior-art-enumeration-drift.ps1" sibling of
REM     this file over this "check-prior-art-enumeration-drift.cmd" file itself -- if both exist, `check-prior-art-enumeration-drift`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "check-prior-art-enumeration-drift.py"
...
echo [check-prior-art-enumeration-drift] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [check-prior-art-enumeration-drift] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0check-prior-art-enumeration-drift.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0check-prior-art-enumeration-drift.py" %*
exit /b %ERRORLEVEL%
