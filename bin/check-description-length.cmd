@echo off
setlocal
REM Windows launcher for check-description-length.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "check-description-length.ps1" sibling of
REM     this file over this "check-description-length.cmd" file itself -- if both exist, `check-description-length`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "check-description-length.py"
...
echo [check-description-length] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [check-description-length] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0check-description-length.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0check-description-length.py" %*
exit /b %ERRORLEVEL%
