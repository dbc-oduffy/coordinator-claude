@echo off
setlocal
REM Windows launcher for cruft-sweep.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "cruft-sweep.ps1" sibling of
REM     this file over this "cruft-sweep.cmd" file itself -- if both exist, `cruft-sweep`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "cruft-sweep.py"
...
echo [cruft-sweep] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [cruft-sweep] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0cruft-sweep.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0cruft-sweep.py" %*
exit /b %ERRORLEVEL%
