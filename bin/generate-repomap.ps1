# generate-repomap.ps1 — python-direct Windows launcher for generate-repomap.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "generate-repomap.ps1" file over
#     any co-located "generate-repomap.cmd" sibling -- `generate-repomap` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "generate-repomap.py"
...
$_entry = Join-Path $_here 'generate-repomap.py'
...
[Console]::Error.WriteLine('[generate-repomap] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[generate-repomap] Install Python: https://www.python.org/downloads/windows/')
exit 127
