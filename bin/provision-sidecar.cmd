@echo off
setlocal
REM Windows launcher for provision-sidecar.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "provision-sidecar.ps1" sibling of
REM     this file over this "provision-sidecar.cmd" file itself -- if both exist, `provision-sidecar`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "provision-sidecar.py"
...
echo [provision-sidecar] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [provision-sidecar] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0provision-sidecar.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0provision-sidecar.py" %*
exit /b %ERRORLEVEL%
