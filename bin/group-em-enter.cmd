@echo off
setlocal
REM Windows launcher for group-em-enter.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "group-em-enter.ps1" sibling of
REM     this file over this "group-em-enter.cmd" file itself -- if both exist, `group-em-enter`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "group-em-enter.py"
...
echo [group-em-enter] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [group-em-enter] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0group-em-enter.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0group-em-enter.py" %*
exit /b %ERRORLEVEL%
