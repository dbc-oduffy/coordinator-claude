@echo off
setlocal
REM Windows launcher for coordinator-setup-state.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "coordinator-setup-state.ps1" sibling of
REM     this file over this "coordinator-setup-state.cmd" file itself -- if both exist, `coordinator-setup-state`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "coordinator-setup-state.py"
...
echo [coordinator-setup-state] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [coordinator-setup-state] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0coordinator-setup-state.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0coordinator-setup-state.py" %*
exit /b %ERRORLEVEL%
