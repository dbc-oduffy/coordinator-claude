# coordinator-install.ps1 — python-direct Windows launcher for coordinator-install.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "coordinator-install.ps1" file over
#     any co-located "coordinator-install.cmd" sibling -- `coordinator-install` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "coordinator-install.py"
...
$_entry = Join-Path $_here 'coordinator-install.py'
...
[Console]::Error.WriteLine('[coordinator-install] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[coordinator-install] Install Python: https://www.python.org/downloads/windows/')
exit 127
