# corpus-currency-probe.ps1 — python-direct Windows launcher for corpus-currency-probe.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "corpus-currency-probe.ps1" file over
#     any co-located "corpus-currency-probe.cmd" sibling -- `corpus-currency-probe` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "corpus-currency-probe.py"
...
$_entry = Join-Path $_here 'corpus-currency-probe.py'
...
[Console]::Error.WriteLine('[corpus-currency-probe] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[corpus-currency-probe] Install Python: https://www.python.org/downloads/windows/')
exit 127
