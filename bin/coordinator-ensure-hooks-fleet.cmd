@echo off
setlocal
REM Windows launcher for coordinator-ensure-hooks-fleet — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "coordinator-ensure-hooks-fleet.ps1" sibling of
REM     this file over this "coordinator-ensure-hooks-fleet.cmd" file itself -- if both exist, `coordinator-ensure-hooks-fleet`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "coordinator-ensure-hooks-fleet"
...
echo [coordinator-ensure-hooks-fleet] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [coordinator-ensure-hooks-fleet] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0coordinator-ensure-hooks-fleet" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0coordinator-ensure-hooks-fleet" %*
exit /b %ERRORLEVEL%
