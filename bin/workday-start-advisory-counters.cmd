@echo off
setlocal
REM Windows launcher for workday-start-advisory-counters.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "workday-start-advisory-counters.ps1" sibling of
REM     this file over this "workday-start-advisory-counters.cmd" file itself -- if both exist, `workday-start-advisory-counters`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "workday-start-advisory-counters.py"
...
echo [workday-start-advisory-counters] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [workday-start-advisory-counters] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0workday-start-advisory-counters.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0workday-start-advisory-counters.py" %*
exit /b %ERRORLEVEL%
