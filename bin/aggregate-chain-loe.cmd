@echo off
setlocal
REM Windows launcher for aggregate-chain-loe.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "aggregate-chain-loe.ps1" sibling of
REM     this file over this "aggregate-chain-loe.cmd" file itself -- if both exist, `aggregate-chain-loe`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "aggregate-chain-loe.py"
...
echo [aggregate-chain-loe] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [aggregate-chain-loe] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0aggregate-chain-loe.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0aggregate-chain-loe.py" %*
exit /b %ERRORLEVEL%
