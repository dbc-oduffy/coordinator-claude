@echo off
setlocal
REM Windows launcher for bug-sweep-probes.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "bug-sweep-probes.ps1" sibling of
REM     this file over this "bug-sweep-probes.cmd" file itself -- if both exist, `bug-sweep-probes`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "bug-sweep-probes.py"
...
echo [bug-sweep-probes] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [bug-sweep-probes] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0bug-sweep-probes.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0bug-sweep-probes.py" %*
exit /b %ERRORLEVEL%
