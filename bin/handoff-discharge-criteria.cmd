@echo off
setlocal
REM Windows launcher for handoff-discharge-criteria.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "handoff-discharge-criteria.ps1" sibling of
REM     this file over this "handoff-discharge-criteria.cmd" file itself -- if both exist, `handoff-discharge-criteria`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "handoff-discharge-criteria.py"
...
echo [handoff-discharge-criteria] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [handoff-discharge-criteria] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0handoff-discharge-criteria.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0handoff-discharge-criteria.py" %*
exit /b %ERRORLEVEL%
