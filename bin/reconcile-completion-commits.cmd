@echo off
setlocal
REM Windows launcher for reconcile-completion-commits.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "reconcile-completion-commits.ps1" sibling of
REM     this file over this "reconcile-completion-commits.cmd" file itself -- if both exist, `reconcile-completion-commits`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "reconcile-completion-commits.py"
...
echo [reconcile-completion-commits] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [reconcile-completion-commits] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0reconcile-completion-commits.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0reconcile-completion-commits.py" %*
exit /b %ERRORLEVEL%
