@echo off
setlocal
REM Windows launcher for hook-run.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "hook-run.ps1" sibling of
REM     this file over this "hook-run.cmd" file itself -- if both exist, `hook-run`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "hook-run.py"
...
echo [hook-run] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [hook-run] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
