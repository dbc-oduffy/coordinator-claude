@echo off
setlocal
REM Windows launcher for standup.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "standup.ps1" sibling of
REM     this file over this "standup.cmd" file itself -- if both exist, `standup`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "standup.py"
...
echo [standup] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [standup] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
