...
# Graceful-degradation: --subject doctrine fail-loud on non-DoE machines → WARN+skip.
_wn_q=""
if _wn_q="$(coordinator_state_root --central --subject doctrine 2>/dev/null)"; then
  QUEUE_DIR="${_wn_q}/improvement-queue"
else
  echo "  WARN: central doctrine state root unresolvable — skipping improvement queue (non-DoE machine)" >&2
  QUEUE_DIR=""
fi
if [[ -d "$QUEUE_DIR" ]]; then
