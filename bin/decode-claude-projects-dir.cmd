@echo off
setlocal
REM Windows launcher for decode-claude-projects-dir.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "decode-claude-projects-dir.ps1" sibling of
REM     this file over this "decode-claude-projects-dir.cmd" file itself -- if both exist, `decode-claude-projects-dir`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "decode-claude-projects-dir.py"
...
echo [decode-claude-projects-dir] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [decode-claude-projects-dir] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0decode-claude-projects-dir.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0decode-claude-projects-dir.py" %*
exit /b %ERRORLEVEL%
