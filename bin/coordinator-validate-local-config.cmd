@echo off
setlocal
REM Windows launcher for coordinator-validate-local-config.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "coordinator-validate-local-config.ps1" sibling of
REM     this file over this "coordinator-validate-local-config.cmd" file itself -- if both exist, `coordinator-validate-local-config`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "coordinator-validate-local-config.py"
...
echo [coordinator-validate-local-config] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [coordinator-validate-local-config] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0coordinator-validate-local-config.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0coordinator-validate-local-config.py" %*
exit /b %ERRORLEVEL%
