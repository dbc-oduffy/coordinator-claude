@echo off
setlocal
REM Windows launcher for assert-no-terminal-plans-in-live.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "assert-no-terminal-plans-in-live.ps1" sibling of
REM     this file over this "assert-no-terminal-plans-in-live.cmd" file itself -- if both exist, `assert-no-terminal-plans-in-live`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "assert-no-terminal-plans-in-live.py"
...
echo [assert-no-terminal-plans-in-live] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [assert-no-terminal-plans-in-live] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0assert-no-terminal-plans-in-live.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0assert-no-terminal-plans-in-live.py" %*
exit /b %ERRORLEVEL%
