# session-liveness-cli.ps1 — python-direct Windows launcher for session-liveness-cli.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "session-liveness-cli.ps1" file over
#     any co-located "session-liveness-cli.cmd" sibling -- `session-liveness-cli` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "session-liveness-cli.py"
...
$_entry = Join-Path $_here 'session-liveness-cli.py'
...
[Console]::Error.WriteLine('[session-liveness-cli] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[session-liveness-cli] Install Python: https://www.python.org/downloads/windows/')
exit 127
