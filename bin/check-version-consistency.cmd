@echo off
setlocal
REM Windows launcher for check-version-consistency.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "check-version-consistency.ps1" sibling of
REM     this file over this "check-version-consistency.cmd" file itself -- if both exist, `check-version-consistency`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "check-version-consistency.py"
...
echo [check-version-consistency] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [check-version-consistency] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0check-version-consistency.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0check-version-consistency.py" %*
exit /b %ERRORLEVEL%
