@echo off
setlocal
REM Windows launcher for backfill-initiative-fk.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "backfill-initiative-fk.ps1" sibling of
REM     this file over this "backfill-initiative-fk.cmd" file itself -- if both exist, `backfill-initiative-fk`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "backfill-initiative-fk.py"
...
echo [backfill-initiative-fk] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [backfill-initiative-fk] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127

:run_baked
"%_py%" "%~dp0backfill-initiative-fk.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0backfill-initiative-fk.py" %*
exit /b %ERRORLEVEL%
