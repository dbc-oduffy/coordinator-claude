# check-doctrine-status.ps1 — python-direct Windows launcher for check-doctrine-status.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "check-doctrine-status.ps1" file over
#     any co-located "check-doctrine-status.cmd" sibling -- `check-doctrine-status` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "check-doctrine-status.py"
...
$_entry = Join-Path $_here 'check-doctrine-status.py'
...
[Console]::Error.WriteLine('[check-doctrine-status] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[check-doctrine-status] Install Python: https://www.python.org/downloads/windows/')
exit 127
