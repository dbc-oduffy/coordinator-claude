# parse-resolves-trailer.ps1 — python-direct Windows launcher for parse-resolves-trailer.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "parse-resolves-trailer.ps1" file over
#     any co-located "parse-resolves-trailer.cmd" sibling -- `parse-resolves-trailer` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "parse-resolves-trailer.py"
...
$_entry = Join-Path $_here 'parse-resolves-trailer.py'
...
[Console]::Error.WriteLine('[parse-resolves-trailer] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[parse-resolves-trailer] Install Python: https://www.python.org/downloads/windows/')
exit 127
