@echo off
setlocal
REM Windows launcher for check-posix-exec-assumptions.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "check-posix-exec-assumptions.ps1" sibling of
REM     this file over this "check-posix-exec-assumptions.cmd" file itself -- if both exist, `check-posix-exec-assumptions`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "check-posix-exec-assumptions.py"
...
echo [check-posix-exec-assumptions] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [check-posix-exec-assumptions] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0check-posix-exec-assumptions.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0check-posix-exec-assumptions.py" %*
exit /b %ERRORLEVEL%
