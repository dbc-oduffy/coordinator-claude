@echo off
setlocal
REM Windows launcher for goal-kr-evidence.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "goal-kr-evidence.ps1" sibling of
REM     this file over this "goal-kr-evidence.cmd" file itself -- if both exist, `goal-kr-evidence`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "goal-kr-evidence.py"
...
echo [goal-kr-evidence] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [goal-kr-evidence] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0goal-kr-evidence.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0goal-kr-evidence.py" %*
exit /b %ERRORLEVEL%
