@echo off
setlocal
REM Windows launcher for tier-last-run.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "tier-last-run.ps1" sibling of
REM     this file over this "tier-last-run.cmd" file itself -- if both exist, `tier-last-run`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "tier-last-run.py"
...
echo [tier-last-run] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [tier-last-run] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0tier-last-run.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0tier-last-run.py" %*
exit /b %ERRORLEVEL%
