# learn-lessons-reconcile-candidates.ps1 — python-direct Windows launcher for learn-lessons-reconcile-candidates.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "learn-lessons-reconcile-candidates.ps1" file over
#     any co-located "learn-lessons-reconcile-candidates.cmd" sibling -- `learn-lessons-reconcile-candidates` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "learn-lessons-reconcile-candidates.py"
...
$_entry = Join-Path $_here 'learn-lessons-reconcile-candidates.py'
...
[Console]::Error.WriteLine('[learn-lessons-reconcile-candidates] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[learn-lessons-reconcile-candidates] Install Python: https://www.python.org/downloads/windows/')
exit 127
