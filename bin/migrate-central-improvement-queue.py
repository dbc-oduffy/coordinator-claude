...
def _normalise_from_repo(raw: str) -> str:
    ...
    Priority:
      ...
      3. Raw string as-is (preserves project-rag, example-game-workbench-repo, etc.).
    """
    stripped = raw.strip()
    ...
    if stripped.startswith("~/.claude"):
        return "claude-central-em"
    # Return raw — peer repos (project-rag, example-game-workbench-repo, etc.) keep their name.
    return stripped
...
def main() -> int:
    parser = argparse.ArgumentParser(
        ...
        description=(
            ...
            "Spec backlink: C2a of 2026-06-22-example-cockpit-repo-cockpit-contract plan."
        ),
    )
    parser.add_argument(
        "--input",
