# generate-claudemeta-manifest.ps1 — python-direct Windows launcher for generate-claudemeta-manifest.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "generate-claudemeta-manifest.ps1" file over
#     any co-located "generate-claudemeta-manifest.cmd" sibling -- `generate-claudemeta-manifest` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "generate-claudemeta-manifest.py"
...
$_entry = Join-Path $_here 'generate-claudemeta-manifest.py'
...
[Console]::Error.WriteLine('[generate-claudemeta-manifest] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[generate-claudemeta-manifest] Install Python: https://www.python.org/downloads/windows/')
exit 127
