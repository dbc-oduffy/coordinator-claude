@echo off
setlocal
REM Windows launcher for gen-settings-hooks.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "gen-settings-hooks.ps1" sibling of
REM     this file over this "gen-settings-hooks.cmd" file itself -- if both exist, `gen-settings-hooks`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "gen-settings-hooks.py"
...
echo [gen-settings-hooks] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [gen-settings-hooks] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0gen-settings-hooks.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0gen-settings-hooks.py" %*
exit /b %ERRORLEVEL%
