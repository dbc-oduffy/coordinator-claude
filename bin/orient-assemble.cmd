@echo off
setlocal
REM Windows launcher for orient-assemble.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "orient-assemble.ps1" sibling of
REM     this file over this "orient-assemble.cmd" file itself -- if both exist, `orient-assemble`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "orient-assemble.py"
...
echo [orient-assemble] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [orient-assemble] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0orient-assemble.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0orient-assemble.py" %*
exit /b %ERRORLEVEL%
