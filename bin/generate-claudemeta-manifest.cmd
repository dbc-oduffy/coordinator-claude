@echo off
setlocal
REM Windows launcher for generate-claudemeta-manifest.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "generate-claudemeta-manifest.ps1" sibling of
REM     this file over this "generate-claudemeta-manifest.cmd" file itself -- if both exist, `generate-claudemeta-manifest`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "generate-claudemeta-manifest.py"
...
echo [generate-claudemeta-manifest] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [generate-claudemeta-manifest] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0generate-claudemeta-manifest.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0generate-claudemeta-manifest.py" %*
exit /b %ERRORLEVEL%
