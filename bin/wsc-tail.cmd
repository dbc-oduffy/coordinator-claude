@echo off
setlocal
REM Windows launcher for wsc-tail.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "wsc-tail.ps1" sibling of
REM     this file over this "wsc-tail.cmd" file itself -- if both exist, `wsc-tail`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "wsc-tail.py"
...
echo [wsc-tail] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [wsc-tail] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
