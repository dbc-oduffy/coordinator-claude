@echo off
setlocal
REM Windows launcher for extract-lessons.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "extract-lessons.ps1" sibling of
REM     this file over this "extract-lessons.cmd" file itself -- if both exist, `extract-lessons`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "extract-lessons.py"
...
echo [extract-lessons] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [extract-lessons] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0extract-lessons.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0extract-lessons.py" %*
exit /b %ERRORLEVEL%
