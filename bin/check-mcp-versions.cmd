@echo off
setlocal
REM Windows launcher for check-mcp-versions.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "check-mcp-versions.ps1" sibling of
REM     this file over this "check-mcp-versions.cmd" file itself -- if both exist, `check-mcp-versions`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "check-mcp-versions.py"
...
echo [check-mcp-versions] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [check-mcp-versions] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0check-mcp-versions.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0check-mcp-versions.py" %*
exit /b %ERRORLEVEL%
