@echo off
setlocal
REM Windows launcher for quick-wrap-assemble.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "quick-wrap-assemble.ps1" sibling of
REM     this file over this "quick-wrap-assemble.cmd" file itself -- if both exist, `quick-wrap-assemble`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "quick-wrap-assemble.py"
...
echo [quick-wrap-assemble] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [quick-wrap-assemble] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0quick-wrap-assemble.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0quick-wrap-assemble.py" %*
exit /b %ERRORLEVEL%
