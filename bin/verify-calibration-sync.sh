...
if [[ ! -f "$_CS_LIB" ]]; then
    ...
    if [[ -n "$_doe_root" ]] && [[ -d "$_doe_root/coordinator" ]]; then
        _CS_LIB="${CLAUDE_PLUGIN_ROOT:-${_doe_root}/coordinator}/lib/coordinator-session.sh"
        # shellcheck source=../lib/coordinator-trusted-root-guard.sh
        source "$(cd "$(dirname "$0")/.." && pwd)/lib/coordinator-trusted-root-guard.sh"
        coordinator_trusted_root_guard --mode=fail-loud --root="$_CS_LIB" --site="$0"
    fi
fi
if [[ -f "$_CS_LIB" ]]; then
...
# --- consumer list sourced from snippet-registry CLI ---
# Spec backlink: docs/plans/2026-06-15-snippet-sync-consumer-registry.md § C5a
# The CLI owns all consumer resolution — $PLUGIN_ROOT-relative paths, sibling ../
# paths, and example-game-repo conditionals — so this script no longer needs EXAMPLE_GAME_REPO_ROOT
# or machine-local resolution.
...
mapfile -t HARDCODED_CONSUMERS < <("$REGISTRY_CLI" list-consumers reviewer-calibration)
...
# Review: the Staff Engineer R2 finding 0 — factor shared extraction primitive; replace inlined awk.
extract_block() {
    local file="$1"
