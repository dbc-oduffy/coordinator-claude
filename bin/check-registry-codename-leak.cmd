@echo off
setlocal
REM Windows launcher for check-registry-codename-leak.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "check-registry-codename-leak.ps1" sibling of
REM     this file over this "check-registry-codename-leak.cmd" file itself -- if both exist, `check-registry-codename-leak`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "check-registry-codename-leak.py"
...
echo [check-registry-codename-leak] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [check-registry-codename-leak] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0check-registry-codename-leak.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0check-registry-codename-leak.py" %*
exit /b %ERRORLEVEL%
