@echo off
setlocal
REM Windows launcher for resolve-peer-address.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "resolve-peer-address.ps1" sibling of
REM     this file over this "resolve-peer-address.cmd" file itself -- if both exist, `resolve-peer-address`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "resolve-peer-address.py"
...
echo [resolve-peer-address] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [resolve-peer-address] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0resolve-peer-address.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0resolve-peer-address.py" %*
exit /b %ERRORLEVEL%
