# coordinator-queue-close.ps1 — python-direct Windows launcher for coordinator-queue-close (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "coordinator-queue-close.ps1" file over
#     any co-located "coordinator-queue-close.cmd" sibling -- `coordinator-queue-close` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "coordinator-queue-close"
...
$_entry = Join-Path $_here 'coordinator-queue-close'
...
[Console]::Error.WriteLine('[coordinator-queue-close] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[coordinator-queue-close] Install Python: https://www.python.org/downloads/windows/')
exit 127
