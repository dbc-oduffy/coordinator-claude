# triage-plan-candidates.ps1 — python-direct Windows launcher for triage-plan-candidates.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "triage-plan-candidates.ps1" file over
#     any co-located "triage-plan-candidates.cmd" sibling -- `triage-plan-candidates` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "triage-plan-candidates.py"
...
$_entry = Join-Path $_here 'triage-plan-candidates.py'
...
[Console]::Error.WriteLine('[triage-plan-candidates] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[triage-plan-candidates] Install Python: https://www.python.org/downloads/windows/')
exit 127
