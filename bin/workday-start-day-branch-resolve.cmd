@echo off
setlocal
REM Windows launcher for workday-start-day-branch-resolve.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "workday-start-day-branch-resolve.ps1" sibling of
REM     this file over this "workday-start-day-branch-resolve.cmd" file itself -- if both exist, `workday-start-day-branch-resolve`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "workday-start-day-branch-resolve.py"
...
echo [workday-start-day-branch-resolve] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [workday-start-day-branch-resolve] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0workday-start-day-branch-resolve.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0workday-start-day-branch-resolve.py" %*
exit /b %ERRORLEVEL%
