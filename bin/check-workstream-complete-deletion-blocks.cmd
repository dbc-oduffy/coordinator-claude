@echo off
setlocal
REM Windows launcher for check-workstream-complete-deletion-blocks.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "check-workstream-complete-deletion-blocks.ps1" sibling of
REM     this file over this "check-workstream-complete-deletion-blocks.cmd" file itself -- if both exist, `check-workstream-complete-deletion-blocks`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "check-workstream-complete-deletion-blocks.py"
...
echo [check-workstream-complete-deletion-blocks] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [check-workstream-complete-deletion-blocks] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0check-workstream-complete-deletion-blocks.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0check-workstream-complete-deletion-blocks.py" %*
exit /b %ERRORLEVEL%
