@echo off
setlocal
REM Windows launcher for check-deferral-orphan-memo.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "check-deferral-orphan-memo.ps1" sibling of
REM     this file over this "check-deferral-orphan-memo.cmd" file itself -- if both exist, `check-deferral-orphan-memo`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "check-deferral-orphan-memo.py"
...
echo [check-deferral-orphan-memo] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [check-deferral-orphan-memo] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0check-deferral-orphan-memo.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0check-deferral-orphan-memo.py" %*
exit /b %ERRORLEVEL%
