@echo off
setlocal
REM Windows launcher for new-project-scaffold.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "new-project-scaffold.ps1" sibling of
REM     this file over this "new-project-scaffold.cmd" file itself -- if both exist, `new-project-scaffold`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "new-project-scaffold.py"
...
echo [new-project-scaffold] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [new-project-scaffold] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0new-project-scaffold.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0new-project-scaffold.py" %*
exit /b %ERRORLEVEL%
