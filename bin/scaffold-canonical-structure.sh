    ...
    if in_readme_block:
        ...
        if term_new_entry or term_toplevel or term_entry_field or term_comment_bleed:
            ...
            if term_new_entry:
                ...
                current = {"path": path_val, "creation": "", "schema": None, "gitkeep": False, "readme": None, "template": None}
                continue
            elif term_toplevel:
...
for entry in entries:
    ...
    # Emit eager directory entries (path ends with /) and eager file entries
    # (path does NOT end with / and carries a template: field).
    if creation != "eager":
        continue
    is_dir = path.endswith("/")
    is_file = not is_dir and bool(template)
    if not is_dir and not is_file:
        continue

    # Transport: encode for tab-delimited line transport.
    # Column 1: path
    # Column 2: gitkeep flag ("1" = create .gitkeep; "0" = create README or file)
    # Column 3: readme text ("-" = no README; non-dash = README content)
    # Column 4: template path ("-" = no template; non-dash = template-relative path)
    #
    ...
    # Column 4 (template) is a plain path token — no newlines, no encoding hazard.
    ...
    if is_dir:
        ...
        # File entry: no gitkeep, no readme — template carries the content source
        gitkeep_flag = "0"
        readme_transport = "-"
        template_col = template
    print(f"{path}\t{gitkeep_flag}\t{readme_transport}\t{template_col}")
PYEOF
)
...
if [[ -z "$ENTRIES" ]]; then
    echo "scaffold-canonical-structure.sh: no eager directory or file entries found in manifest — nothing to do"
    exit 0
fi
...
while IFS=$'\t' read -r entry_path gitkeep_flag readme_text template_rel; do
    [[ -z "$entry_path" ]] && continue

    # Branch: directory entry (path ends with /) vs file entry (no trailing /)
    if [[ "$entry_path" == */ ]]; then
        # -----------------------------------------------------------------------
        # DIRECTORY entry — existing logic (unchanged)
        # -----------------------------------------------------------------------
    ...
    elif [[ "${template_rel:-}" != "-" && -n "${template_rel:-}" ]]; then
        # -----------------------------------------------------------------------
        # FILE entry — scaffolded from a static template (no substitution)
        # -----------------------------------------------------------------------

        target="${ROOT_PATH}/${entry_path}"
        template_abs="$(dirname "$MANIFEST")/${template_rel}"

        # Review: code-reviewer — dry-run must warn-and-continue, not hard-exit; hard-exit only on live run
        if [[ ! -f "$template_abs" ]]; then
            if [[ $DRY_RUN -eq 1 ]]; then
                echo "[dry-run] ERROR: declared template not found: $template_abs (would exit 2 on live run)" >&2
                continue
            else
                echo "scaffold-canonical-structure.sh: declared template not found: $template_abs" >&2
                exit 2
            fi
        fi
        ...
        if [[ -f "$target" ]]; then
            if [[ $DRY_RUN -eq 1 ]]; then
                # Review: code-reviewer — align spacing with dir-branch: "skip (exists) file:" (29) + 2sp = col 31
                echo "[dry-run] skip (exists) file:  ${entry_path}"
            fi
            SKIPPED=$((SKIPPED + 1))
        else
            if [[ $DRY_RUN -eq 1 ]]; then
                # Review: code-reviewer — align spacing with dir-branch: "would create file:" (28) + 3sp = col 31
                echo "[dry-run] would create file:   ${entry_path}"
            else
                mkdir -p "$(dirname "$target")"
                cp "$template_abs" "$target"
                # Review: code-reviewer — align spacing with dir-branch: "created file:" (13) + 3sp = col 16
                echo "created file:   ${entry_path}"
                CREATED_FILES=$((CREATED_FILES + 1))
            fi
        fi
    else
        # Review: code-reviewer — defensive else: non-dir entry with no template sentinel emits a warning
        echo "scaffold-canonical-structure.sh: WARNING: non-dir entry '${entry_path}' has no template — skipped" >&2
    fi
...
if [[ $DRY_RUN -eq 1 ]]; then
    ...
    echo "scaffold complete: ${CREATED_DIRS} dir(s) created, ${CREATED_READMES} README(s) created, ${CREATED_GITKEEPS} .gitkeep(s) created, ${CREATED_FILES} file(s) created, ${SKIPPED} already-present item(s) skipped"
fi
