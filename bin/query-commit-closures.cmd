@echo off
setlocal
REM Windows launcher for query-commit-closures.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "query-commit-closures.ps1" sibling of
REM     this file over this "query-commit-closures.cmd" file itself -- if both exist, `query-commit-closures`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "query-commit-closures.py"
...
echo [query-commit-closures] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [query-commit-closures] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0query-commit-closures.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0query-commit-closures.py" %*
exit /b %ERRORLEVEL%
