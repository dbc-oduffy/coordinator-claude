@echo off
setlocal
REM Windows launcher for check-machine-path-leak.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "check-machine-path-leak.ps1" sibling of
REM     this file over this "check-machine-path-leak.cmd" file itself -- if both exist, `check-machine-path-leak`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "check-machine-path-leak.py"
...
echo [check-machine-path-leak] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [check-machine-path-leak] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0check-machine-path-leak.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0check-machine-path-leak.py" %*
exit /b %ERRORLEVEL%
