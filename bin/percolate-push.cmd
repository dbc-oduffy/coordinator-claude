@echo off
setlocal
REM Windows launcher for percolate-push.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "percolate-push.ps1" sibling of
REM     this file over this "percolate-push.cmd" file itself -- if both exist, `percolate-push`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "percolate-push.py"
...
echo [percolate-push] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [percolate-push] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0percolate-push.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0percolate-push.py" %*
exit /b %ERRORLEVEL%
