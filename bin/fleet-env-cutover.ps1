# fleet-env-cutover.ps1 — python-direct Windows launcher for fleet-env-cutover.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "fleet-env-cutover.ps1" file over
#     any co-located "fleet-env-cutover.cmd" sibling -- `fleet-env-cutover` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "fleet-env-cutover.py"
...
$_entry = Join-Path $_here 'fleet-env-cutover.py'
...
[Console]::Error.WriteLine('[fleet-env-cutover] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[fleet-env-cutover] Install Python: https://www.python.org/downloads/windows/')
exit 127
