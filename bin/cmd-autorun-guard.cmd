@echo off
setlocal
REM Windows launcher for cmd-autorun-guard.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "cmd-autorun-guard.ps1" sibling of
REM     this file over this "cmd-autorun-guard.cmd" file itself -- if both exist, `cmd-autorun-guard`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "cmd-autorun-guard.py"
...
echo [cmd-autorun-guard] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [cmd-autorun-guard] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0cmd-autorun-guard.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0cmd-autorun-guard.py" %*
exit /b %ERRORLEVEL%
