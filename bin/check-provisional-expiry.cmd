@echo off
setlocal
REM Windows launcher for check-provisional-expiry.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "check-provisional-expiry.ps1" sibling of
REM     this file over this "check-provisional-expiry.cmd" file itself -- if both exist, `check-provisional-expiry`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "check-provisional-expiry.py"
...
echo [check-provisional-expiry] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [check-provisional-expiry] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0check-provisional-expiry.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0check-provisional-expiry.py" %*
exit /b %ERRORLEVEL%
