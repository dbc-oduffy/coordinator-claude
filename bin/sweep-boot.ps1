# sweep-boot.ps1 — python-direct Windows launcher for sweep-boot.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "sweep-boot.ps1" file over
#     any co-located "sweep-boot.cmd" sibling -- `sweep-boot` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "sweep-boot.py"
...
$_entry = Join-Path $_here 'sweep-boot.py'
...
[Console]::Error.WriteLine('[sweep-boot] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[sweep-boot] Install Python: https://www.python.org/downloads/windows/')
exit 127
