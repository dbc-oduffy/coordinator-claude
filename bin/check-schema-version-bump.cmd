@echo off
setlocal
REM Windows launcher for check-schema-version-bump.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "check-schema-version-bump.ps1" sibling of
REM     this file over this "check-schema-version-bump.cmd" file itself -- if both exist, `check-schema-version-bump`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "check-schema-version-bump.py"
...
echo [check-schema-version-bump] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [check-schema-version-bump] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0check-schema-version-bump.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0check-schema-version-bump.py" %*
exit /b %ERRORLEVEL%
