@echo off
setlocal
REM Windows launcher for land-wave.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "land-wave.ps1" sibling of
REM     this file over this "land-wave.cmd" file itself -- if both exist, `land-wave`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "land-wave.py"
...
echo [land-wave] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [land-wave] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
