# survey-consume-gate.ps1 — python-direct Windows launcher for survey-consume-gate.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "survey-consume-gate.ps1" file over
#     any co-located "survey-consume-gate.cmd" sibling -- `survey-consume-gate` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "survey-consume-gate.py"
...
$_entry = Join-Path $_here 'survey-consume-gate.py'
...
[Console]::Error.WriteLine('[survey-consume-gate] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[survey-consume-gate] Install Python: https://www.python.org/downloads/windows/')
exit 127
