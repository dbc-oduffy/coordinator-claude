 ...
 *
 * Usage:
 *   refresh-queries [--root <path>] [--check] [--files <path>[,<path>...]]
 *
 *   --check  Don't write files; exit 1 if any file would change. For CI.
 *   --files  Comma-separated exact path(s) to markdown files to scan (resolved relative to
 *            --root, or absolute). No glob support. Default (omitted): full recursive walk
 *            of **‌/*.md under root, excluding node_modules/.git/archive/.
...
function parseArgs(argv) {
  ...
  const opts = { root: null, check: false, files: null };
...
function processFile(filePath, root, checkMode) {
  ...
  while (true) {
    ...
    // Depth-correct link emission (cross-repo/inbox/2026-07-01-refresh-queries-relative-path-bug.md):
    // markdown links resolve relative to the containing file's directory, not repo root, so
    // the record paths must be recomputed per-file via path.relative(dirname(filePath), target).
    const expansion = formatRecords(records, queryOpts, { root, fromDir: path.dirname(filePath) });
...
// ---------------------------------------------------------------------------
// Resolve --files into a concrete exact-path file list.
// Splits on comma, resolves each entry relative to root (or absolute as-is),
// keeps only existing .md files. No glob engine — exact-path allowlist only.
// ---------------------------------------------------------------------------
function resolveFilesOpt(filesOpt, root) {
  const entries = filesOpt.split(',').map((s) => s.trim()).filter(Boolean);
  const resolved = [];
  for (const entry of entries) {
    ...
    try {
      ...
      // Missing file — skip silently (allowlist semantics, not an error).
    }
  }
  // Review: code-reviewer Finding 4 — non-error diagnostic (not a failure) so a caller
  // debugging "why didn't my --files run anything" gets a signal instead of silence.
  if (resolved.length < entries.length) {
    process.stderr.write(`Note: ${entries.length - resolved.length} of ${entries.length} --files entries skipped (not found or not .md)\n`);
  }
  return resolved;
}
...
function main() {
  ...
  // Review: code-reviewer Finding 2 — discriminate on presence (opts.files !== null), not on
  // string-equality against the magic default, so a caller who explicitly passes the literal
  // default value still gets routed through the exact-path allowlist instead of silently
  // falling through to a full-tree walk.
  const files = (opts.files !== null)
    ? resolveFilesOpt(opts.files, root)
    : walkMd(root);
  let totalChanged = 0;
  let totalErrors = 0;
