# coordinator-gate.ps1 — python-direct Windows launcher for coordinator-gate.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "coordinator-gate.ps1" file over
#     any co-located "coordinator-gate.cmd" sibling -- `coordinator-gate` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "coordinator-gate.py"
...
$_entry = Join-Path $_here 'coordinator-gate.py'
...
[Console]::Error.WriteLine('[coordinator-gate] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[coordinator-gate] Install Python: https://www.python.org/downloads/windows/')
exit 127
