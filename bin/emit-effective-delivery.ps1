# emit-effective-delivery.ps1 — python-direct Windows launcher for emit-effective-delivery.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "emit-effective-delivery.ps1" file over
#     any co-located "emit-effective-delivery.cmd" sibling -- `emit-effective-delivery` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "emit-effective-delivery.py"
...
[Console]::Error.WriteLine('[emit-effective-delivery] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[emit-effective-delivery] Install Python: https://www.python.org/downloads/windows/')
exit 127
