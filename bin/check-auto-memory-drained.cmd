@echo off
setlocal
REM Windows launcher for check-auto-memory-drained.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "check-auto-memory-drained.ps1" sibling of
REM     this file over this "check-auto-memory-drained.cmd" file itself -- if both exist, `check-auto-memory-drained`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "check-auto-memory-drained.py"
...
echo [check-auto-memory-drained] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [check-auto-memory-drained] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0check-auto-memory-drained.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0check-auto-memory-drained.py" %*
exit /b %ERRORLEVEL%
