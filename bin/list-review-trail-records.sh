...
# ---------------------------------------------------------------------------
# State root discovery — use coordinator_state_root (per-repo seam) to resolve
# the correct state dir for the caller's context.
# COORDINATOR_ROOT is an explicit override for tests (interpreted as a state root
# when it contains "/state", or as a repo root otherwise for backward compat).
...
# archive/review-trail/. Use the per-repo seam (C4) rather than git toplevel directly
# so meta-repo cwd redirects to example-orchestration-hub/state/ correctly.
# ---------------------------------------------------------------------------

_lrtr_state_root=""
if [[ -n "${COORDINATOR_ROOT:-}" ]]; then
  # Test/caller override: if it looks like a state root (ends in /state), use as-is;
  # otherwise treat as repo root and append /state for backward compat.
  if [[ "${COORDINATOR_ROOT}" == */state ]]; then
    _lrtr_state_root="${COORDINATOR_ROOT}"
  else
    _lrtr_state_root="${COORDINATOR_ROOT}/state"
  fi
else
  _lrtr_state_root="$(coordinator_state_root 2>/dev/null || true)"
  if [[ -z "${_lrtr_state_root}" ]]; then
...
LIVE_DIR="${_lrtr_state_root}/review-trail"
ARCHIVE_DIR="${_lrtr_state_root%/state}/archive/review-trail"
