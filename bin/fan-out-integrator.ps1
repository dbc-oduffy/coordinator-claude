# fan-out-integrator.ps1 — python-direct Windows launcher for fan-out-integrator.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "fan-out-integrator.ps1" file over
#     any co-located "fan-out-integrator.cmd" sibling -- `fan-out-integrator` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "fan-out-integrator.py"
...
$_entry = Join-Path $_here 'fan-out-integrator.py'
...
[Console]::Error.WriteLine('[fan-out-integrator] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[fan-out-integrator] Install Python: https://www.python.org/downloads/windows/')
exit 127
