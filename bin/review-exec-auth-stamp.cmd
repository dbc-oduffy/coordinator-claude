@echo off
setlocal
REM Windows launcher for review-exec-auth-stamp.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "review-exec-auth-stamp.ps1" sibling of
REM     this file over this "review-exec-auth-stamp.cmd" file itself -- if both exist, `review-exec-auth-stamp`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "review-exec-auth-stamp.py"
...
echo [review-exec-auth-stamp] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [review-exec-auth-stamp] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0review-exec-auth-stamp.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0review-exec-auth-stamp.py" %*
exit /b %ERRORLEVEL%
