# generate-tested-platforms.ps1 — python-direct Windows launcher for generate-tested-platforms.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "generate-tested-platforms.ps1" file over
#     any co-located "generate-tested-platforms.cmd" sibling -- `generate-tested-platforms` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "generate-tested-platforms.py"
...
$_entry = Join-Path $_here 'generate-tested-platforms.py'
...
[Console]::Error.WriteLine('[generate-tested-platforms] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[generate-tested-platforms] Install Python: https://www.python.org/downloads/windows/')
exit 127
