@echo off
setlocal
REM Windows launcher for workday-complete-backfill-scan.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "workday-complete-backfill-scan.ps1" sibling of
REM     this file over this "workday-complete-backfill-scan.cmd" file itself -- if both exist, `workday-complete-backfill-scan`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "workday-complete-backfill-scan.py"
...
echo [workday-complete-backfill-scan] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [workday-complete-backfill-scan] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0workday-complete-backfill-scan.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0workday-complete-backfill-scan.py" %*
exit /b %ERRORLEVEL%
