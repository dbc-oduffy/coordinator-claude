# memo-outbox-tracking-guard.ps1 — python-direct Windows launcher for memo-outbox-tracking-guard.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "memo-outbox-tracking-guard.ps1" file over
#     any co-located "memo-outbox-tracking-guard.cmd" sibling -- `memo-outbox-tracking-guard` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "memo-outbox-tracking-guard.py"
...
$_entry = Join-Path $_here 'memo-outbox-tracking-guard.py'
...
[Console]::Error.WriteLine('[memo-outbox-tracking-guard] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[memo-outbox-tracking-guard] Install Python: https://www.python.org/downloads/windows/')
exit 127
