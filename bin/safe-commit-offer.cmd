@echo off
setlocal
REM Windows launcher for safe-commit-offer.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "safe-commit-offer.ps1" sibling of
REM     this file over this "safe-commit-offer.cmd" file itself -- if both exist, `safe-commit-offer`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "safe-commit-offer.py"
...
echo [safe-commit-offer] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [safe-commit-offer] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0safe-commit-offer.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0safe-commit-offer.py" %*
exit /b %ERRORLEVEL%
