@echo off
setlocal
REM Windows launcher for stitch-observer-sidecar.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "stitch-observer-sidecar.ps1" sibling of
REM     this file over this "stitch-observer-sidecar.cmd" file itself -- if both exist, `stitch-observer-sidecar`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "stitch-observer-sidecar.py"
...
echo [stitch-observer-sidecar] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [stitch-observer-sidecar] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0stitch-observer-sidecar.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0stitch-observer-sidecar.py" %*
exit /b %ERRORLEVEL%
