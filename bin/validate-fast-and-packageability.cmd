@echo off
setlocal
REM Windows launcher for validate-fast-and-packageability.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "validate-fast-and-packageability.ps1" sibling of
REM     this file over this "validate-fast-and-packageability.cmd" file itself -- if both exist, `validate-fast-and-packageability`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "validate-fast-and-packageability.py"
...
echo [validate-fast-and-packageability] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [validate-fast-and-packageability] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0validate-fast-and-packageability.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0validate-fast-and-packageability.py" %*
exit /b %ERRORLEVEL%
