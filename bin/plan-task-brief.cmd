@echo off
setlocal
REM Windows launcher for plan-task-brief.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "plan-task-brief.ps1" sibling of
REM     this file over this "plan-task-brief.cmd" file itself -- if both exist, `plan-task-brief`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "plan-task-brief.py"
...
echo [plan-task-brief] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [plan-task-brief] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0plan-task-brief.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0plan-task-brief.py" %*
exit /b %ERRORLEVEL%
