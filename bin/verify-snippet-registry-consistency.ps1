# verify-snippet-registry-consistency.ps1 — python-direct Windows launcher for verify-snippet-registry-consistency.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "verify-snippet-registry-consistency.ps1" file over
#     any co-located "verify-snippet-registry-consistency.cmd" sibling -- `verify-snippet-registry-consistency` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "verify-snippet-registry-consistency.py"
...
$_entry = Join-Path $_here 'verify-snippet-registry-consistency.py'
...
[Console]::Error.WriteLine('[verify-snippet-registry-consistency] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[verify-snippet-registry-consistency] Install Python: https://www.python.org/downloads/windows/')
exit 127
