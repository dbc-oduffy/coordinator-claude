@echo off
setlocal
REM Windows launcher for render-template-tree.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "render-template-tree.ps1" sibling of
REM     this file over this "render-template-tree.cmd" file itself -- if both exist, `render-template-tree`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "render-template-tree.py"
...
echo [render-template-tree] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [render-template-tree] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0render-template-tree.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0render-template-tree.py" %*
exit /b %ERRORLEVEL%
