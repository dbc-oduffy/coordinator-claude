@echo off
setlocal
REM Windows launcher for query-routine-signals.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "query-routine-signals.ps1" sibling of
REM     this file over this "query-routine-signals.cmd" file itself -- if both exist, `query-routine-signals`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "query-routine-signals.py"
...
echo [query-routine-signals] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [query-routine-signals] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0query-routine-signals.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0query-routine-signals.py" %*
exit /b %ERRORLEVEL%
