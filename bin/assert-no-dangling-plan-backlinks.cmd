@echo off
setlocal
REM Windows launcher for assert-no-dangling-plan-backlinks.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "assert-no-dangling-plan-backlinks.ps1" sibling of
REM     this file over this "assert-no-dangling-plan-backlinks.cmd" file itself -- if both exist, `assert-no-dangling-plan-backlinks`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "assert-no-dangling-plan-backlinks.py"
...
echo [assert-no-dangling-plan-backlinks] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [assert-no-dangling-plan-backlinks] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0assert-no-dangling-plan-backlinks.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0assert-no-dangling-plan-backlinks.py" %*
exit /b %ERRORLEVEL%
