# warm-engine-stop.ps1 — python-direct Windows launcher for warm-engine-stop.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "warm-engine-stop.ps1" file over
#     any co-located "warm-engine-stop.cmd" sibling -- `warm-engine-stop` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "warm-engine-stop.py"
...
$_entry = Join-Path $_here 'warm-engine-stop.py'
...
[Console]::Error.WriteLine('[warm-engine-stop] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[warm-engine-stop] Install Python: https://www.python.org/downloads/windows/')
exit 127
