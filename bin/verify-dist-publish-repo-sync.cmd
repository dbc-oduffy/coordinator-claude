@echo off
setlocal
REM Windows launcher for verify-dist-publish-repo-sync.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "verify-dist-publish-repo-sync.ps1" sibling of
REM     this file over this "verify-dist-publish-repo-sync.cmd" file itself -- if both exist, `verify-dist-publish-repo-sync`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "verify-dist-publish-repo-sync.py"
...
echo [verify-dist-publish-repo-sync] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [verify-dist-publish-repo-sync] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0verify-dist-publish-repo-sync.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0verify-dist-publish-repo-sync.py" %*
exit /b %ERRORLEVEL%
