@echo off
setlocal
REM Windows launcher for wsc-session-disposition.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "wsc-session-disposition.ps1" sibling of
REM     this file over this "wsc-session-disposition.cmd" file itself -- if both exist, `wsc-session-disposition`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "wsc-session-disposition.py"
...
echo [wsc-session-disposition] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [wsc-session-disposition] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0wsc-session-disposition.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0wsc-session-disposition.py" %*
exit /b %ERRORLEVEL%
