# coordinator-invoke.ps1 — python-direct Windows launcher for coordinator-invoke.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "coordinator-invoke.ps1" file over
#     any co-located "coordinator-invoke.cmd" sibling -- `coordinator-invoke` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "coordinator-invoke.py"
...
$_entry = Join-Path $_here 'coordinator-invoke.py'
...
[Console]::Error.WriteLine('[coordinator-invoke] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[coordinator-invoke] Install Python: https://www.python.org/downloads/windows/')
exit 127
