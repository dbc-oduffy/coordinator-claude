@echo off
setlocal
REM Windows launcher for probe-memory-headroom.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "probe-memory-headroom.ps1" sibling of
REM     this file over this "probe-memory-headroom.cmd" file itself -- if both exist, `probe-memory-headroom`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "probe-memory-headroom.py"
...
echo [probe-memory-headroom] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [probe-memory-headroom] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0probe-memory-headroom.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0probe-memory-headroom.py" %*
exit /b %ERRORLEVEL%
