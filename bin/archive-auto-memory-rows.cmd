@echo off
setlocal
REM Windows launcher for archive-auto-memory-rows.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "archive-auto-memory-rows.ps1" sibling of
REM     this file over this "archive-auto-memory-rows.cmd" file itself -- if both exist, `archive-auto-memory-rows`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "archive-auto-memory-rows.py"
...
echo [archive-auto-memory-rows] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [archive-auto-memory-rows] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0archive-auto-memory-rows.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0archive-auto-memory-rows.py" %*
exit /b %ERRORLEVEL%
