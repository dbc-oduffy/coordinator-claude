@echo off
setlocal
REM Windows launcher for workweek-trail-scope.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "workweek-trail-scope.ps1" sibling of
REM     this file over this "workweek-trail-scope.cmd" file itself -- if both exist, `workweek-trail-scope`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "workweek-trail-scope.py"
...
echo [workweek-trail-scope] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [workweek-trail-scope] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0workweek-trail-scope.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0workweek-trail-scope.py" %*
exit /b %ERRORLEVEL%
