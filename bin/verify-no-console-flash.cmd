@echo off
setlocal
REM Windows launcher for verify-no-console-flash.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "verify-no-console-flash.ps1" sibling of
REM     this file over this "verify-no-console-flash.cmd" file itself -- if both exist, `verify-no-console-flash`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "verify-no-console-flash.py"
...
echo [verify-no-console-flash] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [verify-no-console-flash] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0verify-no-console-flash.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0verify-no-console-flash.py" %*
exit /b %ERRORLEVEL%
