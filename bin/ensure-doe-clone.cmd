@echo off
setlocal
REM Windows launcher for ensure-doe-clone.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "ensure-doe-clone.ps1" sibling of
REM     this file over this "ensure-doe-clone.cmd" file itself -- if both exist, `ensure-doe-clone`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "ensure-doe-clone.py"
...
echo [ensure-doe-clone] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [ensure-doe-clone] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0ensure-doe-clone.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0ensure-doe-clone.py" %*
exit /b %ERRORLEVEL%
