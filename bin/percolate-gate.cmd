@echo off
setlocal
REM Windows launcher for percolate-gate.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "percolate-gate.ps1" sibling of
REM     this file over this "percolate-gate.cmd" file itself -- if both exist, `percolate-gate`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "percolate-gate.py"
...
echo [percolate-gate] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [percolate-gate] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0percolate-gate.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0percolate-gate.py" %*
exit /b %ERRORLEVEL%
