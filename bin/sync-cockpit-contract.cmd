@echo off
setlocal
REM Windows launcher for sync-cockpit-contract.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "sync-cockpit-contract.ps1" sibling of
REM     this file over this "sync-cockpit-contract.cmd" file itself -- if both exist, `sync-cockpit-contract`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "sync-cockpit-contract.py"
...
echo [sync-cockpit-contract] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [sync-cockpit-contract] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0sync-cockpit-contract.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0sync-cockpit-contract.py" %*
exit /b %ERRORLEVEL%
