@echo off
setlocal
REM Windows launcher for with-suite-mutex — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "with-suite-mutex.ps1" sibling of
REM     this file over this "with-suite-mutex.cmd" file itself -- if both exist, `with-suite-mutex`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "with-suite-mutex"
...
echo [with-suite-mutex] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [with-suite-mutex] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0with-suite-mutex" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0with-suite-mutex" %*
exit /b %ERRORLEVEL%
