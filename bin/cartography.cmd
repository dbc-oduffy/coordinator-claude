@echo off
setlocal
REM Windows launcher for cartography.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "cartography.ps1" sibling of
REM     this file over this "cartography.cmd" file itself -- if both exist, `cartography`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "cartography.py"
...
echo [cartography] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [cartography] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0cartography.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0cartography.py" %*
exit /b %ERRORLEVEL%
