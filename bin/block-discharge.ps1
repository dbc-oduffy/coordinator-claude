# block-discharge.ps1 — python-direct Windows launcher for block-discharge.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "block-discharge.ps1" file over
#     any co-located "block-discharge.cmd" sibling -- `block-discharge` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "block-discharge.py"
...
$_entry = Join-Path $_here 'block-discharge.py'
...
[Console]::Error.WriteLine('[block-discharge] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[block-discharge] Install Python: https://www.python.org/downloads/windows/')
exit 127
