@echo off
setlocal
REM Windows launcher for workstream-complete-assemble.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "workstream-complete-assemble.ps1" sibling of
REM     this file over this "workstream-complete-assemble.cmd" file itself -- if both exist, `workstream-complete-assemble`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "workstream-complete-assemble.py"
...
echo [workstream-complete-assemble] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [workstream-complete-assemble] Install Python: https://www.python.org/downloads/windows/ 1>&2
...
:run_baked
"%_py%" "%~dp0workstream-complete-assemble.py" %*
2>nul rd /s /q "%_LAUNCHER_RAW_CMDLINE_DIR%"
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0workstream-complete-assemble.py" %*
2>nul rd /s /q "%_LAUNCHER_RAW_CMDLINE_DIR%"
exit /b %ERRORLEVEL%
