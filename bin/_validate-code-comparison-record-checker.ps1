# _validate-code-comparison-record-checker.ps1 — python-direct Windows launcher for _validate-code-comparison-record-checker.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "_validate-code-comparison-record-checker.ps1" file over
#     any co-located "_validate-code-comparison-record-checker.cmd" sibling -- `_validate-code-comparison-record-checker` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "_validate-code-comparison-record-checker.py"
...
$_entry = Join-Path $_here '_validate-code-comparison-record-checker.py'
...
[Console]::Error.WriteLine('[_validate-code-comparison-record-checker] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[_validate-code-comparison-record-checker] Install Python: https://www.python.org/downloads/windows/')
exit 127
