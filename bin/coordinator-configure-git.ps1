# coordinator-configure-git.ps1 — python-direct Windows launcher for coordinator-configure-git.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "coordinator-configure-git.ps1" file over
#     any co-located "coordinator-configure-git.cmd" sibling -- `coordinator-configure-git` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "coordinator-configure-git.py"
...
$_entry = Join-Path $_here 'coordinator-configure-git.py'
...
[Console]::Error.WriteLine('[coordinator-configure-git] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[coordinator-configure-git] Install Python: https://www.python.org/downloads/windows/')
exit 127
