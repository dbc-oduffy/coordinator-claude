@echo off
setlocal
REM Windows launcher for query-work-state.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "query-work-state.ps1" sibling of
REM     this file over this "query-work-state.cmd" file itself -- if both exist, `query-work-state`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "query-work-state.py"
...
echo [query-work-state] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [query-work-state] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0query-work-state.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0query-work-state.py" %*
exit /b %ERRORLEVEL%
