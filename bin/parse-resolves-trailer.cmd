@echo off
setlocal
REM Windows launcher for parse-resolves-trailer.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "parse-resolves-trailer.ps1" sibling of
REM     this file over this "parse-resolves-trailer.cmd" file itself -- if both exist, `parse-resolves-trailer`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "parse-resolves-trailer.py"
...
echo [parse-resolves-trailer] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [parse-resolves-trailer] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0parse-resolves-trailer.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0parse-resolves-trailer.py" %*
exit /b %ERRORLEVEL%
