# navi-singleton.ps1 — python-direct Windows launcher for navi-singleton.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "navi-singleton.ps1" file over
#     any co-located "navi-singleton.cmd" sibling -- `navi-singleton` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "navi-singleton.py"
...
$_entry = Join-Path $_here 'navi-singleton.py'
...
[Console]::Error.WriteLine('[navi-singleton] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[navi-singleton] Install Python: https://www.python.org/downloads/windows/')
exit 127
