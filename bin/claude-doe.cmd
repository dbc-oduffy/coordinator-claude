@echo off
setlocal
REM Windows launcher for claude-doe.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "claude-doe.ps1" sibling of
REM     this file over this "claude-doe.cmd" file itself -- if both exist, `claude-doe`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "claude-doe.py"
...
echo [claude-doe] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [claude-doe] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0claude-doe.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0claude-doe.py" %*
exit /b %ERRORLEVEL%
