@echo off
setlocal
REM Windows launcher for coordinator-queue-append.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "coordinator-queue-append.ps1" sibling of
REM     this file over this "coordinator-queue-append.cmd" file itself -- if both exist, `coordinator-queue-append`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "coordinator-queue-append.py"
...
echo [coordinator-queue-append] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [coordinator-queue-append] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0coordinator-queue-append.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0coordinator-queue-append.py" %*
exit /b %ERRORLEVEL%
