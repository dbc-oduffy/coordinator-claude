 ...
 *
 * Inspired by example-game-repo's agent-domain-coverage.test.ts (which enforces TOOL_ORPHANED /
 ...
 * script operate against the live authoring tree instead of the publish mirror.
 *
 * OSS / marketplace installs: sentinel absent → fall back to the published
 * mirror at ~/.claude/plugins/coordinator-claude/.
 *
 * Spec backlink: docs/plans/2026-07-05-doe-cutover-completion-sweep.md § C5.
 */
// Review: code-reviewer — accept optional homeDir so tests can inject a temp home
// without mutating process.env.HOME; production callers pass no argument.
function defaultRoot(homeDir = os.homedir()) {
  const sentinelPath = path.join(homeDir, '.claude', '.doe-root');
  try {
    const doeRoot = fs.readFileSync(sentinelPath, 'utf-8').trim();
    if (doeRoot) return doeRoot;
  } catch (e) {
    // Review: code-reviewer — name catch var; warn on non-ENOENT so an unreadable sentinel
    // on a DoE machine surfaces visibly instead of silently walking the wrong tree.
    if (e.code !== 'ENOENT') console.warn(`warn: .doe-root sentinel unreadable (${e.code}), using OSS fallback`);
    // ENOENT (file absent) is the expected OSS path — stay silent.
  }
  return path.join(homeDir, '.claude', 'plugins', 'coordinator-claude');
}
...
function resolve(ref, kind, artifacts) {
  ...
  if (kind === 'subagent' || kind === 'worker') {
    if (ref.includes(':')) {
      ...
      // out-of-tree (external plugin like example-game-repo-control) and skip silently.
...
const REF_ALLOWLIST = new Set([
  ...
  // Project-specific agent in example-game-repo consumer repo (plugin/game-dev/agents/schema-migration-auditor.md).
  ...
  'game-dev:schema-migration-auditor',  // example-game-repo project-local game-dev agent; coordinator wiki cross-ref only (2026-05-24)
  ...
  // External installed plugin (oduffy-custom/feature-dev + claude-plugins-official), NOT part of
  // the coordinator-claude tree. capability-catalog.md:64 documents its dispatch shape
  // `subagent_type: feature-dev:<agent-name>` — the `<agent-name>` placeholder makes the parser
  // emit the bare prefix `feature-dev`, which can't resolve against the coordinator registry.
  // Same class as the example-game-repo-control:* / superpowers:* external skips, but bare-prefix so it
  // bypasses the colon-prefix external-skip path in resolve().
  'feature-dev',                        // external plugin; capability-catalog dispatch-shape doc, not a coordinator-tree agent (2026-06-27)
  // Documented glob PATTERN, not a concrete reference: docs/wiki/tiered-context-loading.md
  // lists `coordinator:research-*` (the research-scout/research-specialist/research-worker
  // family) inside a Tier-4 subagent_type set. The reference parser strips the trailing `*`,
  // yielding the unresolvable token `coordinator:research-`. Allowlisting the truncated form.
  'coordinator:research-',              // glob pattern for research-* family in tiered-context-loading.md (2026-07-08)
]);
...
function main() {
  ...
  //
  // Exclude review-trail/ — state/review-trail/findings/*.md holds immutable historical
  // code-review findings. Reviewers QUOTE agent/skill references as evidence, including
  // deliberately-wrong ones (e.g. a finding whose entire point is "SKILL.md hardcodes a
  // wrong `coordinator:` prefix"). These are historical evidence, not live dispatch
  // references, so coverage-checking them is a false-positive generator — prune the
  // whole subtree. Excluded by directory basename via walkMarkdown's exclude set.
  for (const file of walkMarkdown(root, new Set(['dist', 'review-trail']))) {
...
// Review: code-reviewer — guard main() so require() for unit-testing defaultRoot() is safe.
