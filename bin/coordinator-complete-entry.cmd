@echo off
setlocal
REM Windows launcher for coordinator-complete-entry.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "coordinator-complete-entry.ps1" sibling of
REM     this file over this "coordinator-complete-entry.cmd" file itself -- if both exist, `coordinator-complete-entry`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "coordinator-complete-entry.py"
...
echo [coordinator-complete-entry] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [coordinator-complete-entry] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0coordinator-complete-entry.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0coordinator-complete-entry.py" %*
exit /b %ERRORLEVEL%
