@echo off
setlocal
REM Windows launcher for probe-cwd-project-rag-relevance.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "probe-cwd-project-rag-relevance.ps1" sibling of
REM     this file over this "probe-cwd-project-rag-relevance.cmd" file itself -- if both exist, `probe-cwd-project-rag-relevance`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "probe-cwd-project-rag-relevance.py"
...
echo [probe-cwd-project-rag-relevance] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [probe-cwd-project-rag-relevance] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127

:run_baked
"%_py%" "%~dp0probe-cwd-project-rag-relevance.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0probe-cwd-project-rag-relevance.py" %*
exit /b %ERRORLEVEL%
