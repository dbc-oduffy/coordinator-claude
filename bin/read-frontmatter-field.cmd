@echo off
setlocal
REM Windows launcher for read-frontmatter-field.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "read-frontmatter-field.ps1" sibling of
REM     this file over this "read-frontmatter-field.cmd" file itself -- if both exist, `read-frontmatter-field`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "read-frontmatter-field.py"
...
echo [read-frontmatter-field] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [read-frontmatter-field] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0read-frontmatter-field.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0read-frontmatter-field.py" %*
exit /b %ERRORLEVEL%
