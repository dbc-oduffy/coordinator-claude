@echo off
setlocal
REM Windows launcher for aggregate-rollup.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "aggregate-rollup.ps1" sibling of
REM     this file over this "aggregate-rollup.cmd" file itself -- if both exist, `aggregate-rollup`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "aggregate-rollup.py"
...
echo [aggregate-rollup] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [aggregate-rollup] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0aggregate-rollup.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0aggregate-rollup.py" %*
exit /b %ERRORLEVEL%
