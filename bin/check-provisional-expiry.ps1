# check-provisional-expiry.ps1 — python-direct Windows launcher for check-provisional-expiry.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "check-provisional-expiry.ps1" file over
#     any co-located "check-provisional-expiry.cmd" sibling -- `check-provisional-expiry` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "check-provisional-expiry.py"
...
$_entry = Join-Path $_here 'check-provisional-expiry.py'
...
[Console]::Error.WriteLine('[check-provisional-expiry] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[check-provisional-expiry] Install Python: https://www.python.org/downloads/windows/')
exit 127
