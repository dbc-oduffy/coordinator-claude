# query-completions.ps1 — python-direct Windows launcher for query-completions.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "query-completions.ps1" file over
#     any co-located "query-completions.cmd" sibling -- `query-completions` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "query-completions.py"
...
$_entry = Join-Path $_here 'query-completions.py'
...
[Console]::Error.WriteLine('[query-completions] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[query-completions] Install Python: https://www.python.org/downloads/windows/')
exit 127
