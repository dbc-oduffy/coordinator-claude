@echo off
setlocal
REM Windows launcher for repo-setup-args-and-register.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "repo-setup-args-and-register.ps1" sibling of
REM     this file over this "repo-setup-args-and-register.cmd" file itself -- if both exist, `repo-setup-args-and-register`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "repo-setup-args-and-register.py"
...
echo [repo-setup-args-and-register] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [repo-setup-args-and-register] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0repo-setup-args-and-register.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0repo-setup-args-and-register.py" %*
exit /b %ERRORLEVEL%
