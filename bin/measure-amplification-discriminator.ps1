# measure-amplification-discriminator.ps1 — python-direct Windows launcher for measure-amplification-discriminator.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "measure-amplification-discriminator.ps1" file over
#     any co-located "measure-amplification-discriminator.cmd" sibling -- `measure-amplification-discriminator` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "measure-amplification-discriminator.py"
...
$_entry = Join-Path $_here 'measure-amplification-discriminator.py'
...
[Console]::Error.WriteLine('[measure-amplification-discriminator] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[measure-amplification-discriminator] Install Python: https://www.python.org/downloads/windows/')
exit 127
