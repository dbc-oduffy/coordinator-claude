@echo off
setlocal
REM Windows launcher for list-review-trail-records.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "list-review-trail-records.ps1" sibling of
REM     this file over this "list-review-trail-records.cmd" file itself -- if both exist, `list-review-trail-records`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "list-review-trail-records.py"
...
echo [list-review-trail-records] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [list-review-trail-records] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0list-review-trail-records.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0list-review-trail-records.py" %*
exit /b %ERRORLEVEL%
