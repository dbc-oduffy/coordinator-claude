@echo off
setlocal
REM Windows launcher for coordinator-doctor-sentinel.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "coordinator-doctor-sentinel.ps1" sibling of
REM     this file over this "coordinator-doctor-sentinel.cmd" file itself -- if both exist, `coordinator-doctor-sentinel`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "coordinator-doctor-sentinel.py"
...
echo [coordinator-doctor-sentinel] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [coordinator-doctor-sentinel] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0coordinator-doctor-sentinel.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0coordinator-doctor-sentinel.py" %*
exit /b %ERRORLEVEL%
