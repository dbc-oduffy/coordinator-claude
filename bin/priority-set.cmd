@echo off
setlocal
REM Windows launcher for priority-set.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "priority-set.ps1" sibling of
REM     this file over this "priority-set.cmd" file itself -- if both exist, `priority-set`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "priority-set.py"
...
echo [priority-set] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [priority-set] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0priority-set.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0priority-set.py" %*
exit /b %ERRORLEVEL%
