# engine-gap-lint.ps1 — python-direct Windows launcher for engine-gap-lint.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "engine-gap-lint.ps1" file over
#     any co-located "engine-gap-lint.cmd" sibling -- `engine-gap-lint` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "engine-gap-lint.py"
...
$_entry = Join-Path $_here 'engine-gap-lint.py'
...
[Console]::Error.WriteLine('[engine-gap-lint] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[engine-gap-lint] Install Python: https://www.python.org/downloads/windows/')
exit 127
