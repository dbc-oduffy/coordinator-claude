@echo off
setlocal
REM Windows launcher for setup-verify.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "setup-verify.ps1" sibling of
REM     this file over this "setup-verify.cmd" file itself -- if both exist, `setup-verify`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "setup-verify.py"
...
echo [setup-verify] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [setup-verify] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0setup-verify.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0setup-verify.py" %*
exit /b %ERRORLEVEL%
