# warm-serve-partition.ps1 — python-direct Windows launcher for warm-serve-partition.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "warm-serve-partition.ps1" file over
#     any co-located "warm-serve-partition.cmd" sibling -- `warm-serve-partition` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "warm-serve-partition.py"
...
$_entry = Join-Path $_here 'warm-serve-partition.py'
...
[Console]::Error.WriteLine('[warm-serve-partition] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[warm-serve-partition] Install Python: https://www.python.org/downloads/windows/')
exit 127
