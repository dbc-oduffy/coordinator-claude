@echo off
setlocal
REM Windows launcher for validate-code-comparison-record.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "validate-code-comparison-record.ps1" sibling of
REM     this file over this "validate-code-comparison-record.cmd" file itself -- if both exist, `validate-code-comparison-record`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "validate-code-comparison-record.py"
...
echo [validate-code-comparison-record] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [validate-code-comparison-record] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0validate-code-comparison-record.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0validate-code-comparison-record.py" %*
exit /b %ERRORLEVEL%
