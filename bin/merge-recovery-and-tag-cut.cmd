@echo off
setlocal
REM Windows launcher for merge-recovery-and-tag-cut.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "merge-recovery-and-tag-cut.ps1" sibling of
REM     this file over this "merge-recovery-and-tag-cut.cmd" file itself -- if both exist, `merge-recovery-and-tag-cut`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "merge-recovery-and-tag-cut.py"
...
echo [merge-recovery-and-tag-cut] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [merge-recovery-and-tag-cut] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0merge-recovery-and-tag-cut.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0merge-recovery-and-tag-cut.py" %*
exit /b %ERRORLEVEL%
