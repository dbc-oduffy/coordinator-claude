...
#
# Usage: check-workstream-complete-deletion-blocks.sh <prepared-commit-msg-file> [-- <pathspec>...]
#
# Passing -- with zero trailing paths is equivalent to omitting it (whole-index mode).
#
# When a pathspec is provided (after a literal --), staged-deletion and staged-all
# reads are scoped to those paths only.  This prevents concurrent-EM sibling
# deletions on the shared index from tripping the inverse check (F3).  Without a
# pathspec the gate inspects the whole index (backward-compatible standalone mode).
#
# Exit codes:
...
# Exit 2 — usage
if [ "$#" -lt 1 ]; then
  printf 'usage: %s <prepared-commit-msg-file> [-- <pathspec>...]\n' "$0" >&2
  exit 2
fi
msg_file="$1"
shift
...
# Optional pathspec: parse literal -- followed by path arguments.
# When non-empty, staged reads are scoped to these paths (concurrent-EM fix).
# When empty, whole-index behavior is preserved (backward-compat).
# Guard: declare array before the conditional so set -u never sees it as unset.
pathspec=()
if [ "$#" -ge 1 ] && [ "$1" = "--" ]; then
  shift
  # Remaining args are the pathspec (may be zero or more paths after the --)
  while [ "$#" -gt 0 ]; do
    pathspec+=("$1")
    shift
  done
...
# Staged reality — scoped to pathspec when provided, whole-index otherwise.
# The length-check branch avoids expanding a possibly-empty array under set -u
# (bash <4.4 treats "${arr[@]}" on an empty declared array as unbound).
# tracked_at_head uses git ls-tree (HEAD snapshot for Kept-claim existence);
# it is intentionally left unscoped — HEAD is not affected by pathspec scope.
if [ "${#pathspec[@]}" -gt 0 ]; then
  staged_deletions=$(git diff --cached --diff-filter=D --name-only -- "${pathspec[@]}" 2>/dev/null || true)
  staged_all=$(git diff --cached --name-only -- "${pathspec[@]}" 2>/dev/null || true)
else
