@echo off
setlocal
REM Windows launcher for workweek-complete-drift-guards.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "workweek-complete-drift-guards.ps1" sibling of
REM     this file over this "workweek-complete-drift-guards.cmd" file itself -- if both exist, `workweek-complete-drift-guards`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "workweek-complete-drift-guards.py"
...
echo [workweek-complete-drift-guards] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [workweek-complete-drift-guards] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0workweek-complete-drift-guards.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0workweek-complete-drift-guards.py" %*
exit /b %ERRORLEVEL%
