@echo off
setlocal
REM Windows launcher for check-deferral-partial-strangle.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "check-deferral-partial-strangle.ps1" sibling of
REM     this file over this "check-deferral-partial-strangle.cmd" file itself -- if both exist, `check-deferral-partial-strangle`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "check-deferral-partial-strangle.py"
...
echo [check-deferral-partial-strangle] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [check-deferral-partial-strangle] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0check-deferral-partial-strangle.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0check-deferral-partial-strangle.py" %*
exit /b %ERRORLEVEL%
