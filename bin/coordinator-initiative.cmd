@echo off
setlocal
REM Windows launcher for coordinator-initiative.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "coordinator-initiative.ps1" sibling of
REM     this file over this "coordinator-initiative.cmd" file itself -- if both exist, `coordinator-initiative`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "coordinator-initiative.py"
...
echo [coordinator-initiative] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [coordinator-initiative] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0coordinator-initiative.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0coordinator-initiative.py" %*
exit /b %ERRORLEVEL%
