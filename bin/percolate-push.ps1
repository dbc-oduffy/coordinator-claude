# percolate-push.ps1 — python-direct Windows launcher for percolate-push.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "percolate-push.ps1" file over
#     any co-located "percolate-push.cmd" sibling -- `percolate-push` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "percolate-push.py"
...
$_entry = Join-Path $_here 'percolate-push.py'
...
[Console]::Error.WriteLine('[percolate-push] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[percolate-push] Install Python: https://www.python.org/downloads/windows/')
exit 127
