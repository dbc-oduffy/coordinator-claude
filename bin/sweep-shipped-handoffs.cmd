@echo off
setlocal
REM Windows launcher for sweep-shipped-handoffs.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "sweep-shipped-handoffs.ps1" sibling of
REM     this file over this "sweep-shipped-handoffs.cmd" file itself -- if both exist, `sweep-shipped-handoffs`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "sweep-shipped-handoffs.py"
...
echo [sweep-shipped-handoffs] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [sweep-shipped-handoffs] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0sweep-shipped-handoffs.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0sweep-shipped-handoffs.py" %*
exit /b %ERRORLEVEL%
