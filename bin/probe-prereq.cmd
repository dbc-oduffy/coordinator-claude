@echo off
setlocal
REM Windows launcher for probe-prereq.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "probe-prereq.ps1" sibling of
REM     this file over this "probe-prereq.cmd" file itself -- if both exist, `probe-prereq`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "probe-prereq.py"
...
echo [probe-prereq] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [probe-prereq] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0probe-prereq.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0probe-prereq.py" %*
exit /b %ERRORLEVEL%
