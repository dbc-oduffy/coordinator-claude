@echo off
setlocal
REM Windows launcher for classify-resolver-callers.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "classify-resolver-callers.ps1" sibling of
REM     this file over this "classify-resolver-callers.cmd" file itself -- if both exist, `classify-resolver-callers`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "classify-resolver-callers.py"
...
echo [classify-resolver-callers] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [classify-resolver-callers] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0classify-resolver-callers.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0classify-resolver-callers.py" %*
exit /b %ERRORLEVEL%
