# wsc-coverage-gate-runner.ps1 — python-direct Windows launcher for wsc-coverage-gate-runner.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "wsc-coverage-gate-runner.ps1" file over
#     any co-located "wsc-coverage-gate-runner.cmd" sibling -- `wsc-coverage-gate-runner` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "wsc-coverage-gate-runner.py"
...
$_entry = Join-Path $_here 'wsc-coverage-gate-runner.py'
...
[Console]::Error.WriteLine('[wsc-coverage-gate-runner] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[wsc-coverage-gate-runner] Install Python: https://www.python.org/downloads/windows/')
exit 127
