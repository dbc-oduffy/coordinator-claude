@echo off
setlocal
REM Windows launcher for refresh-plugin-live-install.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "refresh-plugin-live-install.ps1" sibling of
REM     this file over this "refresh-plugin-live-install.cmd" file itself -- if both exist, `refresh-plugin-live-install`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "refresh-plugin-live-install.py"
...
echo [refresh-plugin-live-install] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [refresh-plugin-live-install] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0refresh-plugin-live-install.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0refresh-plugin-live-install.py" %*
exit /b %ERRORLEVEL%
