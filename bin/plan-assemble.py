...
def main(argv: list[str]) -> int:
    try:
        ...
        print(f"plan-assemble: CLAUDE_KLABAUTER_ROOT resolution failed: {exc}", file=sys.stderr)
        return _TRANSPORT_FAIL
    except ImportError as exc:
        print(f"plan-assemble: coordinator_core.plan_assemble not importable: {exc}", file=sys.stderr)
        return _TRANSPORT_FAIL
