...
_REGEN_SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" 2>/dev/null && pwd)"
# shellcheck source=../lib/coordinator-state-root.sh
source "${_REGEN_SCRIPT_DIR}/../lib/coordinator-state-root.sh"
...
count_handoffs_ready() {
    local d="$STATE_ROOT/handoffs" n=0 f
    [[ ! -d "$d" ]] && { echo 0; return; }
    for f in "$d"/*.md; do
        [[ -f "$f" ]] || continue
...
count_bug_backlog() {
    local f="$STATE_ROOT/bug-backlog.md" n=0
    [[ ! -f "$f" ]] && { echo 0; return; }
    n=$(grep -cE '^- (BS-|\*\*BS-)' "$f" 2>/dev/null || echo 0)
    _clean_int "$n"
}
count_local_queue() {
    local f="$STATE_ROOT/improvement-queue.md" n=0
    [[ ! -f "$f" ]] && { echo 0; return; }
