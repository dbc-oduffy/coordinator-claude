@echo off
setlocal
REM Windows launcher for fix-concrete-path-citations.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "fix-concrete-path-citations.ps1" sibling of
REM     this file over this "fix-concrete-path-citations.cmd" file itself -- if both exist, `fix-concrete-path-citations`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "fix-concrete-path-citations.py"
...
echo [fix-concrete-path-citations] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [fix-concrete-path-citations] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0fix-concrete-path-citations.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0fix-concrete-path-citations.py" %*
exit /b %ERRORLEVEL%
