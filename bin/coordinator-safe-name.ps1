# coordinator-safe-name.ps1 — python-direct Windows launcher for coordinator-safe-name.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "coordinator-safe-name.ps1" file over
#     any co-located "coordinator-safe-name.cmd" sibling -- `coordinator-safe-name` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "coordinator-safe-name.py"
...
$_entry = Join-Path $_here 'coordinator-safe-name.py'
...
[Console]::Error.WriteLine('[coordinator-safe-name] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[coordinator-safe-name] Install Python: https://www.python.org/downloads/windows/')
exit 127
