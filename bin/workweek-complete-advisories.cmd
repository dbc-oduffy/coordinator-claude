@echo off
setlocal
REM Windows launcher for workweek-complete-advisories.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "workweek-complete-advisories.ps1" sibling of
REM     this file over this "workweek-complete-advisories.cmd" file itself -- if both exist, `workweek-complete-advisories`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "workweek-complete-advisories.py"
...
echo [workweek-complete-advisories] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [workweek-complete-advisories] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0workweek-complete-advisories.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0workweek-complete-advisories.py" %*
exit /b %ERRORLEVEL%
