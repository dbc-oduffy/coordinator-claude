@echo off
setlocal
REM Windows launcher for reap-sessions.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "reap-sessions.ps1" sibling of
REM     this file over this "reap-sessions.cmd" file itself -- if both exist, `reap-sessions`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "reap-sessions.py"
...
echo [reap-sessions] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [reap-sessions] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0reap-sessions.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0reap-sessions.py" %*
exit /b %ERRORLEVEL%
