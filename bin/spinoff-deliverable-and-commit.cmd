@echo off
setlocal
REM Windows launcher for spinoff-deliverable-and-commit.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "spinoff-deliverable-and-commit.ps1" sibling of
REM     this file over this "spinoff-deliverable-and-commit.cmd" file itself -- if both exist, `spinoff-deliverable-and-commit`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "spinoff-deliverable-and-commit.py"
...
echo [spinoff-deliverable-and-commit] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [spinoff-deliverable-and-commit] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0spinoff-deliverable-and-commit.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0spinoff-deliverable-and-commit.py" %*
exit /b %ERRORLEVEL%
