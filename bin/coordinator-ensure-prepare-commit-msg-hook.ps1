# coordinator-ensure-prepare-commit-msg-hook.ps1 — python-direct Windows launcher for coordinator-ensure-prepare-commit-msg-hook.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "coordinator-ensure-prepare-commit-msg-hook.ps1" file over
#     any co-located "coordinator-ensure-prepare-commit-msg-hook.cmd" sibling -- `coordinator-ensure-prepare-commit-msg-hook` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "coordinator-ensure-prepare-commit-msg-hook.py"
...
$_entry = Join-Path $_here 'coordinator-ensure-prepare-commit-msg-hook.py'
...
[Console]::Error.WriteLine('[coordinator-ensure-prepare-commit-msg-hook] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[coordinator-ensure-prepare-commit-msg-hook] Install Python: https://www.python.org/downloads/windows/')
exit 127
