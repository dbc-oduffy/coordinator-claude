@echo off
setlocal
REM Windows launcher for list-reverse-drift-cmds.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "list-reverse-drift-cmds.ps1" sibling of
REM     this file over this "list-reverse-drift-cmds.cmd" file itself -- if both exist, `list-reverse-drift-cmds`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "list-reverse-drift-cmds.py"
...
echo [list-reverse-drift-cmds] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [list-reverse-drift-cmds] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0list-reverse-drift-cmds.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0list-reverse-drift-cmds.py" %*
exit /b %ERRORLEVEL%
