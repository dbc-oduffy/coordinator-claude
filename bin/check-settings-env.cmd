@echo off
setlocal
REM Windows launcher for check-settings-env.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "check-settings-env.ps1" sibling of
REM     this file over this "check-settings-env.cmd" file itself -- if both exist, `check-settings-env`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "check-settings-env.py"
...
echo [check-settings-env] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [check-settings-env] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0check-settings-env.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0check-settings-env.py" %*
exit /b %ERRORLEVEL%
