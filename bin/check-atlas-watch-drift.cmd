@echo off
setlocal
REM Windows launcher for check-atlas-watch-drift.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "check-atlas-watch-drift.ps1" sibling of
REM     this file over this "check-atlas-watch-drift.cmd" file itself -- if both exist, `check-atlas-watch-drift`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "check-atlas-watch-drift.py"
...
echo [check-atlas-watch-drift] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [check-atlas-watch-drift] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0check-atlas-watch-drift.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0check-atlas-watch-drift.py" %*
exit /b %ERRORLEVEL%
