# writes-ignored-check.ps1 — python-direct Windows launcher for writes-ignored-check.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "writes-ignored-check.ps1" file over
#     any co-located "writes-ignored-check.cmd" sibling -- `writes-ignored-check` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "writes-ignored-check.py"
...
$_entry = Join-Path $_here 'writes-ignored-check.py'
...
[Console]::Error.WriteLine('[writes-ignored-check] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[writes-ignored-check] Install Python: https://www.python.org/downloads/windows/')
exit 127
