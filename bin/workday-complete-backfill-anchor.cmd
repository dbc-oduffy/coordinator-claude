@echo off
setlocal
REM Windows launcher for workday-complete-backfill-anchor.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "workday-complete-backfill-anchor.ps1" sibling of
REM     this file over this "workday-complete-backfill-anchor.cmd" file itself -- if both exist, `workday-complete-backfill-anchor`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "workday-complete-backfill-anchor.py"
...
echo [workday-complete-backfill-anchor] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [workday-complete-backfill-anchor] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0workday-complete-backfill-anchor.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0workday-complete-backfill-anchor.py" %*
exit /b %ERRORLEVEL%
