@echo off
setlocal
REM Windows launcher for klabauter-channel.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "klabauter-channel.ps1" sibling of
REM     this file over this "klabauter-channel.cmd" file itself -- if both exist, `klabauter-channel`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "klabauter-channel.py"
...
echo [klabauter-channel] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [klabauter-channel] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0klabauter-channel.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0klabauter-channel.py" %*
exit /b %ERRORLEVEL%
