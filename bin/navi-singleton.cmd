@echo off
setlocal
REM Windows launcher for navi-singleton.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "navi-singleton.ps1" sibling of
REM     this file over this "navi-singleton.cmd" file itself -- if both exist, `navi-singleton`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "navi-singleton.py"
...
echo [navi-singleton] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [navi-singleton] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0navi-singleton.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0navi-singleton.py" %*
exit /b %ERRORLEVEL%
