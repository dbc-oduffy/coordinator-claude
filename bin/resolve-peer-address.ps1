# resolve-peer-address.ps1 — python-direct Windows launcher for resolve-peer-address.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "resolve-peer-address.ps1" file over
#     any co-located "resolve-peer-address.cmd" sibling -- `resolve-peer-address` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "resolve-peer-address.py"
...
$_entry = Join-Path $_here 'resolve-peer-address.py'
...
[Console]::Error.WriteLine('[resolve-peer-address] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[resolve-peer-address] Install Python: https://www.python.org/downloads/windows/')
exit 127
