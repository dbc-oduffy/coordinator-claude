# plan-capture-persist.ps1 — python-direct Windows launcher for plan-capture-persist.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "plan-capture-persist.ps1" file over
#     any co-located "plan-capture-persist.cmd" sibling -- `plan-capture-persist` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "plan-capture-persist.py"
...
$_entry = Join-Path $_here 'plan-capture-persist.py'
...
[Console]::Error.WriteLine('[plan-capture-persist] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[plan-capture-persist] Install Python: https://www.python.org/downloads/windows/')
exit 127
