...
gitignore_patterns=()
commit_paths=()
commit_add_paths=()    # paths to `git add` — real, existing worktree/index paths (excludes vanished rename sources)
...
while IFS= read -r status_line; do
  ...
  #    Latent-bug fix: rename lines (xy starts with R) must NOT run this check
  #    against the destination path. `git diff --quiet -- <rename-dest>` exits 0
  #    for a pure content-preserving rename (there is no *unstaged* diff — the
  #    only diff is staged, from the rename itself), which wrongly classified
  #    the destination as EOL-phantom and skipped it, leaving it staged-but-
  #    uncommitted after the AUTO-COMMIT act block ran (the rename source was
  #    committed via commit_paths, but the destination was never added to any
  #    accumulator). Root cause of the vanished-rename-source abort investigated
  #    under this same task; this sibling gap surfaced empirically while
  #    validating the fix (2026-07-09).
  case "$xy" in
    "??"*) : ;;  # untracked — skip EOL check
    R*) : ;;     # rename/copy destination — skip EOL check, let classification below run
    *)
  ...
  if [[ "$commit_match" -eq 1 ]]; then
    commit_paths+=("$path")
    # Review: code-reviewer (Finding 1) — `RD` porcelain (rename staged, then the
    # destination was deleted from the worktree) leaves `$path` (the destination)
    # non-existent on disk. `git add -- <RD-vanished-dest>` doesn't abort loudly like
    # the source-side case; it silently unstages the rename pairing (converts it to a
    # bare deletion of the source), after which the later `git commit -- ... "$path"`
    # fails ("pathspec did not match any file(s) known to git") because the destination
    # no longer exists in the index either. Only `git add` when the destination is still
    # present in the worktree (Y status != D); always fold into commit_paths regardless.
    if [[ "${xy:1:1}" != "D" ]]; then
      commit_add_paths+=("$path")
    fi
  ...
  case "$rest" in
    *" -> "*)
      ...
      if [[ "$src_commit" -eq 1 ]]; then
        # Rename source is already staged as a deletion and no longer exists in the worktree.
        # It must appear in the `git commit` pathspec (so the staged deletion folds into the
        # rename commit) but must NEVER be `git add`ed — `git add` of a vanished path aborts
        # with exit 128. Hence commit_paths only, not commit_add_paths.
        # Review: code-reviewer (Finding 4) — do not record a synthetic "rename-src" root
        # (not a real path prefix, corrupts the human-readable root list) or double-count
        # cnt_commit for what is one logical rename; the destination side already recorded
        # the real root and incremented the count in the main classification loop above.
        commit_paths+=("$src")
        continue
      fi
      ...
      if [[ "$src_gi" -eq 1 ]]; then
        # Review: code-reviewer (spec-adjacent ask) — source is already gone (renamed
        # away) but its staged deletion was previously folded into NEITHER the gitignore
        # act block nor the AUTO-COMMIT act block, leaving it dangling-staged after the
        # script reports OK/exit 0. Fold into commit_paths (not commit_add_paths — the
        # source is vanished) so the AUTO-COMMIT act block commits the staged deletion.
        # Still counted under cnt_gitignore for the summary (gitignore-classified).
        commit_paths+=("$src")
        cnt_gitignore=$(( cnt_gitignore + 1 ))
        continue
      fi
...
if [[ "${#commit_paths[@]}" -gt 0 ]]; then
  ...
  if [[ "$DRY_RUN" -eq 1 ]]; then
    ...
    # git add only the real (existing) paths; rename sources are already staged-deleted
    # and must not be `git add`ed (a vanished pathspec aborts with exit 128). The mask on
    # this git add is intentionally dropped so genuine staging failures surface (memo
    # 2026-07-09 defense-in-depth ask).
    if [[ "${#commit_add_paths[@]}" -gt 0 ]]; then
