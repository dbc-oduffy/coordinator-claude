@echo off
setlocal
REM Windows launcher for sweep-terminal-plans.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "sweep-terminal-plans.ps1" sibling of
REM     this file over this "sweep-terminal-plans.cmd" file itself -- if both exist, `sweep-terminal-plans`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "sweep-terminal-plans.py"
...
echo [sweep-terminal-plans] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [sweep-terminal-plans] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0sweep-terminal-plans.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0sweep-terminal-plans.py" %*
exit /b %ERRORLEVEL%
