...
#
#   2. Operator-private repo / project codename → synthetic OSS-safe placeholder
#      Use this slot for YOUR OWN private project names that are not part of
#      the coordinator system itself (e.g. a client codename, an internal
#      product name, a private experiment repo you happen to cite in a wiki).
#
#      Coordinator-system-authored codenames (the built-in system codenames
#      that ship seeded for every operator) belong in the
#      COMMITTED built-in seed file:
#        coordinator/bin/codename-provenance-seed.sh
#      That file is .percolate-ignore'd so it never ships to the OSS mirror,
#      and is sourced automatically by publish-time-transform.sh. Do NOT add
#      coordinator-authored codenames here — they must fire for all operators
#      who publish the coordinator system, not just for you personally.
#
...
declare -A OPERATOR_NAME_TO_ROLE=(
    ...
    # Class 2 — operator-private project codename → synthetic placeholder
    #   (coordinator system codenames go in codename-provenance-seed.sh, not here)
    # ["my-client-project"]="example-repo"
