# mise-prep-entry.ps1 — python-direct Windows launcher for mise-prep-entry.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "mise-prep-entry.ps1" file over
#     any co-located "mise-prep-entry.cmd" sibling -- `mise-prep-entry` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "mise-prep-entry.py"
...
$_entry = Join-Path $_here 'mise-prep-entry.py'
...
[Console]::Error.WriteLine('[mise-prep-entry] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[mise-prep-entry] Install Python: https://www.python.org/downloads/windows/')
exit 127
