@echo off
setlocal
REM Windows launcher for sync-main.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "sync-main.ps1" sibling of
REM     this file over this "sync-main.cmd" file itself -- if both exist, `sync-main`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "sync-main.py"
...
echo [sync-main] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [sync-main] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
