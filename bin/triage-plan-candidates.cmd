@echo off
setlocal
REM Windows launcher for triage-plan-candidates.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "triage-plan-candidates.ps1" sibling of
REM     this file over this "triage-plan-candidates.cmd" file itself -- if both exist, `triage-plan-candidates`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "triage-plan-candidates.py"
...
echo [triage-plan-candidates] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [triage-plan-candidates] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0triage-plan-candidates.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0triage-plan-candidates.py" %*
exit /b %ERRORLEVEL%
