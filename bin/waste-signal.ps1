# waste-signal.ps1 — python-direct Windows launcher for waste-signal.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "waste-signal.ps1" file over
#     any co-located "waste-signal.cmd" sibling -- `waste-signal` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "waste-signal.py"
...
$_entry = Join-Path $_here 'waste-signal.py'
...
[Console]::Error.WriteLine('[waste-signal] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[waste-signal] Install Python: https://www.python.org/downloads/windows/')
exit 127
