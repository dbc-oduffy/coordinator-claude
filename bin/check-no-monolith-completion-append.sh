...
#   6. Lines under hooks/scripts/tests/ — the monolith-write tripwire's own self-test
#      fixtures carry literal monolith-path JSON strings as test INPUT, not write calls.
#
...
while IFS= read -r line; do
  ...
  # Exception 6: hooks/scripts/tests/ — the monolith-write tripwire's own self-test
  # fixtures (e.g. test-block-completion-monolith-write.sh) carry literal monolith-path
  # JSON strings as test INPUT for the runtime hook, not as write calls. Scanning the
  # tripwire's self-test file always fires the guard it exists to test (false positive
  # latent since the W4.2 self-test relocation). Real writes never live in a test harness.
  echo "$line" | grep -qF "/hooks/scripts/tests/" && continue
...
echo "<!-- TRIPWIRE: comment markers, and hooks/scripts/tests/ self-test fixtures." >&2
