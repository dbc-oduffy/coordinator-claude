@echo off
setlocal
REM Windows launcher for plan-assemble.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "plan-assemble.ps1" sibling of
REM     this file over this "plan-assemble.cmd" file itself -- if both exist, `plan-assemble`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "plan-assemble.py"
...
echo [plan-assemble] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [plan-assemble] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0plan-assemble.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0plan-assemble.py" %*
exit /b %ERRORLEVEL%
