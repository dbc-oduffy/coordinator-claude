@echo off
setlocal
REM Windows launcher for reap-orphaned-in-flight-handoffs.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "reap-orphaned-in-flight-handoffs.ps1" sibling of
REM     this file over this "reap-orphaned-in-flight-handoffs.cmd" file itself -- if both exist, `reap-orphaned-in-flight-handoffs`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "reap-orphaned-in-flight-handoffs.py"
...
echo [reap-orphaned-in-flight-handoffs] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [reap-orphaned-in-flight-handoffs] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0reap-orphaned-in-flight-handoffs.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0reap-orphaned-in-flight-handoffs.py" %*
exit /b %ERRORLEVEL%
