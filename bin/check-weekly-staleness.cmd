@echo off
setlocal
REM Windows launcher for check-weekly-staleness.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "check-weekly-staleness.ps1" sibling of
REM     this file over this "check-weekly-staleness.cmd" file itself -- if both exist, `check-weekly-staleness`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "check-weekly-staleness.py"
...
echo [check-weekly-staleness] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [check-weekly-staleness] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0check-weekly-staleness.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0check-weekly-staleness.py" %*
exit /b %ERRORLEVEL%
