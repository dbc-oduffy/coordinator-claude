@echo off
setlocal
REM Windows launcher for doctor.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "doctor.ps1" sibling of
REM     this file over this "doctor.cmd" file itself -- if both exist, `doctor`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "doctor.py"
...
echo [doctor] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [doctor] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
