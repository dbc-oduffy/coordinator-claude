@echo off
setlocal
REM Windows launcher for tier-u-grant-cli.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "tier-u-grant-cli.ps1" sibling of
REM     this file over this "tier-u-grant-cli.cmd" file itself -- if both exist, `tier-u-grant-cli`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "tier-u-grant-cli.py"
...
echo [tier-u-grant-cli] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [tier-u-grant-cli] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0tier-u-grant-cli.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0tier-u-grant-cli.py" %*
exit /b %ERRORLEVEL%
