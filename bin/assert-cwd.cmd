@echo off
setlocal
REM Windows launcher for assert-cwd.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "assert-cwd.ps1" sibling of
REM     this file over this "assert-cwd.cmd" file itself -- if both exist, `assert-cwd`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "assert-cwd.py"
...
echo [assert-cwd] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [assert-cwd] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0assert-cwd.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0assert-cwd.py" %*
exit /b %ERRORLEVEL%
