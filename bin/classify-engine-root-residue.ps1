# classify-engine-root-residue.ps1 — python-direct Windows launcher for classify-engine-root-residue.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "classify-engine-root-residue.ps1" file over
#     any co-located "classify-engine-root-residue.cmd" sibling -- `classify-engine-root-residue` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "classify-engine-root-residue.py"
...
$_entry = Join-Path $_here 'classify-engine-root-residue.py'
...
[Console]::Error.WriteLine('[classify-engine-root-residue] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[classify-engine-root-residue] Install Python: https://www.python.org/downloads/windows/')
exit 127
