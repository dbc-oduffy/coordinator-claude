@echo off
setlocal
REM Windows launcher for staff-session-assemble.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "staff-session-assemble.ps1" sibling of
REM     this file over this "staff-session-assemble.cmd" file itself -- if both exist, `staff-session-assemble`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "staff-session-assemble.py"
...
:run_baked
"%_py%" "%~dp0staff-session-assemble.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0staff-session-assemble.py" %*
exit /b %ERRORLEVEL%
