# query-commit-closures.ps1 — python-direct Windows launcher for query-commit-closures.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "query-commit-closures.ps1" file over
#     any co-located "query-commit-closures.cmd" sibling -- `query-commit-closures` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "query-commit-closures.py"
...
$_entry = Join-Path $_here 'query-commit-closures.py'
...
[Console]::Error.WriteLine('[query-commit-closures] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[query-commit-closures] Install Python: https://www.python.org/downloads/windows/')
exit 127
