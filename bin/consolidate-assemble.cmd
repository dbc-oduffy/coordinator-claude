@echo off
setlocal
REM Windows launcher for consolidate-assemble.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "consolidate-assemble.ps1" sibling of
REM     this file over this "consolidate-assemble.cmd" file itself -- if both exist, `consolidate-assemble`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "consolidate-assemble.py"
...
echo [consolidate-assemble] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [consolidate-assemble] Install Python: https://www.python.org/downloads/windows/ 1>&2
...
:run_baked
"%_py%" "%~dp0consolidate-assemble.py" %*
2>nul rd /s /q "%_LAUNCHER_RAW_CMDLINE_DIR%"
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0consolidate-assemble.py" %*
2>nul rd /s /q "%_LAUNCHER_RAW_CMDLINE_DIR%"
exit /b %ERRORLEVEL%
