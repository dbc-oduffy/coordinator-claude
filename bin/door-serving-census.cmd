@echo off
setlocal
REM Windows launcher for door-serving-census.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "door-serving-census.ps1" sibling of
REM     this file over this "door-serving-census.cmd" file itself -- if both exist, `door-serving-census`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "door-serving-census.py"
...
echo [door-serving-census] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [door-serving-census] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0door-serving-census.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0door-serving-census.py" %*
exit /b %ERRORLEVEL%
