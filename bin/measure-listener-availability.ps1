# measure-listener-availability.ps1 — python-direct Windows launcher for measure-listener-availability.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "measure-listener-availability.ps1" file over
#     any co-located "measure-listener-availability.cmd" sibling -- `measure-listener-availability` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "measure-listener-availability.py"
...
$_entry = Join-Path $_here 'measure-listener-availability.py'
...
[Console]::Error.WriteLine('[measure-listener-availability] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[measure-listener-availability] Install Python: https://www.python.org/downloads/windows/')
exit 127
