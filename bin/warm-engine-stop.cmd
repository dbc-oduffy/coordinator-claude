@echo off
setlocal
REM Windows launcher for warm-engine-stop.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "warm-engine-stop.ps1" sibling of
REM     this file over this "warm-engine-stop.cmd" file itself -- if both exist, `warm-engine-stop`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "warm-engine-stop.py"
...
echo [warm-engine-stop] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [warm-engine-stop] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0warm-engine-stop.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0warm-engine-stop.py" %*
exit /b %ERRORLEVEL%
