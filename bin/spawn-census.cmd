@echo off
setlocal
REM Windows launcher for spawn-census — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "spawn-census.ps1" sibling of
REM     this file over this "spawn-census.cmd" file itself -- if both exist, `spawn-census`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "spawn-census"
...
echo [spawn-census] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [spawn-census] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
