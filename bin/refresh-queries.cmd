@echo off
setlocal
REM Windows launcher for refresh-queries.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "refresh-queries.ps1" sibling of
REM     this file over this "refresh-queries.cmd" file itself -- if both exist, `refresh-queries`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "refresh-queries.py"
...
echo [refresh-queries] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [refresh-queries] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0refresh-queries.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0refresh-queries.py" %*
exit /b %ERRORLEVEL%
