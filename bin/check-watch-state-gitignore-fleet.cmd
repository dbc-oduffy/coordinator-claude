@echo off
setlocal
REM Windows launcher for check-watch-state-gitignore-fleet.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "check-watch-state-gitignore-fleet.ps1" sibling of
REM     this file over this "check-watch-state-gitignore-fleet.cmd" file itself -- if both exist, `check-watch-state-gitignore-fleet`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "check-watch-state-gitignore-fleet.py"
...
echo [check-watch-state-gitignore-fleet] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [check-watch-state-gitignore-fleet] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0check-watch-state-gitignore-fleet.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0check-watch-state-gitignore-fleet.py" %*
exit /b %ERRORLEVEL%
