@echo off
setlocal
REM Windows launcher for coordinator-session-loe.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "coordinator-session-loe.ps1" sibling of
REM     this file over this "coordinator-session-loe.cmd" file itself -- if both exist, `coordinator-session-loe`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "coordinator-session-loe.py"
...
echo [coordinator-session-loe] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [coordinator-session-loe] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0coordinator-session-loe.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0coordinator-session-loe.py" %*
exit /b %ERRORLEVEL%
