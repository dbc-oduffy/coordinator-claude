@echo off
setlocal
REM Windows launcher for merge-assemble.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "merge-assemble.ps1" sibling of
REM     this file over this "merge-assemble.cmd" file itself -- if both exist, `merge-assemble`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "merge-assemble.py"
...
echo [merge-assemble] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [merge-assemble] Install Python: https://www.python.org/downloads/windows/ 1>&2
...
:run_baked
"%_py%" "%~dp0merge-assemble.py" %*
2>nul rd /s /q "%_LAUNCHER_RAW_CMDLINE_DIR%"
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0merge-assemble.py" %*
2>nul rd /s /q "%_LAUNCHER_RAW_CMDLINE_DIR%"
exit /b %ERRORLEVEL%
