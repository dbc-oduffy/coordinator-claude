@echo off
setlocal
REM Windows launcher for fleet-env.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "fleet-env.ps1" sibling of
REM     this file over this "fleet-env.cmd" file itself -- if both exist, `fleet-env`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "fleet-env.py"
...
echo [fleet-env] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [fleet-env] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
