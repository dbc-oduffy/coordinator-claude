@echo off
setlocal
REM Windows launcher for check-em-environment.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "check-em-environment.ps1" sibling of
REM     this file over this "check-em-environment.cmd" file itself -- if both exist, `check-em-environment`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "check-em-environment.py"
...
echo [check-em-environment] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [check-em-environment] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0check-em-environment.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0check-em-environment.py" %*
exit /b %ERRORLEVEL%
