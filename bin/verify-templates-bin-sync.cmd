@echo off
setlocal
REM Windows launcher for verify-templates-bin-sync.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "verify-templates-bin-sync.ps1" sibling of
REM     this file over this "verify-templates-bin-sync.cmd" file itself -- if both exist, `verify-templates-bin-sync`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "verify-templates-bin-sync.py"
...
echo [verify-templates-bin-sync] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [verify-templates-bin-sync] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0verify-templates-bin-sync.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0verify-templates-bin-sync.py" %*
exit /b %ERRORLEVEL%
