...
# Source portable timeout primitive (cs_timeout) — conditional: missing lib → probe soft-degrades,
# dispatch tool still runs. Mirrors release-currency.sh:85-88 conditional-source pattern.
# cs_timeout absence is guarded at the call site below (declare -f cs_timeout guard).
# Review: code-reviewer S3 F2 — unconditional source hard-aborts dispatch on missing lib under set -e.
[[ -f "${SCRIPT_DIR}/../lib/coordinator-watchdog.sh" ]] && source "${SCRIPT_DIR}/../lib/coordinator-watchdog.sh"
...
if [[ -n "$PLAN_PATH" ]]; then
    ...
    for i in "${!CHUNK_IDS[@]}"; do
        ...
        if [[ ! -f "$sidecar_path" ]]; then
            coordinator-doc-new --type flight-recorder --plan "$PLAN_PATH" --chunk "$cid" >/dev/null
        fi
    done
fi
...
if [[ -f "${SCRIPT_DIR}/probe-memory-headroom.sh" ]]; then
    ...
    # external tool can never stall dispatch; cs_timeout is portable across GNU/BSD/macOS.
    # Guard: if the watchdog lib is absent (partial install), fall back to bare bash —
    # restores pre-retrofit soft-degradation; || PROBE_OUT="" preserved in both paths.
    # Review: code-reviewer S3 F2 — guard cs_timeout call for absent watchdog lib case.
    if declare -f cs_timeout >/dev/null 2>&1; then
        PROBE_OUT="$(cs_timeout 10 -- bash "${SCRIPT_DIR}/probe-memory-headroom.sh" 2>/dev/null)" || PROBE_OUT=""
    else
