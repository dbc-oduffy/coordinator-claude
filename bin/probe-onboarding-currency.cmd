@echo off
setlocal
REM Windows launcher for probe-onboarding-currency.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "probe-onboarding-currency.ps1" sibling of
REM     this file over this "probe-onboarding-currency.cmd" file itself -- if both exist, `probe-onboarding-currency`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "probe-onboarding-currency.py"
...
echo [probe-onboarding-currency] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [probe-onboarding-currency] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0probe-onboarding-currency.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0probe-onboarding-currency.py" %*
exit /b %ERRORLEVEL%
