# derive-session-hierarchy.ps1 — python-direct Windows launcher for derive-session-hierarchy.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "derive-session-hierarchy.ps1" file over
#     any co-located "derive-session-hierarchy.cmd" sibling -- `derive-session-hierarchy` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "derive-session-hierarchy.py"
...
$_entry = Join-Path $_here 'derive-session-hierarchy.py'
...
[Console]::Error.WriteLine('[derive-session-hierarchy] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[derive-session-hierarchy] Install Python: https://www.python.org/downloads/windows/')
exit 127
