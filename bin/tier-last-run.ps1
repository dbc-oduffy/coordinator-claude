# tier-last-run.ps1 — python-direct Windows launcher for tier-last-run.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "tier-last-run.ps1" file over
#     any co-located "tier-last-run.cmd" sibling -- `tier-last-run` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "tier-last-run.py"
...
$_entry = Join-Path $_here 'tier-last-run.py'
...
[Console]::Error.WriteLine('[tier-last-run] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[tier-last-run] Install Python: https://www.python.org/downloads/windows/')
exit 127
