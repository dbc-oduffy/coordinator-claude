@echo off
setlocal
REM Windows launcher for check-anchor-freshness.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "check-anchor-freshness.ps1" sibling of
REM     this file over this "check-anchor-freshness.cmd" file itself -- if both exist, `check-anchor-freshness`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "check-anchor-freshness.py"
...
echo [check-anchor-freshness] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [check-anchor-freshness] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0check-anchor-freshness.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0check-anchor-freshness.py" %*
exit /b %ERRORLEVEL%
