@echo off
setlocal
REM Windows launcher for fan-out-dispatch.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "fan-out-dispatch.ps1" sibling of
REM     this file over this "fan-out-dispatch.cmd" file itself -- if both exist, `fan-out-dispatch`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "fan-out-dispatch.py"
...
echo [fan-out-dispatch] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [fan-out-dispatch] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0fan-out-dispatch.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0fan-out-dispatch.py" %*
exit /b %ERRORLEVEL%
