@echo off
setlocal
REM Windows launcher for render-template.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "render-template.ps1" sibling of
REM     this file over this "render-template.cmd" file itself -- if both exist, `render-template`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "render-template.py"
...
echo [render-template] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [render-template] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0render-template.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0render-template.py" %*
exit /b %ERRORLEVEL%
