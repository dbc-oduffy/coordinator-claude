@echo off
setlocal
REM Windows launcher for query-completion-rollups.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "query-completion-rollups.ps1" sibling of
REM     this file over this "query-completion-rollups.cmd" file itself -- if both exist, `query-completion-rollups`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "query-completion-rollups.py"
...
echo [query-completion-rollups] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [query-completion-rollups] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0query-completion-rollups.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0query-completion-rollups.py" %*
exit /b %ERRORLEVEL%
