@echo off
setlocal
REM Windows launcher for migrate-bug-backlog.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "migrate-bug-backlog.ps1" sibling of
REM     this file over this "migrate-bug-backlog.cmd" file itself -- if both exist, `migrate-bug-backlog`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "migrate-bug-backlog.py"
...
echo [migrate-bug-backlog] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [migrate-bug-backlog] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127

:run_baked
"%_py%" "%~dp0migrate-bug-backlog.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0migrate-bug-backlog.py" %*
exit /b %ERRORLEVEL%
