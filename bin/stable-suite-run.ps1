# stable-suite-run.ps1 — python-direct Windows launcher for stable-suite-run.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "stable-suite-run.ps1" file over
#     any co-located "stable-suite-run.cmd" sibling -- `stable-suite-run` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "stable-suite-run.py"
...
$_entry = Join-Path $_here 'stable-suite-run.py'
...
[Console]::Error.WriteLine('[stable-suite-run] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[stable-suite-run] Install Python: https://www.python.org/downloads/windows/')
exit 127
