...
#   7. legacy: conditional-consumer parity (registry vs. script example-game-repo block)
#
# Spec backlinks:
...
if [ "${1:-}" = "--list" ]; then
    ...
    for name in "${SNIPPET_NAMES[@]}"; do
        ...
        echo "check:conditional_parity[$name] — (legacy only) conditional consumers: registry.toml vs. example-game-repo block in verify script"
    done
    exit 0
fi
...
# Extract conditional example-game-repo consumers: look for HARDCODED_CONSUMERS+=("$_EXAMPLE_GAME_REPO_ROOT/<path>")
cond_entries = re.findall(r'HARDCODED_CONSUMERS\+=\("?\$_EXAMPLE_GAME_REPO_ROOT/([^")]+)"?\)', content)
for cpath in cond_entries:
    print(f"COND {cpath}")

# Detect which condition_key is used (the script uses repos.example_game_workbench_repo)
if 'repos.example_game_workbench_repo' in content:
    print("COND_KEY repos.example_game_workbench_repo")
PYEOF
}
...
for name in "${SNIPPET_NAMES[@]}"; do
    ...
    # Build script conditional paths (relative to example-game-repo root)
    SCRIPT_COND_PATHS=""
    while IFS= read -r line; do
        case "$line" in
            "COND "*)
    ...
    if [ "$sorted_reg_cond" != "$sorted_scr_cond" ]; then
        ...
        if [ -n "$MISSING_FROM_REG" ]; then
            echo "FAIL [conditional-parity] $name: example-game-repo paths in script but missing from registry conditional_consumer:" >&2
            while IFS= read -r p; do
                [ -n "$p" ] && echo "  missing-from-registry: $p" >&2
            done <<< "$MISSING_FROM_REG"
        fi
        if [ -n "$MISSING_FROM_SCR" ]; then
            echo "FAIL [conditional-parity] $name: example-game-repo paths in registry conditional_consumer but missing from script:" >&2
            while IFS= read -r p; do
                [ -n "$p" ] && echo "  missing-from-script: $p" >&2
    ...
    # Verify condition_key consistency for example-game-repo conditionals
    # Review: code-reviewer F2 — check fires whenever registry has conditional_consumer entries
    # (REG_COND non-empty), not only when the script has a example-game-repo conditional (SCR_COND).
    # Previously, a registry entry with a malformed condition_key AND no script example-game-repo block
    ...
    if [ -n "$REG_COND" ]; then
        ...
        while IFS= read -r ckey; do
            [ -z "$ckey" ] && continue
            if [ "$ckey" != "repos.example_game_workbench_repo" ]; then
                echo "FAIL [conditional-parity] $name: unexpected condition_key in registry: '$ckey'" >&2
                OVERALL_EXIT=1
            fi
        done <<< "$REG_KEYS"
    elif [ -n "$SCR_COND" ]; then
        echo "FAIL [conditional-parity] $name: script has example-game-repo conditional consumer but registry has none" >&2
        OVERALL_EXIT=1
    fi
done
