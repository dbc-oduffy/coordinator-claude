@echo off
setlocal
REM Windows launcher for migrate-cross-repo-layout.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "migrate-cross-repo-layout.ps1" sibling of
REM     this file over this "migrate-cross-repo-layout.cmd" file itself -- if both exist, `migrate-cross-repo-layout`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "migrate-cross-repo-layout.py"
...
echo [migrate-cross-repo-layout] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [migrate-cross-repo-layout] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127

:run_baked
"%_py%" "%~dp0migrate-cross-repo-layout.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0migrate-cross-repo-layout.py" %*
exit /b %ERRORLEVEL%
