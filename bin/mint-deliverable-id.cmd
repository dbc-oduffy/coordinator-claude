@echo off
setlocal
REM Windows launcher for mint-deliverable-id.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "mint-deliverable-id.ps1" sibling of
REM     this file over this "mint-deliverable-id.cmd" file itself -- if both exist, `mint-deliverable-id`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "mint-deliverable-id.py"
...
echo [mint-deliverable-id] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [mint-deliverable-id] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0mint-deliverable-id.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0mint-deliverable-id.py" %*
exit /b %ERRORLEVEL%
