@echo off
setlocal
REM Windows launcher for sweep-terminal-handoffs.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "sweep-terminal-handoffs.ps1" sibling of
REM     this file over this "sweep-terminal-handoffs.cmd" file itself -- if both exist, `sweep-terminal-handoffs`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "sweep-terminal-handoffs.py"
...
echo [sweep-terminal-handoffs] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [sweep-terminal-handoffs] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0sweep-terminal-handoffs.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0sweep-terminal-handoffs.py" %*
exit /b %ERRORLEVEL%
