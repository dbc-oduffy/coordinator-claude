@echo off
setlocal
REM Windows launcher for sync-plugin-wiki.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "sync-plugin-wiki.ps1" sibling of
REM     this file over this "sync-plugin-wiki.cmd" file itself -- if both exist, `sync-plugin-wiki`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "sync-plugin-wiki.py"
...
echo [sync-plugin-wiki] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [sync-plugin-wiki] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0sync-plugin-wiki.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0sync-plugin-wiki.py" %*
exit /b %ERRORLEVEL%
