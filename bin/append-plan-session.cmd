@echo off
setlocal
REM Windows launcher for append-plan-session.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "append-plan-session.ps1" sibling of
REM     this file over this "append-plan-session.cmd" file itself -- if both exist, `append-plan-session`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "append-plan-session.py"
...
echo [append-plan-session] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [append-plan-session] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0append-plan-session.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0append-plan-session.py" %*
exit /b %ERRORLEVEL%
