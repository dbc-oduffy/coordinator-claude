@echo off
setlocal
REM Windows launcher for gen-launcher-shim.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "gen-launcher-shim.ps1" sibling of
REM     this file over this "gen-launcher-shim.cmd" file itself -- if both exist, `gen-launcher-shim`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "gen-launcher-shim.py"
...
echo [gen-launcher-shim] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [gen-launcher-shim] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0gen-launcher-shim.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0gen-launcher-shim.py" %*
exit /b %ERRORLEVEL%
