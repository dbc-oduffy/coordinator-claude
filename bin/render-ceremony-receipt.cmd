@echo off
setlocal
REM Windows launcher for render-ceremony-receipt.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "render-ceremony-receipt.ps1" sibling of
REM     this file over this "render-ceremony-receipt.cmd" file itself -- if both exist, `render-ceremony-receipt`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "render-ceremony-receipt.py"
...
echo [render-ceremony-receipt] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [render-ceremony-receipt] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0render-ceremony-receipt.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0render-ceremony-receipt.py" %*
exit /b %ERRORLEVEL%
