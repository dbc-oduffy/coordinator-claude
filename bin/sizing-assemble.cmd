@echo off
setlocal
REM Windows launcher for sizing-assemble.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "sizing-assemble.ps1" sibling of
REM     this file over this "sizing-assemble.cmd" file itself -- if both exist, `sizing-assemble`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "sizing-assemble.py"
...
echo [sizing-assemble] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [sizing-assemble] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0sizing-assemble.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0sizing-assemble.py" %*
exit /b %ERRORLEVEL%
