@echo off
setlocal
REM Windows launcher for verify-arch-audit-atlas-refresh.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "verify-arch-audit-atlas-refresh.ps1" sibling of
REM     this file over this "verify-arch-audit-atlas-refresh.cmd" file itself -- if both exist, `verify-arch-audit-atlas-refresh`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "verify-arch-audit-atlas-refresh.py"
...
echo [verify-arch-audit-atlas-refresh] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [verify-arch-audit-atlas-refresh] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0verify-arch-audit-atlas-refresh.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0verify-arch-audit-atlas-refresh.py" %*
exit /b %ERRORLEVEL%
