@echo off
setlocal
REM Windows launcher for coordinator-resolve-validation-cmd.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "coordinator-resolve-validation-cmd.ps1" sibling of
REM     this file over this "coordinator-resolve-validation-cmd.cmd" file itself -- if both exist, `coordinator-resolve-validation-cmd`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "coordinator-resolve-validation-cmd.py"
...
echo [coordinator-resolve-validation-cmd] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [coordinator-resolve-validation-cmd] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0coordinator-resolve-validation-cmd.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0coordinator-resolve-validation-cmd.py" %*
exit /b %ERRORLEVEL%
