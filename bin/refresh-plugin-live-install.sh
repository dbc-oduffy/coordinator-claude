...
if prop_mode == "source_is_live":
    ...
    # (example-game-repo-em 2026-06-09: project-rag junction live at 0.10.0, pip show 0.8.0).
...
# NOT hardcode `install-plugin.sh <plugin> --no-enable`: the example-game-repo trio gates
# standalone component installs behind EXAMPLE_GAME_REPO_UMBRELLA_INSTALL=1 (the gate at
...
# component has no forwarder at all. The correct invocation is example-game-repo-install
...
if [[ "$PROPAGATION_MODE" == "copy_install" ]]; then
    ...
    # The cross-repo memo asks the example-game-repo installer to self-register refresh_cmd
...
refresh-plugin-live-install.sh: [copy_install] $PLUGIN has no refresh_cmd registered — cannot auto-refresh.
  Refresh manually, then re-run the probe (check-plugin-drift.sh $PLUGIN):
    example-game-repo trio (umbrella):  /example-game-repo:install
