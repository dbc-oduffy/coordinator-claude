@echo off
setlocal
REM Windows launcher for workday-complete-step2_5-dirty-tree.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "workday-complete-step2_5-dirty-tree.ps1" sibling of
REM     this file over this "workday-complete-step2_5-dirty-tree.cmd" file itself -- if both exist, `workday-complete-step2_5-dirty-tree`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "workday-complete-step2_5-dirty-tree.py"
...
echo [workday-complete-step2_5-dirty-tree] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [workday-complete-step2_5-dirty-tree] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0workday-complete-step2_5-dirty-tree.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0workday-complete-step2_5-dirty-tree.py" %*
exit /b %ERRORLEVEL%
