@echo off
setlocal
REM Windows launcher for list-orphaned-plans.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "list-orphaned-plans.ps1" sibling of
REM     this file over this "list-orphaned-plans.cmd" file itself -- if both exist, `list-orphaned-plans`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "list-orphaned-plans.py"
...
echo [list-orphaned-plans] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [list-orphaned-plans] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0list-orphaned-plans.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0list-orphaned-plans.py" %*
exit /b %ERRORLEVEL%
