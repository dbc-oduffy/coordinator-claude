@echo off
setlocal
REM Windows launcher for corpus-currency-probe.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "corpus-currency-probe.ps1" sibling of
REM     this file over this "corpus-currency-probe.cmd" file itself -- if both exist, `corpus-currency-probe`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "corpus-currency-probe.py"
...
echo [corpus-currency-probe] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [corpus-currency-probe] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0corpus-currency-probe.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0corpus-currency-probe.py" %*
exit /b %ERRORLEVEL%
