@echo off
setlocal
REM Windows launcher for workday-start-health-probes.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "workday-start-health-probes.ps1" sibling of
REM     this file over this "workday-start-health-probes.cmd" file itself -- if both exist, `workday-start-health-probes`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "workday-start-health-probes.py"
...
echo [workday-start-health-probes] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [workday-start-health-probes] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0workday-start-health-probes.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0workday-start-health-probes.py" %*
exit /b %ERRORLEVEL%
