@echo off
setlocal
REM Windows launcher for rollup-derive.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "rollup-derive.ps1" sibling of
REM     this file over this "rollup-derive.cmd" file itself -- if both exist, `rollup-derive`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "rollup-derive.py"
...
echo [rollup-derive] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [rollup-derive] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0rollup-derive.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0rollup-derive.py" %*
exit /b %ERRORLEVEL%
