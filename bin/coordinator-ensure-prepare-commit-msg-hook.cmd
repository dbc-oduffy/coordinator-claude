@echo off
setlocal
REM Windows launcher for coordinator-ensure-prepare-commit-msg-hook.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "coordinator-ensure-prepare-commit-msg-hook.ps1" sibling of
REM     this file over this "coordinator-ensure-prepare-commit-msg-hook.cmd" file itself -- if both exist, `coordinator-ensure-prepare-commit-msg-hook`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "coordinator-ensure-prepare-commit-msg-hook.py"
...
echo [coordinator-ensure-prepare-commit-msg-hook] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [coordinator-ensure-prepare-commit-msg-hook] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0coordinator-ensure-prepare-commit-msg-hook.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0coordinator-ensure-prepare-commit-msg-hook.py" %*
exit /b %ERRORLEVEL%
