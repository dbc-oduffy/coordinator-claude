@echo off
setlocal
REM Windows launcher for check-no-illegal-paths.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "check-no-illegal-paths.ps1" sibling of
REM     this file over this "check-no-illegal-paths.cmd" file itself -- if both exist, `check-no-illegal-paths`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "check-no-illegal-paths.py"
...
echo [check-no-illegal-paths] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [check-no-illegal-paths] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0check-no-illegal-paths.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0check-no-illegal-paths.py" %*
exit /b %ERRORLEVEL%
