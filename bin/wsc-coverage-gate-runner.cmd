@echo off
setlocal
REM Windows launcher for wsc-coverage-gate-runner.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "wsc-coverage-gate-runner.ps1" sibling of
REM     this file over this "wsc-coverage-gate-runner.cmd" file itself -- if both exist, `wsc-coverage-gate-runner`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "wsc-coverage-gate-runner.py"
...
echo [wsc-coverage-gate-runner] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [wsc-coverage-gate-runner] Install Python: https://www.python.org/downloads/windows/ 1>&2
...
:run_baked
"%_py%" "%~dp0wsc-coverage-gate-runner.py" %*
2>nul rd /s /q "%_LAUNCHER_RAW_CMDLINE_DIR%"
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0wsc-coverage-gate-runner.py" %*
2>nul rd /s /q "%_LAUNCHER_RAW_CMDLINE_DIR%"
exit /b %ERRORLEVEL%
