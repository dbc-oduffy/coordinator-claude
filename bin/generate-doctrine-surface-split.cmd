@echo off
setlocal
REM Windows launcher for generate-doctrine-surface-split.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "generate-doctrine-surface-split.ps1" sibling of
REM     this file over this "generate-doctrine-surface-split.cmd" file itself -- if both exist, `generate-doctrine-surface-split`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "generate-doctrine-surface-split.py"
...
echo [generate-doctrine-surface-split] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [generate-doctrine-surface-split] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0generate-doctrine-surface-split.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0generate-doctrine-surface-split.py" %*
exit /b %ERRORLEVEL%
