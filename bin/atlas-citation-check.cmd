@echo off
setlocal
REM Windows launcher for atlas-citation-check.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "atlas-citation-check.ps1" sibling of
REM     this file over this "atlas-citation-check.cmd" file itself -- if both exist, `atlas-citation-check`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "atlas-citation-check.py"
...
echo [atlas-citation-check] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [atlas-citation-check] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0atlas-citation-check.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0atlas-citation-check.py" %*
exit /b %ERRORLEVEL%
