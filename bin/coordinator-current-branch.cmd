@echo off
setlocal
REM Windows launcher for coordinator-current-branch.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "coordinator-current-branch.ps1" sibling of
REM     this file over this "coordinator-current-branch.cmd" file itself -- if both exist, `coordinator-current-branch`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "coordinator-current-branch.py"
...
echo [coordinator-current-branch] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [coordinator-current-branch] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0coordinator-current-branch.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0coordinator-current-branch.py" %*
exit /b %ERRORLEVEL%
