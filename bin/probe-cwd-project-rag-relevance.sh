...
_PROBE_SETTINGS_HOME_LIB="${_PROBE_SCRIPT_DIR}/../lib/settings-home.sh"
if [[ -f "$_PROBE_SETTINGS_HOME_LIB" ]]; then
    # shellcheck source=../lib/settings-home.sh
    source "$_PROBE_SETTINGS_HOME_LIB"
fi
unset _PROBE_SETTINGS_HOME_LIB _PROBE_SCRIPT_DIR
...
if [[ -n "${COORDINATOR_TEST_HOME:-}" ]]; then
  ...
  # Bridge COORDINATOR_TEST_HOME to the settings-home seam sandbox redirect.
  # Tests that set COORDINATOR_TEST_HOME expect it to act as the home base;
  # propagate to COORDINATOR_SETTINGS_HOME (seam override) when not already set,
  # so _coordinator_settings_home() resolves relative to the test sandbox.
  if [[ -z "${COORDINATOR_SETTINGS_HOME:-}" ]]; then
    COORDINATOR_SETTINGS_HOME="${COORDINATOR_TEST_HOME}/.coordinator-claude-settings"
  fi
else
  HOME_BASE="$HOME"
fi
...
# Resolve machine-local registry via the settings-home seam (C1).
# machine-local has moved out of ~/.claude into <settings-home>/machine-local.
if declare -f _coordinator_settings_home >/dev/null 2>&1; then
  _probe_ml_dir="$(_coordinator_settings_home)/machine-local"
else
  # Seam unavailable (cold bootstrap) — derive from HOME_BASE using seam convention.
  _probe_ml_dir="${HOME_BASE}/.coordinator-claude-settings/machine-local"
fi
REGISTRY_LOCAL_TOML="${_probe_ml_dir}/registry.local.toml"
REGISTRY_TOML="${_probe_ml_dir}/registry.toml"
unset _probe_ml_dir
