@echo off
setlocal
REM Windows launcher for write-identity-file.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "write-identity-file.ps1" sibling of
REM     this file over this "write-identity-file.cmd" file itself -- if both exist, `write-identity-file`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "write-identity-file.py"
...
echo [write-identity-file] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [write-identity-file] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0write-identity-file.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0write-identity-file.py" %*
exit /b %ERRORLEVEL%
