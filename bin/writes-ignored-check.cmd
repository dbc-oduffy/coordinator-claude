@echo off
setlocal
REM Windows launcher for writes-ignored-check.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "writes-ignored-check.ps1" sibling of
REM     this file over this "writes-ignored-check.cmd" file itself -- if both exist, `writes-ignored-check`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "writes-ignored-check.py"
...
echo [writes-ignored-check] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [writes-ignored-check] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0writes-ignored-check.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0writes-ignored-check.py" %*
exit /b %ERRORLEVEL%
