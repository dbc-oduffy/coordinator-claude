@echo off
setlocal
REM Windows launcher for plan-tasks-resolve — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "plan-tasks-resolve.ps1" sibling of
REM     this file over this "plan-tasks-resolve.cmd" file itself -- if both exist, `plan-tasks-resolve`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "plan-tasks-resolve"
...
echo [plan-tasks-resolve] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [plan-tasks-resolve] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0plan-tasks-resolve" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0plan-tasks-resolve" %*
exit /b %ERRORLEVEL%
