 ...
 *   6. Roadmap-kind row shows stub_id and blocked_by columns
...
describe('render-handoff-tracker — table shape', () => {
  ...
  it('Spinoffs table has expected header columns including stub_id and blocked_by', () => {
    const md = renderRepoSection(tmpRoot);
    ...
    // roadmap spinoff exists in fixtures — stub_id and blocked_by must appear
    assert.ok(headerLine.includes('stub_id'), `Expected stub_id in spinoff header: ${headerLine}`);
...
// ---------------------------------------------------------------------------
// Test 6: Roadmap-kind row shows stub_id and blocked_by
// ---------------------------------------------------------------------------

describe('render-handoff-tracker — roadmap spinoff columns', () => {
  it('stub_id "rm-5" appears in spinoffs table', () => {
    const md = renderRepoSection(tmpRoot);
    const spinoffsIdx = md.indexOf('## Spinoffs');
    const spinoffsSection = md.slice(spinoffsIdx);
    assert.ok(spinoffsSection.includes('rm-5'), `Expected stub_id "rm-5" in Spinoffs section; got:\n${spinoffsSection.slice(0, 400)}`);
  });
