@echo off
setlocal
REM Windows launcher for emit-cockpit-snapshot.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "emit-cockpit-snapshot.ps1" sibling of
REM     this file over this "emit-cockpit-snapshot.cmd" file itself -- if both exist, `emit-cockpit-snapshot`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "emit-cockpit-snapshot.py"
...
echo [emit-cockpit-snapshot] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [emit-cockpit-snapshot] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0emit-cockpit-snapshot.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0emit-cockpit-snapshot.py" %*
exit /b %ERRORLEVEL%
