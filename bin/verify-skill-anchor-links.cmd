@echo off
setlocal
REM Windows launcher for verify-skill-anchor-links.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "verify-skill-anchor-links.ps1" sibling of
REM     this file over this "verify-skill-anchor-links.cmd" file itself -- if both exist, `verify-skill-anchor-links`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "verify-skill-anchor-links.py"
...
echo [verify-skill-anchor-links] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [verify-skill-anchor-links] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0verify-skill-anchor-links.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0verify-skill-anchor-links.py" %*
exit /b %ERRORLEVEL%
