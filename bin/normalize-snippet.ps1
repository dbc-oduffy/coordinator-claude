# normalize-snippet.ps1 — python-direct Windows launcher for normalize-snippet.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "normalize-snippet.ps1" file over
#     any co-located "normalize-snippet.cmd" sibling -- `normalize-snippet` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "normalize-snippet.py"
...
$_entry = Join-Path $_here 'normalize-snippet.py'
...
[Console]::Error.WriteLine('[normalize-snippet] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[normalize-snippet] Install Python: https://www.python.org/downloads/windows/')
exit 127
