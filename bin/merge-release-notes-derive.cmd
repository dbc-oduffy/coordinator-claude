@echo off
setlocal
REM Windows launcher for merge-release-notes-derive.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "merge-release-notes-derive.ps1" sibling of
REM     this file over this "merge-release-notes-derive.cmd" file itself -- if both exist, `merge-release-notes-derive`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "merge-release-notes-derive.py"
...
echo [merge-release-notes-derive] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [merge-release-notes-derive] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0merge-release-notes-derive.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0merge-release-notes-derive.py" %*
exit /b %ERRORLEVEL%
