@echo off
setlocal
REM Windows launcher for check-machine-local-regeneratability.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "check-machine-local-regeneratability.ps1" sibling of
REM     this file over this "check-machine-local-regeneratability.cmd" file itself -- if both exist, `check-machine-local-regeneratability`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "check-machine-local-regeneratability.py"
...
echo [check-machine-local-regeneratability] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [check-machine-local-regeneratability] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0check-machine-local-regeneratability.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0check-machine-local-regeneratability.py" %*
exit /b %ERRORLEVEL%
