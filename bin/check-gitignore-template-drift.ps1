# check-gitignore-template-drift.ps1 — python-direct Windows launcher for check-gitignore-template-drift.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "check-gitignore-template-drift.ps1" file over
#     any co-located "check-gitignore-template-drift.cmd" sibling -- `check-gitignore-template-drift` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "check-gitignore-template-drift.py"
...
$_entry = Join-Path $_here 'check-gitignore-template-drift.py'
...
[Console]::Error.WriteLine('[check-gitignore-template-drift] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[check-gitignore-template-drift] Install Python: https://www.python.org/downloads/windows/')
exit 127
