@echo off
setlocal
REM Windows launcher for baton-drift-sweep.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "baton-drift-sweep.ps1" sibling of
REM     this file over this "baton-drift-sweep.cmd" file itself -- if both exist, `baton-drift-sweep`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "baton-drift-sweep.py"
...
echo [baton-drift-sweep] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [baton-drift-sweep] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0baton-drift-sweep.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0baton-drift-sweep.py" %*
exit /b %ERRORLEVEL%
