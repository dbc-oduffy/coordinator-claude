@echo off
setlocal
REM Windows launcher for coordinator-tasks-mirror.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "coordinator-tasks-mirror.ps1" sibling of
REM     this file over this "coordinator-tasks-mirror.cmd" file itself -- if both exist, `coordinator-tasks-mirror`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "coordinator-tasks-mirror.py"
...
echo [coordinator-tasks-mirror] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [coordinator-tasks-mirror] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0coordinator-tasks-mirror.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0coordinator-tasks-mirror.py" %*
exit /b %ERRORLEVEL%
