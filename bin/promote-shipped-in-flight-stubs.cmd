@echo off
setlocal
REM Windows launcher for promote-shipped-in-flight-stubs.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "promote-shipped-in-flight-stubs.ps1" sibling of
REM     this file over this "promote-shipped-in-flight-stubs.cmd" file itself -- if both exist, `promote-shipped-in-flight-stubs`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "promote-shipped-in-flight-stubs.py"
...
echo [promote-shipped-in-flight-stubs] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [promote-shipped-in-flight-stubs] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0promote-shipped-in-flight-stubs.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0promote-shipped-in-flight-stubs.py" %*
exit /b %ERRORLEVEL%
