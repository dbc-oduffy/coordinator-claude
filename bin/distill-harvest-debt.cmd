@echo off
setlocal
REM Windows launcher for distill-harvest-debt.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "distill-harvest-debt.ps1" sibling of
REM     this file over this "distill-harvest-debt.cmd" file itself -- if both exist, `distill-harvest-debt`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "distill-harvest-debt.py"
...
echo [distill-harvest-debt] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [distill-harvest-debt] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0distill-harvest-debt.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0distill-harvest-debt.py" %*
exit /b %ERRORLEVEL%
