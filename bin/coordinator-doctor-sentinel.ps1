# coordinator-doctor-sentinel.ps1 — python-direct Windows launcher for coordinator-doctor-sentinel.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "coordinator-doctor-sentinel.ps1" file over
#     any co-located "coordinator-doctor-sentinel.cmd" sibling -- `coordinator-doctor-sentinel` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "coordinator-doctor-sentinel.py"
...
$_entry = Join-Path $_here 'coordinator-doctor-sentinel.py'
...
[Console]::Error.WriteLine('[coordinator-doctor-sentinel] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[coordinator-doctor-sentinel] Install Python: https://www.python.org/downloads/windows/')
exit 127
