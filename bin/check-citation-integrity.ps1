# check-citation-integrity.ps1 — python-direct Windows launcher for check-citation-integrity.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "check-citation-integrity.ps1" file over
#     any co-located "check-citation-integrity.cmd" sibling -- `check-citation-integrity` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "check-citation-integrity.py"
...
$_entry = Join-Path $_here 'check-citation-integrity.py'
...
[Console]::Error.WriteLine('[check-citation-integrity] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[check-citation-integrity] Install Python: https://www.python.org/downloads/windows/')
exit 127
