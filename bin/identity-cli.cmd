@echo off
setlocal
REM Windows launcher for identity-cli.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "identity-cli.ps1" sibling of
REM     this file over this "identity-cli.cmd" file itself -- if both exist, `identity-cli`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "identity-cli.py"
...
echo [identity-cli] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [identity-cli] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0identity-cli.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0identity-cli.py" %*
exit /b %ERRORLEVEL%
