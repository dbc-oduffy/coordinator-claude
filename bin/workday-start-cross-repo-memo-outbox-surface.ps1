# workday-start-cross-repo-memo-outbox-surface.ps1 — python-direct Windows launcher for workday-start-cross-repo-memo-outbox-surface.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "workday-start-cross-repo-memo-outbox-surface.ps1" file over
#     any co-located "workday-start-cross-repo-memo-outbox-surface.cmd" sibling -- `workday-start-cross-repo-memo-outbox-surface` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "workday-start-cross-repo-memo-outbox-surface.py"
...
[Console]::Error.WriteLine('[workday-start-cross-repo-memo-outbox-surface] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[workday-start-cross-repo-memo-outbox-surface] Install Python: https://www.python.org/downloads/windows/')
exit 127
