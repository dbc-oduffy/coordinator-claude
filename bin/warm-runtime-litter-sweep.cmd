@echo off
setlocal
REM Windows launcher for warm-runtime-litter-sweep.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "warm-runtime-litter-sweep.ps1" sibling of
REM     this file over this "warm-runtime-litter-sweep.cmd" file itself -- if both exist, `warm-runtime-litter-sweep`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "warm-runtime-litter-sweep.py"
...
echo [warm-runtime-litter-sweep] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [warm-runtime-litter-sweep] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0warm-runtime-litter-sweep.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0warm-runtime-litter-sweep.py" %*
exit /b %ERRORLEVEL%
