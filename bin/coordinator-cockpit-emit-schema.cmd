@echo off
setlocal
REM Windows launcher for coordinator-cockpit-emit-schema.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "coordinator-cockpit-emit-schema.ps1" sibling of
REM     this file over this "coordinator-cockpit-emit-schema.cmd" file itself -- if both exist, `coordinator-cockpit-emit-schema`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "coordinator-cockpit-emit-schema.py"
...
echo [coordinator-cockpit-emit-schema] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [coordinator-cockpit-emit-schema] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0coordinator-cockpit-emit-schema.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0coordinator-cockpit-emit-schema.py" %*
exit /b %ERRORLEVEL%
