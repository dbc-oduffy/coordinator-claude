@echo off
setlocal
REM Windows launcher for _validate-code-comparison-record-checker.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "_validate-code-comparison-record-checker.ps1" sibling of
REM     this file over this "_validate-code-comparison-record-checker.cmd" file itself -- if both exist, `_validate-code-comparison-record-checker`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "_validate-code-comparison-record-checker.py"
...
echo [_validate-code-comparison-record-checker] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [_validate-code-comparison-record-checker] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0_validate-code-comparison-record-checker.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0_validate-code-comparison-record-checker.py" %*
exit /b %ERRORLEVEL%
