@echo off
setlocal
REM Windows launcher for emit-cadence.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "emit-cadence.ps1" sibling of
REM     this file over this "emit-cadence.cmd" file itself -- if both exist, `emit-cadence`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "emit-cadence.py"
...
echo [emit-cadence] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [emit-cadence] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0emit-cadence.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0emit-cadence.py" %*
exit /b %ERRORLEVEL%
