@echo off
setlocal
REM Windows launcher for workday-start-step0.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "workday-start-step0.ps1" sibling of
REM     this file over this "workday-start-step0.cmd" file itself -- if both exist, `workday-start-step0`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "workday-start-step0.py"
...
echo [workday-start-step0] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [workday-start-step0] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0workday-start-step0.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0workday-start-step0.py" %*
exit /b %ERRORLEVEL%
