@echo off
setlocal
REM Windows launcher for detect-project-runtime.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "detect-project-runtime.ps1" sibling of
REM     this file over this "detect-project-runtime.cmd" file itself -- if both exist, `detect-project-runtime`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "detect-project-runtime.py"
...
echo [detect-project-runtime] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [detect-project-runtime] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0detect-project-runtime.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0detect-project-runtime.py" %*
exit /b %ERRORLEVEL%
