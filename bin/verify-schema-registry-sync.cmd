@echo off
setlocal
REM Windows launcher for verify-schema-registry-sync.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "verify-schema-registry-sync.ps1" sibling of
REM     this file over this "verify-schema-registry-sync.cmd" file itself -- if both exist, `verify-schema-registry-sync`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "verify-schema-registry-sync.py"
...
echo [verify-schema-registry-sync] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [verify-schema-registry-sync] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0verify-schema-registry-sync.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0verify-schema-registry-sync.py" %*
exit /b %ERRORLEVEL%
