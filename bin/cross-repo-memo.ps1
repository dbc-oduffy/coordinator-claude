# cross-repo-memo.ps1 — python-direct Windows launcher for cross-repo-memo.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "cross-repo-memo.ps1" file over
#     any co-located "cross-repo-memo.cmd" sibling -- `cross-repo-memo` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "cross-repo-memo.py"
...
$_entry = Join-Path $_here 'cross-repo-memo.py'
...
[Console]::Error.WriteLine('[cross-repo-memo] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[cross-repo-memo] Install Python: https://www.python.org/downloads/windows/')
exit 127
