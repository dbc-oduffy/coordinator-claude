# coordinator-fold-execution-record.ps1 — python-direct Windows launcher for coordinator-fold-execution-record.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "coordinator-fold-execution-record.ps1" file over
#     any co-located "coordinator-fold-execution-record.cmd" sibling -- `coordinator-fold-execution-record` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "coordinator-fold-execution-record.py"
...
$_entry = Join-Path $_here 'coordinator-fold-execution-record.py'
...
[Console]::Error.WriteLine('[coordinator-fold-execution-record] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[coordinator-fold-execution-record] Install Python: https://www.python.org/downloads/windows/')
exit 127
