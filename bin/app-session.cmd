@echo off
setlocal
REM Windows launcher for app-session.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "app-session.ps1" sibling of
REM     this file over this "app-session.cmd" file itself -- if both exist, `app-session`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "app-session.py"
...
echo [app-session] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [app-session] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0app-session.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0app-session.py" %*
exit /b %ERRORLEVEL%
