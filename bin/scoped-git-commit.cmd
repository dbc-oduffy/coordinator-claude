@echo off
setlocal
REM Windows launcher for scoped-git-commit — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "scoped-git-commit.ps1" sibling of
REM     this file over this "scoped-git-commit.cmd" file itself -- if both exist, `scoped-git-commit`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "scoped-git-commit"
...
echo [scoped-git-commit] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [scoped-git-commit] Install Python: https://www.python.org/downloads/windows/ 1>&2
...
:run_baked
"%_py%" "%~dp0scoped-git-commit" %*
2>nul rd /s /q "%_LAUNCHER_RAW_CMDLINE_DIR%"
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0scoped-git-commit" %*
2>nul rd /s /q "%_LAUNCHER_RAW_CMDLINE_DIR%"
exit /b %ERRORLEVEL%
