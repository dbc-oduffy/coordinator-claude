@echo off
setlocal
REM Windows launcher for regen-cockpit-schema.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "regen-cockpit-schema.ps1" sibling of
REM     this file over this "regen-cockpit-schema.cmd" file itself -- if both exist, `regen-cockpit-schema`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "regen-cockpit-schema.py"
...
echo [regen-cockpit-schema] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [regen-cockpit-schema] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0regen-cockpit-schema.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0regen-cockpit-schema.py" %*
exit /b %ERRORLEVEL%
