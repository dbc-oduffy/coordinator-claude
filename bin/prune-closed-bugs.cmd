@echo off
setlocal
REM Windows launcher for prune-closed-bugs.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "prune-closed-bugs.ps1" sibling of
REM     this file over this "prune-closed-bugs.cmd" file itself -- if both exist, `prune-closed-bugs`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "prune-closed-bugs.py"
...
echo [prune-closed-bugs] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [prune-closed-bugs] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0prune-closed-bugs.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0prune-closed-bugs.py" %*
exit /b %ERRORLEVEL%
