@echo off
setlocal
REM Windows launcher for cross-repo-memo.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "cross-repo-memo.ps1" sibling of
REM     this file over this "cross-repo-memo.cmd" file itself -- if both exist, `cross-repo-memo`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "cross-repo-memo.py"
...
echo [cross-repo-memo] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [cross-repo-memo] Install Python: https://www.python.org/downloads/windows/ 1>&2
...
:run_baked
"%_py%" "%~dp0cross-repo-memo.py" %*
2>nul rd /s /q "%_LAUNCHER_RAW_CMDLINE_DIR%"
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0cross-repo-memo.py" %*
2>nul rd /s /q "%_LAUNCHER_RAW_CMDLINE_DIR%"
exit /b %ERRORLEVEL%
