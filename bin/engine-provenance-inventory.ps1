# engine-provenance-inventory.ps1 — python-direct Windows launcher for engine-provenance-inventory.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "engine-provenance-inventory.ps1" file over
#     any co-located "engine-provenance-inventory.cmd" sibling -- `engine-provenance-inventory` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "engine-provenance-inventory.py"
...
$_entry = Join-Path $_here 'engine-provenance-inventory.py'
...
[Console]::Error.WriteLine('[engine-provenance-inventory] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[engine-provenance-inventory] Install Python: https://www.python.org/downloads/windows/')
exit 127
