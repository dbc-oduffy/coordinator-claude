# check-sidecar-fill.ps1 — python-direct Windows launcher for check-sidecar-fill.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "check-sidecar-fill.ps1" file over
#     any co-located "check-sidecar-fill.cmd" sibling -- `check-sidecar-fill` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "check-sidecar-fill.py"
...
$_entry = Join-Path $_here 'check-sidecar-fill.py'
...
[Console]::Error.WriteLine('[check-sidecar-fill] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[check-sidecar-fill] Install Python: https://www.python.org/downloads/windows/')
exit 127
