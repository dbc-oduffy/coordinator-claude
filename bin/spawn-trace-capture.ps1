# spawn-trace-capture.ps1 — python-direct Windows launcher for spawn-trace-capture.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "spawn-trace-capture.ps1" file over
#     any co-located "spawn-trace-capture.cmd" sibling -- `spawn-trace-capture` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "spawn-trace-capture.py"
...
$_entry = Join-Path $_here 'spawn-trace-capture.py'
...
[Console]::Error.WriteLine('[spawn-trace-capture] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[spawn-trace-capture] Install Python: https://www.python.org/downloads/windows/')
exit 127
