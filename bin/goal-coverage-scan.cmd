@echo off
setlocal
REM Windows launcher for goal-coverage-scan.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "goal-coverage-scan.ps1" sibling of
REM     this file over this "goal-coverage-scan.cmd" file itself -- if both exist, `goal-coverage-scan`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "goal-coverage-scan.py"
...
echo [goal-coverage-scan] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [goal-coverage-scan] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0goal-coverage-scan.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0goal-coverage-scan.py" %*
exit /b %ERRORLEVEL%
