@echo off
setlocal
REM Windows launcher for measure-listener-availability.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "measure-listener-availability.ps1" sibling of
REM     this file over this "measure-listener-availability.cmd" file itself -- if both exist, `measure-listener-availability`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "measure-listener-availability.py"
...
echo [measure-listener-availability] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [measure-listener-availability] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0measure-listener-availability.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0measure-listener-availability.py" %*
exit /b %ERRORLEVEL%
