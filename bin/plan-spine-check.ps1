# plan-spine-check.ps1 — python-direct Windows launcher for plan-spine-check.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "plan-spine-check.ps1" file over
#     any co-located "plan-spine-check.cmd" sibling -- `plan-spine-check` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "plan-spine-check.py"
...
$_entry = Join-Path $_here 'plan-spine-check.py'
...
[Console]::Error.WriteLine('[plan-spine-check] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[plan-spine-check] Install Python: https://www.python.org/downloads/windows/')
exit 127
