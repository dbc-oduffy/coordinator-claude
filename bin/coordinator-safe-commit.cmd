@echo off
setlocal
REM Windows launcher for coordinator-safe-commit.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "coordinator-safe-commit.ps1" sibling of
REM     this file over this "coordinator-safe-commit.cmd" file itself -- if both exist, `coordinator-safe-commit`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "coordinator-safe-commit.py"
...
echo [coordinator-safe-commit] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [coordinator-safe-commit] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0coordinator-safe-commit.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0coordinator-safe-commit.py" %*
exit /b %ERRORLEVEL%
