@echo off
setlocal
REM Windows launcher for schema-drift-gate.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "schema-drift-gate.ps1" sibling of
REM     this file over this "schema-drift-gate.cmd" file itself -- if both exist, `schema-drift-gate`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "schema-drift-gate.py"
...
echo [schema-drift-gate] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [schema-drift-gate] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0schema-drift-gate.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0schema-drift-gate.py" %*
exit /b %ERRORLEVEL%
