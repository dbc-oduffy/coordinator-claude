@echo off
setlocal
REM Windows launcher for close-origin-stub-on-ship.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "close-origin-stub-on-ship.ps1" sibling of
REM     this file over this "close-origin-stub-on-ship.cmd" file itself -- if both exist, `close-origin-stub-on-ship`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "close-origin-stub-on-ship.py"
...
echo [close-origin-stub-on-ship] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [close-origin-stub-on-ship] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0close-origin-stub-on-ship.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0close-origin-stub-on-ship.py" %*
exit /b %ERRORLEVEL%
