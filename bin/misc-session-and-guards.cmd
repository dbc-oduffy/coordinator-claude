@echo off
setlocal
REM Windows launcher for misc-session-and-guards.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "misc-session-and-guards.ps1" sibling of
REM     this file over this "misc-session-and-guards.cmd" file itself -- if both exist, `misc-session-and-guards`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "misc-session-and-guards.py"
...
echo [misc-session-and-guards] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [misc-session-and-guards] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0misc-session-and-guards.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0misc-session-and-guards.py" %*
exit /b %ERRORLEVEL%
