@echo off
setlocal
REM Windows launcher for render-handoff-tracker.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "render-handoff-tracker.ps1" sibling of
REM     this file over this "render-handoff-tracker.cmd" file itself -- if both exist, `render-handoff-tracker`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "render-handoff-tracker.py"
...
echo [render-handoff-tracker] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [render-handoff-tracker] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127

:run_baked
"%_py%" "%~dp0render-handoff-tracker.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0render-handoff-tracker.py" %*
exit /b %ERRORLEVEL%
