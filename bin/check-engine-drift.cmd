@echo off
setlocal
REM Windows launcher for check-engine-drift.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "check-engine-drift.ps1" sibling of
REM     this file over this "check-engine-drift.cmd" file itself -- if both exist, `check-engine-drift`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "check-engine-drift.py"
...
echo [check-engine-drift] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [check-engine-drift] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0check-engine-drift.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0check-engine-drift.py" %*
exit /b %ERRORLEVEL%
