@echo off
setlocal
REM Windows launcher for expired-plan-gates.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "expired-plan-gates.ps1" sibling of
REM     this file over this "expired-plan-gates.cmd" file itself -- if both exist, `expired-plan-gates`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "expired-plan-gates.py"
...
echo [expired-plan-gates] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [expired-plan-gates] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0expired-plan-gates.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0expired-plan-gates.py" %*
exit /b %ERRORLEVEL%
