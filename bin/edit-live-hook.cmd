@echo off
setlocal
REM Windows launcher for edit-live-hook.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "edit-live-hook.ps1" sibling of
REM     this file over this "edit-live-hook.cmd" file itself -- if both exist, `edit-live-hook`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "edit-live-hook.py"
...
echo [edit-live-hook] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [edit-live-hook] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0edit-live-hook.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0edit-live-hook.py" %*
exit /b %ERRORLEVEL%
