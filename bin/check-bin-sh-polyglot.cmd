@echo off
setlocal
REM Windows launcher for check-bin-sh-polyglot.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "check-bin-sh-polyglot.ps1" sibling of
REM     this file over this "check-bin-sh-polyglot.cmd" file itself -- if both exist, `check-bin-sh-polyglot`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "check-bin-sh-polyglot.py"
...
echo [check-bin-sh-polyglot] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [check-bin-sh-polyglot] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0check-bin-sh-polyglot.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0check-bin-sh-polyglot.py" %*
exit /b %ERRORLEVEL%
