@echo off
setlocal
REM Windows launcher for update-docs-probes.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "update-docs-probes.ps1" sibling of
REM     this file over this "update-docs-probes.cmd" file itself -- if both exist, `update-docs-probes`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "update-docs-probes.py"
...
echo [update-docs-probes] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [update-docs-probes] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0update-docs-probes.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0update-docs-probes.py" %*
exit /b %ERRORLEVEL%
