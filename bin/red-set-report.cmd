@echo off
setlocal
REM Windows launcher for red-set-report.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "red-set-report.ps1" sibling of
REM     this file over this "red-set-report.cmd" file itself -- if both exist, `red-set-report`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "red-set-report.py"
...
echo [red-set-report] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [red-set-report] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0red-set-report.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0red-set-report.py" %*
exit /b %ERRORLEVEL%
