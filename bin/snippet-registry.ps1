# snippet-registry.ps1 — python-direct Windows launcher for snippet-registry.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "snippet-registry.ps1" file over
#     any co-located "snippet-registry.cmd" sibling -- `snippet-registry` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "snippet-registry.py"
...
$_entry = Join-Path $_here 'snippet-registry.py'
...
[Console]::Error.WriteLine('[snippet-registry] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[snippet-registry] Install Python: https://www.python.org/downloads/windows/')
exit 127
