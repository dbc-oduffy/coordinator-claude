@echo off
setlocal
REM Windows launcher for regenerate-orientation-cache.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "regenerate-orientation-cache.ps1" sibling of
REM     this file over this "regenerate-orientation-cache.cmd" file itself -- if both exist, `regenerate-orientation-cache`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "regenerate-orientation-cache.py"
...
echo [regenerate-orientation-cache] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [regenerate-orientation-cache] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0regenerate-orientation-cache.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0regenerate-orientation-cache.py" %*
exit /b %ERRORLEVEL%
