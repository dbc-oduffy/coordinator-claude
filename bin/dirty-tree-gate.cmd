@echo off
setlocal
REM Windows launcher for dirty-tree-gate.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "dirty-tree-gate.ps1" sibling of
REM     this file over this "dirty-tree-gate.cmd" file itself -- if both exist, `dirty-tree-gate`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "dirty-tree-gate.py"
...
echo [dirty-tree-gate] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [dirty-tree-gate] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0dirty-tree-gate.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0dirty-tree-gate.py" %*
exit /b %ERRORLEVEL%
