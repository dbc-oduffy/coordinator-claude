@echo off
setlocal
REM Windows launcher for migrate-provenance-stamp.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "migrate-provenance-stamp.ps1" sibling of
REM     this file over this "migrate-provenance-stamp.cmd" file itself -- if both exist, `migrate-provenance-stamp`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "migrate-provenance-stamp.py"
...
echo [migrate-provenance-stamp] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [migrate-provenance-stamp] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127

:run_baked
"%_py%" "%~dp0migrate-provenance-stamp.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0migrate-provenance-stamp.py" %*
exit /b %ERRORLEVEL%
