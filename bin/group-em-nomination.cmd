@echo off
setlocal
REM Windows launcher for group-em-nomination.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "group-em-nomination.ps1" sibling of
REM     this file over this "group-em-nomination.cmd" file itself -- if both exist, `group-em-nomination`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "group-em-nomination.py"
...
echo [group-em-nomination] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [group-em-nomination] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0group-em-nomination.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0group-em-nomination.py" %*
exit /b %ERRORLEVEL%
