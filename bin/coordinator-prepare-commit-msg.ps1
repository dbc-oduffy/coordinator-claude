# coordinator-prepare-commit-msg.ps1 — python-direct Windows launcher for coordinator-prepare-commit-msg.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "coordinator-prepare-commit-msg.ps1" file over
#     any co-located "coordinator-prepare-commit-msg.cmd" sibling -- `coordinator-prepare-commit-msg` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "coordinator-prepare-commit-msg.py"
...
$_entry = Join-Path $_here 'coordinator-prepare-commit-msg.py'
...
[Console]::Error.WriteLine('[coordinator-prepare-commit-msg] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[coordinator-prepare-commit-msg] Install Python: https://www.python.org/downloads/windows/')
exit 127
