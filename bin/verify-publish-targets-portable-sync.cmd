@echo off
setlocal
REM Windows launcher for verify-publish-targets-portable-sync.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "verify-publish-targets-portable-sync.ps1" sibling of
REM     this file over this "verify-publish-targets-portable-sync.cmd" file itself -- if both exist, `verify-publish-targets-portable-sync`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "verify-publish-targets-portable-sync.py"
...
echo [verify-publish-targets-portable-sync] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [verify-publish-targets-portable-sync] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0verify-publish-targets-portable-sync.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0verify-publish-targets-portable-sync.py" %*
exit /b %ERRORLEVEL%
