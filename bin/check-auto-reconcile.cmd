@echo off
setlocal
REM Windows launcher for check-auto-reconcile.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "check-auto-reconcile.ps1" sibling of
REM     this file over this "check-auto-reconcile.cmd" file itself -- if both exist, `check-auto-reconcile`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "check-auto-reconcile.py"
...
echo [check-auto-reconcile] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [check-auto-reconcile] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0check-auto-reconcile.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0check-auto-reconcile.py" %*
exit /b %ERRORLEVEL%
