# query-handoff-columns.ps1 — python-direct Windows launcher for query-handoff-columns.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "query-handoff-columns.ps1" file over
#     any co-located "query-handoff-columns.cmd" sibling -- `query-handoff-columns` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "query-handoff-columns.py"
...
$_entry = Join-Path $_here 'query-handoff-columns.py'
...
[Console]::Error.WriteLine('[query-handoff-columns] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[query-handoff-columns] Install Python: https://www.python.org/downloads/windows/')
exit 127
