# workstream-complete-assemble.ps1 — python-direct Windows launcher for workstream-complete-assemble.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "workstream-complete-assemble.ps1" file over
#     any co-located "workstream-complete-assemble.cmd" sibling -- `workstream-complete-assemble` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "workstream-complete-assemble.py"
...
$_entry = Join-Path $_here 'workstream-complete-assemble.py'
...
[Console]::Error.WriteLine('[workstream-complete-assemble] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[workstream-complete-assemble] Install Python: https://www.python.org/downloads/windows/')
exit 127
