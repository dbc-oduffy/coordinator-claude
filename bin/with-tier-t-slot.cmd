@echo off
setlocal
REM Windows launcher for with-tier-t-slot — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "with-tier-t-slot.ps1" sibling of
REM     this file over this "with-tier-t-slot.cmd" file itself -- if both exist, `with-tier-t-slot`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "with-tier-t-slot"
...
echo [with-tier-t-slot] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [with-tier-t-slot] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0with-tier-t-slot" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0with-tier-t-slot" %*
exit /b %ERRORLEVEL%
