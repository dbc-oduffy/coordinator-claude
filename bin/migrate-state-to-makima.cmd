@echo off
setlocal
REM Windows launcher for migrate-state-to-claude-klabauter.sh — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "migrate-state-to-claude-klabauter.ps1" sibling of
REM     this file over this "migrate-state-to-claude-klabauter.cmd" file itself -- if both exist, `migrate-state-to-claude-klabauter`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "migrate-state-to-claude-klabauter.sh"
...
echo [migrate-state-to-claude-klabauter] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [migrate-state-to-claude-klabauter] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127

:run_baked
"%_py%" "%~dp0migrate-state-to-claude-klabauter.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0migrate-state-to-claude-klabauter.py" %*
exit /b %ERRORLEVEL%
