@echo off
setlocal
REM Windows launcher for count-distill-backlog.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "count-distill-backlog.ps1" sibling of
REM     this file over this "count-distill-backlog.cmd" file itself -- if both exist, `count-distill-backlog`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "count-distill-backlog.py"
...
echo [count-distill-backlog] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [count-distill-backlog] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0count-distill-backlog.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0count-distill-backlog.py" %*
exit /b %ERRORLEVEL%
