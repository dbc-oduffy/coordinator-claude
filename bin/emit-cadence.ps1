# emit-cadence.ps1 — python-direct Windows launcher for emit-cadence.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "emit-cadence.ps1" file over
#     any co-located "emit-cadence.cmd" sibling -- `emit-cadence` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "emit-cadence.py"
...
$_entry = Join-Path $_here 'emit-cadence.py'
...
[Console]::Error.WriteLine('[emit-cadence] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[emit-cadence] Install Python: https://www.python.org/downloads/windows/')
exit 127
