@echo off
setlocal
REM Windows launcher for check-rag-state.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "check-rag-state.ps1" sibling of
REM     this file over this "check-rag-state.cmd" file itself -- if both exist, `check-rag-state`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "check-rag-state.py"
...
echo [check-rag-state] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [check-rag-state] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0check-rag-state.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0check-rag-state.py" %*
exit /b %ERRORLEVEL%
