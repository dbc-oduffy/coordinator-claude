@echo off
setlocal
REM Windows launcher for repo-census.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "repo-census.ps1" sibling of
REM     this file over this "repo-census.cmd" file itself -- if both exist, `repo-census`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "repo-census.py"
...
echo [repo-census] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [repo-census] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0repo-census.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0repo-census.py" %*
exit /b %ERRORLEVEL%
