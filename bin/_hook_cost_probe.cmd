@echo off
setlocal
REM Windows launcher for _hook_cost_probe.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "_hook_cost_probe.ps1" sibling of
REM     this file over this "_hook_cost_probe.cmd" file itself -- if both exist, `_hook_cost_probe`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "_hook_cost_probe.py"
...
echo [_hook_cost_probe] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [_hook_cost_probe] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0_hook_cost_probe.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0_hook_cost_probe.py" %*
exit /b %ERRORLEVEL%
