@echo off
setlocal
REM Windows launcher for check-no-monolith-completion-append.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "check-no-monolith-completion-append.ps1" sibling of
REM     this file over this "check-no-monolith-completion-append.cmd" file itself -- if both exist, `check-no-monolith-completion-append`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "check-no-monolith-completion-append.py"
...
echo [check-no-monolith-completion-append] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [check-no-monolith-completion-append] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0check-no-monolith-completion-append.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0check-no-monolith-completion-append.py" %*
exit /b %ERRORLEVEL%
