# staff-session-assemble.ps1 — python-direct Windows launcher for staff-session-assemble.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "staff-session-assemble.ps1" file over
#     any co-located "staff-session-assemble.cmd" sibling -- `staff-session-assemble` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "staff-session-assemble.py"
...
$_entry = Join-Path $_here 'staff-session-assemble.py'
...
[Console]::Error.WriteLine('[staff-session-assemble] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[staff-session-assemble] Install Python: https://www.python.org/downloads/windows/')
exit 127
