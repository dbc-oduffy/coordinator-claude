@echo off
setlocal
REM Windows launcher for migrate-debt-backlog.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "migrate-debt-backlog.ps1" sibling of
REM     this file over this "migrate-debt-backlog.cmd" file itself -- if both exist, `migrate-debt-backlog`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "migrate-debt-backlog.py"
...
echo [migrate-debt-backlog] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [migrate-debt-backlog] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127

:run_baked
"%_py%" "%~dp0migrate-debt-backlog.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0migrate-debt-backlog.py" %*
exit /b %ERRORLEVEL%
