@echo off
setlocal
REM Windows launcher for check-global-doctrine-mirror.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "check-global-doctrine-mirror.ps1" sibling of
REM     this file over this "check-global-doctrine-mirror.cmd" file itself -- if both exist, `check-global-doctrine-mirror`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "check-global-doctrine-mirror.py"
...
echo [check-global-doctrine-mirror] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [check-global-doctrine-mirror] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0check-global-doctrine-mirror.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0check-global-doctrine-mirror.py" %*
exit /b %ERRORLEVEL%
