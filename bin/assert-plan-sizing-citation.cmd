@echo off
setlocal
REM Windows launcher for assert-plan-sizing-citation.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "assert-plan-sizing-citation.ps1" sibling of
REM     this file over this "assert-plan-sizing-citation.cmd" file itself -- if both exist, `assert-plan-sizing-citation`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "assert-plan-sizing-citation.py"
...
echo [assert-plan-sizing-citation] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [assert-plan-sizing-citation] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0assert-plan-sizing-citation.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0assert-plan-sizing-citation.py" %*
exit /b %ERRORLEVEL%
