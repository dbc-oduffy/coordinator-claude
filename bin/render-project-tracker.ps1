# render-project-tracker.ps1 — python-direct Windows launcher for render-project-tracker.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "render-project-tracker.ps1" file over
#     any co-located "render-project-tracker.cmd" sibling -- `render-project-tracker` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "render-project-tracker.py"
...
$_entry = Join-Path $_here 'render-project-tracker.py'
...
[Console]::Error.WriteLine('[render-project-tracker] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[render-project-tracker] Install Python: https://www.python.org/downloads/windows/')
exit 127
