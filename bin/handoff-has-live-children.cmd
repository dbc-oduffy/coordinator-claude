@echo off
setlocal
REM Windows launcher for handoff-has-live-children.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "handoff-has-live-children.ps1" sibling of
REM     this file over this "handoff-has-live-children.cmd" file itself -- if both exist, `handoff-has-live-children`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "handoff-has-live-children.py"
...
echo [handoff-has-live-children] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [handoff-has-live-children] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0handoff-has-live-children.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0handoff-has-live-children.py" %*
exit /b %ERRORLEVEL%
