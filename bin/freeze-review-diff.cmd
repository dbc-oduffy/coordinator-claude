@echo off
setlocal
REM Windows launcher for freeze-review-diff.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "freeze-review-diff.ps1" sibling of
REM     this file over this "freeze-review-diff.cmd" file itself -- if both exist, `freeze-review-diff`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "freeze-review-diff.py"
...
echo [freeze-review-diff] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [freeze-review-diff] Install Python: https://www.python.org/downloads/windows/ 1>&2
...
:run_baked
"%_py%" "%~dp0freeze-review-diff.py" %*
2>nul rd /s /q "%_LAUNCHER_RAW_CMDLINE_DIR%"
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0freeze-review-diff.py" %*
2>nul rd /s /q "%_LAUNCHER_RAW_CMDLINE_DIR%"
exit /b %ERRORLEVEL%
