@echo off
setlocal
REM Windows launcher for backfill-chain-ancestry-waiver-paths.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "backfill-chain-ancestry-waiver-paths.ps1" sibling of
REM     this file over this "backfill-chain-ancestry-waiver-paths.cmd" file itself -- if both exist, `backfill-chain-ancestry-waiver-paths`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "backfill-chain-ancestry-waiver-paths.py"
...
echo [backfill-chain-ancestry-waiver-paths] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [backfill-chain-ancestry-waiver-paths] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127

:run_baked
"%_py%" "%~dp0backfill-chain-ancestry-waiver-paths.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0backfill-chain-ancestry-waiver-paths.py" %*
exit /b %ERRORLEVEL%
