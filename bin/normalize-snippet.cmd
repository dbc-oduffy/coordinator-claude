@echo off
setlocal
REM Windows launcher for normalize-snippet.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "normalize-snippet.ps1" sibling of
REM     this file over this "normalize-snippet.cmd" file itself -- if both exist, `normalize-snippet`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "normalize-snippet.py"
...
echo [normalize-snippet] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [normalize-snippet] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0normalize-snippet.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0normalize-snippet.py" %*
exit /b %ERRORLEVEL%
