@echo off
setlocal
REM Windows launcher for review-coverage-gate.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "review-coverage-gate.ps1" sibling of
REM     this file over this "review-coverage-gate.cmd" file itself -- if both exist, `review-coverage-gate`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "review-coverage-gate.py"
...
echo [review-coverage-gate] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [review-coverage-gate] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127

:run_baked
"%_py%" "%~dp0review-coverage-gate.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0review-coverage-gate.py" %*
exit /b %ERRORLEVEL%
