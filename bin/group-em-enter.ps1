# group-em-enter.ps1 — python-direct Windows launcher for group-em-enter.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "group-em-enter.ps1" file over
#     any co-located "group-em-enter.cmd" sibling -- `group-em-enter` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "group-em-enter.py"
...
$_entry = Join-Path $_here 'group-em-enter.py'
...
[Console]::Error.WriteLine('[group-em-enter] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[group-em-enter] Install Python: https://www.python.org/downloads/windows/')
exit 127
