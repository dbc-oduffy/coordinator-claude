# measure-hook-class-cost.ps1 — python-direct Windows launcher for measure-hook-class-cost.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "measure-hook-class-cost.ps1" file over
#     any co-located "measure-hook-class-cost.cmd" sibling -- `measure-hook-class-cost` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "measure-hook-class-cost.py"
...
$_entry = Join-Path $_here 'measure-hook-class-cost.py'
...
[Console]::Error.WriteLine('[measure-hook-class-cost] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[measure-hook-class-cost] Install Python: https://www.python.org/downloads/windows/')
exit 127
