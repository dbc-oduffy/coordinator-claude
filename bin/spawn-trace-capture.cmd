@echo off
setlocal
REM Windows launcher for spawn-trace-capture.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "spawn-trace-capture.ps1" sibling of
REM     this file over this "spawn-trace-capture.cmd" file itself -- if both exist, `spawn-trace-capture`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "spawn-trace-capture.py"
...
echo [spawn-trace-capture] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [spawn-trace-capture] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0spawn-trace-capture.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0spawn-trace-capture.py" %*
exit /b %ERRORLEVEL%
