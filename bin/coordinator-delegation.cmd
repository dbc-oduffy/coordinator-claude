@echo off
setlocal
REM Windows launcher for coordinator-delegation.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "coordinator-delegation.ps1" sibling of
REM     this file over this "coordinator-delegation.cmd" file itself -- if both exist, `coordinator-delegation`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "coordinator-delegation.py"
...
echo [coordinator-delegation] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [coordinator-delegation] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0coordinator-delegation.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0coordinator-delegation.py" %*
exit /b %ERRORLEVEL%
