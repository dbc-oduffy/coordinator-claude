@echo off
setlocal
REM Windows launcher for plan-tasks-grouping-digest.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "plan-tasks-grouping-digest.ps1" sibling of
REM     this file over this "plan-tasks-grouping-digest.cmd" file itself -- if both exist, `plan-tasks-grouping-digest`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "plan-tasks-grouping-digest.py"
...
echo [plan-tasks-grouping-digest] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [plan-tasks-grouping-digest] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0plan-tasks-grouping-digest.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0plan-tasks-grouping-digest.py" %*
exit /b %ERRORLEVEL%
