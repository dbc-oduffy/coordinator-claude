@echo off
setlocal
REM Windows launcher for sweep-boot.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "sweep-boot.ps1" sibling of
REM     this file over this "sweep-boot.cmd" file itself -- if both exist, `sweep-boot`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "sweep-boot.py"
...
echo [sweep-boot] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [sweep-boot] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0sweep-boot.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0sweep-boot.py" %*
exit /b %ERRORLEVEL%
