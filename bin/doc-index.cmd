@echo off
setlocal
REM Windows launcher for doc-index.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "doc-index.ps1" sibling of
REM     this file over this "doc-index.cmd" file itself -- if both exist, `doc-index`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "doc-index.py"
...
echo [doc-index] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [doc-index] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
