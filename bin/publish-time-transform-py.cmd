@echo off
setlocal
REM Windows launcher for publish-time-transform-py.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "publish-time-transform-py.ps1" sibling of
REM     this file over this "publish-time-transform-py.cmd" file itself -- if both exist, `publish-time-transform-py`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "publish-time-transform-py.py"
...
echo [publish-time-transform-py] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [publish-time-transform-py] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0publish-time-transform-py.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0publish-time-transform-py.py" %*
exit /b %ERRORLEVEL%
