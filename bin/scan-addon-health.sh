...
# Non-plugin consumer repos (e.g. example-cockpit-repo) get their own presence-based
# lane rather than a synthetic dir under plugins/ — plugins/<name>/ is reserved
# for installed marketplace plugins. Main-verdict-loop-only; see the pass
# headers below for why passes 2/3 do NOT iterate this root.
# → docs/wiki/addon-health-sentinel.md § Consumer sentinels (non-plugin repos)
CONSUMER_ROOT="${COORDINATOR_CONSUMER_HEALTH_ROOT:-$HOME/.claude}"

# Exit only if BOTH roots are absent: a consumer-primary machine (coordinator
# resolved via --plugin-dir, no marketplace plugins) has no $PLUGINS_ROOT but
# may still carry consumer sentinels under $CONSUMER_ROOT — gating on PLUGINS_ROOT
# alone would silently drop the consumer verdict lane the main loop adds.
[[ -d "$PLUGINS_ROOT" || -d "$CONSUMER_ROOT" ]] || exit 0
...
if [[ "$MODE" == "--check-sentinel-presence" ]]; then
  ...
  if [[ "$sentinel_count" -eq 0 ]]; then
    echo "addon-health: no doctor sentinels found across ${installed_count} installed plugin(s) — run /coordinator:install and your plugin doctors to bootstrap"
  fi
  exit 0
fi
...
# Main verdict loop (pass 1). Globs BOTH $PLUGINS_ROOT and $CONSUMER_ROOT —
# verdict surfacing is the only thing non-plugin consumers get (see passes 2/3
# below, which stay PLUGINS_ROOT-scoped by design). The two roots are disjoint
# by construction (PLUGINS_ROOT is one level deeper than CONSUMER_ROOT), so a
# real plugin is never double-counted; presence of the exact
# data/doctor-last-run.json filename is the deliberate opt-in, not a dir-shape
# check. → docs/wiki/addon-health-sentinel.md § Consumer sentinels (non-plugin repos)
shopt -s nullglob 2>/dev/null || true
for sentinel in "$PLUGINS_ROOT"/*/data/doctor-last-run.json "$CONSUMER_ROOT"/*/data/doctor-last-run.json; do
  [[ -f "$sentinel" ]] || continue

  # Derive plugin/consumer name from the sentinel path itself — NOT from
  # $PLUGINS_ROOT — so a consumer sentinel (rooted at $CONSUMER_ROOT) resolves
  # correctly too: .../<plugin-or-consumer>/data/doctor-last-run.json
...
#
# PLUGINS_ROOT-scoped BY DESIGN — do NOT extend to $CONSUMER_ROOT. The
# absent-sentinel nag is a plugin-identity concern (it walks every installed
# plugin dir looking for a declared-but-never-run doctor); non-plugin
# consumers surface a verdict only, never a nag. Extending this pass to
# $CONSUMER_ROOT would find-scan every first-level dir under ~/.claude
# (~30 dirs: state/, sessions/, tasks/, cache/, ...) for commands/doctor.md,
# manufacturing false-nag exposure. → docs/wiki/addon-health-sentinel.md
...
#
# PLUGINS_ROOT-scoped BY DESIGN — do NOT extend to $CONSUMER_ROOT, same
# rationale as pass 2: this is a plugin-identity hook-integrity probe, not a
# verdict surface. Non-plugin consumers get verdict-only in the main loop;
# extending the hook-script crawl to $CONSUMER_ROOT would probe every
# first-level dir under ~/.claude for hooks/hooks.json, turning a targeted
# check into a ~30-dir filesystem crawl. → docs/wiki/addon-health-sentinel.md
# § Consumer sentinels (non-plugin repos)
if [[ "$MODE" == "--red-and-stale" ]]; then
  shopt -s nullglob 2>/dev/null || true
...
" "$plugin_dir" 2>/dev/null) || true
    ...
    if [[ -n "$missing" ]]; then
      while IFS= read -r rel; do
        [[ -n "$rel" ]] || continue
        echo "[health] ${plugin}: SessionStart hook references missing script '${rel}' (declared in hooks/hooks.json, not on disk — Claude Code silently skips it). Re-run the plugin's installer or /coordinator:install."
      done <<< "$missing"
    fi
  done
fi
