...
# Review: code-reviewer F7 — disarm trap first, then clean up inline. The old order (rm then
# trap-disarm) created a double-delete window if a signal fired between the two lines; with
# trap disarmed first the explicit rm is the sole deletion path.
trap - EXIT; rm -f "$_subst_tmp"  # disarm before output-section trap takes over
