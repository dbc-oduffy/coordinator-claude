...
# Review: code-reviewer — confirm the finding scopes to exactly repos.project_rag (not repos.example-sim-repo,
# which IS declared in the tracked registry.toml; the finding must not fire for it).
_assert_stderr_not_contains "(b) gitignored session-accumulated - repos.example-sim-repo NOT flagged (has tracked baseline)" "repos.example-sim-repo" "$TEST_STDERR"
