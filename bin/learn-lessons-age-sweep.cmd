@echo off
setlocal
REM Windows launcher for learn-lessons-age-sweep.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "learn-lessons-age-sweep.ps1" sibling of
REM     this file over this "learn-lessons-age-sweep.cmd" file itself -- if both exist, `learn-lessons-age-sweep`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "learn-lessons-age-sweep.py"
...
echo [learn-lessons-age-sweep] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [learn-lessons-age-sweep] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0learn-lessons-age-sweep.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0learn-lessons-age-sweep.py" %*
exit /b %ERRORLEVEL%
