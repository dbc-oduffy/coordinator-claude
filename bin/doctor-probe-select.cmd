@echo off
setlocal
REM Windows launcher for doctor-probe-select.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "doctor-probe-select.ps1" sibling of
REM     this file over this "doctor-probe-select.cmd" file itself -- if both exist, `doctor-probe-select`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "doctor-probe-select.py"
...
echo [doctor-probe-select] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [doctor-probe-select] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0doctor-probe-select.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0doctor-probe-select.py" %*
exit /b %ERRORLEVEL%
