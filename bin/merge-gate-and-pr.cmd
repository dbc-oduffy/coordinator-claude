@echo off
setlocal
REM Windows launcher for merge-gate-and-pr.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "merge-gate-and-pr.ps1" sibling of
REM     this file over this "merge-gate-and-pr.cmd" file itself -- if both exist, `merge-gate-and-pr`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "merge-gate-and-pr.py"
...
echo [merge-gate-and-pr] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [merge-gate-and-pr] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0merge-gate-and-pr.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0merge-gate-and-pr.py" %*
exit /b %ERRORLEVEL%
