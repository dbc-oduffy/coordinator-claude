@echo off
setlocal
REM Windows launcher for install-doe-claude-precommit-hook.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "install-doe-claude-precommit-hook.ps1" sibling of
REM     this file over this "install-doe-claude-precommit-hook.cmd" file itself -- if both exist, `install-doe-claude-precommit-hook`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "install-doe-claude-precommit-hook.py"
...
echo [install-doe-claude-precommit-hook] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [install-doe-claude-precommit-hook] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0install-doe-claude-precommit-hook.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0install-doe-claude-precommit-hook.py" %*
exit /b %ERRORLEVEL%
