@echo off
setlocal
REM Windows launcher for coordinator-render-rollup.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "coordinator-render-rollup.ps1" sibling of
REM     this file over this "coordinator-render-rollup.cmd" file itself -- if both exist, `coordinator-render-rollup`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "coordinator-render-rollup.py"
...
echo [coordinator-render-rollup] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [coordinator-render-rollup] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0coordinator-render-rollup.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0coordinator-render-rollup.py" %*
exit /b %ERRORLEVEL%
