@echo off
setlocal
REM Windows launcher for install-health-run.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "install-health-run.ps1" sibling of
REM     this file over this "install-health-run.cmd" file itself -- if both exist, `install-health-run`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "install-health-run.py"
...
echo [install-health-run] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [install-health-run] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0install-health-run.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0install-health-run.py" %*
exit /b %ERRORLEVEL%
