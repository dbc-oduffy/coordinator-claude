@echo off
setlocal
REM Windows launcher for sweep-installed-hook-name-refs.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "sweep-installed-hook-name-refs.ps1" sibling of
REM     this file over this "sweep-installed-hook-name-refs.cmd" file itself -- if both exist, `sweep-installed-hook-name-refs`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "sweep-installed-hook-name-refs.py"
...
echo [sweep-installed-hook-name-refs] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [sweep-installed-hook-name-refs] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0sweep-installed-hook-name-refs.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0sweep-installed-hook-name-refs.py" %*
exit /b %ERRORLEVEL%
