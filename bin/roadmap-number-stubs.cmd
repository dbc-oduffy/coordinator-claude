@echo off
setlocal
REM Windows launcher for roadmap-number-stubs.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "roadmap-number-stubs.ps1" sibling of
REM     this file over this "roadmap-number-stubs.cmd" file itself -- if both exist, `roadmap-number-stubs`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "roadmap-number-stubs.py"
...
echo [roadmap-number-stubs] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [roadmap-number-stubs] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0roadmap-number-stubs.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0roadmap-number-stubs.py" %*
exit /b %ERRORLEVEL%
