@echo off
setlocal
REM Windows launcher for wsc-close.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "wsc-close.ps1" sibling of
REM     this file over this "wsc-close.cmd" file itself -- if both exist, `wsc-close`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "wsc-close.py"
...
echo [wsc-close] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [wsc-close] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
