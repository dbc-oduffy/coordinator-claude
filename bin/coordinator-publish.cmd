@echo off
setlocal
REM Windows launcher for coordinator-publish.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "coordinator-publish.ps1" sibling of
REM     this file over this "coordinator-publish.cmd" file itself -- if both exist, `coordinator-publish`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "coordinator-publish.py"
...
echo [coordinator-publish] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [coordinator-publish] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0coordinator-publish.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0coordinator-publish.py" %*
exit /b %ERRORLEVEL%
