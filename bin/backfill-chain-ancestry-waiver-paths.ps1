# backfill-chain-ancestry-waiver-paths.ps1 — python-direct Windows launcher for backfill-chain-ancestry-waiver-paths.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "backfill-chain-ancestry-waiver-paths.ps1" file over
#     any co-located "backfill-chain-ancestry-waiver-paths.cmd" sibling -- `backfill-chain-ancestry-waiver-paths` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "backfill-chain-ancestry-waiver-paths.py"
...
$_entry = Join-Path $_here 'backfill-chain-ancestry-waiver-paths.py'
...
[Console]::Error.WriteLine('[backfill-chain-ancestry-waiver-paths] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[backfill-chain-ancestry-waiver-paths] Install Python: https://www.python.org/downloads/windows/')
exit 127
