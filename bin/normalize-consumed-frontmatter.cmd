@echo off
setlocal
REM Windows launcher for normalize-consumed-frontmatter.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "normalize-consumed-frontmatter.ps1" sibling of
REM     this file over this "normalize-consumed-frontmatter.cmd" file itself -- if both exist, `normalize-consumed-frontmatter`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "normalize-consumed-frontmatter.py"
...
echo [normalize-consumed-frontmatter] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [normalize-consumed-frontmatter] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0normalize-consumed-frontmatter.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0normalize-consumed-frontmatter.py" %*
exit /b %ERRORLEVEL%
