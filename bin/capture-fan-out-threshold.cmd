@echo off
setlocal
REM Windows launcher for capture-fan-out-threshold.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "capture-fan-out-threshold.ps1" sibling of
REM     this file over this "capture-fan-out-threshold.cmd" file itself -- if both exist, `capture-fan-out-threshold`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "capture-fan-out-threshold.py"
...
echo [capture-fan-out-threshold] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [capture-fan-out-threshold] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0capture-fan-out-threshold.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0capture-fan-out-threshold.py" %*
exit /b %ERRORLEVEL%
