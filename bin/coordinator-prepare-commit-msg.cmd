@echo off
setlocal
REM Windows launcher for coordinator-prepare-commit-msg.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "coordinator-prepare-commit-msg.ps1" sibling of
REM     this file over this "coordinator-prepare-commit-msg.cmd" file itself -- if both exist, `coordinator-prepare-commit-msg`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "coordinator-prepare-commit-msg.py"
...
echo [coordinator-prepare-commit-msg] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [coordinator-prepare-commit-msg] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0coordinator-prepare-commit-msg.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0coordinator-prepare-commit-msg.py" %*
exit /b %ERRORLEVEL%
