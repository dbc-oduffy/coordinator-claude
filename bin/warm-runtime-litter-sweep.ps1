# warm-runtime-litter-sweep.ps1 — python-direct Windows launcher for warm-runtime-litter-sweep.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "warm-runtime-litter-sweep.ps1" file over
#     any co-located "warm-runtime-litter-sweep.cmd" sibling -- `warm-runtime-litter-sweep` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "warm-runtime-litter-sweep.py"
...
$_entry = Join-Path $_here 'warm-runtime-litter-sweep.py'
...
[Console]::Error.WriteLine('[warm-runtime-litter-sweep] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[warm-runtime-litter-sweep] Install Python: https://www.python.org/downloads/windows/')
exit 127
