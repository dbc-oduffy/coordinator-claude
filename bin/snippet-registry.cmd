@echo off
setlocal
REM Windows launcher for snippet-registry.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "snippet-registry.ps1" sibling of
REM     this file over this "snippet-registry.cmd" file itself -- if both exist, `snippet-registry`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "snippet-registry.py"
...
echo [snippet-registry] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [snippet-registry] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0snippet-registry.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0snippet-registry.py" %*
exit /b %ERRORLEVEL%
