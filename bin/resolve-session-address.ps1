# resolve-session-address.ps1 — python-direct Windows launcher for resolve-session-address.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "resolve-session-address.ps1" file over
#     any co-located "resolve-session-address.cmd" sibling -- `resolve-session-address` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "resolve-session-address.py"
...
$_entry = Join-Path $_here 'resolve-session-address.py'
...
[Console]::Error.WriteLine('[resolve-session-address] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[resolve-session-address] Install Python: https://www.python.org/downloads/windows/')
exit 127
