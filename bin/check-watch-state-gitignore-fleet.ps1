# check-watch-state-gitignore-fleet.ps1 — python-direct Windows launcher for check-watch-state-gitignore-fleet.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "check-watch-state-gitignore-fleet.ps1" file over
#     any co-located "check-watch-state-gitignore-fleet.cmd" sibling -- `check-watch-state-gitignore-fleet` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "check-watch-state-gitignore-fleet.py"
...
$_entry = Join-Path $_here 'check-watch-state-gitignore-fleet.py'
...
[Console]::Error.WriteLine('[check-watch-state-gitignore-fleet] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[check-watch-state-gitignore-fleet] Install Python: https://www.python.org/downloads/windows/')
exit 127
