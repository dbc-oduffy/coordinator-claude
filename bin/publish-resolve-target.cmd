@echo off
setlocal
REM Windows launcher for publish-resolve-target.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "publish-resolve-target.ps1" sibling of
REM     this file over this "publish-resolve-target.cmd" file itself -- if both exist, `publish-resolve-target`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "publish-resolve-target.py"
...
echo [publish-resolve-target] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [publish-resolve-target] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0publish-resolve-target.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0publish-resolve-target.py" %*
exit /b %ERRORLEVEL%
