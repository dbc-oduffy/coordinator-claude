@echo off
setlocal
REM Windows launcher for setup-rag-decision.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "setup-rag-decision.ps1" sibling of
REM     this file over this "setup-rag-decision.cmd" file itself -- if both exist, `setup-rag-decision`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "setup-rag-decision.py"
...
echo [setup-rag-decision] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [setup-rag-decision] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127

:run_baked
"%_py%" "%~dp0setup-rag-decision.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0setup-rag-decision.py" %*
exit /b %ERRORLEVEL%
