@echo off
setlocal
REM Windows launcher for bootstrap-repo.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "bootstrap-repo.ps1" sibling of
REM     this file over this "bootstrap-repo.cmd" file itself -- if both exist, `bootstrap-repo`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "bootstrap-repo.py"
...
echo [bootstrap-repo] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [bootstrap-repo] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127

:run_baked
"%_py%" "%~dp0bootstrap-repo.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0bootstrap-repo.py" %*
exit /b %ERRORLEVEL%
