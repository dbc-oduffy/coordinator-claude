@echo off
setlocal
REM Windows launcher for coordinator-is-meta-repo.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "coordinator-is-meta-repo.ps1" sibling of
REM     this file over this "coordinator-is-meta-repo.cmd" file itself -- if both exist, `coordinator-is-meta-repo`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "coordinator-is-meta-repo.py"
...
echo [coordinator-is-meta-repo] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [coordinator-is-meta-repo] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127

:run_baked
"%_py%" "%~dp0coordinator-is-meta-repo.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0coordinator-is-meta-repo.py" %*
exit /b %ERRORLEVEL%
