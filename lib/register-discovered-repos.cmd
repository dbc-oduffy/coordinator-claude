@echo off
setlocal
REM Windows launcher for register-discovered-repos.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "register-discovered-repos.ps1" sibling of
REM     this file over this "register-discovered-repos.cmd" file itself -- if both exist, `register-discovered-repos`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "register-discovered-repos.py"
...
echo [register-discovered-repos] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [register-discovered-repos] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127

:run_baked
"%_py%" "%~dp0register-discovered-repos.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0register-discovered-repos.py" %*
exit /b %ERRORLEVEL%
