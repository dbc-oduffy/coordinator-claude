@echo off
setlocal
REM Windows launcher for trusted-root-guard-cli.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "trusted-root-guard-cli.ps1" sibling of
REM     this file over this "trusted-root-guard-cli.cmd" file itself -- if both exist, `trusted-root-guard-cli`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "trusted-root-guard-cli.py"
...
echo [trusted-root-guard-cli] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [trusted-root-guard-cli] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127

:run_baked
"%_py%" "%~dp0trusted-root-guard-cli.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0trusted-root-guard-cli.py" %*
exit /b %ERRORLEVEL%
