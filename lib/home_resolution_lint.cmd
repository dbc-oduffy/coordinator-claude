@echo off
setlocal
REM Windows launcher for home_resolution_lint.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "home_resolution_lint.ps1" sibling of
REM     this file over this "home_resolution_lint.cmd" file itself -- if both exist, `home_resolution_lint`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "home_resolution_lint.py"
...
echo [home_resolution_lint] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [home_resolution_lint] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127

:run_baked
"%_py%" "%~dp0home_resolution_lint.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0home_resolution_lint.py" %*
exit /b %ERRORLEVEL%
