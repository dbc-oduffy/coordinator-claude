@echo off
setlocal
REM Windows launcher for bootstrap-orchestrate.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "bootstrap-orchestrate.ps1" sibling of
REM     this file over this "bootstrap-orchestrate.cmd" file itself -- if both exist, `bootstrap-orchestrate`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "bootstrap-orchestrate.py"
...
echo [bootstrap-orchestrate] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [bootstrap-orchestrate] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127

:run_baked
"%_py%" "%~dp0bootstrap-orchestrate.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0bootstrap-orchestrate.py" %*
exit /b %ERRORLEVEL%
