@echo off
setlocal
REM Windows launcher for review-coverage-core.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "review-coverage-core.ps1" sibling of
REM     this file over this "review-coverage-core.cmd" file itself -- if both exist, `review-coverage-core`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "review-coverage-core.py"
...
echo [review-coverage-core] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [review-coverage-core] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127

:run_baked
"%_py%" "%~dp0review-coverage-core.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0review-coverage-core.py" %*
exit /b %ERRORLEVEL%
