...
#
# Consumers:
#   - hooks/scripts/session-ensure-branch.sh (SessionStart hook — active branch-ensure)
#   (hooks/scripts/block-off-daily-branch.sh was the former PreToolUse consumer;
#   retired 2026-07-05, strang-04, replaced by session-ensure-branch.sh)
#
...
# Check 6 fully decommissioned 2026-05-07 per PM call. The block-off-daily hook
# itself retired 2026-07-05 (strang-04). Stale-day branches are handled by the
# /workday-start rename prompt, not a commit block.
#
...
# cs_compute_contributor — echo the coordinator contributor slug, always lowercase.
# Resolution order: $COORDINATOR_CONTRIBUTOR → machine-local registry coordinator.contributor_slug
#   → cs_compute_contributor_live (sanitized git user.email seed) → "unknown"
# Registry-preferring canonical resolver — mirrors cs_compute_machine's shape exactly
# (env escape valve → registry key → live/seed fallback → single-tail-emit).
# Spec backlink: docs/plans/2026-07-08-mcollab-01-contributor-slug.md § Design (Resolver)
cs_compute_contributor() {
  local c
  if [[ -n "${COORDINATOR_CONTRIBUTOR:-}" ]]; then
    c="$COORDINATOR_CONTRIBUTOR"
  else
    local _ml_bin="${MACHINE_LOCAL_BIN:-machine-local}"
    local _slug=""
    if command -v "$_ml_bin" &>/dev/null; then
      _slug=$("$_ml_bin" get --default "" coordinator.contributor_slug 2>/dev/null || true)
    fi
    if [[ -n "$_slug" ]]; then
      c="$_slug"
    else
      c=$(cs_compute_contributor_live)   # sanitized user.email seed, or "unknown"
    fi
  fi
  echo "${c:-unknown}" | tr '[:upper:]' '[:lower:]'
}

# cs_compute_contributor_live — echo the coordinator contributor slug without a registry read.
# Resolution order: $COORDINATOR_CONTRIBUTOR → sanitized git user.email local-part → "unknown"
# Purpose: seed source for the registry key and drift-detection comparator for workday-start.
# Using this avoids circularity (seeding from the registry-preferring resolver). The @domain
# portion of user.email is dropped before sanitizing (PII-minimize) — only the local-part seeds
# the slug. A malformed value with no `@` passes through unchanged and is sanitized as-is.
cs_compute_contributor_live() {
  local c raw
  # NOTE: env COORDINATOR_CONTRIBUTOR is intentionally honoured here — enables
  # standalone/drift-detection calls to _live independent of
  # cs_compute_contributor's own env-check (mirrors cs_compute_machine_live above).
  if [[ -n "${COORDINATOR_CONTRIBUTOR:-}" ]]; then
    c="$COORDINATOR_CONTRIBUTOR"
  else
    raw=$(git config --get user.email 2>/dev/null || true)
    raw="${raw%%@*}"                                    # local-part only (drop @domain — PII-minimize)
    c=$(cs_sanitize_slug "$raw")
  fi
  [[ -z "$c" ]] && c="unknown"
  echo "$c" | tr '[:upper:]' '[:lower:]'
}

# cs_sanitize_slug <raw> — bash port of the _memo_filename idiom (cross-repo-memo:868-870).
# lowercase → collapse non-[a-z0-9-] runs to single dash → collapse dashes → strip leading/trailing dashes
# Lowercases internally — callers do not need to pre-lowercase their input.
cs_sanitize_slug() {
  # Review: code-reviewer F1 — ${1,,} is bash-4-only; file's own convention
  # (cs_compute_machine/_live, two functions above) uses tr for portability.
  local s; s=$(printf '%s' "$1" | tr '[:upper:]' '[:lower:]')
  s="${s//[^a-z0-9-]/-}"       # non-charset → dash
  while [[ "$s" == *--* ]]; do s="${s//--/-}"; done   # collapse dash runs
  s="${s#-}"; s="${s%-}"       # strip leading/trailing
  printf '%s' "$s"
}
