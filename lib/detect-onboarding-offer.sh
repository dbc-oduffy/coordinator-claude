...
detect_onboarding_offer() {
    ...
    if [[ -x "$probe_script" ]]; then
        local probe_status
        probe_status="$(COORDINATOR_CURRENCY_REPO_ROOT="$repo_root" COORDINATOR_CURRENCY_PLUGIN_ROOT="$plugin_root" bash "$probe_script" 2>/dev/null || true)"
    ...
    if [[ -f "$lib" ]]; then
        ...
        #
        # NOTE (C2, 2026-07-03): NOT repointed to coordinator_is_meta_repo — this is a
        # DISTINCT concept.  source_is_live asks "is the coordinator plugin co-located with
        # the repo being checked?" (plugin path inside repo/plugins/).  coordinator_is_meta_repo
        # asks "is the repo's git root the claude-home dir?" (via CLAUDE_HOME).  These diverge
        # when DETECT_ONBOARDING_PLUGIN_ROOT is overridden: a non-standard plugin root makes
        # source_is_live=false even if the repo IS the meta-repo, which is the correct behavior
        # here (no stamp expected when the co-located source is absent).  Repointing would change
        # observable behavior in test/CI override scenarios.  Flagged in C2 DONE report.
