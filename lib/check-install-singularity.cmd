@echo off
setlocal
REM Windows launcher for check-install-singularity.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "check-install-singularity.ps1" sibling of
REM     this file over this "check-install-singularity.cmd" file itself -- if both exist, `check-install-singularity`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "check-install-singularity.py"
...
echo [check-install-singularity] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [check-install-singularity] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127

:run_baked
"%_py%" "%~dp0check-install-singularity.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0check-install-singularity.py" %*
exit /b %ERRORLEVEL%
