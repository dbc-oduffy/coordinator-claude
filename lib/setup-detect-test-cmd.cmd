@echo off
setlocal
REM Windows launcher for setup-detect-test-cmd.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "setup-detect-test-cmd.ps1" sibling of
REM     this file over this "setup-detect-test-cmd.cmd" file itself -- if both exist, `setup-detect-test-cmd`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "setup-detect-test-cmd.py"
...
echo [setup-detect-test-cmd] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [setup-detect-test-cmd] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127

:run_baked
"%_py%" "%~dp0setup-detect-test-cmd.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0setup-detect-test-cmd.py" %*
exit /b %ERRORLEVEL%
