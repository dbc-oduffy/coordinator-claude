@echo off
setlocal
REM Windows launcher for detect-hardware.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "detect-hardware.ps1" sibling of
REM     this file over this "detect-hardware.cmd" file itself -- if both exist, `detect-hardware`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "detect-hardware.py"
...
echo [detect-hardware] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [detect-hardware] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0detect-hardware.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0detect-hardware.py" %*
exit /b %ERRORLEVEL%
