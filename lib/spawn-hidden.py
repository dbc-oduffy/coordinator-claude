"""spawn-hidden.py — Windows-console-window-suppressing launcher for coordinator scripts.
...
USAGE
-----
    lib/spawn-hidden.py [--stdin-mode=safe|pipe] <interpreter> [args...]
...
def _parse_args(argv: list[str]) -> tuple[str, list[str]] | None:
    ...
    while i < len(argv):
        ...
        if arg.startswith("--stdin-mode="):
            print(f"spawn-hidden.py: unknown --stdin-mode value: {arg}", file=sys.stderr)
            return None
        if arg == "--":
            i += 1
            break
    ...
    if i >= len(argv):
        print("spawn-hidden.py: no interpreter specified", file=sys.stderr)
        print(
            "Usage: spawn-hidden.py [--stdin-mode=safe|pipe] <interpreter> [args...]",
            file=sys.stderr,
        )
        return None
