...
if [[ ! -f "$_BOOTSTRAP_LIB" ]]; then
  ...
  if [ -z "$_doe_root" ] || [ ! -d "$_doe_root/coordinator" ]; then echo "ERROR: ~/.claude/.doe-root missing/invalid — re-run coordinator:install" >&2; exit 1; fi
  _BOOTSTRAP_LIB="${CLAUDE_PLUGIN_ROOT:-$_doe_root/coordinator}/lib/resolve-python.sh"
  # shellcheck source=/dev/null
  source "$(dirname "${BASH_SOURCE[0]}")/coordinator-trusted-root-guard.sh"
  coordinator_trusted_root_guard --mode=fail-loud --root="$_BOOTSTRAP_LIB" --site="$0"
fi
if [[ -f "$_BOOTSTRAP_LIB" ]]; then
  # shellcheck source=/dev/null
