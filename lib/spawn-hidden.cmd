@echo off
setlocal
REM Windows launcher for spawn-hidden.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "spawn-hidden.ps1" sibling of
REM     this file over this "spawn-hidden.cmd" file itself -- if both exist, `spawn-hidden`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "spawn-hidden.py"
...
echo [spawn-hidden] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [spawn-hidden] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127

:run_baked
"%_py%" "%~dp0spawn-hidden.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0spawn-hidden.py" %*
exit /b %ERRORLEVEL%
