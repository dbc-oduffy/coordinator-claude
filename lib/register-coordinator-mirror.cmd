@echo off
setlocal
REM Windows launcher for register-coordinator-mirror.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "register-coordinator-mirror.ps1" sibling of
REM     this file over this "register-coordinator-mirror.cmd" file itself -- if both exist, `register-coordinator-mirror`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "register-coordinator-mirror.py"
...
echo [register-coordinator-mirror] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [register-coordinator-mirror] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127

:run_baked
"%_py%" "%~dp0register-coordinator-mirror.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0register-coordinator-mirror.py" %*
exit /b %ERRORLEVEL%
