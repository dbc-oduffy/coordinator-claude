@echo off
setlocal
REM Windows launcher for settings_home.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "settings_home.ps1" sibling of
REM     this file over this "settings_home.cmd" file itself -- if both exist, `settings_home`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "settings_home.py"
...
echo [settings_home] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [settings_home] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127

:run_baked
"%_py%" "%~dp0settings_home.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0settings_home.py" %*
exit /b %ERRORLEVEL%
