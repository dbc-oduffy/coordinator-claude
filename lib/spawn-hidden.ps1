# spawn-hidden.ps1 — python-direct Windows launcher for spawn-hidden.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "spawn-hidden.ps1" file over
#     any co-located "spawn-hidden.cmd" sibling -- `spawn-hidden` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "spawn-hidden.py"
...
$_entry = Join-Path $_here 'spawn-hidden.py'
...
[Console]::Error.WriteLine('[spawn-hidden] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[spawn-hidden] Install Python: https://www.python.org/downloads/windows/')
exit 127
