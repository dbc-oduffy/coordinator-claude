@echo off
setlocal
REM Windows launcher for coordinator-artifact-subject.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "coordinator-artifact-subject.ps1" sibling of
REM     this file over this "coordinator-artifact-subject.cmd" file itself -- if both exist, `coordinator-artifact-subject`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "coordinator-artifact-subject.py"
...
echo [coordinator-artifact-subject] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [coordinator-artifact-subject] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127

:run_baked
"%_py%" "%~dp0coordinator-artifact-subject.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0coordinator-artifact-subject.py" %*
exit /b %ERRORLEVEL%
