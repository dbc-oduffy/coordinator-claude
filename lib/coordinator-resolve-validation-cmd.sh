...
#     source "$_LIB"
#   fi
# Preferred for in-tree bin/ scripts under this same plugin tree (e.g.
# coordinator-ceremony-hook.sh): a BASH_SOURCE-relative resolution, which is
# install-location-agnostic and works under --plugin-dir live resolution (not
# just the $HOME/.claude mirror):
#   PLUGIN_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
#   _LIB="${PLUGIN_ROOT}/lib/coordinator-resolve-validation-cmd.sh"
#   source "$_LIB"
# Keep the $HOME/.claude/... form above for genuinely out-of-tree consumers.
#
# Consumers:
...
#   - coordinator/bin/coordinator-ceremony-hook.sh (cs_read_local_md_key — per-repo ceremony post-command key lookup)
#
...
_cs_metachar_warn() {
  ...
  case "$s" in
    *'$('*|*'`'*|*'; '*)
      # Review: code-reviewer F1 — suppression-hint text was fast-test-cmd-scoped
      # ("...FAST_TEST_CMD_SUPPRESS...") even for non-fast-test callers (e.g. the
      # ceremony hook). Reworded caller-neutral; legacy env var name is unchanged
      # and still the only suppression knob (backward-compatible).
      echo "[$caller] WARN (step=$step): resolved value contains shell metacharacters (\$( or backtick or ;) — confirm this is intended; set COORDINATOR_FAST_TEST_CMD_SUPPRESS_METACHAR_WARN=1 to silence this warning for this and other callers." >&2
      ;;
  esac
}
...
# cs_read_local_md_key <repo_root> <key>
