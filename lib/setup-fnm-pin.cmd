@echo off
setlocal
REM Windows launcher for setup-fnm-pin.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "setup-fnm-pin.ps1" sibling of
REM     this file over this "setup-fnm-pin.cmd" file itself -- if both exist, `setup-fnm-pin`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "setup-fnm-pin.py"
...
echo [setup-fnm-pin] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [setup-fnm-pin] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127

:run_baked
"%_py%" "%~dp0setup-fnm-pin.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0setup-fnm-pin.py" %*
exit /b %ERRORLEVEL%
