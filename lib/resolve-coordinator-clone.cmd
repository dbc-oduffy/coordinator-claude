@echo off
setlocal
REM Windows launcher for resolve-coordinator-clone.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "resolve-coordinator-clone.ps1" sibling of
REM     this file over this "resolve-coordinator-clone.cmd" file itself -- if both exist, `resolve-coordinator-clone`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "resolve-coordinator-clone.py"
...
echo [resolve-coordinator-clone] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [resolve-coordinator-clone] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127

:run_baked
"%_py%" "%~dp0resolve-coordinator-clone.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0resolve-coordinator-clone.py" %*
exit /b %ERRORLEVEL%
