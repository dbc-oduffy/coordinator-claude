@echo off
setlocal
REM Windows launcher for setup-seed-health-ledger.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "setup-seed-health-ledger.ps1" sibling of
REM     this file over this "setup-seed-health-ledger.cmd" file itself -- if both exist, `setup-seed-health-ledger`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "setup-seed-health-ledger.py"
...
:run_py3
py -3 "%~dp0setup-seed-health-ledger.py" %*
exit /b %ERRORLEVEL%
