@echo off
setlocal
REM Windows launcher for read_doe_root_pointer.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "read_doe_root_pointer.ps1" sibling of
REM     this file over this "read_doe_root_pointer.cmd" file itself -- if both exist, `read_doe_root_pointer`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "read_doe_root_pointer.py"
...
echo [read_doe_root_pointer] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [read_doe_root_pointer] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127

:run_baked
"%_py%" "%~dp0read_doe_root_pointer.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0read_doe_root_pointer.py" %*
exit /b %ERRORLEVEL%
