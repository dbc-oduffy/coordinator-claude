@echo off
setlocal
REM Windows launcher for coordinator-state-root.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "coordinator-state-root.ps1" sibling of
REM     this file over this "coordinator-state-root.cmd" file itself -- if both exist, `coordinator-state-root`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "coordinator-state-root.py"
...
echo [coordinator-state-root] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [coordinator-state-root] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127

:run_baked
"%_py%" "%~dp0coordinator-state-root.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0coordinator-state-root.py" %*
exit /b %ERRORLEVEL%
