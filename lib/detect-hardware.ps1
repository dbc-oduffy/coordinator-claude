# detect-hardware.ps1 — python-direct Windows launcher for detect-hardware.py (NO bash re-exec).
...
#   - On bare-name resolution, PowerShell prefers this "detect-hardware.ps1" file over
#     any co-located "detect-hardware.cmd" sibling -- `detect-hardware` typed bare in a pwsh
...
#
# Resolves a Python interpreter and runs the co-located entrypoint "detect-hardware.py"
...
$_entry = Join-Path $_here 'detect-hardware.py'
...
[Console]::Error.WriteLine('[detect-hardware] ERROR: no Python interpreter found (python.exe / py -3).')
[Console]::Error.WriteLine('[detect-hardware] Install Python: https://www.python.org/downloads/windows/')
exit 127
