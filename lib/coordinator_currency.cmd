@echo off
setlocal
REM Windows launcher for coordinator_currency.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "coordinator_currency.ps1" sibling of
REM     this file over this "coordinator_currency.cmd" file itself -- if both exist, `coordinator_currency`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "coordinator_currency.py"
...
echo [coordinator_currency] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [coordinator_currency] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127

:run_baked
"%_py%" "%~dp0coordinator_currency.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0coordinator_currency.py" %*
exit /b %ERRORLEVEL%
