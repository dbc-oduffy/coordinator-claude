@echo off
setlocal
REM Windows launcher for install-substrate.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "install-substrate.ps1" sibling of
REM     this file over this "install-substrate.cmd" file itself -- if both exist, `install-substrate`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "install-substrate.py"
...
echo [install-substrate] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [install-substrate] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127

:run_baked
"%_py%" "%~dp0install-substrate.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0install-substrate.py" %*
exit /b %ERRORLEVEL%
