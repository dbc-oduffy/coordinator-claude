@echo off
setlocal
REM Windows launcher for install.py — python-direct (NO bash re-exec).
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "install.py"
...
echo [install] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [install] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
