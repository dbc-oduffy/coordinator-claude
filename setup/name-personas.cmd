@echo off
setlocal
REM Windows launcher for name-personas.py — python-direct (NO bash re-exec).
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "name-personas.py"
...
echo [name-personas] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [name-personas] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127

:run_baked
"%_py%" "%~dp0name-personas.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0name-personas.py" %*
exit /b %ERRORLEVEL%
