@echo off
setlocal
REM Windows launcher for dev-sync.py — python-direct (NO bash re-exec).
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "dev-sync.py"
...
echo [dev-sync] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [dev-sync] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
