...
### Pre-Dispatch Verification (example-repo T1.1, single landing across 3 files)
...
1. **Dispatch all items in the wave concurrently.** For each item:
   ...
   - Dispatch to a Sonnet executor agent with `run_in_background: true` and `mode: "auto"`. The prompt must include: the full spec (or path to it), the item's file footprint from Phase 2, and the constraints below.
...
**Hibernate:**
1. Verify push: `git log "origin/$(coordinator-current-branch)..HEAD" 2>/dev/null` should be empty for all session work
