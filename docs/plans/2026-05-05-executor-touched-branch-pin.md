...
The Staff Engineer review identified that two factual premises were unverified and that the platform-gotchas wiki was cited for a claim it does not make. Probe both before approving Options below.
...
**Cross-repo guard (per the Staff Engineer Finding 1).** Executors routinely cross repos (`~/.claude` ↔ project repos). Without a guard, the hook in the executor's repo could write into a non-existent / wrong parent dir.
...
**Where parent-capture lives (per the Staff Engineer Finding 5).** Probe 0.2 step 4 answers: does `SessionStart` fire for subagent dispatches?
...
- `plugins/coordinator/hooks/scripts/track-touched-files.sh` — detect parent session, cross-repo guard, write to parent's touched.txt as well as own. ~25 lines (was estimated 15 before cross-repo design).
- `plugins/coordinator/lib/coordinator-session.sh` — `cs_init` writes `parent.txt` as `<id>:<git_root>` from the source identified by Probe 0.1.
...
- `coordinator/CLAUDE.md` — strike "After every executor-ending dispatch, follow with explicit-path commit" rule **in a separate, second commit** after one verification session passes (the Staff Engineer Finding 6).
...
- Concurrency: two executors dispatched in parallel — both writes land in EM's touched.txt. Duplicate entries are benign; `coordinator-safe-commit:398–401` candidate_map dedups them. Note in test report (the Staff Engineer Finding 7).

### Coupling acknowledgement (per the Staff Engineer Finding 2)
...
The Staff Engineer Finding 3 corrected my earlier framing. Executors are LLM agents, not deterministic processes — a doctrine line "before any git commit, run git branch --show-current" is the class of instruction that gets dropped under context pressure. Only the bash helper fails closed.
...
- `plugins/coordinator/bin/coordinator-safe-commit` — add `--expected-branch` flag, mismatch abort. ~15 lines, plumbs into flag-parsing block (lines 43–90).
...
- `plugins/coordinator/agents/executor.md` — pass-through standing order. ~3 lines.
...
5. **Doctrine sweep last (separate commit, after one verification session, per the Staff Engineer Finding 6):** strike obsolete workarounds in `coordinator/CLAUDE.md` and improvement queue. Until that strike, the existing "follow-up explicit-path commit" rule stays in place as a safety net.

## Out of scope

- General `--include-executor-touched <id>` flag (Option 2 above; reconsider only if Probe 0.1 collapses Option 1).
- Refactoring the session-id sentinel to fix the "last writer wins" concurrency issue. **Note coupling: Option 1 adds a second sentinel-read on the same hot path; the eventual rework must address both call sites.** (the Staff Engineer Finding 2.)
...
- `plugins/coordinator/hooks/scripts/track-touched-files.sh`
- `plugins/coordinator/lib/coordinator-session.sh` (`cs_init`, lines 171–180)
- `plugins/coordinator/bin/coordinator-safe-commit` (flag block lines 43–90)
...
- `plugins/coordinator/bin/coordinator-safe-commit:112–171` (session-id resolution)
- `plugins/coordinator/bin/coordinator-safe-commit:347–452` (--scope-from logic)
- `plugins/coordinator/bin/coordinator-safe-commit:404–429` (cross-session subtraction)
- `plugins/coordinator/hooks/scripts/track-touched-files.sh:64–87` (session-id + path)
- `plugins/coordinator/hooks/scripts/track-touched-files.sh:92–108` (slow-path / fallback cs_init invocation)
- `plugins/coordinator/lib/coordinator-session.sh:171–180` (cs_init meta.json write)
- `plugins/coordinator/pipelines/workday-start-internals.md:6–26` (only known checkout site in tree)
