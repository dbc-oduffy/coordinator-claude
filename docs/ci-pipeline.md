...
| Persona names | `check-persona-names.py` | Persona display names (Patrik, Sid, Camelia, Palí, Fru, Zolí, YK) appearing in canonical-layer files — depersonalize hook should have substituted to role labels |
...
- `plugins/coordinator/bin/depersonalize-for-publish.sh` — the depersonalize hook's substitution map legitimately holds persona names as keys (e.g. `["Patrik"]="the Staff Engineer"`). Stripping them would break the hook.
