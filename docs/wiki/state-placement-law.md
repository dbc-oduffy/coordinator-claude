...
> **Purpose.**
> Durable rule for where every session-authored artifact lands after the engine repo's state-home
...
`~/.claude`, compounding structural rot. The fix: **relocate working data to the engine repo** and route every artifact-authoring surface through a resolver seam, so future pickups
...
**A further reason `~/.claude` is the wrong home for routine working state: it is a monitored
surface.** Agent reads/writes under `~/.claude` trigger Anthropic monitoring alerts, independent of
whether a given path happens to sit outside a relocation sweep. A doctrine claim that some specific
`~/.claude` path is "safe" because it isn't swept does not account for this — install batons,
handoffs, and other routine working state belong off `~/.claude` (settings-home, or a sibling
repo's `state/`) regardless of sweep status.
...
| **Install-baton rendezvous** (shared install/orient batons — `kind: spinoff` batons carrying `install_chain_order:` or an orient-leg discriminator, seeded by a conforming repo's installer or coordinator's own onboarding flow) | **`$(coordinator-settings-home)/state/handoffs/`** — machine-shared, per-machine install substrate | Distinct from per-repo `state/handoffs/` (row above — session-scoped, `$GIT_ROOT`-resolved) and from central engine-repo state (row above — meta-repo session state). Mints the same `<settings-home>` root prefix as the C9 `<settings-home>/<repo-id>/` install-status-ledger / chain-walk-visited-set precedent — both are per-machine install substrate, a plane distinct from the engine repo's meta-repo session state — but the rendezvous is NOT scoped under `<settings-home>/<repo-id>/` (it is machine-shared, not per-repo-id). See `agent-install-contract.md` § The rendezvous and § Relocation boundary. Compat: legacy `~/.claude/state/handoffs/` read as fallback during the transition window. |
| **Meta-repo operational docs**: top-level `~/.claude/docs/{plans,research,problems}/` whose deliverable does NOT edit the plugin source tree | **the engine repo** | Physically moved at migration (not auto-resolved); new artifacts authored via seam-routed surfaces land in the engine repo. Doctrine-subject plans land in the doctrine repo's single `docs/plans/` — see § Plan Homes below. |
| **Plugin source** (`bin/`, `lib/`, `hooks/`, `skills/`, `agents/`) | **the doctrine repo's clone — `coordinator/` tree, resolved live via `--plugin-dir`** | Current tooling shape: coordinator plugin source is resolved live from the external doctrine-repo clone (`$REPO_DOE_CLAUDE/coordinator/`) via Claude Code `--plugin-dir`; the `~/.claude/plugins/coordinator-claude/` marketplace copy is retired/vestigial. OSS publish (the engine repo's `coordinator/bin/publish.py`) sources from the allowlisted plugin subpath inside the doctrine-repo clone (doctrine-repo→OSS percolation). |
| **Wikis** (`docs/wiki/`) | **the doctrine repo's clone — `coordinator/docs/wiki/`, resolved live via `--plugin-dir`** | Same coordinator tree as plugin source (`$REPO_DOE_CLAUDE/coordinator/docs/wiki/`); OSS-published Tier-1 orientation surface — moves would break publish + session boot. |
| **Decisions** (`docs/decisions/`) | **the doctrine repo's clone — `$REPO_DOE_CLAUDE/docs/decisions/`** | Tier-1 orientation. `~/.claude` has no `docs/` path in any commit reachable from this clone's fetched refs, and none untracked in the pre-incident config snapshot; the corpus was seeded into the doctrine-repo clone and is still written there. DR-083 makes the doctrine repo the SSOT for DR-identifier allocation, so the write target and the numbering authority are the same tree. Note the corpus is dual-indexed — some records are date-named files carrying an internal `id: DR-NNN`, so a filename-only scan shows phantom gaps (see `naming-discipline.md`). |
| **`archive/`** | **the engine repo** | Session-init archives `state/handoffs → archive/handoffs` within the state-repo; the engine repo holds `archive/` alongside `state/`. |
...
**The gate is never `status:` alone.** A reaper for this class must condition deletion on session liveness AND/OR an age floor (or both), never bare completion status — a `status: complete` sidecar whose requesting session is still live, or that hasn't cleared the age floor, is preserved regardless of status. This mirrors the two legacy `state/review-trail/findings/` reap legs (marker-present / doctrine-repo-owned; marker-absent-and-aged / engine-owned) and is now the steady-state rule for every `state/subagent-share/` sidecar type (review findings, staff-eng-review, general run-reports — **not** the plan-pipeline lens sidecars, which live at `.coordinator-local/plan-sidecars/` and are UNREAPED-BY-DESIGN, see that row above) via `bin/reap-stale-subagent-sidecars.py` (engine-repo-resident; see the `state/subagent-share/` row above).

**Why this is a named op, not an ad-hoc `/distill` purge.** A directory-purge sweep can't distinguish "folded and safe to delete" from "never folded, still load-bearing" — that's exactly the failure mode a naive `status:`-only sweep hits, reaping a sidecar an in-flight concurrent session is still writing to. Pinned in **this repo** by `coordinator/tests/test_distill_subagent_share_sweep.py` (`test_gate_is_liveness_or_age_not_status_alone`) — a doctrine-repo path, not an engine-repo one, and the only pin this clause has. Delete-by-convention reaping is surgical and typed, scoped to one artifact class at a time — the same shape as `bin/reap-integrated-review-findings.py`.
...
A plan, DR, or doc living in the engine repo records only that **the engine repo is coordinator's central
working-data store** — it makes NO claim about who *owns* or *executes* the work. Ownership is
set by the tri-plane boundary (the doctrine repo's contract-vs-engine decision is the
governing authority, with the custody-vs-projection framing supplied by
the engine repo's state-is-disk-truth decision):
*meaning* (artifact-shape-contract + skills) → coordinator-claude; *emission-write engine + custody
of its own disk-truth bytes* → the engine repo; *query/retrieval capability over a derived, re-projectable
...
coordinator-plugin surfaces sat in the engine repo's `docs/plans/` and read as "the engine repo's work" — it is not.
...
The doctrine repo's authoring-repo root — the *doctrine plane*, sibling to `CLAUDE_KLABAUTER_ROOT` (the *engine
plane*). Resolver primitive `coordinator_doe_root` (engine-native — `coordinator_core/ops/coordinator_doe_root.py`; there is NO `lib/coordinator-doe-root.sh` and never was, so do not reach for a sourced shell form. Reached in practice through `lib/coordinator-state-root.py --central --subject doctrine`, which routes to the doctrine plane), same outer chain
...
itself fails loud. Introduced by the doctrine-repo-authoring seam rewire (W2.1).

### `coordinator_artifact_subject <path>`

Subject-matter classifier (the engine repo's `coordinator/lib/coordinator-artifact-subject.py`, W2.2) — the routing key for
...
on *subject*, not tree location: e.g. an engine-repo install *script* → `engine`, the coordinator install
*skill* → `doctrine`.
...
via the engine repo's `coordinator/lib/coordinator-state-root.py` (or import `coordinator_state_root` from it,
now itself migrated to the engine repo), never as a sourced shell function. Encodes the
...
- `coordinator-state-root.py --central` (no subject/artifact) → `$(coordinator_claude_klabauter_root)/state` unconditionally
  (**backward-compat default — unchanged**; every existing `--central` caller still routes to the engine repo).
- `coordinator-state-root.py --central --subject doctrine` (or `--artifact <doctrine-path>`) →
  `$(coordinator_doe_root)/state` — doctrine-subject central state routes to the doctrine plane. Fail-loud
  (no silent fallback to the engine repo) when `REPO_DOE_CLAUDE` is unresolvable; central-loop callers catch
  and WARN+skip.
...
# CORRECT — routes through the seam (coordinator-state-root.py is engine-repo-resident)
...
**Graceful degradation:** central-loop entry points (the engine repo's `coordinator-lesson-promote`,
...
- `docs/README.md` (meta-repo master index — stays; updated to point cross-repo to engine-repo-resident plans)
- **`.doe-root`** — **a disposable, non-authoritative mirror under `~/.claude`.** The durable, authoritative coordinator-root anchor is now the settings-home machine-local registry (`~/.coordinator-claude-settings/machine-local/registry.local.toml`, key `repos.doe_claude`), mirroring the engine repo's own analogous root-anchor file. `~/.claude/.doe-root` remains as a regenerated, untracked cache (never a committed per-machine value); it is downstream of the registry, not the source of truth. the engine repo's `coordinator/bin/coordinator-settings-home` (the settings-home resolution seam).
...
| `.coordinator-venv/` | `~/.claude/.coordinator-venv/` | `~/.coordinator-claude-settings/.coordinator-venv/` | Rebuilt (never copied) by `install-substrate.py` (C10a) via the native venv builder (`coordinator_core.install.ensure_venv`, engine-repo-resident); legacy venv removed only after rebuild + health probe both confirm healthy. |
...
principle is the doctrine-of-record here — there is no competing doctrine-repo decision-record; the
code-level `topology.md § AC-1` reclassification is the engine repo's own artifact, folded into this
...
- **Tier A — instrumented repos** (run coordinator + the engine repo): owe rich Tier-A emission — the full
...
- **`--subject`/sibling routing** (the engine repo's `coordinator/lib/coordinator-state-root.py`, a CLI trampoline
  over the engine repo's `coordinator_core.state_root.coordinator_state_root`, Rules 1–5) — the
...
and falls back to `~/.claude/state`." That claim is **false on the doctrine-repo *shell* seam when sourced**
...
doctrine-repo-owned latent robustness bug, tracked at low priority. State both halves precisely: a reader
...
Cockpit. A new repo that stands up coordinator + the engine repo inherits per-repo emission automatically
...
- **Tier A/B graceful degradation.** A Tier-A repo (coordinator + the engine repo) emits the full artifact
