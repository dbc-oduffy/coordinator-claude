...
**`## Deviations` audit tables are retired as of 2026-06-15.** Forecast-vs-shipped reconciliation now happens entirely via `(was: <plan-forecast>)` ALLOWLIST annotations in the Decisions Made / API Contracts sections (the load-bearing surface for `/distill` Phase 1 `[SUPERSEDED]` classification). `/workstream-complete` Step 2.4 corrects ALLOWLIST sections in place and no longer appends a `## Deviations` audit table. Historical plans may still carry one — `/distill` continues to drop them as `[EPHEMERAL]` for backwards-compat — but do not author new ones. → `docs/wiki/plan-deviation-reconciliation.md` for the legacy format and the canonical reconciliation contract.
...
> Spec backlink: `docs/plans/2026-07-09-plan-full-coverage-and-deferred-harvest.md § C1`
> Schema: `schemas/plan-tasks.schema.json` (per-row shape), wired into `schemas/plan.schema.json`'s `tasks` property via `$ref`.
...
| `change_kind` | enum | required | The **WIDER UNIVERSAL** enum — SSOT is `docs/wiki/lessons-outbox-schema.md § Change-kind enum`. Ten values: `doctrine-edit`, `agent-prompt-edit`, `hook-edit`, `script-edit`, `snippet-sync-update`, `wiki-new`, `wiki-append`, `skill-edit`, `doc-edit`, `test-edit`. |
| `surface` | string | required | The harvest/coverage **PRIMARY-TARGET** field — a single path or subsystem. Maps to the queue entry's `surface` field ONLY. This is deliberately **not** the Dispatch Ledger write-files set (which may enumerate many files) — `surface` names the one primary target a triager would look at first. |
...
| `deferred` | bool | optional (default `false`) | `true` means this row is NOT shipped by this plan — it is a harvest candidate. |
| `pm_approved` | bool | **required when `deferred: true`** | Records that the PM ratified the scope cut. `false` or absent on a `deferred: true` row is a `plan-coverage-checker` flag. |
| `body` | string block | optional | Multi-line detail. Maps to the queue entry's `body` field. |

**Deferral needs PM ratification — this is not a self-service escape hatch.** A row with
`deferred: true` and no `pm_approved: true` is a plan author quietly cutting scope without
the PM's sign-off; `plan-coverage-checker` exists specifically to catch this. Only rows
with **both** `deferred: true` AND `pm_approved: true` are harvest candidates — the
deferred-harvest CLI never promotes an unratified cut into the improvement queue.

### Harvest routing (informational — the harvest CLI owns the mechanics)

The harvest-eligible slice of `change_kind` is the 8-value project-tier subset
(`script-edit`, `skill-edit`, `wiki-append`, `wiki-new`, `hook-edit`, `agent-prompt-edit`,
`doc-edit`, `test-edit`) — these route to `coordinator-queue-append --schema
...
- **FLAGGED** by `plan-coverage-checker` — the enforcement surface that a plan author
  actually sees at review time.

**Post-scaffold hand edits to the `## Tasks` block are Tier-2 (warn, never block)**, per
`docs/wiki/canonical-artifact-shapes.md § Warn-Not-Block Enforcement Posture` — the same
...
> Spec backlink: `docs/plans/2026-07-09-plan-full-coverage-and-deferred-harvest.md § C5`
...
from the task list — it becomes a `deferred: true` row on the spine (see § Authoring the
block above), and a `deferred: true` row is only a legitimate cut when `pm_approved: true`
sits alongside it. The mechanized enforcement lens for this is `plan-coverage-checker`'s
deferral-flag check: a `deferred: true` row with `pm_approved` false or absent is flagged,
full stop — the checker does not accept an EM's own judgment that a cut was reasonable as
substitute PM ratification. See `## Branch C — Compose the plan body` in
`skills/plan/SKILL.md` (the "soon = now" deferral row and the YAGNI row) for the
authoring-time version of this same discipline, and `## Branch A` in the same skill for
the triage-time scoping check this section extends.
...
Sister to "existence ≠ fit" and `§ Verifying Handoff Premises` (broken-today claims need HEAD verification). (Source: 2026-06-18 example-game-workbench-repo.)
...
### (l.1) Executor briefs authoring `~/.claude` subpaths must pin `${CLAUDE_HOME:-$HOME}/.claude` verbatim

**Any executor brief that writes, reads, or references a path inside `~/.claude` must pin the resolution `${CLAUDE_HOME:-$HOME}/.claude` verbatim in the brief — never hardcode `~/.claude` as a bare tilde-path.** The `CLAUDE_HOME` override is the multi-machine portability seam: on machines where `~/.claude` is symlinked or relocated (e.g., for shared installs or custom home dirs), the tilde expands to the wrong root. An executor brief that hardcodes `~/.claude` bakes the primary machine's path into the executor's commands, fails silently on all other machines, and cannot be caught by the path-verification row above (that row checks repo-relative paths, not meta-repo-relative tilde paths). Mechanism: `${CLAUDE_HOME:-$HOME}/.claude` expands to `CLAUDE_HOME` when set, falling back to `$HOME/.claude`. Backlink: `docs/wiki/machine-local-registry.md § 4a` for the full resolver. Source: 2026-06-20 ~/.claude.
...
**Rule:** before writing "verbatim" or "reuses the exact mechanism from X", enumerate every branch in X (if/else trees, conditional flags, mode switches). Mirror each or explicitly name the deviation and weaken the claim: "reuses the headline invocation, omitting the `--check-only` branch." the Staff Engineer's review surface is the backstop — a "VERBATIM" assertion in a plan is a known attention trigger for the reviewer. Source: 2026-06-11 repo-setup-whoami-self-install.

## Anti-Literal-Tripwire Chunks Must Grep-and-Mark Scoped Docstrings In-Chunk

**A chunk that adds an anti-literal tripwire (a hook or validator that fires on a literal string appearing in a file) must grep every in-scope path for any docstring, comment, or test fixture that uses the literal as a teaching example — and mark each occurrence with a scoped exemption annotation in the SAME chunk, not at validation time.** Anti-literal tripwires fire on docstrings that say "do not write X" or tests that assert the string X appears for negative coverage; by the time the executor reaches the validation step, those false-positive fire sites are already present on disk and the tripwire blocks the merge. Finding and annotating them is the chunk author's obligation, not a follow-up task. At plan-write time, add an explicit sub-step: `grep -rn "<literal>" <scoped-paths> — mark each non-production hit with the tripwire's exemption comment`. Source: 2026-06-09 ~/.claude.
...
*Source: example-game-repo `state/lessons.md` (example-game-repo-L33, central-promoted 2026-05-28).*
...
*Source: example-game-repo `state/lessons.md` (example-game-repo-L115, central-promoted 2026-05-28).*
...
*Source: example-game-repo `state/lessons.md` (example-game-repo-L155, central-promoted 2026-05-28).*
...
**`source_memo:` cross-check — for a plan that actions a cross-repo memo.** Filename + `git log` collision detection misses the case where two sessions route plans from the *same memo* under different slugs (the 2026-06-23 example-game-repo B3 collision: two sessions realized the same accepted memo because nothing cross-checked `source_memo` at plan-write time). When the plan carries (or will carry) a `source_memo:` frontmatter pointer, additionally grep every plan for a sibling citing the same memo basename — `grep -l "source_memo:.*<memo-basename>" docs/plans/*.md` plus a body-mention fallback — and surface any hit to the PM before drafting. This fires in the same Branch B.0 pre-flight as the filename/`git log` checks, before scout/reviewer dispatch. **Residual:** catches plan-routed collisions only; commit-only / `inline` realizations leave no plan carrying `source_memo:` and are instead guarded by the M2.5 memo claim-lock plus the archived `realized_by` claim-of-record (see `cross-repo-communication.md` § Claim-at-pickup parity with handoffs). Spec: `docs/plans/2026-06-23-memo-pickup-realization-claim-visibility.md`.
...
**Why:** the 2026-06-14 codex-reviewer-integration plan wrote to `~/.claude/settings.json` without reading `bin/platform-localize.sh`, which already performed a wholesale-write to `settings.local.json`'s `extraKnownMarketplaces` key from a directory-scan — a two-writer tension on the same key. The Staff Engineer flagged both at R1 as critical/major. The plan body's "Substrate verification" bullets named what existed but not what *wrote* to it.
...
**Why:** the 2026-06-14 distill-phase3d plan, the Staff Engineer R1 F0 (DR-3): Phase 5 expansion specified "Read scout file, locate section_anchor heading, enumerate paths under that section (until next H2 or EOF)." the Staff Engineer caught it as a Markdown-parse contract masquerading as mechanical — path-line format unspecified, H2 boundary unverified. Fix: PM directed tightening scout format to fenced YAML-under-anchor; the consumer parses YAML, not Markdown prose.
...
**Why:** the 2026-06-17 concurrent-pickup-guard-sid-regression: a spike switched `cs_claim_handoff`'s session-id resolution from the machine-shared `.current-session-id` sentinel to per-session `CLAUDE_CODE_SESSION_ID` (correct locally). But DR-110's same-machine concurrent-pickup guard relied on the OLD behavior — it namespaced the atomic-`mkdir` claim path on `<sid>`, and same-machine sessions only collided because they shared the sentinel sid. Per-session sids → distinct claim paths → `mkdir` never collides → the guard silently never fired (split-brain dual pickup). The fix's author saw only the local correctness win, not the downstream consumer that assumed the shared value.
...
Sister to "broken-today claims need HEAD verification" and "verified-at-plan-write is decaying." (Source: 2026-06-19 example-game-workbench-repo.)

### "Verified at plan-write" is a decaying claim within a single long session

**Rule:** when a plan's gate-posture or substrate-blocker matrix asserts a dependency is absent / broken / unlanded ("SB-2: tc-7 not landed, verified 2026-06-19"), re-verify against HEAD at *review-integration* AND at *execute-plan Phase 1* on any session longer than ~1h or with known concurrent branch activity. A concurrent EM can land the blocker mid-session while the plan is in review — the "verified" timestamp is intra-session-stale, not just pickup-to-now stale.
...
**Why:** the 2026-06-08 case (~/.claude): allowlist TOML bucket names were treated as if they equalled detection category names, so an entry was looked up by category and missed — the correct shape iterates all named buckets and matches each. Source: 2026-06-08 ~/.claude.
...
**How to apply:** at plan-write time, when a chunk proposes to change an existing field's meaning, locate the versioning/schema contract and look for a `who-must-ratify-meaning-changes` clause before treating the change as free. You cannot substitute "it's low-risk" for the named approval. (Source: 2026-06-18 example-game-workbench-repo.)
...
**Why:** the 2026-06-15 nudge-quota-exhausted-agent plan (v1→v2 pivot): v1 authored a PostToolUse-on-Agent hook scanning `tool_response.content`. The Staff Engineer review (R1 F1 — DR-2) flagged the field path as hypothesis. EM added Chunk 0 ("EM-owned live-payload capture before executor dispatch"). C0 captured a real payload, revealed `tool_response.content` doesn't exist for async responses, and killed the entire v1 approach before any executor wasted cycles.

**Pattern that works:** temporarily inject capture into an existing hook script (e.g. `agent-completion-log.sh`), trigger one Agent dispatch, sanitize the JSON payload, commit as plan sidecar (`*.c0-evidence.json`). Only after the real payload shape is confirmed do C1 executors begin.
