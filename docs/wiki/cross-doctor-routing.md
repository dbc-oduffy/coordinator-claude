...
3. **Grep our own config surfaces** — `the (now-removed) meta-repo local-doctrine file`, our repo's `.mcp.json`, machine-local sentinels. The operator may have a non-canonical path that our config knows about. Stop on first match.
...
- **Per-machine variance.** `/x/project-rag` on one machine, `~/dev/project-rag` on another, `D:\project-rag` on the third. The registry is per-machine; the cascade reads it. <!-- foreign-path-ok: illustrates the very per-machine variance this doc is about, not a location claim -->
...
- example-game-repo ↔ project-rag (UE-RAG addon)
...
Pattern formalized in addon-doctor-host-routing-pointers-cascade work (project-rag-ue-addon). Canonical implementation: `resolve_peer_repo_pointers()` in the addon's doctor. Lesson promoted from `project-rag-ue-addon` repo's `state/lessons/:356` via `/learn-lessons --mode=central`.
