...
4. **Dispatch a single seam reviewer** — the reviewer self-scaffolds its sidecar in
   `state/review-trail/findings/` and returns a pointer+verdict line. No EM pre-scaffold.
   For `coordinator:code-reviewer` (UNNAMED): the reviewer self-persists by default.
   For a persona reviewer: append `snippets/findings-self-persist-sentinel.md` to the brief.
