...
Coordinator skills come in two shapes. **Narrative skills** (the original pattern) contain prose explaining principles and examples — the EM is expected to absorb them at boot. **Decision-tree skills** contain a tree the EM walks at trigger time, with each branch terminating in a single concrete action.

For skills about *how to use the coordinator workflow* (planning, reviewing, etc.), prefer the decision-tree shape. For wiki pages containing long-form reference doctrine, keep as narrative. See `docs/wiki/skills-corpus/super-skill-architecture.md` for the full decision-tree skill contract (7 rules).
...
**In coordinator, the sibling-file half of that applies and the script half does not** — heavy
reference splits out (this repo spells the sibling directory `residue/`, topic-named, one hop),
executable code does not travel beside a skill. See § Skills With Executable Code — documented
upstream, deliberately not used here.
...
- **Omit `name:` to avoid collisions across plugins.** If two enabled plugins both expose a generic verb (`/setup`, `/doctor`), keep at least the non-primary ones qualified. Coordinator's `/coordinator:install` is intentionally prefixed because example-game-repo and deep-research also have setup commands.
...
> That is the mistrust-shape `docs/wiki/dispatching-parallel-agents/eager-agent-calibration.md` rejects. The reframe: baseline
...
`docs/wiki/dispatching-parallel-agents/eager-agent-calibration.md` § Offer-Shape vs. Friction-as-Warning):
...
framing to coerce compliance is exactly the mistrust-shape `docs/wiki/dispatching-parallel-agents/eager-agent-calibration.md`
...
### Skills With Executable Code — documented upstream, deliberately not used here
...
The divergence is a considered rejection, not an oversight. A bundled script inside a skill
directory is invisible to the engine's test registries, cannot be called by any surface other
than that one skill, and reintroduces the EM-as-transport step the tripwire exists to extirpate.
The sanctioned shape is the single-line invocation of a named CLI resolved per the precedence
ladder in `coordinator/snippets/resolve-coordinator-bin.md` — rung 0 / Shape W on a PowerShell
host, `${COORDINATOR_SETTINGS_HOME:-$HOME/.coordinator-claude-settings}/bin/<cli>` on a POSIX host.
...
> See `coordinator/snippets/em-operating-doctrine.md` § How to Dispatch (no current heading names
> "agent registration" specifically — closest surviving section is the general dispatch doctrine
> there) for further notes on agent registration.
...
An agent declaring `access-mode: read-only` with `Write` in its `tools:` list WILL write —
disk-verified under both `bypassPermissions` and `acceptEdits` permission modes, and via both
...
**`access-mode` is a capability declaration, not a behavioral promise.** The field answers "can
this agent mutate?" (static, lint-checked capability declaration), not "will this agent mutate?"
(body-level discipline). An agent with `Bash` in its tools list for read-only CLI invocation
(e.g. `ls`, `grep`, artifact parsing) correctly declares `read-write` because `Bash` carries
mutation capability — even when the body explicitly prohibits `Edit` / `Write`. The body's "Do
NOT use Edit or Write" is behavioral self-discipline within the `read-write` capability envelope;
it is not a contradiction.

Evidence: `docs/research/spike-verdicts/2026-07-27-boot-context-envelope.md` and the 2026-07-27
census (30 read-write, 2 read-only, 1 keyless). → `coordinator/docs/wiki/coordinator-tripwires/` § validate-agent-tools.
...
## Shell Blocks in Skills and Commands — Extract to `bin/`, Don't Inline

### Rule: bash blocks longer than a one-liner belong in `bin/`, not inline
...
### Rule: CLI doctrine that names an ad-hoc staging path teaches ad-hoc staging

When authoring CLI doctrine — wiki lines, skill steps, agent prompts — **do not instruct the agent to write to a path of its own choosing** (a temp file, an ad-hoc `tasks/<feature>/` file, a `%TEMP%` location). Any instruction of the form "write the body to a temp file and pass `--body-file <path>`" literally teaches the reflex it was meant to avoid: the surface the agent picks decays into cruft the PM eventually catches and sweeps.
...
When readers repeatedly misread a skill/command sub-step as belonging to the *preceding* phase (treating Step Nb as a continuation of Step Na rather than a distinct gate), the durable fix is a **structural rename of the sub-step boundary** — give the misread step its own clearly-distinct name/number so the boundary reads as a boundary — not a clarifying banner ("NOTE: this is a separate phase") bolted onto the existing structure. A banner is prose competing against the structure for the reader's attention; the structure wins, and the misread recurs. The rename moves the signal into the structure itself, where the reader is already looking. Reach for a banner only when the boundary genuinely cannot be renamed (a platform-fixed token, a cross-referenced anchor). (Source: ~/.claude meta-repo.)
