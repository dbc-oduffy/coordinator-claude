...
This checklist is the discharge artifact held against every computed-skills conversion baton at review time.

> Spec backlinks: the extraction-unit decision record (fleet-internal, does not ship);
> `coordinator/docs/wiki/skills-corpus/computed-skills.md` § Generalizing this pattern (AC14);
...
- [ ] **The discharge test.** For every rule the converted surface implies, name the artifact
      ...
      `coordinator/docs/wiki/claude-md-surfaces/invisible-doctrine.md` (the north star this checklist serves).
