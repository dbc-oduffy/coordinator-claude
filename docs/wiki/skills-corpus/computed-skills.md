...
A computed skill replaces a hand-walked decision tree with a single read-only engine CLI that computes the whole routing decision from disk/git/frontmatter state.
...
>
> **Read `docs/wiki/claude-md-surfaces/invisible-doctrine.md` first.** This page is the CONTRACT — the schema,
...
This vocabulary is the doctrinal parent of `docs/wiki/dispatching-parallel-agents/eager-agent-calibration.md` §
...
`docs/wiki/doctrine-authoring/named-contracts-vs-incidental-flags.md` warns against generalizing one op's
...
(`docs/wiki/claude-md-surfaces/rag-bait-conventions.md`, `no-ruling-dates-in-skill-surfaces`) applied here.
...
- **Consumer discipline** — the shared shape only holds if every caller actually uses it. That
  ...
  (`coordinator/docs/wiki/skills-corpus/computed-skills-conversion-checklist.md`, authored this session) plus
...
`docs/wiki/claude-md-surfaces/rag-bait-conventions.md` § negative-spec blocks.
