...
A dual-identity module hazard occurs when a Python module launched as `__main__` and imported by canonical name ends up loaded twice, with each copy holding its own independent state.
...
- [`writing-plans.md`](../planning/writing-plans.md) § Substrate-Verification — add this hazard check to the plan-time substrate audit for any plan that ships mutable module-level state.
- [`pre-dispatch-verification.md`](../dispatching-parallel-agents/pre-dispatch-verification.md) — grep the writer set and reader set for any boot-time global before declaring the writer's call sites complete.
