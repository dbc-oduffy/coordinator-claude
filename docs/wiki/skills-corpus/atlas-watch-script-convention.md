...
An `atlas-watch-script` is a per-system `<name>.watch.sh` sibling file that emits one mechanical FRESH/DRIFT/MISSING line for cheap drift detection between full architecture-audit rotations.
