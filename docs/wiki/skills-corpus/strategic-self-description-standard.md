...
The strategic self-description standard defines the fleet-wide producer/schema contract each repo's own strategic self-description artifact must conform to.
...
- `docs/wiki/skills-corpus/emission-conformance-contract.md § Producer Contract` — the `(owner, repo,
  coordinator_root_path)` join-key tuple `repo_identity` matches.
- `docs/wiki/addon-protocol/addon-chunker-categories.md` — unrelated `curated` vocabulary (RAG-embedding category);
