...
- `setup/install.sh` (publish-repo fresh-install entry point) AND `/coordinator:install` Phase 3 (ongoing maintenance, via `install-substrate.sh`) install `publish.sh`, `publish_sync.py`, and `publish-targets.example.sh` into `~/.claude/setup/`.
...
- If org-slug rewrites are needed, operator copies `~/.claude/plugins/coordinator/bin/depersonalize-identity.example.sh` to `depersonalize-identity.sh` and edits it.
...
Internal sibling-repo codenames (`example-game-repo`, `example-sim-repo`, `example-cockpit-repo`, etc.) appear in coordinator wikis and skills as provenance or incident case-citations. Without remediation they leak private repo names to any OSS user who reads the publish tree. The publish-time transform (`bin/publish-time-transform.sh`) handles this class automatically via a **committed but `.percolate-ignore`'d seed file** — no operator action required.

### Built-in codename seed

`coordinator/bin/codename-provenance-seed.sh` is a committed file that:

- Declares a `CODENAME_TO_PLACEHOLDER` assoc array and a `CODENAME_ORDERED_KEYS` indexed array seeded with coordinator-author private-repo codenames → `example-*-repo` placeholders (longest-match-first, case variants enumerated as separate keys per the `Dónal O'Duffy`/`Donal O'Duffy` committed precedent in the transform).
- Is sourced by `publish-time-transform.sh` at load time via the **optional-sibling guard** (identical pattern to `depersonalize-identity.sh`): `if [[ -f "$_SEED" ]]; then source "$_SEED"; fi`.
- **Does NOT ship to the OSS mirror.** It is listed in `plugins/coordinator-claude/.percolate-ignore` (same entry form as `coordinator/bin/depersonalize-identity.sh`). An OSS consumer who sources an absent file gets a silent no-op — identical to a fresh operator with no `depersonalize-identity.sh`.

The seed file is the home for **coordinator-authored repo codenames** (names that appear in coordinator wikis/skills as provenance). It is NOT the home for an individual operator's own private project names — those belong in the per-operator gitignored `depersonalize-identity.sh` (Class-2 slot). See `depersonalize-doctrine.md` § Codename Provenance for the vocabulary-vs-attribution classification that governs which surface owns what.

**Keep-set — not genericized.** `project-rag` (live MCP server namespace, accepted-exposure PM decision 2026-06-27) and shipped plugin names (`deep-research`, `game-dev`, `web-dev`, `data-science`, `coordinator`) are intentionally absent from the seed map. Rewriting them would corrupt functional identifiers in agent definitions and skill routing tables. See `depersonalize-doctrine.md` § Codename Provenance for the keep-set rationale.
...
**Expected case — `--check` residual gate (mapped codenames only).** `publish-time-transform.sh --check` greps the publish tree for the `PATTERN` built from `ORDERED_KEYS`. Once codenames appear in `CODENAME_ORDERED_KEYS`, any residual occurrence post-`--fix` fails loud. **Scope limit:** `--check` detects only codenames already in `ORDERED_KEYS`. A genuinely-new private repo cited in a new wiki entry passes `--check` clean — `PATTERN` is built from the existing map keys, not from the full registry. The novel-leak guard below closes this gap.

**Unexpected case — registry-derived novel-leak guard.** `bin/check-registry-codename-leak.sh` is wired into the percolate `post-rsync` hooks for both the `coordinator-claude` and `deep-research-claude` targets. It:
...
**None required for built-in codenames.** The seed fires automatically at every publish via the optional-sibling source block in `publish-time-transform.sh`; the registry guard fires automatically via the `post-rsync` hook. Operators do not need to author or configure anything.

**If a new private sibling repo is later cited in a coordinator wiki:** the registry guard will catch it at publish time and fail loud. The fix is to add the new codename and placeholder to `coordinator/bin/codename-provenance-seed.sh` and commit it. This is an EM engineering call; no per-operator file changes are needed for coordinator-authored codenames.
