...
> Runtime self-declaration and content-class routing contract for the project-rag MCP server — what it declares about its own topology and bug-ownership boundary, and how callers route accordingly.
>
...
**Content-agnostic invariant (the Director of Engineering Directive 1):** Host text must contain NO addon names or engine-specific vocabulary. Banned regex: `\b(blueprint|bp_|unreal|example-game-repo|uproject|umap|uasset)\b` — also "BPs"; no `UCharacterMovementComponent`-shape exemplars; no `r.example-sim-repo.EnableFlight`-shape CVar names. Generic content-class phrasing throughout.
...
**`_CAPABILITY_MATRIX_PATH` in `instructions.py`** redirects to `docs/wiki/addon-protocol/host-vs-addons.md` (primary) and `docs/wiki/addon-protocol/corpus-class-taxonomy.md` (secondary). Data envelope keys: `host_addon_contract_path` + `corpus_class_taxonomy_path`.
...
## The three indexes answer different questions — never conflate them

Structural (tree-sitter) tracks HEAD: the never-stale layer. SCIP adds call/reference richness on
a delta cadence and lags by design; tens of commits behind still beats grep. Semantic serves
slow-moving docs ("have we done this before") and exists only where indexed.
...
project-rag's live checkout is refresh-managed (`coordinator/bin/refresh-plugin-live-install.py` periodically runs `git checkout <track_ref>`). Hand-editing the source tree has no lasting effect AND gets silently reverted on next propagation. Configure through the provided verbs:
...
- `docs/wiki/mcp-topology/project-rag-tool-envelope.md` — response envelope schema, verdict enum, provenance fields
- `docs/wiki/addon-protocol/host-vs-addons.md` — host/addon separation of concerns (primary capability matrix)
- `docs/wiki/addon-protocol/corpus-class-taxonomy.md` — corpus class definitions
- `docs/wiki/addon-protocol/host-addon-separation-of-concerns.md` — architectural invariants
