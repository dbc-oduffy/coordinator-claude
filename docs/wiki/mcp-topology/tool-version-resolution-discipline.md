...
*Lesson surface: example-game-workbench-repo — a tool invocation resolved to a different installed version than the operator expected, costing diagnostic time downstream.*
...
- [`substrate-pin-doctrine.md`](../test-design-discipline/substrate-pin-doctrine.md) — substrate pins must lock BOTH index and version; the same logic at the tool layer.
- [`test-environment-discipline.md`](../test-design-discipline/test-environment-discipline.md) — pinned-venv-python resolution for test harnesses.
- [`bash-on-windows-gotchas.md`](../portability/bash-on-windows-gotchas.md) — shebang locality is the bash-side cousin of this rule.
