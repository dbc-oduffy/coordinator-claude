...
- [`implementation-standards-by-domain.md`](../doctrine-authoring/implementation-standards-by-domain.md) — sibling domain-specific standards (observability, DB/indexer, deps, packaging)
- [`verification-before-completion.md`](../em-operating-model/verification-before-completion.md) — green unit tests aren't runtime-readiness when tests don't exercise the framer under load
