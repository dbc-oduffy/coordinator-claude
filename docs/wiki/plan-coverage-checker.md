...
The agent runs four mechanical lenses — coverage (slate-vs-oracle cross-reference), hedge detection (appetite-based deferral patterns), task-spine deferral-ratification and malformed-row detection, and substrate drift (in-repo path/symbol/constant verification) — and writes a sidecar at `<plan-path>.plan-coverage-check.md` with six finding buckets: Missed, Ambiguous, Weak-OOS, Hedges, Unratified-Deferrals-and-Malformed-Rows, and Substrate-drift.
...
### Lens 2b — Task-spine deferral ratification and malformed-row detection
...
**The agent is the enforcement surface for this contract.** The harvest tool defensively parse-or-skips a malformed row (WARN-AND-SKIP — a plan mid-authoring may not have a spine yet); the coverage-checker does not have that luxury. It is the one place a malformed or unratified row gets flagged before it ships silently.

Two things this lens checks, both mechanical:

1. **Malformed rows.** Any row that fails to parse (bad YAML) or is missing a required field (`id`, `title`, `change_kind`, `surface`; `pm_approved` as a *key* — presence only, any boolean value — when `deferred: true`) is flagged — quoted verbatim, with the specific missing/invalid field named. The *value* being literal `true` is checked separately, below.
2. **Unratified deferrals.** Any row with `deferred: true` but `pm_approved` absent, `false`, or anything other than literal `true` gets the finding: **"deferral pending PM ratification — scope is a PM decision, EM preference is not a scope decision."** This fires regardless of how reasonable the deferral looks on its face — the lens checks for the presence of the ratification signal, not the merit of the deferral. Merit is the EM/PM's call, not the checker's.

**FAIL-LOUD, not skip-quiet, when the spine itself is broken.** If the `## Tasks` heading exists but carries zero or more than one `yaml plan-tasks` fenced block underneath, this is the parser-locate rule's defined error case — the checker cannot enforce anything without exactly one spine. It emits verdict `DEGRADED` and stops the spine lens (other lenses still run independently). This differs from the "no `## Tasks` heading at all" case, which is legitimate no-signal (a plan mid-authoring, or predating the task-spine contract) and produces no finding.
...
<!-- Review: code-reviewer — removed direct staff-game-dev.md bullet (game-dev retired from OSS coordinator-claude 2026-05-26); replaced with example-game-repo-resolved entry note to match the live verifier behaviour -->
