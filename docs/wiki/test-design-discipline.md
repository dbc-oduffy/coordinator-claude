...
**Read the authority axis as load rationing, not permission distribution.** What the ladder rations is spawns on a shared box — a full suite lands its whole spawn volume on a tree every live peer session is also working in, a machine-wide event rather than a verification step (`CLAUDE.md § Build & Test`). **Cite that volume from the measurement, never from memory:** `python3 coordinator/tests/_spawn_budget.py` is zero-spawn and prints the current count against the ratchet's budget. A remembered figure decays silently and has been wrong here by a factor of three. Two misreadings follow from reading it as a permission table, and both end in the same place: **"allowed to X only" is a ceiling, never a licence** — that the EM is the one actor permitted to invoke Tier F/U says nothing about whether this run is worth the machine, and "the EM owns this check" assigns the *decision*, not a standing warrant to run. And when the load is the objection, **delegating does not answer it**: a subagent's run spends the same spawns on the same box, so it moves the load rather than removing it. The grant is about the spawn, not about who holds the shell — which is why it does not travel down a dispatch (§ Corollaries).
...
Corollaries: subagents get exactly one rung — Tier T. A dispatch brief must never contain a Tier-F or Tier-U command, and **a live grant does not travel down a dispatch** — it authorizes a load the machine can absorb once, not the same load re-spent by a delegate. An EM told the run is too expensive and answering by dispatching it has not honored the objection; it has hidden the spawns behind an agent boundary. Concurrent suite runs are forbidden outright regardless of tier — see `test-environment-discipline.md` §6 (shared-tree concurrent edits produce fake assertion failures on HEAD-correct constants; §5 covers a narrower, more severe variant — native access violations that abort the pytest process outright).
...
**Any autouse fixture that mocks production primitives must be opt-IN, not opt-out.** `tests/conftest.py`'s autouse `_mock_torch_for_lifespan` patched `_acquire_lock` / `_release_lock` to no-ops UNLESS `@pytest.mark.real_pid_lock` was present. Tests that exercised the real lock primitive never had the marker — so they "passed" against mock no-ops for weeks, then started failing once the production code grew assertions the mocks fail to satisfy. A forgotten opt-out marker means the test runs against a fake, not a clean failure.
...
## 110. A Command-Matching Predicate Is a Grammar Assumption — Fixture Every Grammar the Fleet Drives


**Anything matching command *text* — a guard regex, a hook's tokenizer, a policy allowlist — assumes a shell grammar, and no linter sees it.** ShellCheck and portability review scan shell we *write*; this is shell we *match*, and `r"rm -rf"` is portable Python.

Fixtures spelled in the grammar the matcher already understands cannot fail in the direction of the defect, so the suite goes green independent of coverage. A guard layer of 100 test files and 559 POSIX-idiom fixtures, carrying one PowerShell mention, misses `Remove-Item -Recurse -Force` at full green.

**Rule.** Every command-matching predicate carries at least one fixture in each grammar the fleet drives (POSIX and PowerShell), enforced by a suite-level check that fails when a predicate has fixtures in only one. The check is what survives; a per-guard obligation nobody enforces decays to whichever grammar the last author thought in.

**Assert the differential, not two independent verdicts** — independent assertions let a second-grammar fixture be added and quietly pinned to `allow`. A deny-on-match predicate denies one spelling iff it denies the other. A **default-deny allowlist inverts**: a benign command in the other grammar must ALLOW, since an allowlist fired against an unparsed grammar denies everything and confines the agent to nothing. Direction is per-predicate; backwards pins the defect.
...
The motivating incident: `coordinator/tests` reached an estimated ~4,429 spawns across 98 modules and 122 git call sites, spawning once per parametrized case where one spawn per behaviour would have done — enough to crash live EM sessions sharing the machine. A prior correction excised 79 modules under PM direction; this plan restored their coverage spawn-free. The corpus now measures ~353 estimated spawns against a 354 budget, with zero fixture-construction git call sites outside a single named two-module allowance (see `coordinator/tests/_spawn_budget.py`).

That result holds only as long as the practice below is the default, not a one-time cleanup:

- **Hooks are tested in-process.** `coordinator/tests/_guard_harness.py`'s `invoke_guard` calls the guard's Python entrypoint directly and returns exit status, stdout, stderr, and envelope — an in-process rewrite need not drop any assertion channel a subprocess invocation used to give you. Reach for it before reaching for `subprocess.run([...guard script...])`.
- **Git *state* for a fixture is built as plain files**, via `coordinator/tests/_fake_git.py` — no `git init`/`add`/`commit` subprocess per test case. It also writes real committed history when a test genuinely needs one (`commit_files`), and that history is verified against `git fsck` so the shortcut is never mistaken for a source of undetected corruption.
- **Git *history* is read via `coordinator/tests/_git_history.py`** — a read-only, allowlisted (`log`, `rev-parse`, `show`) wrapper. Reading history is not fixture construction and does not need the plain-files treatment; it needs the allowlist so a read-only call site can never widen into a state-mutating one by accident.
- **Tracked-file enumeration parses `.git/index` directly** via `coordinator/tests/fixtures/hook-message-sweeps/population_scan.py`, rather than shelling `git ls-files` — verified equal to `git ls-files` output at 90/90 before it replaced the subprocess call sites that used it.
