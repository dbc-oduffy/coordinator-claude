...
`session.safe_commit_offer` is a registered engine op (scope `"none"` — identity is not taken from the environment), dialed via `coordinator-invoke session.safe_commit_offer '{"cwd": "<repo>", "session_id": "<caller's id>", "message": "<subject>"}'`, never a bareword. The payload is one positional JSON string; a `k=v` argument list does not parse, and `--repo` is refused outright on a scope-`"none"` op (`-32603`). **Every caller passes both `cwd` and `session_id`.** `cwd` selects which tree is scanned; it does NOT establish identity. The op refuses outright — `caller identity could not be established` — when neither an explicit `params.session_id` nor an identity carried on the wire is present, rather than degrading to the environment of whoever spawned the warm server. Degrading to ambient identity is a defensible fail-safe for a READ; on a commit it is not a degrade but a misattribution of someone else's work, which on a MUTATING no-confirmation op is the worst available failure mode. It computes this session's safe pathspec — the same `MY_SCOPE` computation Component 3 uses (`compute_scope()`, with a dispatched sub-agent's touched-files union added in as an additional candidate set, `"exact"` mode only, never `"broadened"` — see the engine module's own docstring for why: `"broadened"` returns an identical candidate union for every concurrent session, empirically confirmed, which would hand a live peer's files to whichever session's stop-event fires first under an UNATTENDED committer) — and then **commits and pushes it, with no confirmation step of any kind.** It commits via `coordinator_core.git.commit.commit_paths` (the same call `ceremony.commit_v2` makes) and **owns no push at all** — publication is left to whichever cadence checkpoint runs `push_outstanding()` next. It returns what it committed (paths, sha, push state) and what it excluded and why (`owned by session <id>` / `untouched by this session`) in its `rendered` field — echoed AFTER the fact, never as a gate before it. An empty result is a valid "nothing to commit" outcome, not a failure. `/handoff`'s § Safe-Commit Auto-Commit step is the consumer.

**Observed failure: the op can resolve the WRONG session identity even with `cwd`/`session_id` both passed explicitly.**  A live incident showed the op return a peer's session identity and commit five of that peer's files under the caller's message, while leaving the caller's own dirty paths uncommitted. Passing `cwd` explicitly via `--params-file` is the mitigation `/handoff`'s dirty-tree residue names for exactly this failure, on the reasoning that a scope-`"none"` op cannot take identity from the environment; passing it did not prevent the misresolution here. This matters more than an ordinary bug because the op's whole purpose is to stop one session's commit from sweeping a peer's unrelated work — a misresolved identity does not fail safe, it inverts the blast radius, committing exactly the tree the op exists to protect. **Practice:** read the returned `session_id` and confirm it is yours before trusting what was committed; treat `groups[].paths` naming files you don't recognize as the second check, and stop there. If it has already misfired, do not revert the peer's now-committed work on a shared tree — the content is intact, only the attribution is wrong; file the misattribution and commit your own files by hand with an explicit pathspec instead. 
...
### 3c. A deliberately-uncommitted edit parked in a SIBLING repo's tree is not custody — that tree's own auto-commit sweep is a live actor

§ 3b's `session.safe_commit_offer` is this repo's own unattended commit sweep, but the same class of mechanism exists on every repo that has adopted it, and a session working across a repo boundary is exposed to the sibling's sweep, not just its own. Deliberately leaving a live edit uncommitted in a **peer repo's** tree — reasoning that an in-process import already made it "live," so committing can wait — does not account for that tree's own Stop-hook auto-commit sweep. The sweep fires on session end regardless of which repo dispatched the work, picks up the uncommitted edit as ordinary dirty state, and commits it to that repo's HEAD under a generic rescue message with no incident context: an unreviewed change reaches HEAD carried by a session that had no idea what it was committing.

**Rule.** An edit is not "safe uncommitted" merely because something in-process already reads it live. If the edit lives in a repo other than the one your session is scoped to, that repo's own end-of-session mechanics — auto-commit sweeps, safety nets, whatever it runs — are a live actor on it the moment you stop touching it, and they run with none of your incident context. Commit or explicitly hand off anything you leave in a sibling's tree; do not park it on the theory that its liveness elsewhere is a form of custody.
...
### 6b. Scoping a Commit Never Requires Mutating the Working Tree

**When a file you must edit also carries a peer's uncommitted changes, the mechanism is a pathspec, never a revert.** In order of preference: (1) commit by explicit pathspec and leave the peer's hunks dirty — the index and worktree disagreeing on OTHER files does not affect your scoped commit; (2) if you genuinely cannot proceed with their changes present, stash only their path with provenance noted in the message, never a bare revert or checkout, so the content stays recoverable; (3) stop and report, and let the EM decide. Never check out, restore, or hand-revert a hunk you did not author.

**The revert-vs-stash asymmetry is the whole rule: a revert destroys content that exists nowhere else; a stash only relocates it.** A dispatched executor told "commit only paths you authored" on a shared tree achieved that by REVERTING a peer session's two uncommitted hunks in a file it also needed to edit, and never restored them — the peer's in-flight refactor was destroyed outright, with the executor's own commit looking clean and tests passing.

**Corollary for the dispatching EM.** "Commit only paths you authored" is an incomplete instruction on a shared tree — it states the goal and leaves the mechanism to the agent, and the agent will reach for whatever makes the tree look clean, including a revert. Name the mechanism (pathspec) and the prohibition (never revert, restore, or check out a peer's hunks) together, in the same breath, in every dispatch brief touching a repo with concurrent sessions. When reviewing an agent's run-report, treat any mention of reverting/restoring/cleaning the tree as a finding to check, not a housekeeping detail.
...
**A directory pathspec is not a scoped commit — `git add <directory>` stages peer files too**

`git commit -- <paths>` correctly scopes what gets *committed*, but that scoping happens after
staging, not before it. `git add <directory>` (e.g. `git add cross-repo/inbox`, `git add
state/handoffs`) stages **everything dirty under that directory**, regardless of who touched it. On
a shared tree with concurrent sessions, "everything dirty under `state/`" is never just yours. Two
observed instances in one session: `git add cross-repo/inbox cross-repo/archive` swept 18 in-flight
archival moves belonging to other sessions — the delete halves landed in the commit while the
archive-copy halves stayed untracked, briefly leaving memo content live only in the working tree
with no committed counterpart; `git add state/handoffs` swept a peer's mid-flight frontmatter write
plus a second archival deletion with the same untracked-archive-half gap. Both were repaired
in-session by committing the missing add halves — the near-miss is the lesson, not the damage.

**Rule: stage explicit files, never a directory, on a shared tree** — `git add a.md b.md`, never
`git add state/`. Prefer letting `git commit -- <paths>` do the staging in one step so there is no
separate `git add` call to over-scope. When a sweep does happen, repair the **whole** move, not the
half you noticed: an archival move is two halves (delete + archive-copy), and committing the delete
without the add strands the content as untracked — check that every deleted path has a *tracked*
counterpart, not merely an on-disk one.

**Verify commit scope by SHA, not by `HEAD`.** Checking `HEAD` for scope violations after the fact
is itself unreliable on an active tree — `HEAD` has moved by the time you look, because peer
sessions commit on top of yours within seconds, producing false positives. `git show --name-only
<my-sha>` against the SHA your own commit actually produced is the only honest check.
...
**Recovering a stalled executor on a shared branch: hunt the parity gap, not the overlap.** Start with `git log merge-base..HEAD` plus `git show --stat <unfamiliar-sha>` to learn what a sibling actually committed while the executor was stalled, then diff what SHOULD exist against what's committed — both halves of every parity pair (e.g. a `.sh`/`.ps1` counterpart, a schema plus its migration). The gap is the task, not the overlap the two sessions share; the overlap is already safe. Revert any out-of-scope creep a stalled executor added while it was stuck, rather than carrying it forward as if it were intended work. (Source: `state/lessons.md:133`.)
...
**Scoped-pathspec discipline protects your commit, not your work — it is one-directional.** It constrains what YOU take, never what a peer's blanket commit takes from you. A session that staged and committed by explicit pathspec throughout, with no lapse, was still swept twice in twenty minutes by two different concurrent sessions' blanket commits (`git add -A`), once mid-edit and once between staging and committing — its own plan edits landing inside commits titled for unrelated work. Being disciplined yourself is not a defence against an undisciplined peer; the only mitigation is the timing one above — commit soon after editing rather than batching, and re-verify with `git status` before composing a message, since "no changes added to commit" can mean a peer already took them.
...
command's, so it fails open). Engine-side, in the engine repo's
...
(3) A recurrence of (2) four days later, same repo, this rule already documented: partial-stage via `git update-index --cacheinfo` (hold HEAD-plus-one-row), then `git commit -- <paths>` — the trailing pathspec re-read the worktree and discarded the hand-staged content anyway, because the guard's divergence warning was read as advisory. Two peer tripwire rows (`THE-LEDGER-ROW-IS-THE-ONLY-LOE-INPUT`, `SIZING-APPETITE-ANCHOR`) were swept into  alongside the intended one. The takeaway beyond the ruling itself: a `--` pathspec and a hand-staged index are mutually exclusive by construction, not merely a caution — if you partial-stage on a shared tree at all, build the commit in a private `GIT_INDEX_FILE` and never pass a trailing pathspec, full stop, rather than trusting your own care in staging.
...
### `freeze-review-diff` filters by path, not by author — a review-dispatch brief inherits every peer commit on those paths

`freeze-review-diff` (and any diff-range tool that scopes by pathspec rather than by author or session) answers "what changed in these paths," not "what did my session change." On a shared branch, a dispatch brief that describes the diff as "its own commits" hands the reviewer every peer commit that touched the same paths too — the filter has no notion of whose commit it is. A brief that undersells or omits a peer's commit from that range lets a reviewer wave through unreviewed peer work as though it were covered by the review it's actually performing.

**Rule.** When dispatching a review off a path-filtered diff range on a shared branch, enumerate the actual commit SHAs in scope (`git log --oneline -- <paths>` over the range) and name them explicitly in the brief, rather than describing the range only as "my commits on these paths." A reviewer that trusts the brief's framing over the range's actual contents will certify commits nobody reviewed.
...
**A scoped `git add` does not make the commit scoped — the pathspec belongs to the commit, not only the add, and this binds the dispatching EM hardest.** Scoped-commit doctrine reads as a rule about `git add`, which trains attention onto the add step; a correctly-scoped add followed by a bare `git commit` still commits the whole shared index, because a bare commit reads the INDEX, not what you just added to it. On a solo tree the two are indistinguishable, so the habit survives untested until a busy tree punishes it. Verified incidents: an EM-side bare commit intended for 2 files absorbed a concurrent executor's fully-staged 1462-insertion cluster under an unrelated subject — the EM had authored the explicit-pathspec rule into its own executor briefs and violated it EM-side, which is why this binds the EM hardest: the EM commits while N executors hold staged work. Separately, `git add <2 files> && git commit` (no trailing pathspec) intended 2 files and captured 12 — ten review-trail waivers from another session's chain close plus a peer's in-flight plan edit  — with no clean undo available, since reverting hunks you did not author is itself forbidden (§ Scoping a Commit Never Requires Mutating the Working Tree above). Put the pathspec on BOTH halves: `git add -- <paths> && git commit -m "<subject>" -- <paths>`.

**Corroborating incidents, same class, with recovery and guard-improvement findings.** Three further instances of the identical scoped-add-then-bare-commit sweep, each adding something the earlier ones didn't:

- A scoped `git add` followed by a bare `git commit` committed 63 files (4 own + 59 a peer had staged seconds earlier). Recovery: `git reset --soft` plus re-staging, verified byte-identical against the peer's original hunks — but the peer had already pulled the bad commit, so it survives in history under the wrong message regardless of the local repair.
- The same shape swept 60 pre-staged peer paths, including a backwards archive-to-inbox rename that un-archived 49 memos in HEAD. The Bash-PreToolUse scope guard (§ Component 4) fired correctly on the call and was read as noise: it warns identically on the safe pathspec-commit form and the unsafe bare-commit form, because it describes the INDEX state, not the COMMAND shape — it cannot tell "committing worktree bytes for named paths" apart from "committing whatever the shared index holds." The distinguishing signal (a trailing `--` pathspec in the Bash command's own argv) is available to the guard at hook time and is not read. **This is a concrete, fixable guard improvement, not just a doctrine restatement:** Check 5 should classify on argv shape before warning, so a correctly-scoped commit stops tripping the same advisory as the sweep it is meant to catch.
- The sharpest statement of the class: "a scoped commit" is used ambiguously in conversation for both `git add --pathspec` (does NOT protect the following commit) and `git commit -- pathspec` (DOES protect it), and the ambiguity resolves wrong at exactly the point of use. **A pathspec on the commit is a per-commit shield, never a repair** — it protects the one commit it's attached to; the phantom (the foreign staged content) stays armed in the index until that path re-enters some commit, so "I passed a pathspec once" does not mean "I'm safe from here on." In this incident a concurrent peer's unrelated commit swept an executor's uncommitted edit, caught only by `git-commit-agent`'s own subject-vs-payload mismatch check — no scope guard fired. Nine cross-repo memos were separately swept the same day by six different sessions, the same ambiguity recurring independently each time.
- Cross-repo recurrence, same shape twice on the engine repo/doctrine repo pair intended a 2-file commit and landed 3 — a live peer's staged `coordinator_core/session/scope.py` (83+/17-) rode along; the pre-checks verified zero deletions on the two intended paths only and never inspected what else was already staged in the shared index. It recurred at a later `workstream-complete` close: `git add -- <5 named paths>` then a bare `git commit -F -` took 14 files, sweeping 7 chain-ancestry-waiver records a peer chain had staged moments earlier. Left committed rather than reverted (additive provenance only, no peer content modified — reverting would have churned a shared branch for no material gain). Notable near-miss shape: the `wsc-tail --stage-paths` guard had already refused a directory pathspec from the *same session* minutes earlier for exactly this reason — the mechanised guard covered the ceremony's own staging surface, and the EM then reproduced the failure one layer out, in its own hand-run git.
...
### Staging your own hunk to protect a peer's exposes YOU to that peer's next commit

**Rule.** Hunk-staging your own change into the shared index to keep a live peer's uncommitted lines out of your commit (§ Committing the Union above) protects the peer and exposes you: the index is shared, so the next commit on that path by ANY session — a peer's own pathspec commit, a blanket safety commit, a stop-hook auto-commit — takes your staged hunk with it. You are not holding your change in a private area; you have handed it to whoever commits next. It happened twice in one session, each time deliberately hunk-split for a peer's benefit: a doctrine entry staged to keep a peer's in-flight work out of the commit landed inside that peer's own unrelated commit whose message never mentions it; a test file staged pending review landed inside a broad human "safety" commit spanning dozens of files. Content survives both times; attribution and the explaining message do not, and the sweeping commit typically carries no trailer for the real author, so downstream attribution/coverage/provenance surfaces misreport the cause.

**Discipline.** Commit a staged, filtered hunk in the same breath as staging it — the window between `git apply --cached` and your commit is the whole exposure, and on a busy tree it is measured in seconds. Where a wait is genuinely unavoidable, keep the change in the working tree and out of the index instead, and re-derive the hunk split at commit time; an uncommitted worktree hunk is still sweepable by a pathspec commit, but it is not handed to every bare `git commit`. Before re-attempting a deferred commit, check `git log -S'<distinctive string>' -- <path>` — an empty hit on a path you know you edited means it already landed under someone else's message, not that your edit vanished. When it does happen, do not rewrite history to reclaim attribution; record what landed and where.
...
**Path scoping is not authorship scoping, and knowing the rule does not make you immune to it.** `git commit -- <path>` answers "which files", never "whose hunks" — git has no concept of which agent wrote which line of a dirty file, so on a file only one agent is touching the two questions have the same answer, and diverge silently, with no error and no conflict, only once a second agent starts touching the same file. In one incident an EM ran ten parallel agents against a shared branch, had explicitly named this exact hazard earlier in the same session and instructed every agent to re-read files before editing to avoid it — and still committed `dispatch_checks.py -- <path>` for one agent's fix while a second agent's uncommitted hunks were live in the same file, sweeping them under a message that described only the first agent's change. The safeguard applied (path scoping) did not address the failure it was worried about (authorship); diff the file before committing whenever more than one concurrent agent may have touched it, per the discipline above.
...
**The hazard applies even with no staging step on your side at all.** A tight, well-scoped pathspec commit — `git commit -F msg -- <paths>` over an explicit per-file worklist, the recommended scoped-commit shape, with no `git add` and nothing deliberately staged — still silently absorbed a concurrent session's uncommitted worktree edits to the same paths, because a trailing pathspec always commits *worktree* bytes for the named paths regardless of whether anything was staged. No blanket-add guard fires on an explicit pathspec, and no staging step exists for a divergence warning to catch. Scoping the pathspec tightly is not protection by itself — it is what selects *which* peer's in-flight worktree edits get taken when a path is shared. Before a pathspec commit on a shared branch, diff each path and confirm every hunk is yours; where a file mixes your work with a peer's, commit the clean files and leave the mixed ones rather than reverting hunks you did not write. 
**Read this before following the bullet above.** Committing from the shared index with no pathspec, even after proving the staged set is yours via `git diff --cached --name-only`, does not close the hole: the index is shared across concurrent sessions, so a peer can stage between your check and your commit — a real TOCTOU window, not a theoretical one. This is not a hypothetical edge: an EM following this page's own then-current text produced two same-day incidents by exactly this route — the verify-then-commit fallback was followed to the letter and still absorbed a peer's stage in the gap. SC-DR-015's fallback, as first written, was itself unsafe; the private-index recipe below exists because the fallback it replaced could not be made safe by tightening the verification step further.
...
**This recipe uses `commit-tree`, not a plain `git commit`, specifically because a plain `git commit` against a private `GIT_INDEX_FILE` does NOT avoid the shared tree's own pre-commit hook.** A private index isolates staging from the shared *index*, but a plain `git commit` still runs whatever pre-commit hook is wired into the repo, and that hook can restage worktree state — sweeping a live peer's in-flight edit into the commit — in the window between your last verification (`git diff --cached --stat`, against the private index) and the actual write. A verified-correct `git diff --cached --stat` against a temp index is therefore a snapshot, not a guarantee, whenever the commit that follows is a plain `git commit` rather than `commit-tree`: anything the hook does between the check and the write can still widen the commit. This is exactly why the recipe above routes through `commit-tree` (§ "`git commit-tree` fires no hooks at all" two bullets up) rather than a plain `git commit` against the private index — and it is also why the bash guard's own advisory names `ceremony.commit_v2` for unusual staging (partial-hunk, `GIT_INDEX_FILE`): reaching for hand-rolled git plumbing that still ends in a plain `git commit` reopens the hazard the sanctioned recipe exists to close.

**This wanted a tool, not a third prose rule — and now has one.** Requiring an operator to hand-assemble a private index mid-commit was the same failure shape SC-DR-015 exists to name: a rule discharged by remembering. The discharge is now `ceremony.commit_v2` (`coordinator_core/git/commit.py` :: `commit_paths`), and it dissolves the horn rather than picking between its sides: it builds the commit's tree from the explicit `paths` it is given (deletions in `deleted_paths`) instead of reading the index at all, so there is no agree-vs-diverge case to compute and no private-index sequence to assemble. Naming a path in `prefer_staged` (or the blanket `prefer_deliberate_stage`) commits that path's STAGED blob instead of its worktree bytes — the parameter this page's earlier revisions omitted. **Prefer the op over hand-rolling the recipe.** Where the op isn't reachable, the recipe above is still correct — it's what the op implements — but "not partial-staging on a shared tree" is not the fallback advice; call the op.

### A pathspec commit re-reads the worktree and silently undoes a staged `git rm --cached`

**Rule.** `git commit -- <path>` commits the WORKTREE state of that path, bypassing the index — which is exactly wrong when the point of your staging was that the index and worktree deliberately disagree. `git rm --cached <file>` untracks a file while leaving it on disk; stage that plus a `.gitignore` entry and commit with a trailing pathspec (`git commit -F <msg> -- .gitignore <file>`), and git re-reads the still-present worktree file, compares it to HEAD, and calls it "modified" rather than "deleted" — recommitting a fresh copy of the exact file the intent was to untrack. There is no error, no non-zero exit, and no test that catches it: `git show --stat` prints a plausible changed-lines count exactly where a deletion belonged, which is actively reassuring rather than merely silent.

**When the index and worktree deliberately disagree for a path — `git rm --cached`, a partial `add -p`, an intent-to-add — do not use a pathspec commit.** Verify the index is empty of foreign staged work (`git diff --cached --name-only`), stage exactly what you want, confirm with `git diff --cached --name-status` (must show `D  <file>`, not `M`), then commit with NO pathspec so git uses the index you built. Verify the untrack afterward with `git ls-files --error-unmatch <file>` — it must exit non-zero; `git show --stat` alone is not sufficient evidence, because it is exactly what let this pass unnoticed. (Caught before it propagated to ten more repos already staged with the same broken recipe.)
...
**The second question this leaves open — hard-refusing a commit outright when index and worktree disagree — is not answered here.** SC-DR-015 already rules the form (`ceremony.commit_v2` dissolves the horn) and the live `OFFER-PATHSPEC-DIVERGENCE` guard is advisory. Whether that advisory becomes a deny is ruling 5, open at authoring — this page names it as owed and does not answer it.
...
The detection was correct, the staging was correct, and the commit form threw the work away. That is the signature of a rule whose discharge depends on the operator remembering an unwritten precondition — and the EM in this incident already knew the doctrine and had followed the first two steps of it correctly. Prose recited correctly through the staging step still failed at the commit step; that is why `OFFER-PATHSPEC-DIVERGENCE` (§ The trailing pathspec reads the WORKTREE above) is the load-bearing artifact here, not the written rule. A rule an EM can state and still violate is a candidate for a guard, not for a sharper restatement.
...
### A third trigger — a malformed pathspec is NOT always atomic, and never hold staged state across a decision

**The two triggers above (a re-added `git mv` source, a re-added `git rm`'d path) are atomic-fatal-stages-nothing. A generically bad pathspec is not guaranteed to be.** `git add a b c` where `c` is simply a bad/non-matching pathspec has been observed to exit fatal while `a` and `b` are already staged — not "everything except `c`" reverted, but a partial stage left behind. On a shared index, that partial stage then wins over the worktree at the next commit: an archived queue item shipped reading `status: open` because a content flip happened after that partial `add` had already locked in the stale value. The tempting recovery compounds it — `git commit -- <paths>` errors on untracked files ("did not match any file(s)"), so the instinct is to `git add` again, which re-enters the same partial-stage trap.

**Rule.** Never hold staged state across a decision on a shared tree. Make the worktree say exactly what you mean first, then stage and commit that in one call — do not stage some paths, pause to think or re-check, and stage the rest later. Where a batched `git add` genuinely can fail partway, verify with `git diff --cached --name-status` immediately after it, before composing the commit, rather than trusting the fatal/atomic assumption from the two triggers above.
...
**The hazard is standing, not a one-off slip.** Markdown-style backtick quoting of CLI names, file paths and flags is idiomatic in coordinator commit bodies, so this fires on ordinary, well-formed messages, not just careless ones. It also compounds with this fleet's auto-pushing shared branches: the commit exits 0 and pushes before anyone reads the mangled body, and by the time the hole is noticed, amending would rewrite history other sessions may already have pulled — leave the corrupted message in place rather than force-rewriting shared history over it. The same shell-expansion hazard applies to any other double-quoted shell string carrying markdown: memo bodies, `--summary` flags, agent prompts written via Bash.

### `-m` must precede the `--` pathspec separator

**Rule.** Everything after `--` on a `git commit` line is a pathspec — `git commit -- <paths> -m "<subject>"` parses `-m` and the message text themselves as pathspecs and fails with `pathspec did not match any file(s)` for both. The correct order is `git commit -m "<subject>" -- <paths>` (same rule for `git add`). This fails loudly, but on a batched loop of scoped commits it fails EVERY commit in the batch at once — a nine-commit close-out sequence can land zero commits, with the error output repeating the full message body nine times. (Source: doctrine plane.)
...
A scoped commit correctly refused one file because a live peer held a claim on it.
...
Read from the engine as written, not as assumed, against the commit route that preceded
`ceremony.commit_v2` and against `ops/session/safe_commit_offer.py`. The current route for both is
`coordinator_core/git/commit.py` :: `commit_paths` — re-verify a property there before leaning on it:
...
## SC-DR-024 — Audit every SANCTIONED bypass caller, not just the guard: warn-on-stderr before an irreversible sweep is theatre

A sanctioned blanket sweep set the internal bypass marker to skip the very block-blanket hook built to stop unauthorized whole-tree staging, then guarded itself only with a non-blocking stderr warning when its blanket stage overlapped a live sibling's `touched.txt` — so the authorized path silently absorbed a concurrent sibling's in-flight files, exactly the class of hazard the hook exists to prevent.

**Rule.** When you build a guard against an unsafe operation, audit every SANCTIONED caller that bypasses the guard — the carve-out is where the hazard re-enters, and the guard's own coverage says nothing about it. A warning printed to stderr right before an irreversible sweep is theatre (see also SC-DR-012): the foreign files commit regardless of whether anyone reads the warning. Turn the foreign-overlap computation into an actual **exclusion** (subtract live-sibling-claimed paths from the staged set before staging), not a log line emitted after the decision to sweep is already made.
...
### Generate a committed artifact against HEAD, never the working tree, on a shared branch

**Rule.** On a shared branch the working tree is HEAD plus every live peer's uncommitted work. Any artifact generated from the tree inherits that state, and committing it asserts it. Generate the artifact you intend to commit from HEAD content (`git show HEAD:<path>` into a temp input), not from the working tree, so the commit is self-consistent. Regenerate against the working tree afterward and leave *that* delta uncommitted alongside the peer's — both states stay internally true, and when the peer's work lands, the regeneration drift resolves itself as one command, which is the designed signal, not rot.

A generator emitting a manifest block into a shared registry file while a concurrent peer held an uncommitted registration in the same file listed 40 entries where only 39 existed at HEAD; committing the tree-generated block would have shipped a manifest asserting a registration the very commit making the claim did not contain. The failure passes every self-consistency check the artifact and its source agree — and is invisible for exactly that reason. Applies to any emitter, codegen, lockfile, or snapshot test whose input is a file a peer may be editing.

### A regenerator is a sweep with a friendly name, and it offers no pathspec to scope it
