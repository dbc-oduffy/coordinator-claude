...
<!-- Spec backlink: archive/specs/2026-05/2026-05-19-cross-plugin-whoami-contract.md §5 Task 1 -->
...
- **MCP-plugin adopters** — plugins that expose a live MCP tool (e.g. `project_whoami` for project-rag, `example_game_repo_whoami` for example-game-repo-control). Each plugin implements a conformant MCP tool in its own repo and test suite.
...
The contract answers the question every coordinator-level tool or scanner has when it queries a plugin: *"What is this plugin bound to, and is it healthy?"* Previously that question was answered only by project-rag's `project_whoami` tool, whose response shape was project-rag-internal doctrine. As example-game-repo-control joined the ecosystem as a second MCP-bearing plugin, the need for a shared cross-plugin contract became concrete — the wrong-shape arrangement (example-game-repo-control piggybacking on project-rag's tool for cross-plugin introspection) was itself the evidence the abstraction was overdue. PM authorized hoisting the contract to coordinator-claude on 2026-05-19 (DoE memo `~/.claude/cross-repo/archive/2026-05-19-machine-local-doe-reply.md` § 5b — grandfathered pre-cutoff memo).

Each plugin exposes its own named MCP tool (e.g. `project_whoami` for project-rag, `example_game_repo_whoami` for example-game-repo-control). The coordinator validates the envelope shape; plugin-specific extension fields live in the per-plugin `extras` slot, validated by the plugin itself.
...
| `plugin_name` | `str` | Canonical adopter identifier. Use kebab-case: `"project-rag"`, `"example-game-repo-control"`, `"coordinator-session"`. Free string — the schema does not enumerate valid values; adopter identity is established by convention. Must be stable across daemon restarts (or, for non-MCP adopters, across package upgrades). |
...
| `target` | Plugin-specific identifier for what the plugin is bound to. For project-rag: the resolved project root path. For example-game-repo-control: the `.ueproject` file path. `null` when `kind` is `"unbound"` — see **Error semantics** below. |
...
`extras` is a dict keyed by adopter identifier (snake_case: `"project_rag"`, `"example_game_repo_control"`, `"coordinator_session"`). Each adopter writes into its own key. Coordinator validates:
...
- `example-game-repo-control --offline-whoami` — assembles binding/status from on-disk install state.
...
## Consumption pattern — string normalization at the comparison boundary

**Consumer-side `plugin_name` comparisons require `str.strip().upper()` normalization at the comparison boundary, not raw string equality.**

When a coordinator-level scanner or doctor branches on `plugin_name` (routing to plugin-specific logic, matching an expected adopter, deduplicating a result set), the comparison MUST normalize both sides: `received.strip().upper() == "PROJECT-RAG"`. Free-form string fields in a protocol envelope are susceptible to incidental whitespace and capitalization drift as producers evolve. Mocked-string tests that compare against a known literal pass even when this normalization is absent — the real-host smoke test (calling a live `*_whoami` MCP tool and branching on its result) is the calibration step that catches the raw-compare failure. *Source: 2026-06-14 central-improvement-queue #13.*
...
- **Missing install:** run `/coordinator:install` (installs the `coordinator_whoami` package via pip in Phase 3 Step 6).
...
Each plugin's own extension chain writes into its dedicated key inside `extras`. Project-rag's pluggy hookspec chain (`project_rag_register_whoami_extras`, planned for v9 work) writes into `extras["project_rag"]`. example-game-repo-control's eventual extension surface (none today) would write into `extras["example_game_repo_control"]`.
...
1. **Import fails:** `whoami: not installed (run /coordinator:install to install the introspection package)`
...
Plugin whoamis (project-rag, example-game-repo-control, etc.) may appear as optional sub-lines after the session line, but they are not the spine. A session that emits `whoami: bound → (healthy)` is oriented regardless of whether any MCP plugin is currently reachable.
...
## Worked example — example-game-repo-control's planned conformant response (TEMPLATE)

The following illustrates what a conformant `example_game_repo_whoami` response looks like when example-game-repo-control implements its own coordinator-conformant whoami tool. No extension fields are defined for example-game-repo-control today; `extras["example_game_repo_control"]` is a placeholder.

```json
{
  "contract_version": 1,
  "plugin_name": "example-game-repo-control",
  "plugin_version": null,
  "binding": {
    "kind": "bound",
  ...
  "extras": {
    "example_game_repo_control": {}
  }
}
```

When example-game-repo-control cannot locate a `.ueproject` file:
...
1. **DoE memo** `~/.claude/cross-repo/archive/2026-05-19-machine-local-doe-reply.md` § 5b (grandfathered pre-cutoff memo) — PM authorized hoisting the whoami contract from project-rag-internal doctrine to coordinator-claude ownership, citing example-game-repo-control as a second MCP-bearing plugin that needed a shared introspection surface.
...
**Authorship.** The contract shape was produced by the 2026-05-19 authoring session (coordinator EM + PM Dónal O'Duffy). The Director of Engineering (cross-team/cross-repo reviewer, Opus-altitude) ratified the spec-first delivery shape, the namespace disentanglement, and the extras-key regex in the same session.
...
- MCP-plugin adopters (project-rag, example-game-repo-control, any future MCP plugin) each implement a conformant MCP tool in their own repo. The plugin's test suite validates its tool's output against `coordinator_whoami/schemas/whoami-envelope.v1.json`. Each plugin owns whatever it puts inside `extras[<its_plugin_id>]`.
...
<!-- Spec backlink: archive/specs/2026-05/2026-05-19-whoami-substrate-migration.md § Task 9 (R1 subpackage layout) -->
...
example-game-repo-control's in-flight `agentic-doctor` plan (`X:/example-game-workbench-repo/docs/plans/2026-05-19-example-game-repo-control-agentic-doctor.md`) is explicitly held pending this migration — the second adopter is named and blocked on R1 landing. The subpackage layout was established in this phase (R1, PM-authorized 2026-05-19) rather than deferred to a refactor. When example-game-repo-control adopts the contract, its implementation lives at `coordinator_whoami.example_game_repo_control/` (sibling subpackage to `coordinator_whoami.project_rag/`) and reuses the generic surfaces (`contract`, `envelope_base`) without any changes to the package root.
...
When example-game-repo-control or another MCP plugin adopts the contract, the new subpackage lives at `coordinator_whoami.<plugin_name>/` — a sibling of `coordinator_whoami.project_rag/`. The subpackage layout is already in place; no package-root refactor is needed. The new plugin's subpackage authors its own `cli.py`, `envelope.py`, and `addons.py`, reusing `coordinator_whoami.contract` and `coordinator_whoami.envelope_base` from the parent package.
