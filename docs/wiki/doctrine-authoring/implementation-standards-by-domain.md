...
Domain-specific implementation standards — observability, database/indexer correctness,
dependency management, engine plugin packaging — live here rather than in a shared always-on
file.
...
`coordinator/docs/wiki/claude-md-surfaces/claude-md-delivery-topology.md` for the fuller account. It loads only in a session whose cwd is
...
See `docs/wiki/lesson-triage/document-bloat-trim.md` for the general extraction rule.
