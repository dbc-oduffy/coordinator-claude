...
A workaround built on a flag's incidental side effect, rather than its documented contract,
breaks silently the moment the shared code path it depended on is rewired.
...
- → [`substrate-pin-doctrine.md`](../test-design-discipline/substrate-pin-doctrine.md) — sibling pattern: name what you actually depend on
