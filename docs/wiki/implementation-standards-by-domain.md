...
- **An "atomic swap" built from delete-then-rename has a non-atomic boundary — guard the rename and fail loud, naming the temp.** A collection/table swap that builds into `<canon>__building_<tok>`, then `delete(canon)` + `rename(building → canon)` on success, *reads* as atomic but is two calls. If the delete succeeds and the rename then raises, the canonical is gone AND the freshly-built data sits under a `__building_*` name that the next build's orphan sweep deletes unconditionally — permanent data loss. Wrap the rename so a failure fails loud naming the surviving temp (an operator can recover it), and make the orphan sweep spare a `__building_*` sibling that has no live canonical peer. (Source: project-rag, 2026-06-26.)
...
*2026-06-19, example-game-workbench-repo.* [universal]
...
An embed-sidecar VRAM gate compared NVML *system-wide* free VRAM against a floor and 503'd spuriously: NVML counts PyTorch's caching-allocator reserved-but-unused pool as "used" (an `empty_cache()` would hand it back to the driver), so the gate treated the process's OWN reclaimable cache as unavailable. Fix: `effective_free = nvml_free + own_reclaimable` (reserved − allocated) — cross-process usage still counts against you, but your own reclaimable pool does not. Generalizes to any admission gate reading a shared/system-wide sensor (RSS vs own cache, disk-free vs own tempfiles) to decide a per-process action. (Source: project-rag, 2026-06-24.)
...
*Source: example-game-repo `state/lessons.md` (example-game-repo-L11, central-promoted 2026-05-28).*
...
*Source: example-game-repo `state/lessons.md` (example-game-repo-L129, central-promoted 2026-05-28).*
...
*Source: example-game-repo `state/lessons.md` (example-game-repo-L151, central-promoted 2026-05-28).*
...
*Source: example-game-repo `state/lessons.md` (example-game-repo-L211, central-promoted 2026-05-28).*
...
*Source: example-game-repo `state/lessons.md` (example-game-repo-L126), central-pulled 2026-06-20.*
...
*2026-06-18, example-game-workbench-repo.* [universal]
...
*2026-06-27, coordinator.* [universal]

A tool whose runtime correctness depends on a gitignored generated artifact (`dist/`, `build/`, compiled bindings, generated schema files, etc.) must check **freshness** (source newer than built output → stale), not just existence — and fail loud with a build remediation, plus a build guard in the test/pre-run path.

**Why:** A fresh checkout and an existing working tree diverge in opposite ways. After `git clone`, `pnpm install` (or a `prepare` script) produces a current artifact — existence check passes, freshness holds. After editing `src/` without rebuilding, existence check still passes but the artifact is stale — validation fails with opaque errors ("unknown entity", "type mismatch") rather than "rebuild needed." Existence is necessary but not sufficient.

**Pattern (reference implementation: `bin/emit-cockpit-snapshot.sh` lines ~53–61):**
```bash
# existence gate
[[ -f "$DIST_ENTRY" ]] || { echo "ERROR: dist/ not found — run pnpm install" >&2; exit 1; }
# freshness gate
if [[ -n "$(find "$SRC_DIR" -name '*.ts' -newer "$DIST_ENTRY" 2>/dev/null | head -1)" ]]; then
  echo "ERROR: dist/ is STALE — src/ changed since last build; run pnpm run build" >&2; exit 1
fi
```
And `pretest: pnpm run build` in the package `scripts` block so `pnpm test` never runs against a stale artifact.

**How to apply:** (1) After any `[ -f dist/... ]` existence check, add a `find <src> -newer <dist-entry>` staleness check; fail loud with the build command. (2) Add `pretest: <build-cmd>` to the package test entry. (3) Document the gitignored artifact in the tool's setup section.

**Candidates in `bin/` with existence-but-not-freshness gap (EM to triage):**
- `bin/lib/validate-cockpit-record.mjs` — `existsSync(CONTRACT_DIST)` checks presence of `cockpit-contract/dist/index.js` but has no `find src -newer dist` staleness gate (unlike `emit-cockpit-snapshot.sh`). If `src/` changes without a rebuild, the validator silently uses the stale contract.
