...
updated: 2026-07-06
status: active
spec_backlink: docs/decisions/DR-203-cockpit-contract-standing-owner.md
---
...
> reader-first hold below is *unconditional for major bumps*; for minor additive bumps it is a
> softer precaution (coordinate the re-vendor), not an outage-prevention necessity.
> *(2026-07-06: corrected — the "hard-throws on any newer schema_version" claim applied under
> the original cockpit `assertSchemaVersion` gate, which WAS strict. cockpit deliberately
> softened to major-compatible in commit `50a6924` ("relax ingest schema-version gate to
> major-compatible"). The crash-safety rationale for minor bumps was real then and is retired
> now by consumer choice — reversible if a future consumer re-strictens (hence the
> consumer-capability census in Step g). This is a deliberate retirement, not a documentation
> phantom.)*
>
> **Updated 2026-07-03:** added field-addition pattern (deliverable-spine fields on
> existing entities), provenance `ref` ref-conditional convention (Source A / D1), and
> v2.4.0 example row. Spec backlink:
> `docs/plans/2026-07-03-fleet-deliverable-spine-identity-and-facets.md`.
>
> **Producer-identity note (2026-07-08, DR-208/DR-210):** example-orchestration-hub's Python `artifact.emit`
> is now the **sole production cockpit emitter**. `coordinator/bin/emit-cockpit-snapshot.sh`
> is a fail-loud facade stub — its original body was ported to example-orchestration-hub and is no longer
...
> `cross-repo/archive/` DR-208/DR-210 ratification). Out of scope for this repoint pass
...
`docs/decisions/DR-203-cockpit-contract-standing-owner.md`). The surface comprises:

- example-orchestration-hub's Python `artifact.emit` — the records-spine → `cockpit-emission.json`
  projection step (sole production emitter as of DR-208/DR-210).
  `plugins/coordinator/bin/emit-cockpit-snapshot.sh` remains on disk
  as a fail-loud facade stub (zero caller repoints, DR-210 AC8) but is not an independent
...
| 8.14 | `file-attribution` | on-demand Python derivation over `~/.claude/projects/<project>/*.jsonl` | Derived at query time via `bin/derive-file-attribution.py`; aggregated per (session, file) |
...
- **`is_final` liveness join (denoted `D-ISFINAL`).** Session-aware spine types source
  `cs_live_session_ids` from `lib/coordinator-session.sh` for a second read. A `session_id`
...
*(2026-07-06 — DR-204 / D21: relaxed to reader-class-tiered. cockpit deliberately softened
its ingest gate from strict `assertSchemaVersion` to major-compatible `checkSchemaVersion`
in commit `50a6924`; the crash-safety rationale for minor bumps was real then and is retired
now by consumer choice, reversible. See `DECISIONS.md § D21` and
...
**Current census (2026-07-06):** cockpit (a ✓ b ✓ — `checkSchemaVersion` at
...
2. Send an outbox memo to each consumer (`state/memo-outbox/`) requesting re-vendor of the
   bumped contract. Relay to PM for cross-repo delivery. Include the new version string and
...
record governing quarantine semantics); DR-204 / `DECISIONS.md § D21` (sentinel-handshake
...
3. Run the production emit (example-orchestration-hub `artifact.emit` — default path) — this is the first
...
coordination practice (DR-204 / `DECISIONS.md § D21`). The owner decides the batch vs.
...
| Emitter | example-orchestration-hub Python `artifact.emit` (sole producer; `bin/emit-cockpit-snapshot.sh` is a fail-loud facade stub, DR-208/DR-210) |
...
| 2.2.0 | New entity: decision-guide-summary | C1 entity → C4 envelope+codegen → C2/Cn/C3 bump → C5 emitter-wire → C6 sentinel + memo | commits `38636099`…`73de1a4b` (2026-06-27) |
...
- `docs/decisions/DR-203-cockpit-contract-standing-owner.md` — governance decision
