...
`coordinator:setup` Step 3 copies both `.cmd` files from `templates/bin/` to `~/.claude/bin/`. Step 3b (Windows-only conditional) adds `~/.claude/bin` to the Windows user `PATH` if not already present. Both are idempotent.
