...
| **Multi-source list** | `source=["unreal", "example-sim-repo"]` | Per-source resolution then merge (internal backend only) |
...
| Example corpora (e.g. `example-sim-repo`) | No (`applicable_kinds=None`) | Band name only — no umbrella |
...
A two-level scheme (`engine.unreal`, `engine.unity`, vs `example.example-sim-repo`, vs
