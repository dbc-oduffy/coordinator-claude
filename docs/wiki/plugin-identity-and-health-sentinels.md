...
| **Health scanner**   | `scan-addon-health.py` (and future siblings)    | n/a (reader)          | `/workstream-start`, `/workday-start` | None — no-side-effects | Reduces verdicts to RED/AMBER/GREEN   |
...
<!-- Amended by 2026-05-19-cross-plugin-whoami-contract.md: project_whoami reframed from canonical to specific implementation. Decay-discipline content preserved. -->

## Identity surfacing via MCP — `project_whoami`

`project_whoami` is project-rag's MCP introspection tool. Existing fields (`source`, `project_kind`, etc.) surface project-rag's live binding state.
...
1. **Extend the live introspection tool.** For identity-shaped data with no decay model (daemon PID, sidecar revision): add it to the plugin's own introspection tool response. Single writer (MCP), live response, no new infra.
...
The doctrine emerged from a three-EM consult (example-game-repo EM, project-rag host EM, project-rag-ue-addon EM) on 2026-05-19. Authorship credit, for future readers tracing the framing:
