...
Maps which CLAUDE.md-class surfaces actually reach which sessions, and by what mechanism.
...
`docs/wiki/claude-md-surfaces/invisible-doctrine.md`'s discharge test rejects; and a coordinator-owned generic worker
...
- `docs/wiki/claude-md-surfaces/invisible-doctrine.md` — the discharge test, which is why role-by-hook beats
  role-by-reminder.
