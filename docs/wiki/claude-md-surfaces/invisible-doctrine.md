...
States the organizing idea behind the coordinator rebuild that the computed-skills, skills-carry-no-code, and dispatch-seam tracks all share.
...
| don't retire a form while a reader survives | the advance op re-derives consumers and refuses (`coordinator/docs/wiki/ceremony-calibration/cutover-state-machine.md`) |
...
`coordinator/docs/wiki/claude-md-surfaces/claude-md-authoring-and-boot-context.md § 13`). Relationship and posture
