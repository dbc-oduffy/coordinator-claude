...
Defines the `CONTEXT.md` domain-glossary convention adopted for the coordinator-claude system.
...
This variant is documented here so a future session has a paved path. No skill code exists for it today. Do not implement it until a real multi-context repo demonstrates the need.
