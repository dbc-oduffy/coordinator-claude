...
> invariant; `bin/check-version-consistency.sh` mechanically enforces it.
...
| **marketplace.json** | `.claude-plugin/marketplace.json` `.metadata.version` | Catalog manifest version | **Yes** (PM decision 2026-06-22: tracks the plugin version — one number to reason about) |
...
| **git tag** | latest `v*` | Immutable release marker (OSS publish repo only) | **Advisory** — `~/.claude` is `source_is_live` and never tagged; the OSS repo IS tagged at `/merge-to-main` |
...
4. (At `/merge-to-main`, OSS repo only) tag `vX.Y.Z` + GitHub release.
...
`bin/check-version-consistency.sh` asserts the invariant, fail-loud, in both the
...
- **`workweek-complete.md` Step 10** — after the bump, before `/merge-to-main`.
...
Run it manually any time: `bash bin/check-version-consistency.sh [--check-tag]`.
