...
> How to filter peer-repo prior-art lookups when scanning `$(coordinator_state_root --central)/repo-registry.md` (example-orchestration-hub-resident — see `docs/wiki/state-placement-law.md`) for relevant references.
...
4. Skip outcomes get one-line entries in `$(coordinator_state_root --central)/repo-registry.md` (example-orchestration-hub-resident — see `docs/wiki/state-placement-law.md`) so future passes don't re-investigate.
