...
Doctor skills (`/project-rag-ue-addon:doctor`, `/example-game-repo:doctor`, etc.) correctly diagnose addon health when invoked — but nothing surfaces a RED verdict to the operator between invocations. The empirical failure mode (2026-05-19, project-rag-ue-addon): the engine corpus was unreachable from a consumer MCP session for an unknown duration; the doctor would have flagged it RED, but no operator ran it, so a consumer (example-sim-repo) queried the engine corpus, received a silent fallback to project content, and shipped work against the wrong substrate.
...
**The absent-sentinel nag (pass 2) and the SessionStart hook-script probe (pass 3) stay `$PLUGINS_ROOT`-scoped — never extended to `$CONSUMER_ROOT`.** Both passes are plugin-identity concerns (declared-but-never-run doctor; broken hook install); consumers surface a verdict only, never a nag or a hook probe. Extending either pass to the consumer root would turn a targeted probe into a ~30-dir filesystem crawl and manufacture false-nag exposure for any consumer that happens to ship a `commands/doctor.md`-shaped file without ever having written a sentinel. This scoping is the single most likely thing a future well-meaning refactor breaks trying to "fix the inconsistency" between the three passes — it is not an inconsistency, it is the design.
