...
6. **Write the daily summary** to `archive/daily-summaries/YYYY-MM-DD-<machine>.md` (per-machine naming — mirrors `state/week-changelog/<date>-<machine>.md`):
   ...
   COVERED_MACHINE="$(cs_compute_machine)"
   ```
   ...
   **Machine-defer convention (optional):** When a summary covers a day where work was actually
   ...
   This declares that `state/week-changelog/<date>-striker.md` covers this machine's work for
   ...
   changelog target is absent. This guard was added to prevent the 2026-06-30 failure mode where
   a delphipro summary deferred to striker's own changelog that had not been written.
...
After branch consolidation, multiple machine branches (`work/striker/*`, `work/delphipro/*`) share
