...
1. **Install the single `coordinator` plugin at 3.0.0.** If you're setting up
   ...
   in the repo's `README.md` (Quick Start) and `AGENTS.md` / `INSTALL.md` —
...
repo — those paths do not exist in that shape. The equivalent content
lives under the flat `coordinator` plugin tree. If you cloned the repo
