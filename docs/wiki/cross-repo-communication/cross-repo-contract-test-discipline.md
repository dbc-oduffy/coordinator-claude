...
1. **Pin one byte-equal fixture**, committed byte-identical in both repos — e.g. `tests/fixtures/<helper>_contract.json`, a list of `{inputs, expected_output_bytes}` tuples (base64 the bytes so the assertion is exact, including trailing newlines and quote style).
   - **Pin line endings with an explicit `<path> text eol=lf` `.gitattributes` line — never rely on `* text=auto`.** A file whose whole purpose is byte-stability across repos will silently diverge under `autocrlf` if one repo has `* text=auto` (normalizes to CRLF on Windows checkout) and the sibling has no `text=auto` (stores raw LF). git only *warns* ("LF will be replaced by CRLF") — it never errors — so the drift is invisible. Pin `eol=lf` explicitly on every byte-equal fixture **and** on any text file a shell parses (a stray `\r` breaks shell-read manifests). Confirm with `git ls-files --eol <path>` showing `w/lf`. *(Canonical: copying example-game-repo's shell-parsed `cross-repo-sync.manifest`; the `.manifest` extension fell only under `* text=auto` while example-game-repo (no `text=auto`) stored it LF, so the two copies would diverge on the next `autocrlf` checkout.)*
...
3. **Treat stub schemas authored earlier as obsolete-until-proven-live.** Roadmap stubs authored before a migration describe a world that may not exist anymore; re-ground at pickup. → `coordinator/docs/wiki/dispatching-parallel-agents/pre-dispatch-verification.md` (no-fabrication on cited fields; coordinator/CLAUDE.md retired); `cross-repo-handshake-doctrine.md` (the contract-as-hypothesis handshake).
...
**Provenance:** project-rag-ue-addon — **two independent architecture audits the same day** converged on this class from different entry points (doctor-command-surface audit, DSR-2026-05-27-3; addon-pluggy audit, the "silent-subtraction family"). Convergence-as-confidence: a confirmed structural theme, not a one-off.
...
*Source: install-divergence lift; code-reviewer (F1) surfaced a counting bug verbatim in both repos; flagged via cross-repo-memo rather than patching unilaterally.*

## Freeze a consumed-symbol's return vocabulary with an executable guard, not a docstring

**When another repo imports one of your functions and positive-allowlists its return value, codifying that consumed-symbol contract means an executable guard that *fails* on an unratified return-vocabulary extension — a `PUBLIC CONTRACT` docstring alone does not stop drift.**

A downstream repo that imports `resolve_embed_device()` (or any symbol whose *return value* it branches on) and allowlists the reachable set `{"cuda", "mps", "cpu"}` is now coupled to that set. A docstring block declaring the public contract is good documentation — but nothing about it makes a future return-vocab extension (adding `"xpu"`, `"rocm"`) *fail* a test. The next contributor adds the new return path in good faith, the consumer's positive-allowlist silently rejects it at runtime, and the break surfaces only in the downstream repo.

**Rule:** back a consumed-symbol return contract with a test that exercises **every (intent × hardware-state / input-state) combination** and asserts the reachable return set equals the frozen vocabulary — e.g. `test_return_vocabulary_is_frozen_addon_contract` asserting the full reachable set `== {"cuda", "mps", "cpu"}`. A new return value then fails loud in the owning repo, forcing the cross-repo coordination (memo + consumer allowlist widen) *before* it ships, instead of after it breaks the consumer. The docstring documents the contract; the frozen-set test *enforces* it. (Source: 2026-06 project-rag.) Sibling to § version constants symbolic below — a version int is a coarse proxy; the frozen-set test pins the actual vocabulary.
...
- [`round-trip-contract-tests.md`](../test-design-discipline/round-trip-contract-tests.md) — partner-tool integration section: ship the mixed result; workarounds hide the bugs the test was meant to surface. §"Where the Test Lives" (fabricated-on-each-side fixtures lie) is the producer/consumer analogue of the paired-writer rule above.
- [`test-design-discipline.md`](../test-design-discipline.md) — skip-semantics, marker discipline, CI lane composition.
- [`verification-before-completion.md`](../em-operating-model/verification-before-completion.md) — "green CI" is not "shipped working" when the green came from skips.
