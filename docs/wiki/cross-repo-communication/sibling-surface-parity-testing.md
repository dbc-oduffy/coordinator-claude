...
## Cross-Impl Behavioral Parity — the strangler-transport case

The four dimensions above are *structural* (presence of a key/sentinel/version), and this wiki
otherwise defers runtime divergence to `round-trip-contract-tests.md`. There is one sibling
shape where structural parity is provably insufficient: **a caller in language A armed onto a
transport whose canonical implementation is language B** (e.g. a Python CLI calling through a
shell `cc_invoke`). A hand-port of the fail-closed ladder — the raise/return decision across
success / empty / import-error / op-error / timeout — passes every structural check while
silently diverging in *behavior* as the canonical impl evolves.

For this shape, file a **shared golden-vector parity test**: one table of envelope cases, each
run through BOTH implementations, asserting identical raise/return on every case.

```
# Same envelope fixtures → both impls → assert identical disposition
for case in success empty import-error op-error timeout; do
  assert_equal "$(python_impl  "$case")" "$(shell_impl "$case")"
done
```

The golden-vector test is what makes "mirror the byte-oracle" a *durable* guarantee rather than
a one-time snapshot — without it, the port is correct only against the canonical impl as it
existed on port day. (Source: Python CLI → shell `cc_invoke` fail-closed ladder.)
...
- `docs/wiki/test-design-discipline/round-trip-contract-tests.md` — broader contract-test patterns including
...
- `coordinator/docs/wiki/coordinator-tripwires/` § Snippet-sync — tripwire listing which snippets have sync scripts
