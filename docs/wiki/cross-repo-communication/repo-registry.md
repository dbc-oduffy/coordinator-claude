...
Without inline preconditions, the block becomes an orphan auto-generated region that no maintainer knows how to regenerate. Cross-repo sentinel convention lives in the coordinator plugin's `docs/wiki/cross-repo-communication/cross-repo-citation-conventions.md`.
