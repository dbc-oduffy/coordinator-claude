...
Explains how to filter peer-repo prior-art lookups so a scan surfaces genuinely relevant references.

> How to filter peer-repo prior-art lookups when scanning `$(python3 <the engine repo's coordinator/lib/coordinator-state-root.py> --central)/repo-registry.md` (engine-repo-resident — see `docs/wiki/hook-best-practices/state-placement-law.md`) for relevant references.
...
4. Skip outcomes get one-line entries in `$(python3 <the engine repo's coordinator/lib/coordinator-state-root.py> --central)/repo-registry.md` (engine-repo-resident — see `docs/wiki/hook-best-practices/state-placement-law.md`) so future passes don't re-investigate.
