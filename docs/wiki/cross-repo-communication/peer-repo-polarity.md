...
States which sibling repo owns which capability across the UE-specialization / common-language-core split.

> **Doctrine source:** the peer `example-game-workbench-repo` repo's JetBrains gap-closure roadmap, §0 and §12.1. Local working-tree readers know the path; published readers use the peer repo's release docs.
> **Coordinator-claude role:** routing doctrine + EM dispatch authority per §0 line 141–143. Capability ≠ fit.

## The polarity rule

After multi-rag-coexistence (PR-10/13/15) split engine-RAG production out of example-game-repo and into `project-rag-ue-addon`, the four-repo layout settled as:
...
This differs from the host/consumer polarity that governs `project-rag` (host) → `example-game-repo` / `example-sim-repo` / `example-repo` / `example-stats-repo` (consumers): there, the dependency is **one-directional** (consumers depend on the host; the host never depends on them). The addon↔example-game-repo pair is a separate polarity shape — sibling mutual dep — and surfaces in both repos' READMEs must frame it that way, not as one repo publishing to the other.

A reader reaching for "who depends on whom?" between addon and example-game-repo should find the sibling-mutual-dep framing, not infer host/consumer by analogy with the other consumers. (Source: ue-addon README disambiguation — incoming readers were misreading the addon as a publisher-to-example-game-repo rather than a peer.)
...
> ABSTAIN: This request is UE-semantic (touches UObject / specifier / `WITH_EDITOR` / cooked-vs-editor semantics). Per peer-repo polarity (see `docs/wiki/cross-repo-communication/peer-repo-polarity.md`), route through `<addon-namespaced tool>` rather than the producer-agnostic core surface. Suggested next: `<agent>` with goal `<concrete-goal>`.
...
- `docs/wiki/cross-repo-communication/repo-registry.md` — peer-repo registry for prior-art lookups; complements but does not subsume this polarity rule
- `docs/wiki/cross-repo-communication/peer-port-discipline.md` — the adoption/comparison sibling: what to re-verify when an artifact crosses a repo boundary (this wiki owns the *ownership* axis; that one owns the *adoption* axis)
