...
Conventions for citing file:line locations across the 5-repo install chain so citations stay resolvable.
...
Bare `<path>:<line>` is ambiguous across the install chain — multiple peer repos can have a file at the same relative path (e.g. `bin/setup.sh`, `docs/wiki/bug-blitz-residue/cleanup-sweep-hazards.md`). A successor grepping a bare citation either:
