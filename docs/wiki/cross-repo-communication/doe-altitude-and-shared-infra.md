...
`~/.claude` is a backup and harness-config repo (git checkout, remote `example-org/example-doctrine-mirror-repo-v3`) — never a coordinator working repo. It carries no coordinator working data: no `state/`, no `tasks/`, no project scaffold. That data lives in the DoE clone and nowhere else, and `guard-repo-setup-claude-home-refusal.py` refuses a scaffold run targeting it.

It carries its own `.git`, so any resolver finding a repo root by walking up from `cwd` will resolve `~/.claude` and silently redirect state writes into the backup tree. `_engine_root.py::_session_repo_root` excludes it by resolved real path; a new resolver must do the same.

**Which delivery direction owns an item under `~/.claude/`** — "is there a writer for this file?" is the wrong question, since every tracked item already has a delivery owner:
...
2. **Claude Prime backup** — pushed to the private remote. (Harness config, top-level `agents/`, machine-agnostic config.)
3. **Operator-local-by-design** — gitignored, never shipped. (`machine-local/`, session transcripts, per-machine secrets.)

Before classifying anything under `~/.claude/` as an orphan, `git -C ~/.claude remote -v` + `git ls-files <path>`. The migration question is usually "should this move from direction (2) to direction (1)?" — ship via the coordinator installer rather than a Prime clone — not "does this have a writer?"

## There is no channel to Claude Central — DoE-claude *is* the channel

DoE-claude IS Claude Central's working tree. Both directions resolve inside this repo:

- **Outbound.** Global doctrine is authored at `global-doctrine/CLAUDE.md` and re-derived into
  `~/.claude/CLAUDE.md` by a hook; the EM-only channel is `coordinator/snippets/em-operating-doctrine.md`.
  Editing the derived copy is the real error — it is overwritten on next re-derivation.
- **Inbound.** Memos addressed to `claude-central-em` land in this repo's `cross-repo/inbox/`.
  `doctor-proportionality.md` made the full trip, from `example-market-data-repo-em` to a ratified entry.

The absent `repos.claude_central` key is **correct**: `repos.*` addresses peer trees, and Central
is not a peer to this repo. Adding a key would invite the EM to memo itself.

A `~/.claude`-altitude call needs the PM's **assent, not a route** — it is direction-class because
it is fleet-wide, not because it belongs to another team. An EM reporting "no channel upward" has
misfiled a direction-class ask as missing infrastructure.
...
## Canonical worked example: the whoami-sentinel consult
...
**Outcome:** doctrine codified at `docs/wiki/install-playbook-rationale/plugin-identity-and-health-sentinels.md`. Decision shape: **receipt-on-disk, identity-via-MCP, deliberate non-merger.** Revisit only on a concrete incident the three-surface layout couldn't catch.
...
## Hosting a shared contract — sibling plans are substrate, not settle-table checkboxes

As the contract/doctrine host in a multi-repo fleet, a sibling EM's plan is **substrate to read for what it needs FROM you** — not a `status:` field to tick as SETTLED/reviewed. Treating sibling plans as settle-state checkmarks let a blocking dependency *on the host* — an `OwnerNamespace` enum widen the siblings were blocked-waiting on — go unaddressed until the PM flagged it.
