...
> **Authoritative definition:** `docs/wiki/lessons-outbox-schema.md` is the **single source of truth** for the `change_kind` enum. Future enum additions MUST be made there first. The summary table below is for reading convenience only; if the two diverge, `lessons-outbox-schema.md` wins.
