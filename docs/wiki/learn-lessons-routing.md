...
silently leaks every undated `[universal]` accrued in a project's `lessons.md`; the Phase 8
...
> **Authoritative definition:** `docs/wiki/lessons-outbox-schema.md` is the **single source of truth** for the `change_kind` enum — including the `skill-edit` value added in the 2026-06-15 outbox plan. Future enum additions MUST be made there first. The summary table below is for reading convenience only; if the two diverge, `lessons-outbox-schema.md` wins.
...
The 2026-04-27 example-game-repo pass illustrates the signal density: 3 lessons became direct pipeline fixes, 12 became universal coordinator promotions across 15 files — from a single project's lessons file. When the central-mode run is overdue, per-project files are the richest underexplored source of universals.
...
*Source: 2026-05-28 example-game-workbench-repo; central-promoted.*
