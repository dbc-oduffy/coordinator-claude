...
extraction to `example-game-workbench-repo` is Wave 2b.
...
- **Version constant:** `ADDON_PROTOCOL_VERSION = 14` — defined in
  `core/addon_protocol.py` (single source of truth). Bump-notes chain:
    ...
    * `4 → 5` (WS-1 Step 3): Five new hookspecs for multi-language
    ...
    * `5 → 6`: bundles three new façade dataclasses +
      ...
      (5 façades bundled).
    * `6 → 7` (γ-prime): new façade dataclass
    ...
    * `9 → 10` (comprehensive-audit-remediation): Extends `CorpusBand`
    ...
    * `10 → 11` (chunker-author-discoverability-and-residuals):
      ...
      **Z-AMEND-1 refinement (canonical reading from this bump forward):**
      ...
      2. **New opt-in dispatch path (the single_file_runner carve-out).**
         ...
         **Refinement (v12):** The carve-out applies only to
         ...
         **Canonical carve-out 2 instance — `CorpusBand.structural_index_resolver`
         (S-1a):** This field is the clearest application of the
    ...
    * `12 → 13` (fusion-pipeline-n-lane): Reintroduces
      ...
      **v10→v13 ownership-model shift:** At v10 (comprehensive-audit-remediation
      D3), `default_weight` and `authority_pairs` were removed with
    ...
    * `11 → 12` (addon-protocol-v12-and-ue-tool-migration):
      ...
      **Held-docstring replaced.** `AddonMcpDepBundle` docstring at
      ...
      `tasks/2026-05-18-v12-vs-option-c-tiebreaker/the Director of Engineering-verdict.md`.
...
- Tenant-registry write under `<data-home>/process-tenants/` (or
  `<settings-home>/gpu-tenants/` per `spec.tenant_kind`) — neither is under
  `~/.claude`; see `host-resilience-and-bounded-spawn.md` § Tenant registry
...
The 9th hookspec, added in W1. Additive at `ADDON_PROTOCOL_VERSION=2`
...
for the `AddonDiagnostic.anchor_line` AC-8 dedup key — `canonical_id`
...
the stashed callable. **D3 constraint (PM):** addon-callable producers with
...
The UE addon uses this to register `eval/bank_ue.yaml`.
...
`example_game_repo_plugin_enabled` core field (which emits `DeprecationWarning` on read since D4).
...
`core/addon_protocol.py`. Stability contract: adding a defaulted additive
field does NOT mandate an `ADDON_PROTOCOL_VERSION` bump (subject to the
Z-AMEND-1 dispatch-gating refinement in §Overview); required-field
additions, field removal, and field retype DO mandate a bump.
...
- `PreflightVerdict.editor_contention` — editor open but example-game-repo-control
  not registered.
- `PreflightVerdict.editor_required_unavailable` — Mode A required but no
  live editor / example-game-repo-control found.
...
| UE addon | 100–199 | `example-game-workbench-repo` UE addon | Any UE-specific classifier that should fire after core but before generic |
...
Named protocol: **`example_game_repo_python_bridge_v1`**. Field: `bridge.protocol_version = 1`
set at closure-creation site in `project_rag_mcp/preflight.py:make_example_game_repo_python_runner()`
...
| Change | Bump? |
|---|---|
| Signature: input type changes away from `str` | YES → v2 |
| Signature: return type changes away from `dict` | YES → v2 |
...
Example-game-repo doctor renderer must add the `INFO` case correspondingly (cross-repo
...
```python
{
    ...
    "bridge_protocol_version": int | None,  # int (e.g. 1) for example_game_repo_python_bridge_v1, None if no bridge
...
- Invoking the `python_runner` bridge or any `mcp__example_game_repo-control__*` call.
...
*Addon-vs-core collision handling is split from
...
- **B-3 schema ownership.**
  The transition-window B-3 reserved is closed. Phase 6 paired
...
- **B-4 taxonomy** — `_VALID_DOMAINS = {"project","engine","official_docs","external"}`
  ...
  upserts it into `addon_schema_versions` at addon load for observability.
  Both hookspecs use `@hookspec` parallel-call semantics; host iterates
  results and executes DDL + indexes after core DDL.
...
engine-agnostic. No `blueprint`, `bp_`, `unreal`, `example-game-repo`, `uproject`, `umap`, or
...
| `project_rag.addon_commands` | `run_command(argv: list[str] \| None = None) -> int` callable per addon command | Umbrella aggregator (`/example-game-repo:doctor`, future peers) via `importlib.metadata` scan |
...
contract surface for `/example-game-repo:doctor` merge (see §5.6 of the sub-plan).
...
## v13 — CorpusBand.default_weight restored under N-lane ownership
...
## v12 — per-tool project graph.db capability
...
Reference: `tasks/2026-05-18-v12-vs-option-c-tiebreaker/the Director of Engineering-verdict.md`
