...
The contract was ratified via `cross-repo/inbox/2026-06-22-env-stepzero-convergence-reply.md`. The bash reference implementation lives at `scripts/lib/step_zero_emit.sh` in this repo. The **fixture bytes** (`tests/fixtures/step-zero-conformance.json`) are the single normative authority — non-bash consumers conform against the fixture, not against bash `printf` formatting quirks.
...
The bash reference (`scripts/lib/step_zero_emit.sh`, function `_co_pp_json_escape`) implements exactly this five-step sequence. Read the source to confirm — the source is authoritative on what the bash emitter does; the fixture is authoritative on what every conformant emitter must produce.
...
`tests/fixtures/step-zero-conformance.json` is the **single normative authority** for the contract. The bash reference (`scripts/lib/step_zero_emit.sh`) is the worked example for bash consumers — illustrative, not normative. Non-bash consumers (node, Python, PowerShell) conform against the fixture bytes, not against bash `printf` formatting.
...
| **Verbatim-vendor** | A byte-for-byte copy of the bash SSOT (`scripts/lib/step_zero_emit.sh`) vendored into your tree | The **SSOT blob at a pinned SHA**. Conformance is transitive (see below). Do **not** re-vendor the fixture. |
...
If you vendor the bash SSOT (`scripts/lib/step_zero_emit.sh`) **verbatim**, running the fixture is redundant *and* strictly weaker than a byte-identity pin. Prove conformance this way instead:
...
**Why this is sound (transitive conformance):** copy == SSOT (your byte-identity gate) ∧ SSOT ⊨ fixture (coordinator's own `tests/test_step_zero_conformance.sh`) ⇒ copy ⊨ fixture. The verbatim vendor never needs to run the fixture itself.
...
Coordinator ships the bash reference (`scripts/lib/step_zero_emit.sh`) and the conformance fixture. It does **not** ship PowerShell, node, or Python reference emitters. Each consumer repo owns its emitter in its own language; the fixture is the polyglot surface that makes all of them comparable. YAGNI on multi-language emitters until a consumer asks.
...
- `scripts/lib/step_zero_emit.sh` — bash reference implementation (illustrative, not normative).
- `tests/fixtures/step-zero-conformance.json` — normative fixture (v1.1).
- `tests/test_step_zero_conformance.sh` — conformance runner for the bash reference (ships in the coordinator source tree; sibling consumers wire their own runner against the vendored fixture).
