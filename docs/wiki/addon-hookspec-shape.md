...
- Do NOT skip the cross-repo grep when proposing a hookspec for an addon-flavoured concern. Adjacent contracts in sibling repos (`X:/project-rag-ue-addon/...`, `X:/example-game-workbench-repo/...`) must be checked per [[cross-repo-authority-shift-protocol]].
