...
<!-- spec-backlink: docs/plans/2026-05-30-deep-research-synthesis-fidelity-coverage-audit.md § C8, § AC13 -->
...
2. **Sidecar-only output.** The auditor emits one artifact: a coverage-audit sidecar adjacent to
   ...
   contract, upstream in `coordinator/CLAUDE.md § Synthesis Discipline`:
   > "Synthesizers don't rewrite — they assess, fill, and frame. … Never re-author specialist
   > content."
3. **Non-teammate Agent.** Dispatched as a plain `Agent(...)`, not under `TeamCreate` / `team_name`.
...
step — after the synthesis is written, before archive/TeamDelete.
...
instantiation is necessarily per-domain. This follows `coordinator/CLAUDE.md § Self-Improvement Loop`:
...
Per `coordinator/CLAUDE.md § Adding a Convention to the Coordinator System` — conventions decay
...
surfaces; see `docs/wiki/coordinator-tripwires.md` for the registry if a static-grep tripwire is
