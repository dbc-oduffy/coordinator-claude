...
### After the vacate mandate — the authoritative lane is settings-home, and both lanes are read during migration

The vacate mandate moves durable machine-local state out of the resettable `~/.claude` and into the durable settings-home (`~/.coordinator-claude-settings`). A consumer's health sentinel is durable state by this rule: **the governing principle is that a health sentinel must be at least as durable as the component it reports on.** A sentinel stranded on resettable ground outlives its *accuracy*, not its component — after a `~/.claude` reset the scanner reports "doctor has never run" about a consumer that is running fine, because the sentinel was wiped while the (settings-home-resident) component was not.

Therefore, post-mandate:
