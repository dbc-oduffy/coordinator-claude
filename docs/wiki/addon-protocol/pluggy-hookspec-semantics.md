...
3. Grep the central improvement queue and any open spinoff handoffs for the same surface: `rg -n '<seam-shape>' "$(python3 "${REPO_CLAUDE_KLABAUTER:?}"/coordinator/lib/coordinator-state-root.py --central)/improvement-queue/" "$(python3 "${REPO_CLAUDE_KLABAUTER:?}"/coordinator/lib/coordinator-state-root.py --central)/handoffs/"`.
