...
relates_to:
  ...
  - docs/wiki/addon-protocol/cross-corpus-class-addon-contract.md
...
project-rag:docs/wiki/codebase-judgment/typed-surface-over-text-pattern.md
...
**Canonical instance:** `CorpusBand.structural_index_resolver` (S-1a). The
host does not derive the structural index path from `corpus_root` via an
undocumented layout assumption; the UE addon declares the path explicitly via
its own `_make_ue_band()` builder, and the host simply
...
(see project-rag:docs/wiki/thin-wrapper-graceful-fail.md).
