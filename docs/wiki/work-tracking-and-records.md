...
- The status dimension for a handoff is the `deployment_state` enum: `awaiting_gate | ready_to_fire | in_flight | shipped | abandoned`. Query results built from this field are **not terminal by design** — do not invent a parallel status field alongside it.
