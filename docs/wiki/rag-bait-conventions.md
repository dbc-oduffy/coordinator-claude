...
**Where:** In the body of any SKILL.md whose `description:` field acts as an auto-routing
trigger and could pattern-match on tangential prompts.

**What:** A `## Do not use for` markdown section enumerating non-applicability — the
question shapes the skill is NOT for, even when they share surface vocabulary with the
trigger surface.

**Why:** Description-as-trigger skills are high-friction by design — the trigger fires
automatically on every prompt that pattern-matches the description. Over-firing (skill
fires on tangential mentions, the LLM calls the wrong tool, prompt-budget burns on
irrelevant retrieval) is the dominant failure mode. The trigger-boundary block is the
structural mitigation: enumerating non-applicability in the skill body bounds the
implicit description-as-trigger surface that the frontmatter cannot express tersely.

**Distinct from Section 4 "Negative-Spec Block":** Section 4 is a `DO NOT` *code-comment*
annotation at a correction site in source code. Section 5 is a *markdown heading* in a
...
single verb on a single tool) MAY omit it.
