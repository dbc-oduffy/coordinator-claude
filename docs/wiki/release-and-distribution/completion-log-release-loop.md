...
`/merging-to-main` Step 1.5 reads the pending-release file when one exists (or falls back to an
inline pass for emergency releases), formats human-readable release notes, and after the merge
commit — Step 5.5 — flips every `status: pending-release` entry to `status: released`, stamping
`released_in`, `released_at`, and `released_sha` **per-entry**, resolved to the earliest release
tag whose commit history actually contains that entry (not a blanket stamp with the newest tag).
The flip is gated on a release tag having been cut this merge (`$RELEASE_TAG_CUT`), **not** on
the pending-release accumulator file's existence — a consolidation or cross-machine merge that
never produced that file still flips its entries. See `skills/merging-to-main/SKILL.md` Step 5.5.
...
| `status` | enum | `pending-release \| released`. Default `pending-release` on write; `/merging-to-main` flips to `released`. |
| `released_in` | string | Version tag (e.g. `v2.1.0`) — set by `/merging-to-main` |
...
| Field | Type | Notes |
|-------|------|-------|
| `loe` | object | Phase 2 populates: `agent_dispatches`, `opus_dispatches`, `em_tokens`, `tshirt` (`XS\|S\|M\|L\|XL\|XXL`). Phase 1 leaves absent. |
...
thin `query-completions.py` wrapper. All three plan-specified query shapes
...
> block, cite the installed settings-home forwarder per the precedence ladder in
> `coordinator/snippets/resolve-coordinator-bin.md` — rung 0 / Shape W on a PowerShell host,
> the settings-home forwarder for `query-completions` on a
> POSIX host — because the bare extensionless name does not resolve on PATH (only `.sh`/`.js`
> ship; `command -v query-completions` is MISSING even with `bin/` on PATH). Silent-empty
> otherwise. See `docs/wiki/claude-md-surfaces/claude-code-platform-gotchas.md` § "Coordinator scripts are on PATH"
> (label-vs-executable).
...
archive/daily-summaries/                  # /workday-complete Step 4 — per-machine naming
  YYYY-MM-DD-<machine>.md
state/week-changelog/
  YYYY-MM-DD-<machine>.md                 # /workday-complete step9 daily block (per-machine)
  YYYY-MM-DD-pending-release.md           # /workweek-complete output
archive/release-notes/
  YYYY-MM-DD-vX.Y.Z.md                   # /merging-to-main output
```
...
(platform-injected, per-session, unclobberable — Claude Code ≥ ~2.1.150).
...
`/workstream-complete`'s `d-complete-entry` directive checks for a monolith at `archive/completed/YYYY-MM.md` (root
...
(use when migration was handled manually). Documented in `coordinator/docs/wiki/coordinator-tripwires/`.
...
A static-grep tripwire (`check-no-monolith-completion-append.py`, registered in
`coordinator/docs/wiki/coordinator-tripwires/`) fires on any write path to
...
argument. Full detail in `coordinator/docs/wiki/coordinator-tripwires/`.

---

## Cross-references

- **Handoff schema** — `coordinator/schemas/handoff.schema.json`. Phase 2 extended the
...
- **`bin/query-records`** — the underlying query engine; completion entries are
  ...
  `query-completions`.
- **`coordinator/docs/wiki/coordinator-tripwires/`** — tripwire registry entry for
  `monolithic-completion-log-write-check`.
- **`docs/wiki/ceremony-calibration/workday-workweek-cadence.md`** — daily/weekly ceremony context.
- **`docs/wiki/ceremony-calibration/workstream-complete-review.md`** — review marker trail that feeds
...
| Phase 3 | Consumer wiring: architecture-survey, bug-sweep, distill, learn-lessons, workday-start, workweek-complete, spinoff, the Staff Engineer/the VP-Product Reviewer personas, debt-triage | spec complete — awaiting Phase 1 gate |
...
The `loe` object defined in the Phase 1 schema (§ Phase 2 / Phase 3 forward-compat fields) is now populated by `coordinator-session-loe.py` as part of `/workstream-complete`'s `d-complete-entry` directive, immediately before the per-entry file is written.

**Schema slot** (from `coordinator/schemas/completion-entry.schema.json`):
...
loe:
  ...
  tshirt: <XS|S|M|L|XL|XXL>    # derived t-shirt size (see sizing table below)
```

**T-shirt sizing heuristic** (implemented in `coordinator-session-loe.py`):
...
| XXL | > 90 dispatches OR > 12 Opus dispatches OR ≥ 2,000,000 `em_tokens` — rare for a single session; `chain_loe` (below) is the typical path to XXL |

The `tshirt` field is the primary consumer signal. Raw counts are forensic.

**How `loe` is populated:** `/workstream-complete`'s `d-complete-entry` directive invokes `coordinator-session-loe.py` (Phase 2 augmentation, Chunk 3) which reads session telemetry and writes the `loe:` block into the completion entry. The script is idempotent — re-running on an entry that already has `loe:` is a no-op.
...
### Chain-terminal aggregation via aggregate-chain-loe.py

When a workstream-complete closes a chain (a completion entry with `chain_terminal: true` AND a non-null `chain:`), `coordinator-session-loe.py` invokes `aggregate-chain-loe.py` to produce the chain-level LoE summary.
...
**Primary precedent:** `state/lessons/` lesson at line ~361: *"Ship-and-watch telemetry needs a calendared review or it's not real."* The tier-usage telemetry lesson established that instrumentation without a concrete consumer path is ceremony. Phase 2 LoE tracking ships with a concrete consumer (workweek-complete Step 8.5 LoE high-water check) and a concrete PM-facing output (XL chain-terminal entries in the weekly summary) — not a "we'll glance if drift appears" design.
...
Six handoffs across two weeks, each individually M or S. The chain-terminal workstream-complete runs `aggregate-chain-loe.py` and produces:
...
The `chain_loe.tshirt: XL` is what workweek-complete Step 8.5 surfaces to the PM with the chain span (e.g. "6 sessions, <start date> to <end date>").

---

### Phase 2 cross-references

- **Handoff schema** — `coordinator/schemas/handoff.schema.json` carries a comment block documenting the `## Session Ledger` body convention (not a frontmatter field).
...
- **`aggregate-chain-loe.py`** — chain-walk script invoked at workstream-complete for `chain_terminal: true` entries.
- **`coordinator-session-loe.py`** — per-session LoE collector; called by `/workstream-complete`'s `d-complete-entry` directive (Chunk 3 augmentation).
...
(XS=1, S=2, M=4, L=8, XL=16, XXL=32), not by commit churn. Churn-based scoring conflates
...
|--------|--------|--------|
| roadmap | XXL, L, or XL | Highlights |
| roadmap | S or M | Notable |
| roadmap | XS | Other (likely doc/spec) |
| bugfix | XXL or XL | Notable (call out explicitly) |
| bugfix | S/M/L | Other (unless user-visible) |
| tech-debt or infra | XXL or XL | Other (flag for PM awareness) |
...
**debt-triage:**
LoE-weighted prioritization: surface tech-debt and infra entries with `tshirt=XL` (or `XXL`) or
`chain_loe.tshirt=XL` (or `XXL`) as highest-priority triage candidates. High-LoE debt entries
...
awk '/<!-- BEGIN repo-registry -->/,/<!-- END repo-registry -->/' "$(python3 "${_cc_claude_klabauter}/coordinator/lib/coordinator-state-root.py" --central)/repo-registry.md" \
  | grep -E '^\s*shortname:' \
  | awk '{print $2}' \
  | while read -r shortname; do
      path="$("${_cc_claude_klabauter}/coordinator/bin/resolve-repo-path.py" "$shortname")"
      [ -n "$path" ] || continue   # unregistered on this machine — skip (fail-loud-skip)
      printf '%s\n' "$path"
    done
```

`resolve-repo-path.py` emits the on-disk path on stdout, or empty (exit 0) when the shortname
has no registered path on this machine — skip those entries rather than treating empty as a path.
Then iterate the resolved paths and invoke `query-completions` with `--root <path>` in each
sibling repo.
