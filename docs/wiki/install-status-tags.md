...
**`~/.example-game-repo/setup-state.json` vs `~/.claude/example-game-repo/install-status.json` are distinct files by design** (2026-05-08 example-game-workbench-repo). `~/.example-game-repo/setup-state.json` is the Wave-4 persistent mode state file; `~/.claude/example-game-repo/install-status.json` is per-run telemetry from agentic-install-hardening. Checker corpora and prior-art lookups must document both as canonical separately. Conflating them produces false Conflict findings.
