...
motivated the original split (the Data Science Reviewer memo). Vendored-parity rule
...
| Site | Path | Role |
|---|---|---|
| Addon gate (authority) | `project-rag-ue-addon` repo's `scripts/gate_structural_index.py:KNOWN_GOOD_PAIRS` | Write-side: rejects unknown pairs at ingestion |
| Addon read mirror | `project-rag-ue-addon` repo's `<…>` (parity-tested) | Addon-side handlers |
...
`get_corpus_class_sources(cls)` recomputes from the pair set on every call. No `functools.cache`. Reasons (the Staff Engineer P2-2):
