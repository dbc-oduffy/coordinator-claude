...
table so that example-game-repo-callable wrappers (and any other consumer that parses the rendered
...
| `coordinator_whoami` | `coordinator_whoami` package | Whether the `coordinator_whoami` Python package is importable via the coordinator venv, and what Phase 3 Step 6 did about it. Values: `ready` (importable, venv healthy — emitted in both normal and check-only modes when venv is healthy) / `rebuilt` (venv was absent or broken; `ensure-coordinator-venv.sh` rebuilt it and the package is now importable) / `would write` (under `--check-only`: venv would be created or rebuilt) / `failed (<reason>)` (venv creation or pip install failed). <!-- Review: code-reviewer F1/F6 — collapsed `would write (offer-D)` → `would write`; expanded description to cover both absent and broken venv cases. --> |
| `brew_present` | Homebrew presence (macOS) | **macOS only** — Whether Homebrew is installed at `/opt/homebrew/bin/brew` (Apple Silicon) or `/usr/local/bin/brew` (Intel). Pure read; no `--check-only` divergence. Values: `ready` / `failed (Homebrew not installed)`. |
| `brew_bash_installed` | brew bash ≥ 4.3 (macOS) | **macOS only** — Whether a brew-managed bash ≥ 4.3 is present. Values: `ready (<version> at <path>)` / `failed (declined)` / `failed (brew install bash error: <stderr tail>)` / `would write (offer-B)` (last variant under `--check-only`). |
| `shellenv_block` | Login-shell shellenv block (macOS) | **macOS only** — Whether the marker-guarded brew shellenv block is present and active in the user's login rc. Values: `ready (already present in <rc-path>)` / `ready (appended to <rc-path>)` / `ready (sentinel present — eval status unprobed in check-only mode)` / `failed (declined)` / `failed (rc not writable: <rc-path>)` / `failed (rc parent dir not writable: <dir>)` / `failed (append failed mid-write — inspect <rc-path> for partial sentinel)` / `failed (append succeeded but sentinel absent — inspect <rc-path>)` / `failed (sentinel present, eval not active — inspect <rc-path>)` / `would write (offer-C, target: <rc-path>)` (last variant under `--check-only`). |
| `bash_version` | Bash version (PATH-resolved) | Whether the PATH-resolved bash meets the ≥ 4.3 floor required by coordinator scripts. Values: `ready (<version> at <path>)` / `failed (no bash on PATH)` / `failed (<version> below 4.3 nameref floor)` / `failed (<version> — bash ≥ 4.3 required)`. <!-- Review: code-reviewer F3+F8 — bash_version row emitted by install.md:72-75,208 was missing from this table (pre-existing gap); added in the same commit that fixes the shellenv_block value set. --> |

**Total: 20 rows** (11 pre-redesign + 3 new + 1 persona-customization row + 1 coordinator-whoami row + 3 brew bash / shellenv rows + 1 bash_version row). The `coordinator_whoami` row's value vocabulary was extended 2026-06-20 to include `rebuilt` (venv self-heal) — the row count is unchanged.
...
| `rebuilt` | The coordinator venv was absent or broken; `ensure-coordinator-venv.sh` rebuilt it and the component is now functional. Emitted by the `coordinator_whoami` row when Phase 3 Step 6 had to self-heal the venv. This is a successful outcome (not a failure) — the component is healthy after the rebuild. |
...
example-game-repo wrappers (`scripts/example_game_repo_setup.sh`, `scripts/example_game_repo_setup.ps1`) and any other
