...
| `meta_repo_doctrine` | Meta-repo the (now-removed) meta-repo local-doctrine file | Whether `the (now-removed) meta-repo local-doctrine file` exists and is non-empty |
...
| `render_template_helper` | Render-template helper | **new** — Whether `render-template.py` is present and executable in the coordinator plugin directory |
| `persona_customization` | Persona customization | Whether persona names were customized or kept at defaults under this run |
| `coordinator_whoami` | `coordinator_whoami` package | **Retired** — the `coordinator_whoami` package does not exist. This identifier is preserved per the append-only stability contract; no installer path emits it. |
...
| `bash_version` | Bash version (PATH-resolved) | Whether the PATH-resolved bash meets the ≥ 4.3 floor required by coordinator scripts. Values: `ready (<version> at <path>)` / `failed (no bash on PATH)` / `failed (<version> below 4.3 nameref floor)` / `failed (<version> — bash ≥ 4.3 required)`. |
...
| `rebuilt` | The coordinator venv was absent or broken; `coordinator_core.install.ensure_venv` rebuilt it and the component is now functional. Its sole emitter is the `coordinator_whoami` row (§ Check identifiers), which is retired and unemitted; the value is preserved in this vocabulary per the append-only stability contract. This is a successful outcome (not a failure) — the component is healthy after the rebuild. |
