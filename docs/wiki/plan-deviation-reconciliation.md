...
Acceptance Criteria oracle tables (consumed by `check-acceptance-oracle.sh`) are **immutable
in their Criterion and Test columns**. Free-text mutation of those cells would corrupt the
structured oracle the gate parser relies on.

When an AC shipped differently from what was forecast:

1. **Status column** — change to `shipped-differently`
2. **Note column** — add a brief note naming what shipped
3. **Substantive delta** — record in the `## Deviations` table (see below) AND in the Decisions Made section

The Criterion and Test cells remain as authored. The `## Deviations` table and Decisions Made are
...
**Rule.** Before treating a distill-forked DR as genuinely open, reconcile it against shipped reality: grep the codebase for the decision's subject, check the git log for commits that settled the question, and read the relevant `## Deviations` table in the source plan. If the shipped code answers the question, update the wiki entry in-place (remove the "open" marker, write the actual decision) — do not defer as a "separate triage item." An unreconciled "open" DR in a wiki is doctrine rot that misleads future reviewers and prior-art checkers.
