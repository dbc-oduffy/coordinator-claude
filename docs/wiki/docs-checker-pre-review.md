...
## Symbol Existence ≠ Call-Shape Exists — Verify Arity, Not Just Presence
...
This is distinct from the "fix already shipped" scope limit (§ Scope Limit above): that covers the case where the bug has already been fixed; this covers the case where the API signature was never what the plan assumed at the arity level. Both are "docs-checker green ≠ sufficient" but on orthogonal axes.
...
The EM Decision Rules table is calibrated against the current Claude model's training distribution. Re-evaluate when the underlying model changes. A note is recorded in the central structured queue (`state/improvement-queue/`, `queue_scope: central`, via `coordinator-queue-append --schema improvement-queue --queue-scope central`) to flag this for the next model upgrade.
