...
**Incident:** `awaiting_gate` handoffs were filtered behind a 14-day staleness gate. The PM needed cross-workstream context that never crossed the threshold. The skill cited the spec faithfully, but the EM obeyed the filter too literally — the filter was structurally wrong, not just miscalibrated.
...
- → `docs/wiki/skills-corpus/tiered-context-loading.md` (boot-time orientation budget and tier discipline)
- → `coordinator/skills/handoff/SKILL.md` § Handoff Lineage
  (handoff frontmatter `deployment_state` enum: `awaiting_gate | ready_to_fire | in_flight |
  shipped | abandoned`)
