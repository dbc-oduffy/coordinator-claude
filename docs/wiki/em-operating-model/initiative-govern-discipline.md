...
2. **Is the cluster large enough?** A cluster below the ≥3-item floor does not
...
4. **Does the cluster span multiple artifact types or a long time horizon?** Single-type,
   short-horizon clusters belong in the queue triage, not in the initiative taxonomy — see
   § Queue-triage themes vs true initiatives for the carve-out this creates on the shared FK.
...
**What distinguishes a triage-theme value from a true initiative.** A true initiative
(minted via `coordinator-initiative create`, § Sweep Cadence) clears the full cut-bar above:
it is distinctive enough to navigate the board by, and it spans either multiple artifact
types or a horizon long enough to warrant standing govern attention (burn-down or dated,
§ Prioritization Discipline). A **triage-theme** value is narrower on both axes by
construction — it names a title-keyword cluster of ≥3 unattached rows *within a single
queue family* (bug, debt, or improvement — never spanning types), surfaced by
`detect-initiative-candidates` during a single triage pass and written only at the moment a
cluster **graduates to a themed baton** (a `/debt-triage`- or `/bug-blitz`-class terminus —
see `coordinator/docs/wiki/ceremony-calibration/queue-terminus-doctrine.md`). It carries no `status`, no
`target_date`, and no `state/initiatives/<id>.yaml` entity of its own — it is a bare string
value on the FK, not a minted initiative record. A triage-theme is a *candidate that
graduated to a baton*, not a candidate that graduated to an initiative; those are two
different graduations sharing one field.

**Can a graduated theme later become a real initiative?** Yes, but not automatically. If a
triage-theme value recurs across sweeps, keeps clearing the ≥3 floor, and starts showing the
breadth (multi-type) or horizon (standing, not one-off) that Check 4 asks for, a human can
promote it the ordinary way: `coordinator-initiative create` with that theme's label, then
`attach` the accumulated member rows and the baton to the new entity — the surface-and-confirm
rule (§ The Surface-and-Confirm Graduation Rule) governs that promotion exactly as it governs
any other candidate-to-initiative cut. Nothing about writing a triage-theme value pre-empts
or shortcuts that decision; it is ordinary attach-to-existing-or-create judgment applied
later, by a human, to evidence the triage passes happened to accumulate.
...
surface-and-confirm caution this wiki already applies to initiative candidates applies here
to theme candidates.

**Why both the rows and the baton carry the value.** The value is written on the member
rows **and** on the baton itself, so the theme is traceable in both directions — from a row
to the baton it graduated into, and from the baton back to every row it summarizes. This
bidirectional traceability is the reason the existing `initiative` FK was chosen over minting
a second grouping field: a new `theme`/`subsystem` field would exist only on the member
rows, with nothing on the baton to join back through, giving the corpus two competing
cluster spines instead of one. Reusing the FK that already exists on all three queue schemas
(`improvement-queue.schema.json:71`, `debt-backlog.schema.json:74`,
`bug-backlog.schema.json:78`) and on `handoff.schema.json:184` — unpopulated on all 366
entries prior to this carve-out — costs nothing schematically and buys the row↔baton join
for free.

**Negative-spec.** Do NOT read a populated `initiative` value on a queue row as evidence a
named initiative exists — check `state/initiatives/<id>.yaml` before assuming the string is
more than a triage theme. Do NOT bulk-populate the FK across a queue family as a one-time
sweep; population happens one graduation at a time, at a terminus, by an EM exercising the
same judgment the detector's ~50% false-positive rate exists to require.

### The ≥3-item cluster floor
