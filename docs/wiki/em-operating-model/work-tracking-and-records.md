...
- the engine repo's `coordinator/bin/aggregate-chain-loe.py` walks the `predecessor:` chain from a terminal handoff backward, reading both `tasks/handoffs/` and `tasks/handoffs/archive/**/`. It maintains a visited-set as a cycle guard, parses *all* Session Ledger blocks per handoff (N blocks → N records), and dedupes on `session_id`. Degrades gracefully on missing/malformed data.
