...
> The helper's JSON keys are still named `the Staff Engineer`/`patrik_seam_files` for back-compat; post-restructure the `the Staff Engineer` SHA set is the code-semantics chunking input and `patrik_seam_files` additionally feeds the Staff Engineer's Layer-2 pass in Step 7.5.
