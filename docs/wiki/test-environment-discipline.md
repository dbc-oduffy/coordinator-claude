...
Two recurrent foot-guns from project-rag that violate the hermetic-test contract in subtle ways. Both look like "making tests simpler" but actually hide real failure modes.
...
*Source: L20, test-design §32. project-rag.*
...
*Recurs: ragaddon-L8, ragaddon-L427. [universal]*
...
*Source: ragaddon-L8 (second failure mode).*
...
*Source: example-sim-repo `state/lessons/` (L12, central-promoted). [universal]*
...
*Source: example-sim-repo `state/lessons/` (L14, central-promoted). [universal]*
...
**Rule.** When using a `--purge`, `--clean`, or equivalent flag on a pipeline whose rebuild path is unproven in the current environment, run WITHOUT the flag first to verify the full rebuild succeeds end-to-end. Only purge after a non-purge run has produced a known-good rebuilt state. Concrete failure mode: a retrieval-index rebuild uses `--purge` to match a handoff's framing of "start from a clean state"; a consumer schema mismatch then means nothing rebuilds the vector store, and `semantic_search` and `blended_query` go from working to broken — a net regression, even though the prior state was usable.
...
## 11. Tests Writing to a Real User-Config Path Must Isolate via ENV Vars, Not Just Function Patch — xdist-Fragile

*Source: project-rag-ue-addon L149. [universal]*

A test that patches a config-dir resolver function (`claude_home_dir()`, `get_config_root()`, etc.) to return a `tmp_path` looks hermetic — but under `pytest-xdist` parallel workers, an unpatched code path can bypass the function patch and write into the operator's live home dir. The function patch is applied in one worker's address space; another code path (or the same path re-imported before the patch) reaches the real dir.

**Concrete failure (project-rag-ue-addon).** `register-plugin` tests patched `claude_home_dir()`, but under xdist workers they intermittently wrote test fixtures into the operator's live `~/.claude/settings.json`, silently corrupting plugin-enable state mid-session.

**Rule.** For any test that exercises code resolving+writing a home/config dir, use `monkeypatch.setenv` on the underlying env vars (`CLAUDE_HOME`, `HOME`, `USERPROFILE`) to point at `tmp_path` as belt-and-suspenders **in addition to** patching the resolver function. An unpatched code path under concurrency can bypass the function patch and reach the real home.

**The smell is: a test writes to a real dotfile dir at all.** Any test that, under any execution path (even a path the author didn't intend), could reach `~/.claude/` or `~/` for writes is a reliability hazard in a concurrent suite.

Composes with §4 (autouse HOME-isolation fixtures are inherited by spawned subprocesses — the env-var isolation is the complement to the function-patch layer, not a substitute) and §1 (resolve to the pinned venv python, not `sys.executable` — same class of ambient-inheritance foot-gun).

## 12. An Isolation Lever Silently Deprecated to a No-Op Turns an "Isolated" Test Into One That REUSES and POLLUTES the Dev Environment

*Source: project-rag. [universal]*

A test that passes an isolation flag to a script/installer — `-UseVenvFallback` "to create a `.venv` inside the temp dir so no dev env is perturbed" (its docstring's promise) — becomes silently non-isolating when that flag has been deprecated to a no-op somewhere in the production code. The installer then targets `$RepoRoot/.venv` = the **dev** venv (which has editable-installed packages), so the "isolated" test reuses and mutates the developer's real environment while still reading green.

**Rule.** An isolation lever (a `--tmp`/`--fallback`/`--isolated` flag, an env var, a fixture) is only load-bearing while the production code honors it. When a test's isolation depends on such a lever, **assert the isolation actually happened** — check the resolved target path is under `tmp_path`, not the repo/dev root — rather than trusting the flag by name. A flag deprecated to a no-op is invisible at the call site; the assertion that the isolation landed is the guard. Composes with §11 (tests writing a real user-config path must isolate via ENV vars, not just a function patch) and §5 (native-store reads on a multi-session machine).

## 13. A Gate That Shells a Script by Bare Path Exec-Fails (126) on a Non-Executable File — Interpret, Don't Exec

*Source: project-rag, 2026-06. [universal]*

A test/gate/wrapper that runs a script by bare path on a command line (`bash -c "path/to/run.sh …"`, or `path/to/run.sh` directly) is an **exec** attempt: if the script is `-rw-r--r--` (not `+x`) — the common state for a checked-in helper — the exec fails with `Permission denied`, exit **126**, and the gate reads a false RED that has nothing to do with the script's logic.

**Rule.** To run a script whose executable bit you don't control, **interpret** it explicitly — `bash path/to/run.sh` / `sh path/to/run.sh` — never invoke it by bare path. Exit 126 on a script invocation is an exec/permission artifact, not a script failure; check the file mode before diagnosing the script. Composes with test-design-discipline.md §58 (`bash -n` failure does not prove a shipped script is broken) and §55 (a real run is the control-flow ceiling; a syntax/exec check is not).

## 14. A Shared Lazy-Launched Service Needs Per-Test Root Isolation and Per-Connect Token Reads

*Source: coordinator_core contract tests.*

Contract tests that talk to a single machine-global, lazy-launched service (a daemon, a multiplex socket broker) inherit two coupled foot-guns. Both read green until a concurrent or respawn race exposes them.

**Per-test service-root isolation.** When every test shares one lazy-launched service and the reaper only fires under a set env knob (`_reap_lazy_core` reaps only when `COORDINATOR_SVC_ROOT` is set), a stale or skewed service leaks across tests → flaky cross-test contention. **Rule.** Any test using a shared external service pins a *fresh* isolated service root per test — a fixture that sets a unique `COORDINATOR_SVC_ROOT`, passes it into each subprocess via `env=`, and reaps it on teardown. This is the service-lifecycle analogue of §11 (isolate via env vars, not just a function patch).

**Per-connect token reads — never cache.** A global-multiplex service that rotates its per-repo token *unconditionally on every (re)start* (writing the endpoint sentinel BEFORE the token) makes any token read at fixture setup go stale on the next respawn, and a read fired the instant the socket appears may race an absent token. **Rule.** Test clients call `read_token(repo)` fresh on *each connect*, inside the readiness-retry loop — never once at setup, never pre-provisioned test-side via `write_tokens` (a plan that pre-provisions is wrong; the Staff Engineer F1).

**Socket-path length.** `COORDINATOR_SVC_ROOT` (or any AF_UNIX socket root) must be a short `/tmp` path, not a pytest `tmp_path` — macOS caps `sun_path` at 103 bytes and a deep pytest temp dir silently overruns it.

## 15. Retiring a Transport Without Deleting Its Contract Tests HANGS the Suite

*Source: coordinator_core transport migration.*

When a daemon/socket transport is retired — its entrypoint becomes an exit-1 stub — the old contract tests do **not** fail fast. They block on a socket-ready poll that never resolves and hang the entire suite, turning it un-runnable rather than red. A green→hang transition is worse than a green→red one: no failure line points at the cause.

**Rule.** A migration that retires a transport must sweep and delete its daemon-era tests and fixtures *as part of the same change* — order: tests first, then the now-orphaned fixtures. Grep for every poller of the retired socket/entrypoint, not just the tests that name it directly. Composes with §5 (a hang/native-crash is a distinct failure surface from an assertion) — a poll-forever hang, like a segfault, produces no catchable Python exception.
...
**Rule.** Quarantine each surfaced red — skip-with-named-reason + a bug-backlog entry — ship the gate green-on-known-state, and fork a spinoff to fix the quarantined slice. Do **not** silently fix contract surfaces inline (collision risk with concurrent work on the relocated code), and do **not** hollow the gate to green without tracking. The gate's value is that it now *sees* the drift; the fix is a separate, tracked workstream.
