...
`coordinator-tripwires.md`): it transparently auto-rewrites to the prompt-free `git -C <path> …`
...
**First response is to REDUCE CONCURRENCY, not wait-and-re-fire.** Re-firing the same fan-out into the same throttle without lowering agent count changes the clock, not the variable that matters (see `coordinator-tripwires.md` § RE-FIRE-INTO-THROTTLE).
