...
## A guard's own installer-drift and tool-scope bugs are still a concurrent-EM hazard, not just an install defect

Two tool-shaped gaps worth naming even though the fix routes elsewhere: (1) `scoped-git-commit --stage-patch` can leave the pre-commit blobs of the paths it just committed staged in the SHARED index — a staged reversal of the commit that just landed, discovered because a plain `git commit` (no `--stage-patch`) on the same tree left the index clean while every `--stage-patch` invocation left residue. (2) A commit guard's per-session content-provenance table only registers a hash when the session's own top-level process runs `Write`/`Edit`; a Bash-executed edit (`sed`, a `python -c` rewrite) or a dispatched subagent's `Write` both bypass that registration, so a later scoped commit can refuse with "on-disk content no longer matches the content this session last recorded" even though the diff is entirely the session's own work and no peer touched the file. Both read, at first glance, like contamination from a peer; both are actually the tool's own scope tracking failing to see edits it didn't make itself. Before escalating either as a peer-contamination incident, check whether the edit path bypassed the tool's own recording mechanism.
