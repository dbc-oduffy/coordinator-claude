...
| DoE (all repos) | `$(coordinator_state_root --central)/doe-handoff-tracker.md` (example-orchestration-hub — see `state-placement-law.md`) | `/workstream-start` (coordinator meta-repo), ad-hoc `--all-repos` |
...
primitives (`stub_id`, `roadmap_id`, `blocked_by`). It appears in the **daily tracker** as a
...
# DoE mode (all repos from machine-local, writes to example-orchestration-hub at $(coordinator_state_root --central)/doe-handoff-tracker.md — see state-placement-law.md):
...
- `state/handoff-tracker.md` and `doe-handoff-tracker.md` (in example-orchestration-hub at `$(coordinator_state_root --central)/doe-handoff-tracker.md` — see `state-placement-law.md`) are NOT published
