...
anything, `deployment_state` in `{shipped, abandoned}`). **Vocabulary-migration hazard:** Branch B's
terminal `deployment_state` set exists as three separate in-code copies
(`handoff_reconcile._CLOSED_DEPLOYMENT_STATES`, `reconcile.gate_eval._TERMINAL_STATES`,
`_common._TERMINAL_DEPLOYMENT_STATES`) — when `abandoned` is eventually renamed `closed`, all three
must move in lockstep or the branches silently diverge again. This triplication is a named
DRY-extract target for a future consolidation pass, not yet done.
