...
**Design-as-offers principle.** The preamble leads with the better alternative ("did you mean `repos.project_rag`?"), not the violation. Offer-shape — assume willing collaboration; mistrust-shape (warn/block/nag without alternative) fights eagerness rather than redirecting it. See `docs/wiki/dispatching-parallel-agents/eager-agent-calibration.md`.
