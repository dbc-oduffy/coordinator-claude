...
Hardening `subprocess.run`/`Popen` calls against Windows locale-inherited `UnicodeDecodeError`.

> Source: `hl-031-wiki-only-python-subprocess-locale` — promoted from example-game-repo project lessons.
...
### uv-managed venvs and the stub-launcher flash
...
Also: a doctrine bullet stating an unverified *conclusion* can mislead for weeks — verify pytest runner behavior against live process state, not the bullet. Source: project-rag.

## `Path.write_text` on Windows Emits CRLF — Pass `newline='\n'` for Unix Consumers

*Source: project-rag state/lessons.md:35. [universal]*
...
*Source: project-rag L5 + example-game-workbench-repo L13.*
...
## Byte-porting a bash hook to naked Windows python — four recurring footguns

*Source: hook-migration. [universal]*

When byte-porting a bash hook to a native `python3.exe` (no Git-Bash shim), four bugs recur across nearly every port and must be checked **every time**:

1. **Hardcoded `/tmp` sentinels.** Git-Bash `/tmp` is an MSYS mount invisible to native python — a sentinel written to `/tmp/foo` by the `.sh` oracle and read from `/tmp/foo` by the port lands in two different places. Use `tempfile.gettempdir()`, never a literal `/tmp`.

2. **`os.kill(pid, 0)` for liveness.** On Windows CPython, `os.kill(pid, 0)` does **not** perform a harmless probe — signal `0` routes through `TerminateProcess` and can **kill the process being probed**. Use `OpenProcess` + `GetExitCodeProcess` (via `ctypes`) to test liveness. (This is the naked-python analog of the `RAW-PID-LIVENESS` ban — see
`coordinator/docs/wiki/coordinator-tripwires.md § RAW-PID-LIVENESS` (formerly `coordinator/CLAUDE.md`
§ Verification Before Done, retired — see `coordinator/docs/wiki/em-operating-model/verification-before-completion.md`
for the successor content).)

3. **`sys.stderr.write` silently up-converts `\n` → `\r\n`.** Text-mode stderr on Windows CRLF-translates, breaking byte-parity with the `.sh` oracle that emits bare `\n`. Write via `sys.stderr.buffer.write(text.encode())` for byte-exact output. (Same CRLF hazard as `Path.write_text` above — different write site, identical root cause.)

4. **Bare `subprocess.run` pops a focus-stealing console window.** Pass `creationflags=getattr(subprocess, "CREATE_NO_WINDOW", 0)` at every spawn site — see § Windows headless spawn above for the uv-venv stub-launcher exception (`pythonw.exe`).

**Also enforce the C8 invariant:** no internal `subprocess` timeout may exceed the hook's own `hooks.json` timeout — an inner timeout longer than the outer budget lets the harness kill the hook mid-cleanup, defeating the bounded-lifecycle guarantee (§ `communicate(timeout=)`).
