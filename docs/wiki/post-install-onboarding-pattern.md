...
- [`agent-install-contract.md`](agent-install-contract.md) § Install-spinoff layer — the **install-execution** counterpart to this **onboarding-tour** doctrine. This wiki governs the leaf-fires-the-tour rule (suppress double-offers); the contract's install-spinoff layer governs how a multi-repo *install itself* is sequenced durably across the coordinator reboot: coordinator onboarding is the handoff, each queued downstream repo is a `kind: spinoff` baton in the machine-shared install-baton rendezvous (at `$(coordinator-settings-home)/state/handoffs/`, inline fallback `${CLAUDE_HOME:-$HOME}/.coordinator-claude-settings/state/handoffs` — see `state-placement-law.md`), stitched onto one install-chain spine. Tour = "what now?"; install-spinoff layer = "the install is still coming, here's the tracked chain." <!-- (the Director of Engineering review, item 3) repointed off the example-orchestration-hub-central `coordinator_state_root --central` idiom — that root is a distinct rendezvous (see state-placement-law.md taxonomy); the install baton lives at settings-home, not example-orchestration-hub. -->
- [`surface-unfinished-onboarding.sh`](../../hooks/scripts/surface-unfinished-onboarding.sh) — the SessionStart `startup` hook that closes the loop this pattern describes: if the operator restarts Claude Code after `/coordinator:install` without running the guided orientation tour, the hook detects installed-but-not-onboarded (via `coordinator-setup-state.sh check orientation_completed` — re-gated per the Director of Engineering review item 1, not `install-status.json` completeness) and **surfaces** the pointer itself — the operator is never expected to type a `~/.claude/...` path by hand. Mode A (implemented): guarded-seed `continue-onboarding-and-installation.md` into the settings-home rendezvous above (only-if-absent, per `spinoff-handoffs.md` § Install Seed Loop) and print one `/pickup <path>` line. Self-extinguishes once `orientation_completed` is recorded. Fail-safe: any hook error exits 0 silently, never wedging the session.
