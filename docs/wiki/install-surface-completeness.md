...
The 2026-05-20/21 cleanup wave consumed roughly two days of EM time tracing exactly this pattern across the project-rag / project-rag-ue-addon / example-game-workbench-repo trio.
...
**Bootstrap gap — machine-local/ creation and seeding.** `lib/install-substrate.sh:81` (`mkdir -p "$_ml_dst"`) DOES create `~/.claude/machine-local/` on the `coordinator:install` Phase 3 path — the prior claim here that "no current installer creates machine-local/" was stale. The OSS path (`plugins/coordinator/dist/publish-repo-setup/install.sh`, ~59.9 KB) deliberately used a "minimal mirror" that historically skipped `install-substrate.sh` entirely (lines 944–954 of the dist installer name the cause: `install-substrate.sh` exposed no partial-invocation flag). Per the 2026-06-23 setup-time-substrate-completeness plan (PM decision), this is now corrected: C7a adds a `--setup-only` flag to `install-substrate.sh`, and C7b wires the OSS dist installer to call it, so both populations — `coordinator:install` users AND OSS `setup/install.sh` users — receive the full machine-local layer (dir creation, tracked baselines including `hardware.toml`, registry seed, bin/ resolver shims, hardware audit). The deprecated `publish-targets.sh` fallback silently activates when the registry directory is absent; it remains in place as a bootstrap backstop until all installations have been migrated to the new seeding path. Any work that depends on `machine-local/` keys must verify which install population the operator used and direct them to the appropriate re-run path (see `coordinator-doctor.md` hardware-probe remediation text for the two-population wording).
...
*Source: example-game-workbench-repo, 2026-05-29. [universal]*
...
(Source: 2026-06-18, example-game-workbench-repo.) Composes with the SHA-sentinel `copy_install` drift class in `machine-local-registry.md § propagation_mode = "copy_install"` (which notes the installer's banner/BOM injection defeats naive content-diff).
...
(Source: 2026-06-19, example-game-workbench-repo.) Composes with § Versioned gates / Post-Consumer Gates (advisory-WARN) and the doctor-EM-substitute framing — a deleted readiness probe is the absence of the consumer's self-service diagnostic.

## A runtime sidecar provisions on the host's resolved interpreter, not a separate phase

*Source: 2026-06-19, example-game-workbench-repo (gpu_sidecar cu130 trap); consult memo 2026-06-20.*
...
Empirical basis (2026-06-17, BettaAir coordinator-install-seed-phase dogfood): a manifest-only re-publish left the published script stale; the dogfood failed first-pass (`Unknown argument: --phase`).

### Contract-required publish targets in `publish-targets.example.sh` silently drop on new devices — use machine-local registry

The active `setup/publish-targets.sh` is gitignored/per-machine. A fresh device that hand-copies only the `mirror` row from `publish-targets.example.sh` loses the contract-required `toplevel-install` and `toplevel-wiki` targets with no error — those rows simply don't exist in the per-machine file.

**Fix:** write required targets into the machine-local registry via `machine-local array-append publish.targets` / `machine-local array-set publish.targets`, combined with a registry-aware `setup/bin/ensure-required-targets.sh` pre-flight that derives and injects required toplevel rows at publish time. Documentation telling operators to hand-copy rows is install-surface-incomplete — the enforcement must be structural, not prose.

Empirical basis (2026-06-17, BettaAir coordinator-install-seed-phase C0/C0b).
...
Empirical basis (2026-06-19, BettaAir portability-tracked-per-machine-config C5): moving a machine-specific marketplace PATH from tracked `settings.json` to `settings.local.json` was durable. But the harness re-asserted active `enabledPlugins` entries back into `settings.json` within the same session. Prior to 2026-06-30: A Step 3.8 oracle (acceptance oracle, now retired) that gated on `settings.local.json` contents flipped RED after a clean commit because the harness had reorganized those contents between the commit and the oracle check.

## Worked example: whoami venv pin (2026-06-20)

An editable `pip install -e` of `coordinator_whoami` into the bare system `python3` failed in two independent ways that were invisible at authoring time. First, Homebrew Python and Debian Python now refuse direct `pip install` into the system interpreter (PEP 668 — "externally managed environment"), so the install command errored on fresh macOS/Linux machines. Second, even where the install initially succeeded, a system Python bump (e.g. Homebrew upgrading from 3.12 to 3.13) silently invalidated the `.dist-info` entry — `import coordinator_whoami` raised `ImportError` after the upgrade with no signal pointing at the coordinator install.

The fix: `bin/ensure-coordinator-venv.sh` creates a coordinator-owned venv at `~/.coordinator-venv/`, seeds `coordinator_whoami` via editable install into that venv, and writes the absolute interpreter path to the machine-local registry as `coordinator.python`. `lib/resolve-python.sh` now resolves `COORDINATOR_PYTHON` env → `machine-local get coordinator.python` → PATH fallback, with a loud error if the pin is stale. The coordinator doctor (P-5) uses the resolved interpreter, not bare `python3`, so it catches venv-absent and pin-stale states and self-heals via `ensure-coordinator-venv.sh`.
...
`scripts/normalize-env.sh` is the sole writer for fixable env-prereq conditions. Its safety contract:
...
- **Bootstrap-trap surface** — `normalize-env.sh` must parse cleanly under bash 3.2 (stock macOS) because it may be invoked before brew bash is installed; no bash-4+ syntax in the script body. See `coordinator-installer-shape.md` § Step Zero and `cross-platform-shell-portability.md`.

The probe lib (`prereq_probe.sh`) and the fixer (`normalize-env.sh`) are separated by design: **probe lib = read-only SSOT, fixer = sole writer, gate (`--preflight`) = reader**. This prevents the probe from mutating state mid-check (vacuous-pass anti-pattern) and keeps the fixer auditable as the only mutation surface.

### Deferred verification — Windows-specific items (open items in the OS/version test matrix)

**Windows PowerShell 5.1 execution.** Whether `scripts/setup.ps1` actually executes correctly under Windows PowerShell 5.1 (`powershell.exe`) is UNVERIFIED — this is untestable on the macOS authoring machine. The 5.1 fallback path is specified and the pwsh probe WARNs when 5.1 is the only available shell, but functional end-to-end verification requires a Windows box. Tracked as an open item in the version/OS test matrix; needs a Striker (Windows) run before this item can be closed.
...
**AC10 — PATH/shim rollback round-trip.** Whether `--restore` correctly reverses PATH and shim mutations applied by `normalize-env.sh` on Windows is macOS-unverifiable. The backup mechanism is structurally sound from code inspection, but the round-trip (apply → restore → verify unchanged state) requires a Windows box to confirm. Both AC9 and AC10 are tracked as Windows-deferred in the version/OS test matrix alongside the PowerShell 5.1 item above.

> **CI enforcement arm.** Closing the deferred items above (PowerShell 5.1 execution, AC9–AC10 consent/rollback under a real Windows environment) requires a multi-OS CI matrix with an honest measurement gate. → `cross-platform-ci-discipline.md`
...
> Spec backlink: `archive/specs/2026-06/2026-06-11-exec-bit-install-surface-completion.md`; decision: `docs/decisions/DR-151-exec-bit-install-surface-completion.md`
...
See `state/handoffs/2026-06-11_160705_coordinator-install-brew-bash-action.md` for the spinoff that surfaced this; `docs/decisions/DR-148-require-bash4-on-macos.md` for the underlying policy.

**2026-06-25 extension: the login-shell-orphan dimension.** A related incident revealed a new failure mode: an operator `chsh`'d to brew bash believing DR-148 required it. It does not — DR-148's requirement is on the **PATH-resolved interpreter** (`bash <script>` resolution), not on login-shell identity. Switching the login shell to brew bash via `chsh` caused `~/.local/bin` to disappear from PATH in fresh terminals: macOS bash reads `~/.bash_profile` on login, not the zsh rc files (`~/.zshrc`, `~/.zprofile`) where the operator's PATH additions lived. An absent or minimal `~/.bash_profile` silently strands those entries.

**Coordinator's posture:** coordinator never requires or suggests `chsh`. When it detects a bash login shell with an absent or minimal `~/.bash_profile`, it offers to reconstruct `~/.bash_profile` — consent-gated, backed up before write, and reverted via `--restore` if needed (the same discipline all `normalize-env.sh` mutations follow). The installer also SUPPORTS operators who are already on a bash login shell and whose environment is intact. What it never does is proactively offer to change login shell identity.
...
# Note: example is simplified. The shipped normalize-env.sh implementation adds
...
<!-- Review: code-reviewer F7 — fixed three divergences from the real C2 impl:
     ...
     (3) missing '[ -r ~/.bashrc ]' guard; added to match normalize-env.sh line 502.
...
Cross-reference: `docs/decisions/DR-148-require-bash4-on-macos.md § Amendment 2026-06-25` for the policy ruling; `docs/wiki/cross-platform-shell-portability.md § PATH vs login shell (DR-148)` for the interpreter-identity summary.
...
Defer any of these four steps to a follow-up and the LFS cost persists indefinitely. (Source: coordinator-improvement-queue L117, 2026-06-15.)
...
*2026-06-24, claude-central (DoE), discharging the cross-repo `ask` memo `cross-repo/inbox/2026-06-24-git-lfs-step-zero-requirement.md` from the project-rag-ue-addon EM.*
...
How to apply: drift-audit scripts (e.g. `check-plugin-drift.sh`) should compare files individually, stripping documented post-copy mutations before diffing, rather than hashing directory trees. Composes with § Committed publish-mirror of executable scripts (freshness-compare modulo banner). (Source: coordinator-improvement-queue L154, 2026-06-20.)
