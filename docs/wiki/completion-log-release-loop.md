...
<!-- Review: code-reviewer — stale bare `state/repo-registry.md` path left uncorrected while
     the parallel section in repo-registry.md was already fixed to the example-orchestration-hub-resident seam. -->
When a Phase 3 consumer needs to query completion entries across sibling repos registered in
`$(coordinator_state_root --central)/repo-registry.md`, DO NOT use `yq` to parse the registry. The registry is markdown
...
The registry no longer carries a `path:` field — on-disk paths are machine-local-derived and
resolved per `shortname` via `coordinator/bin/resolve-repo-path.sh`. Extract each entry's
`shortname:` from the registry, then resolve it to a path:
```bash
awk '/<!-- BEGIN repo-registry -->/,/<!-- END repo-registry -->/' "$(coordinator_state_root --central)/repo-registry.md" \
  | grep -E '^\s*shortname:' \
  | awk '{print $2}' \
  | while read -r shortname; do
      path="$("${CLAUDE_PLUGIN_ROOT}/bin/resolve-repo-path.sh" "$shortname")"
      [ -n "$path" ] || continue   # unregistered on this machine — skip (fail-loud-skip)
      printf '%s\n' "$path"
    done
```

`resolve-repo-path.sh` emits the on-disk path on stdout, or empty (exit 0) when the shortname
