...
<!-- Review: code-reviewer — implementation type is str | None; validate_corpus_band rejects Path objects with "Convert Path objects to str before assigning" -->
...
<!-- Review: code-reviewer — implementation uses list[str]; frozenset is rejected by validate_corpus_band -->
...
### `structural_index_resolver: Callable[[], Path] | None = None`  *(added S-1a, 2026-05-19)*
...
<!-- Review: code-reviewer — added (i)/(ii)/(iii) labels to match addon-protocol.md:222-236 -->
...
- `band_name` must be a non-empty string. <!-- Review: code-reviewer — corpus_root and applicable_kinds both default to None and validate_corpus_band does not require them to be set -->
- `default_weight` is a non-negative float.
...
**Empirically (addon-EM rev-4 sweep 2026-05-19):** the addon's `A-F-22`
