...
|---|---|---|---|
| **local** | `/update-docs` Phase 6 OR direct invoke from project root | Auto-applies bounded items; surfaces structural changes to PM | Pruned `state/lessons/` entries, append-only wiki edits |
...
**Default mode detection:** `/learn-lessons` without `--mode` arg inspects cwd. Running from `~/.claude` central → default `central`; else default `local`.
...
Phase 5 strips need `git pull --rebase` + scoped pathspec + clean-state check before edit. If `state/lessons/` has uncommitted local edits OR recent commits within the triage window, surface to PM rather than auto-strip — a concurrent peer EM may have added entries the synthesis didn't see.

## Capturing lessons that should promote

When writing a `[universal]` lesson in a project's `state/lessons/` capture queue, the route depends on the proposed target:

| Lesson type | Proposed target | Route |
|---|---|---|
...
Empirical scale for `~/.claude` central + 4 the checkout root project repos as of 2026-05-05:

| Repo | Tagged | Untagged candidates |
|---|---|---|
| example-game-repo | 87 | 19 |
| project-rag | 31 | 0 |
| coordinator | 24 | 7 |
| example-sim-repo | 3 | 12 |
| **Total** | **145** | **38** |
...
**When a lesson in `state/lessons/` turns out to have an inverted rule (postmortem-revealed or PM-overridden), neutralise the entry in-place — do not delete it.**
**Why:** Deletion loses the "why" of the inversion and invites future re-derivation of the same wrong rule. An in-place annotation shows both the original and the corrected version.
**How to apply:** per `skills/learn-lessons/SKILL.md` § Phase 4 reversed-lesson annotation, add a blockquote directly under the entry's `body` text — `> **INVERTED YYYY-MM-DD:** <one-line reason for reversal> (replaced by: <new doctrine pointer>)` — in the same `state/lessons/<date>-<slug>.yaml` file. Deletion is reserved for lessons that were factually wrong from the start or exact duplicates, not for "we changed our minds" reversals. This composes with the delete-on-resolution rule for queue entries (queue entries are discarded once resolved; lesson entries carry a correction note that stays visible).

*Source: example-game-repo `state/lessons/` (example-game-repo-L131).*

### Local vs. Central Concurrent Run — Strip Only the Pre-Window Subset

**`/learn-lessons` local mode on a `state/lessons/` directory a central run is concurrently processing is a concurrent-edit hazard — reconcile delta-vs-full scope before stripping.**

Local mode enumerates the full directory (no `--since`) and counts every uncovered universal; central extracts only the delta since its last run. A local "N universals pending" figure is inflated — most are already-promoted-but-never-stripped pre-window entries (e.g. local reported 73, central's authoritative delta was 1 new universal).

**How to apply:** when a `~/.claude/tasks/learn-lessons-<today>/` central run is active on the same directory, strip only the pre-window subset (entries before the last-central-run date) and leave the delta window and universals to central. Re-enumerate `state/lessons/` and match the target entry by content (per `skills/learn-lessons/SKILL.md` § Apply order) before any strip — never against a cached listing. Source: project-rag. [universal]
...
*Source: project-rag state/lessons.md:60. [universal]*
...
*Source: example-game-repo state/lessons.md:9. [universal]*
...
**Haiku workers dispatched against `state/lessons/` extraction have a documented fabrication failure mode. Default lessons-extraction scouts to Sonnet, or EM-author from source.**

Empirical evidence (a delta `/learn-lessons` central run, 28 entries, 3 parallel Haiku scouts):

- **project-rag scout** — fabricated plausible-but-wrong records (entries cited that did not match the source file).
- **example-game-repo scout** — dumped malformed inline output; no usable file artifact.
- **addon scout** — never wrote (silent zero-tool-use failure).
...
*Source: project-rag delta run; tasks/learn-lessons-2026-05-24/routing-manifest.md.*

Related: `snippets/text-only-recovery-preamble.md`; `skills/learn-lessons/SKILL.md` § Phase 2 — Fan-out scouts.
