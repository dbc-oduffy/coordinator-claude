...
2. **Improvement-queue triage** — the daily/weekly cadence over the central structured queue in example-orchestration-hub at `$(coordinator_state_root --central)/improvement-queue/` (`queue_scope: central` entries — see `state-placement-law.md`).
...
Every entry in the central structured queue (example-orchestration-hub — `$(coordinator_state_root --central)/improvement-queue/`, `queue_scope: central` — see `state-placement-law.md`) should route to one of:
- **universal** — applies to any coordinator user / any project type → keep in the central queue in example-orchestration-hub (`queue_scope: central`, append via `coordinator-queue-append --schema improvement-queue --queue-scope central`)
...
> `docs/wiki/lessons-outbox-schema.md` — including `skill-edit` (added 2026-06-15). The
...
<!-- Amended 2026-06-15: routing-contract update per docs/plans/2026-06-15-universal-lesson-routing-mechanical-capture.md § C9a -->

When writing a `[universal]` lesson in a project's `state/lessons.md`, the route depends on the proposed target:

| Lesson type | Proposed target | Route |
|---|---|---|
| `[universal]` | Central coordinator wiki (`~/.claude/docs/wiki/`) | Invoke `coordinator-lesson-promote` CLI → `state/lessons-outbox/<ts>-<slug>.yaml`. Do **not** use `coordinator-queue-append` targeting the central queue (`$(coordinator_state_root --central)/improvement-queue/`, `queue_scope: central`) for this path — the outbox is the correct surface. |
| `[universal]` | Project-local wiki (`docs/wiki/` in the project repo) | Auto-apply locally via `/learn-lessons` local-mode — unchanged. |
| `[universal]` | Unknown / not yet determined | Manual-review path: write the tagged lesson to `state/lessons.md` and surface at next `/learn-lessons` central run. |
| Project-specific | Any | `state/improvement-queue.md` in the project repo; `/debt-triage` migrates to `debt-backlog.md` at triage time. |

**Source of truth for this routing contract:** `CLAUDE.md § Improvement Queue` (Routing contract paragraph, 2026-06-15). **Schema for outbox YAML entries:** `docs/wiki/lessons-outbox-schema.md`.
...
*Source: example-game-repo `state/lessons.md` (example-game-repo-L131, central-promoted 2026-05-28).*
...
*Source: example-game-repo state/lessons.md:9, 2026-05-29. [universal]*
...
**Haiku workers dispatched against `state/lessons.md` extraction have a documented fabrication failure mode. Default lessons-extraction scouts to Sonnet, or EM-author from source.**

Empirical evidence (2026-05-24 delta `/learn-lessons` central run, 28 entries, 3 parallel Haiku scouts):
...
**Why this is a structural failure mode, not a one-off.** Lessons-extraction is a high-paraphrase task on unstructured prose — the worker must read a war-story bullet, normalise it to a schema, and emit a record that grounds back to the cited line. Haiku underperforms on the grounding step specifically: it produces output that *looks* like a routing record but does not match the source bullet. The `bin/extract-lessons.py` deterministic-extraction gate (lesson-triage.md § Deterministic Extraction Discipline) is the load-bearing defence against this; downstream classification and routing should run on the extractor's `id`-grounded output, not on fresh Haiku reads of `state/lessons.md`.

**Operating rule.**

- Phase 2 fan-out scouts default to **Sonnet** (`general-purpose`), not Haiku, when the input is a raw `state/lessons.md`.
- Haiku may be used downstream of `bin/extract-lessons.py` when the input is the deterministic `extracted.yaml` (the grounding is already done; Haiku only echoes IDs and adds a routing field).
...
*Source: 2026-05-24 project-rag delta run; tasks/learn-lessons-2026-05-24/routing-manifest.md.*

Related: DR-152 (this decision); `snippets/text-only-recovery-preamble.md`; `skills/learn-lessons/SKILL.md` § Phase 2 — Fan-out scouts.
