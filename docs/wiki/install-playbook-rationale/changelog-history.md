# Changelog history — releases before v2.7.0

**What this is.** The pre-consolidation release history of coordinator-claude: every changelog
entry for **v2.5.1 and older**, moved here when `CHANGELOG.md` was slimmed down to the releases
an upgrader actually needs. Entry content, dates, version numbers, and the corrections and
amendments attached to individual entries are unchanged from the original — that record is not
a defect to be edited away. What changed in the move: internal pointers a public reader cannot
resolve (paths under private working trees, bare decision-record ids, commit SHAs, named private
sibling repos) were reduced to plain descriptive text, since this page ships in the public seed
...
Minor release. Headline change: centralize the `CLAUDE_HOME` / `$HOME/.claude` path-resolution definition as a load-bearing module that ships with `/coordinator:setup`, so peer-repo install scripts (the RAG plugin, example-game-repo, deep-research, future Python/TS/Rust consumers) consume one canonical resolver instead of inlining a precedence chain in each repo. Also trims four pre-existing skills/commands to fit the CI spec-line-count ceiling (>500 lines), and ships the ergonomic-substrate / eager-agent-calibration chunks (meta-ask preamble snippet + sync verifier + templates-mirror verifier + new wiki).

### Added

- **`coordinator/lib/claude-home/`** — new load-bearing module that resolves the `$HOME` analog (`CLAUDE_HOME → HOME → USERPROFILE → Path.home()`), `$HOME/.claude.json`, `$HOME/.claude/`, `$HOME/.claude/machine-local/`, `$HOME/.claude/plugins/`. Ships as `bin/claude-home` (CLI), `bin/_claude_home.py` (Python module + CLI; importable for path resolvers AND atomic JSON read/write of `$HOME/.claude.json` with BOM-tolerant read + JSONDecodeError enrichment), and `bin/claude-home.cmd` (Windows shim). Co-located README and 17-test stdlib-`unittest` suite (no pytest dep). Installed to `$HOME/.claude/bin/` by `/coordinator:setup` Phase 3 Step 3. The `lib/<module>/` location signals "cross-repo contract surface, do not customize." Peer repos retire their local copies (e.g., the RAG plugin's own `scripts/_claude_config.py`) by shelling out to `claude-home` or importing `_claude_home`.
- **`docs/wiki/hook-best-practices/machine-local-registry.md § 4a`** — new doctrine section: filesystem-layout invariant (`.claude.json` and `.claude/` are SIBLINGS under `$HOME`, never nested); resolution-order precedence with rationale for why `CLAUDE_HOME` ranks ABOVE `HOME` (unlike `MACHINE_LOCAL_<KEY>` env vars which rank BELOW the registry); generic `read_config` / `write_config` JSON I/O surface; alignment policy naming the RAG plugin as the canonical consumer to retire its inline copy.
...
- **`coordinator/lib/discover-working-repos.sh`** — new helper encapsulating `/coordinator:setup` Phase 2 Step 4 working-repos discovery (Tier A: `$HOME/.claude/projects/` activity record; Tier B: common dev-folder layouts). Tier C interactive prompt remains in `setup.md`.
- **`coordinator/lib/workweek-trail-scope.sh`** — new helper encapsulating the `/workweek-complete` Step 7 prelude logic (parses the week-changelog header file under `state/week-changelog/`, globs the JSON records under `state/review-trail/`, computes `reviewer_scope = unreviewed_week_SHAs ∪ cross-segment-seam SHAs`, writes a weekly reviewer-scope file back to `state/review-trail/`). Subprocess-only, fail-loud, spec-backlinked. Same shape as `install-substrate.sh`.
...
- **`coordinator/docs/wiki/dispatching-parallel-agents/eager-agent-calibration.md`** — new wiki capturing the design-as-offers ethos: agent-facing tooling defaults to offer-shape (lead with the better alternative), not nag-shape. Referenced from `$HOME/.claude/CLAUDE.md § Implementation Standards`.
...
- **`skills/learn-lessons/SKILL.md`** — 593 → 499 lines. Prose compaction (merged duplicate Anti-Patterns entries, condensed the Director-of-Engineering-adjudication rationale, tightened Local-mode auto-apply bounds). All five modes-and-cadence rules, the four-check gate, the routing schema, and the Change-Kind taxonomy intact.
...
### Also in 2.2.0 — earlier `coordinator_whoami` + `$HOME/.claude/machine-local/` substrate work

Closes incompleteness in the 2026-05-19 `coordinator_whoami` + `$HOME/.claude/machine-local/` migration that the RAG plugin's UE-addon dogfood doctor invocation surfaced as cross-team friction.

#### Added

- **`docs/wiki/install-playbook-rationale/coordinator-doctor.md`** — wiki-shaped agentic-steps doctor (explicitly NOT a slash skill) with nine probes covering machine-local registry and `coordinator_whoami` substrate. Downstream plugin doctors (example-game-repo, the RAG plugin, and its UE addon) cite this wiki as the canonical health-verification surface; reinvention is named as a doctrine violation. Cross-team directives at Chunk 1 §5 bind: (a) coordinator-substrate probes MUST use delegation or augmentation shape; (b) binding-health classification MUST cite P-6 (live whoami) not P-7 (config-presence file read). Reviewed standalone by the Director of Engineering 2026-05-20 (Director-of-Engineering altitude, cross-team seam).
...
- **`docs/wiki/hook-best-practices/machine-local-registry.md`** prose aligned to template ground-truth: `repos.example-game-repo` → `repos.example_game_workbench_repo` across §4 (naming + shell examples) and §8(f) anti-pattern. Template (`registry.toml.example`) is operative; the wiki now matches.
- **`docs/wiki/install-playbook-rationale/percolate-setup.md` Step 2** default-registers via `machine-local set publish.targets.<name>`; the legacy `publish-targets.sh` path is reachable only via `--legacy` flag.
...
- **`/session-start` orientation health-check** now invokes `python3 -m coordinator_whoami.project_rag --human` (gated on `coordinator_whoami` importability) as a spot-check of the coordinator/RAG-plugin binding. Cites `coordinator-doctor.md` P-6.
...
- **The RAG plugin's doctor skill — PLUGIN_ROOT discovery** — prepended a `machine-local get repos.project_rag` lookup; a hardcoded machine-specific candidate removed from the fallback loop.
- **`coordinator_whoami/project_rag/cli.py:270` HTTP-transport classification** — `_probe_claude()` now accepts `entry.get("url")` alongside `command` and `args`. Post-multi-RAG MCP daemon entries (HTTP transport, no `command` field) no longer mis-classify as `"broken"`. Mirror of the RAG plugin's UE-addon F4 finding from the same dogfood run.
- **`docs/wiki/hook-best-practices/wiring-env-source-of-truth.md`** — `status: current` → `status: deprecated`, `superseded_by: machine-local-registry.md`, with a prominent deprecation banner naming the dogfood-friction trigger. Wiring.env retirement is the worked precedent for the broader `$HOME/.<project>/` → `$HOME/.claude/<project>/` migration pattern.
- **`docs/wiki/addon-protocol/authoring-an-addon.md:118`** — `required_env` row now flags the RAG plugin's legacy per-user wiring-env file as transitional/deprecated and names `$HOME/.claude/machine-local/project_rag.toml [env]` as the canonical successor for future addon authors.
...
- **`/session-start` `<plugin-cli-path>` literal placeholder** — resolved with an inline `$HOME/.claude.json`-parsing snippet sourced from `commands/workday-start.md` Step 3.6.
- **Global `CLAUDE.md` line 16 parenthetical** — described `publish-targets.sh` as "(machine-local)" which conflated the legacy file with the canonical `$HOME/.claude/machine-local/` registry. Reworded to "(per-machine legacy file, superseded by `$HOME/.claude/machine-local/`)".

#### Migration

No breaking changes. Operators with `$HOME/.claude/machine-local/` already populated need no action. Operators on a fresh install: `/coordinator:setup` Phase 3 lays down the substrate; populate `registry.local.toml` with sibling-repo roots. Verification: run the nine probes in `coordinator-doctor.md` §3 (on Windows Git Bash, substitute `py -3` for `python3`).
...
- **Plugin-bundled wiki convention** — when a plugin file cites a wiki guide, the guide MUST live inside the plugin at `<plugin-root>/docs/wiki/<name>.md`. Project-level wikis (architecture atlas, codebase-specific) stay in the consumer's `$HOME/.claude/docs/wiki/`.

### Changed

- **Mandatory end-of-run Sonnet code review in mise-en-place Phase 6.** `/mise-en-place` now requires a minimum-Sonnet review on the cumulative diff before declaring the run complete; fires in both standard and hibernate modes.
...
- **Bare-slash skill invocations standardized for 24 coordinator skills/commands.** `name:` frontmatter doctrine captured in `docs/wiki/skills-corpus/writing-skills.md`.
...
- Dropped a hardcoded peer-repo mount-path list from `staff-eng.md` routing note (Step 2c MEDIUM).
...
- **`coordinator:writing-plans` → `coordinator:plan`.** Skill body refactored from prose to decision-tree super-skill (Branch A triage / B substrate / C compose / Exit). Long-form doctrine extracted to `docs/wiki/planning/writing-plans.md`. Existing `Skill(coordinator:writing-plans)` invocations break.
...
- Accumulated `$HOME/.claude/projects/<slug>/memory/sessions/` data dirs can be deleted; nothing reads from them.
