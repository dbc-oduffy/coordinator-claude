...
| `[info] <plugin>: copy_install — no version.txt sentinel (installer did not write one; see example-game-repo memo)` | Honest degraded state; no sentinel yet; see Known Limitation #1 | 0 |
...
- Runs `refresh_cmd` from `source_path` (e.g. `bash scripts/install-control-plugin.sh --allow-standalone --no-enable`, from the `example-game-workbench-repo` source repo), with a snapshot + REPLACE-semantics rollback on failure. Git-state and venv-state legs are skipped entirely.
- The coordinator does **not** hardcode the install invocation: the example-game-repo trio refuses bare standalone component installs (`EXAMPLE_GAME_REPO_UMBRELLA_INSTALL=1` gate), reachable only via the forwarders' `--allow-standalone` passthrough (and the docs component has no forwarder). The correct command is installer-internal knowledge, so it lives in `refresh_cmd`. `--no-enable` bypasses `enable_plugin.py` (the `settings.json` lock).
- **No `refresh_cmd` registered → refresh prints the manual path (`/example-game-repo:install` or the per-component forwarder) and exits non-zero.** It never guesses an invocation or silently no-ops.

Takes `<plugin>` name from the `plugin.mirrors.*` registry in `~/.claude/machine-local/registry.local.toml`. **Default (non-interactive) atomic path: idempotent and resumable** — re-running converges to the same fully-refreshed state. The `--interactive` partial path is the one deliberate non-idempotent escape hatch: HEAD is left behind when any file is skipped, producing a MIXED live-install state. The convergence step is a subsequent plain `refresh-plugin-live-install.py <plugin>` (non-interactive), which re-applies the full atomic checkout and restores the idempotency invariant. **Never auto-applied** — operator runs it after `check-plugin-drift.py` flags drift.
...
**Why `refresh_cmd` and not a direct `install-plugin.sh` call.** The example-game-repo trio refuses bare standalone component installs via the `EXAMPLE_GAME_REPO_UMBRELLA_INSTALL=1` gate; the correct command is installer-internal knowledge. A registry-supplied `refresh_cmd` keeps the coordinator generic and routes around umbrella gates without coordinator knowing about them. Dogfood-proven: both refresh-success and refresh-failure paths verified in a live round-trip.

**No `refresh_cmd` registered → refresh prints the manual path (`/example-game-repo:install` or the per-component forwarder) and exits non-zero.** Never guesses or silently no-ops.

`track_ref` and `dist_name` do not apply to `copy_install` entries.

`propagation_mode = "source_is_live"` applies to self-install plugins (coordinator-claude itself, installed over `~/.claude/`). `check-plugin-drift.py` treats these as structural no-ops — there is no separate live checkout to diverge.
...
than a git checkout. The canonical example is the `example-game-workbench-repo` trio (`example-game-repo`,
`example-game-repo-control`, `game-dev`), installed via `scripts/install-plugin.sh` from the example-game-repo
source repo.

### SHA-sentinel — why not content-diff?

The installer writes a 40-char source HEAD SHA to `<live_path>/version.txt` at copy time.
The probe compares this sentinel to `git -C <source_path> rev-parse HEAD` (after first
verifying that `git -C <source_path> rev-parse --show-toplevel` resolves to `<source_path>` —
see guard rule below). No network fetch — the local HEAD is the comparison target, since these
plugins develop on `work/*` branches ahead of `origin/main`.

**`git -C <dir>` silently escapes to the enclosing repo when `<dir>` has no nested `.git`.** When `<dir>` is not its own work-tree root, `git -C <dir>` climbs to the nearest ancestor that IS a git repo and reports THAT repo's HEAD/status — producing a false drift verdict with no error. Guard: before trusting any `git -C <dir>` output, verify `git -C <dir> rev-parse --show-toplevel` equals `<dir>`. If it doesn't match, abort the probe and surface a clear error rather than a phantom drift signal. This guard is required on every drift/health probe that receives a directory path from external configuration (registry, env var, CLI arg). *Source: queue, claude-central.*
...
1. **SHA-sentinel catches committed drift only.** Uncommitted source edits are invisible:
   ...
   lags behind. Acceptable: a 3-day-drift incident was committed
...
3. **Clean-install reproducibility** depends on the example-game-repo installer self-registering
...
4. **`rm -rf` restore guard.** The post-flight restore guard is hardened to check that `LIVE_PATH` is a resolved-physical-path under `$PLUGINS_DIR` (not a self-compare). This matters because `refresh_cmd` is operator-controlled registry content — the trust model was confirmed: `refresh_cmd` adds no privilege-escalation beyond the ability to write the registry. The restore itself is a true REPLACE (`rsync -a --delete`), not an additive copy — this prevents orphan files left behind by a failed install from corrupting the next refresh's state. <!-- src: plan10-053 -->

5. **Installer `--help` header is stale.** The example-game-repo installer's canonical-name header text omits `example-game-repo` from its list of recognized plugin names, even though `example-game-repo`, `example-game-repo-control`, and `game-dev` are all verified-valid `copy_install` arguments (cross-checked against `marketplace.json` plugin names + aliases). Cosmetic only — does not affect actual install/refresh behavior. <!-- src: plan10-075, plan10-052 -->

---

## Live-Ahead-of-Source Drift — the Outward-Only Corollary
...
**Sync tooling must not encode the banned direction.** `verify-templates-setup-sync.py --fix` copied LIVE→template (template tracks live) — the exact inversion the outward-only doctrine bans; running it would clobber template fixes with stale live bytes. Drift-sync tooling must be **direction-aware**: expose the sanctioned template→live reinstall as a first-class verb, and make the drift *check* direction-aware rather than assuming live is canonical.

A broader periodic audit is worth running: enumerate `~/.claude/` live files with uncommitted drift from their committed templates — this one was found by accident. See also `Directionality Verification Before Back-Propagation` below (the read side of the same asymmetry: never back-propagate before classifying which end is ahead).

---

## Drift-Guard Mechanism — Content-Hash, Not mtime or Presence

The SHA-sentinel primitive encodes a general rule for **any** materialized-from-source artifact guard: gate on a content hash of the *source*, never on a weaker proxy. Two proxies recur and both are false-negative machines:
...
## Why R-9(b): `check-plugin-drift.py` at Deployment Time, Source-Side Tripwires at Authoring Time
...
`check-plugin-drift.py` catches **deployment-time skew** — the live checkout is stale, or the venv hasn't been refreshed since `pyproject.toml` changed. It fires at the runtime level.
...
- `docs/wiki/hook-best-practices/machine-local-registry.md § plugin.mirrors` — registry schema and value-writing discipline; § 12 copy_install subsection
- `docs/wiki/addon-protocol/addon-health-sentinel.md` — health sentinel convention; design contrast documented above
- `docs/wiki/install-playbook-rationale/plugin-identity-and-health-sentinels.md` — scanner-is-reader-never-writer rule
