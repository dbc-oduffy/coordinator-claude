...
related:
  ...
  - docs/wiki/install-playbook-rationale/live-install-drift-audit.md
  - docs/wiki/install-playbook-rationale/install-surface-completeness.md
  - docs/wiki/cross-repo-communication/cross-repo-handshake-doctrine.md
keywords:
...
match is treated as a malformed sentinel by `check-plugin-drift.py` (which emits
...
**Doctor surface for absence.** `check-plugin-drift.py` emits `[info] <plugin>: copy_install —
...
run against `source_is_live` plugins (`check-plugin-drift.py` treats them as structural no-ops).
...
classifier gives them the diff and lets them decide; it cannot tell them "the published version says
the same thing in a better structure — your edit is still valid, you just need to rebase it."
For skills with heavy consumer customization (e.g. `skills/workstream-start/SKILL.md` with project-specific
...
**Partially realized (`/coordinator-update`).** The OSS-only `/coordinator-update` skill
...
continuity surface** (a `tasks/` directory, a `handoffs/` file, a memory entry, an entry under
`lessons/`) — even once — formalize the registry. The registry shape is: a machine-readable list of
...
changing the normalization semantics), review the same section in `check-plugin-drift.py:134-215`
...
carries the same cross-tool discipline note pointing back at `check-plugin-drift.py` as its
...
the existing precedence is accepted and the polarity surface is kept to three values. A genuine `ambiguous`
case is one where the operator's "polarity opposite of the bucket label" reading is content-semantic
...
bucket label alone is the footgun this disambiguation closes. Consumer scripts that
...
(`docs/wiki/test-design-discipline/round-trip-contract-tests.md`). The consumer-side leg (a real consumer recovery
...
2. If you run a `copy_install` script that copies from a source repo: invoke
   ...
   copy step. This is the sentinel the drift probe (`check-plugin-drift.py`) and the classifier
...
## Trust-Guard Sourcing — Source From Your OWN Location, Never the Root You Validate

*[universal]. Source: v3split-01 promotion.*

A trust-guard (e.g. `cc-root-source-guard`) validates that a plugin/install root is the genuine one
before trusting code under it. When a duplicated *inline* trust-guard is promoted into a *sourced*
function, each call site MUST source that function via a `BASH_SOURCE`/`dirname`-relative path — its
**own** install location — NOT via the `CLAUDE_PLUGIN_ROOT` / `_cc_root` value the guard is about to
validate. Sourcing the guard *from* the root it is meant to validate is **circular trust**: a
malicious root simply supplies a fake guard that returns "valid," and the check is defeatable by the
very thing it exists to catch.

**Negative-spec (load-bearing).** A presence-scan (grep for "the guard is sourced at every call
site") does NOT catch this — the promotion that shipped this bug had the guard sourced at all 29 lib
source-lines; only a **functional net** (actually source + call each site and observe the check
fire) caught it. The failure is invisible to static inventory because every call site *looks*
guarded; the defect is *where the guard comes from*, not whether it is present.

**How to apply.** Add a source-trust lint battery that greps every call site and asserts the guard
is sourced via a `BASH_SOURCE`-relative path (its own dir), and fails loud on any call site that
sources it via `CLAUDE_PLUGIN_ROOT`/`_cc_root` or any variable derived from the root under
validation.
...
- [`cross-repo-handshake-doctrine.md`](../cross-repo-communication/cross-repo-handshake-doctrine.md) — § Carve-out:
