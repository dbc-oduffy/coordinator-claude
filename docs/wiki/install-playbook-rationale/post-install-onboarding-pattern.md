...
- **Install-chain link:** the tail of the setup skill. Offer-shaped per [design-as-offers](../dispatching-parallel-agents/eager-agent-calibration.md) — lead with the invitation ("want a tour?"), not a nag. Skip under any `--check-only` mode.
- **Wiki home:** the repo's `docs/wiki/install-playbook-rationale/getting-started.md` (or reference-docs equivalent), registered in the wiki index. If the repo percolates to a publish target, confirm the wiki dir ships.
...
- [`eager-agent-calibration.md`](../dispatching-parallel-agents/eager-agent-calibration.md) — design-as-offers: the offer-shape (not nag) discipline the install-chain link must follow.
- [`cross-repo-communication.md`](../cross-repo-communication.md) — seeding this pattern to sibling repos is doctrine seeding (doctrine-repo altitude); wiring a specific sibling's install chain is that EM's install-surface work, relayed via the PM.
- Hard-dependency reference plan (sibling repo): `2026-05-23-ue-flavored-guided-tour-wrap.md` under project-rag-ue-addon's `docs/plans/` — the example-game-repo/UE-addon application that surfaced the leaf-onboarding variant.
- [`agent-install-contract.md`](./agent-install-contract.md) § Install-spinoff layer — the **install-execution** counterpart to this **onboarding-tour** doctrine. This wiki governs the leaf-fires-the-tour rule (suppress double-offers); the contract's install-spinoff layer governs how a multi-repo *install itself* is sequenced durably across the coordinator reboot: coordinator onboarding is the handoff, each queued downstream repo is a `kind: spinoff` baton in the machine-shared install-baton rendezvous (at `$(coordinator-settings-home)/state/handoffs/`, inline fallback `${CLAUDE_HOME:-$HOME}/.coordinator-claude-settings/state/handoffs` — see `state-placement-law.md`), stitched onto one install-chain spine. Tour = "what now?"; install-spinoff layer = "the install is still coming, here's the tracked chain." <!-- (the Director of Engineering review, item 3) repointed off the engine-central `coordinator-state-root.py --central` idiom — that root is a distinct rendezvous (see state-placement-law.md taxonomy); the install baton lives at settings-home, not the engine repo. -->
- **`surface-unfinished-onboarding.sh` — retired, no boot-time replacement.** The SessionStart hook once described here (a `startup`-matcher hook that detected installed-but-not-onboarded via `coordinator-setup-state.py check orientation_completed` and auto-seeded a `/pickup` nudge on the operator's next restart) was deleted as part of the hooks-gutting directive: SessionStart hooks were pared to the single fast synchronous orientation-cache injector (`project-orientation.py --lightweight`) for Windows boot-performance reasons — see `coordinator/hooks/hooks.json` (only one `SessionStart` entry survives, matcher `startup|clear|compact`). There is no auto-surface-on-boot replacement; the loop this pattern describes now closes only **within the install session itself**, not on a later restart — `/coordinator:install`'s own Phase 7 status table and terminal-message gate (`commands/install.md`) check `orientation_completed` and refuse unconditional success language until the tour has run or been explicitly skipped. An operator who restarts before completing the tour gets no automatic nudge; they pick the tour back up manually (e.g. via `/pickup` against whatever install-baton handoff was seeded, or by re-running `/coordinator:install`) rather than being auto-prompted at next boot.
- [`produce-not-prescribe.md`](../doctrine-authoring/produce-not-prescribe.md) — setup-class skills produce minimum-viable substrate; this doctrine is about the *conversation*, that one is about the *substrate*.
