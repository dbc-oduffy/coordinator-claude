...
- `docs/wiki/install-playbook-rationale/percolate-setup.md` — percolation pipeline where hook scoping first mattered
- `docs/wiki/install-playbook-rationale/plugin-extraction-and-distribution.md` — publish pipeline (the engine repo's `coordinator/bin/publish.py`) that drives the rsync invocations
- `docs/wiki/install-playbook-rationale/agentic-install-integrity.md` — classifier, sentinel, and three deferred extensions including §3 (the new-file mutation carve-out this section generalises)
