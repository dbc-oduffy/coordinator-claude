...
**Rule.** The compress step is two-phase, never one: (1) confirm-or-fold — grep the target wiki for the rule; if absent, fold it there *first*; (2) only then compress the inline prose to the pointer. Pairs with § Verify structure-preserving compression via count invariants — the integrity check there is "did I drop a pointer"; the integrity check here is "does the pointer's target exist." (Source: coord-meta, 2026-06-09.)
