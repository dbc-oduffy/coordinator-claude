...
# Source lib to get cs_compute_machine_live (env→hostname, no registry read).
...
if ! machine-local has coordinator.machine_slug 2>/dev/null; then
  source "${CLAUDE_PLUGIN_ROOT}/lib/coordinator-daily-branch.sh"
  _slug="$(cs_compute_machine_live)"
...
# Source lib to get cs_compute_contributor_live (env→user.email, no registry read).
...
if ! machine-local has coordinator.contributor_slug 2>/dev/null; then
  source "${CLAUDE_PLUGIN_ROOT}/lib/coordinator-daily-branch.sh"
  _slug="$(cs_compute_contributor_live)"
