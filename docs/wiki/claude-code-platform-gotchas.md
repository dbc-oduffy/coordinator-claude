...
A SessionStart hook that wants to inject context into the session must write it to **plain stdout** — the `{"hookSpecificOutput": {"additionalContext": …}}` JSON envelope is honored only for `PreToolUse` (where `permissionDecision` lives). A SessionStart hook emitting the `additionalContext` envelope gets nothing surfaced; design SessionStart hook output as raw stdout text. (Source: 2026-06-01, claude-central.)
...
*2026-06-17, project-rag cross-platform staleness-hook.* [universal]
...
### Microsoft Store Python is sandboxed — `lib/resolve-python.sh` actively rejects it

On Windows, `%LOCALAPPDATA%\Microsoft\WindowsApps\python*.exe` are symlinks into `C:\Program Files\WindowsApps\PythonSoftwareFoundation.Python.3.*\` — the Microsoft Store install. It's sandboxed: filesystem and registry writes are redirected to per-package virtualized locations, `pip --user` lands in a hidden tree, package permissions are inconsistent. Always prefer the python.org installer at `%LOCALAPPDATA%\Programs\Python\Python3*\` (per-user) or `C:\Program Files\Python3*\` (system).

`lib/resolve-python.sh` handles this for any code that sources it:
1. Directly probes python.org install dirs first, windowless variant preferred (no PATH dependency).
2. Falls back to `command -v` PATH lookup but **rejects any match whose absolute path contains `WindowsApps`** (`_resolve_python_is_store`).
3. Falls back to the `py`/`pyw` launcher with `-3` (the launcher reads PEP 514 registry, which prefers python.org).
4. Seeds PATH with the resolved interpreter's directory so child processes that re-resolve by bare name (`subprocess.run(["python3"])`, `uv`, `pip`, venv shims) find the same install.

**Symptom when the resolver isn't used:** an agent shell whose ad-hoc script calls bare `python3` triggers Windows' `"Select an app to open 'python3'"` file-association dialog (popup with Figma/Firefox/Notepad/etc.). This happens when the agent's PATH doesn't contain a working Python directory and Windows falls through to ShellExecute. Fix at the source: source `resolve-python.sh` and call `"$PYTHON_BIN" "${PYTHON_ARGS[@]}" ...`. Observed 2026-06-16.

**Known gap — on-demand `bin/` tools that self-resolve.** Several `bin/` scripts (e.g. `verify-calibration-sync.sh`, `verify-docs-checker-sync.sh`) define their own `PYTHON_BIN="$(command -v python3 || command -v python || true)"` inline and do not source `lib/resolve-python.sh`. These callers bypass Store Python rejection and PATH seeding. They should be updated to source `lib/resolve-python.sh` instead.
...
> **WARNING: this minimal snippet does NOT reject Store Python or seed PATH. For production callers, source `lib/resolve-python.sh` (see preceding section).** The snippets below are retained as documentation of the pattern's history and for contexts where sourcing the lib is not feasible.
...
**Cross-repo resolver shape.** Shape β (shared runtime lib at `lib/resolve-python.sh` inside the coordinator plugin) is now the live deployment — sourced by hooks, lib helpers, and several bin scripts. Shape α (vendor per-repo) is deprecated for coordinator-internal scripts. DR-059's deferral is superseded.
...
**Fix:** `coordinator:install` Step 3 installs `machine-local.cmd` and `python3.cmd` shims on the Windows user PATH, and Step 3c runs a health check (detect orphan reparse-point stubs, detect Store-alias-on-PATH, detect missing Python). The shims are found by `CreateProcess` before any `ShellExecute` fallback fires.
...
The Claude Code harness prepends every installed plugin's `bin/` dir to PATH for tool/hook execution (verify: `echo "$PATH" | tr ':' '\n' | grep -i claude` shows each `plugins/*/bin` — that is the harness-provided guarantee). Note: `~/.claude/bin` is NOT harness-injected; it appears in the PATH grep only on Windows because `install-substrate.sh` Step 3b writes it there, and is absent on macOS/Linux. The harness `plugins/*/bin` injection is cross-platform — so it reproduces on every machine running the plugin, the strongest install-surface guarantee we have. A coordinator script (`fan-out-dispatch.sh`, `coordinator-safe-commit`, `check-shipped-on-main.sh`, …) therefore resolves by **bare name from any cwd, in any repo**.
...
*2026-06-15, coordinator session.* [universal]
...
Source: platform-gotcha hardening 2026-07-01, claude-central.

### An agent executing runbook prose collapses per-dep loops — needs an explicit anti-collapse caution

*2026-06-17, example-game-workbench-repo install chain.* [universal]
...
Alternatively, switch to the Bash tool for the call and use a POSIX heredoc. (Source: 2026-05-24 example-game-workbench-repo)
...
Source: central improvement queue (2026-04-29, example-game-workbench-repo).
...
**Why static grep fails here:** SessionStart hooks run via `sh -c`; a transient or since-repaired hook (such as a `.git/hooks/post-commit` before `coordinator-ensure-post-commit-hook` rewrote it) can fire an error that the current on-disk config no longer contains. Static grep proves "not in current config," not "didn't fire at the observed startup."

Empirical basis (2026-06-17, BettaAir): a macOS session showed `SessionStart:startup hook error … /bin/sh: powershell.exe: command not found` (×2). An exhaustive static grep of every enabled plugin's `hooks.json`, every referenced script, and all settings layers found every `powershell.exe` call site correctly platform-guarded. A fresh `claude --debug hooks --debug-file /tmp/h.log -p ok` reproduced no error and named the culprit immediately. The static sweep cost ~20 tool calls and never did.
...
Empirical basis (2026-06-18, BettaAir plan-persistence-hook-automation C0, Claude Code 2.1.170).
