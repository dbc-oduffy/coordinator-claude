...
### Same-repo concurrent-session coordination is not cross-repo-memo

A concurrent EM session in the **same** repo and branch is a shared-bus problem, not a cross-repo one: `cross-repo-memo` resolves the sender's own id as the receiver (a self-send), and `SendMessage` does not reach a session that is not a registered peer with its own identity. Coordinate via the shared handoff plus PM-relay instead, and record any owed cross-EM follow-on as a tracked **DIVERGENCE** entry in your own plan doc rather than editing the sibling session's owned files directly — the same-repo case still has an ownership boundary and a concurrent-edit collision risk even though there is no repo boundary to route a memo across.
...
9. **An upstream's worked example is parameterized — identify which profile/config produced it before citing it as your own risk.** An upstream docstring example ("lexical=0.5, facet_signal=1.0 → composite 0.65") can be arithmetically correct yet computed under a *different* profile's weights than yours — citing it as justification for a constant on your own side inherits the wrong risk if your profile's weight differs enough to change whether the scenario ever clears the floor. Re-derive the arithmetic at your own parameters before citing an upstream worked example as a risk justification, and show the re-derivation in the comment so a reviewer can catch a mismatch.
...
### Cite sibling-repo code by symbol plus verbatim quote, not line number — and re-verify at the wave boundary

A line-number citation into an actively-developed sibling repo is stale by construction, not by bad luck. *(Canonical: a verified `file:line` citation into a sibling repo went stale within hours when that repo landed a fix above the cited line — the citation still resolved to a real line, just the wrong one.)* This generalizes the fleet's own "cite by enclosing function, not line number" convention (claude-klabauter's `CLAUDE.md`) to cross-repo citation practice: cite the enclosing symbol plus a verbatim quote of the cited text, and treat the line number as secondary, SHA-pinned decoration — never the primary locator.

**The fan-out corollary — per-chunk checks cannot see cross-chunk invalidation.** A `file:line` citation authored in one fan-out-wave chunk can be invalidated by a *sibling* chunk in the *same* wave, and every per-chunk check still passes, because each chunk's own verification runs before the other chunks have landed. Re-verifying a citation at authoring time only proves it was correct at that moment — it says nothing about what a sibling chunk does five minutes later. **Re-verify every wiki `file:line` citation after ALL a workstream's chunks have landed, not at authoring time.** Prefer by-name (symbol) citations over line numbers wherever the choice exists, date any line number you do keep, and treat the wave boundary — not chunk-authoring time — as the only vantage point from which cross-chunk collisions are visible at all.
...
**Both EMs across a cross-repo diagnosis session reasoned correctly from their own half of the evidence and reached wrong conclusions repeatedly.** One session produced a false swallowed-report claim, a mismatched-vintage comparison, a wrong-mirror verification, and a misclassified duplicate defect — four distinct wrong conclusions, each internally consistent with the evidence its author could see. Every correction came from handing over something falsifiable — a verbatim log line, an `ls`, an mtime — never from a theory of the other side's code. **Send "here is what I can see that you cannot," never "here is my theory of your code."** A diagnosis about a sibling's internals is a hypothesis regardless of how carefully it was reasoned; only the peer checking it against their own disk closes it. A peer's memo can also *inherit* a misdiagnosis from your own repo rather than originate one — a host's "consume our RAM reader, yours under-reads" memo was built on our own handoff's wrong premise, and running the peer's actual reader live (both returned ~326GB) disproved it and reframed the fix. Disproving an inherited-premise memo is running their code as a consumer, not reading it — reading confirms only that the premise is internally consistent, not that it is true.

**Peer agreement is not measurement — two sessions reading a silent instrument produce two confident readings, not one verified one.** Three times in one session, two EM sessions coordinating by message converged on the same wrong conclusion about a system neither was currently measuring: a killed workflow reported running normally on evidence that couldn't show otherwise, an endorsed "fix" that was both wrong and unbuildable, and a memo that went stale within hours because neither party re-measured before acting on it. Agreement between two sessions feels like corroboration, but when neither side has actually measured the thing in question, agreement only doubles the confidence of a guess — it does not verify it. **The agreement itself is usually the least-checked step in an exchange.** Before acting on a cross-repo consensus, ask which side actually measured the claim just now, on disk — if neither did, the consensus is two unmeasured guesses, not one confirmed fact.
...
- **On a co-located fleet, a sibling's *runtime* state is on disk too — test the limit before asserting it is unobservable.** The bullet above is usually read as "their code is readable"; it extends unchanged to their venv, their DB, their running gate, and their branch — all directly executable or queryable from the same box, not merely readable as source. Before drafting a memo asking a question the sibling's own disk can answer, run their gate yourself. *(Near-miss: a false "is the consumer down?" question nearly went out as a memo when the sibling's own health check was one shell call away and would have answered it directly.)*
- **A differently-named sibling branch on the same host is not evidence of a remote machine.** Branch names are per-repo, not per-host — a sibling checkout on the *same* box can carry a branch name that looks machine-specific (e.g. a per-machine naming convention) purely because that convention is scoped to the repo, not the host it happens to be checked out on. Verify the sibling's actual machine (not just its branch name) before standing down from cross-repo work on the assumption that the sibling is remote and unreachable.
...
## Onboarding a new repo doesn't make it a memo destination — register it explicitly

`coordinator:repo-setup` (single-repo) scaffolds the in-repo `cross-repo/inbox/` channel but does **not** write the machine-local `repos.<name>` entry or the `working-repos.yaml` row — so a freshly-onboarded repo is a receiving channel with no address: no sibling can `--to <name>-em` it. After onboarding any new repo, run `machine-local set repos.<name> <path>`, add the `working-repos.yaml` row, and verify with `cross-repo-memo --list-receivers`. This is a repo-setup gap, not a memo-CLI defect — the CLI correctly refuses an unregistered receiver; the gap is that onboarding does not register one.

### Summon an oversized repo's EM via a thin markdown-only clone

To reach the EM of a very large repo (e.g. a ~100GB UE game repo) without pulling the full checkout, a git partial + sparse clone of just its markdown resolves it cheaply:

```sh
git clone --filter=blob:none --no-checkout --depth=1 <remote> <dest>
cd <dest>
...
This yields tens of MB with all docs. Then set the machine-local registry pointer (`machine-local set repos.<name> <dest>`) so `<name>-em` resolves as a `cross-repo-memo` receiver and a session can `cd` there. **Caveat:** the registry key now points at a lossy partial checkout — fine for doc/summon work, wrong for anything downstream that expects a full tree; don't leave the registry pointed at the thin clone once the real clone is available.
...
**Flipping a memo's `status:` (or `deployment_state:`/`decision:`) via Edit must REPLACE the key, never append a new line.** The `old_string` passed to the edit has to capture and overwrite the existing `key: value` line in place. Adding a fresh `status:` line below the old one creates a duplicate-key frontmatter bug that silently defeats any `-m1`/first-match consumer — the inbox listers and claim tooling above read the *first* occurrence, so a duplicated key can leave a memo reading `open` to every lister while a later line says `in_progress`. Grep the key after the edit to confirm exactly one occurrence.

## `cs_action_memo`: `--decision`/`--decision-note` and `--actioned-note` are mutually exclusive

When actioning a memo via `cs_action_memo`, passing `--decision`/`--decision-note` together with `--actioned-note` in the same call fails ("mutually exclusive"). For a plain `fyi`/`consult` closure, use `--actioned-note` alone; `--decision`/`--decision-note` is for the `ask`/`proposal` accept-or-decline path specifically. Pick one shape per call rather than trying to combine them.
...
**The same risk widens on a long, multi-hour ceremony, not just at commit time.** A fresh cross-repo reply memo can land mid-run and be caught by a reviewer before the EM notices it — the longer the session, the more stale the inbox snapshot taken at session start becomes. Before finalizing any review or plan, re-scan `cross-repo/inbox/` for memos newer than session start; premise state drifts under you over the course of a long ceremony, not only between sessions. 
...
## Changing a two-sided ratified contract's key is never a producer-side checkbox

When a producer doc alters the key or anchor of a boundary that was ratified on **both** sides — e.g. a consumer's diff anchor pinned in a shared doc — that change routes via `cross-repo-memo` plus consumer acknowledgment. It is never a self-marked "done" in the producer's own acceptance trace. Mark such an item `PENDING-consumer-ack`, not done, until the receiver has actually confirmed the new key against their own read surface. A producer's confidence that the rename is trivial does not substitute for the consumer's own verification that their side still resolves the anchor.

## Develop-ahead against a peer's shipped-but-unmerged surface — flag-gate and verify at the seam

When a producer repo has **shipped** a surface on a work branch but not yet merged to `main`, the consumer can build ahead of the merge rather than blocking on it: flag-gate the consumer's use of the surface against the pinned (even partly-unbuilt) contract, mock the not-yet-installed CLI or module in tests, and make the peer's eventual merge a flag-flip rather than a build blocker. Cutover itself stays a separate, PM-gated step — develop-ahead shortens the wait for *building*, not for *shipping*.

## Pickup-reconcile scans the cross-repo inbox for in-flight version bumps before a plan claims a version number

A spinoff or pickup that assumes the next contract version number is free (e.g. an additive `v2.3.0 → v2.4.0` bump) can collide with a separate, PM-ratified, in-flight cross-repo bump to the *same* contract that is sitting unactioned in the consumer repo's `cross-repo/inbox/`. Two workstreams cannot share a version number. Reading the inbound-memo queue during pickup-reconcile — not just the handoff's cited files — surfaces the collision *before* planning, converting a mid-execution contract conflict into a clean PM sequencing decision (e.g. the colliding work becomes a follow-on release after the in-flight one). **Rule:** on any cross-repo consumer coupled to a versioned contract, the pickup premise-verification pass greps the inbound-memo queue for pending version bumps to that same contract.
...
- **A sibling's reply memo describes their PLANNED surface, not their shipped one — verify the shipped symbol before building a client against the memo's letter.** A spinoff spec faithfully quoted a sibling's own door-reply memo naming a "planned" call shape; by ship time the sibling had split it into a differently-named verb, deliberately, for a security reason the memo never mentioned — composing the memo's literal shape would have applied only a coarser-grained ACL check and reopened a leak class the sibling had engineered against. A memo is a point-in-time statement of intent; intent-to-implementation drift is normal, not sloppiness, especially when the sibling's own review reshapes the design after sending. Before building any client against a sibling's door, read the shipped symbol, its registration site, and its return-envelope constructor on their disk — treat the memo as the pointer to *where* to look, never as the contract itself. A handoff/spinoff citing a memo for an API shape inherits the memo as its premise to re-verify; a faithful transcription of a stale memo is still a stale premise.
...
**The reflex to skip this — "the ball must be with them" — is often itself unverified.** A PM directive to unblock a sibling was premised on a third repo's report that "the ball is with them," which that third repo itself had caveated as unverifiable ("I cannot see their unmerged branches"). Reading the sibling's tree directly showed they had already received the real unblock memo, rebased, reviewed, and held a PM-authorized ready-to-fire execution baton — the memo would have told a mid-flight team to start work they'd been ready to do for two days. The asymmetry to exploit: a sibling EM must caveat claims about *you* because they cannot see your tree, but *you* can read *their* working tree directly (git log, plan frontmatter, handoff `deployment_state`) — so a second-hand claim about a sibling's state is a hypothesis with a cheap local witness, even when it arrives already caveated by its source.

**Reading a sibling's tree is not by itself proof of landed state — a plain file read cannot distinguish a live editing buffer from shipped code.** Verifying a file "on disk" and verifying it "at HEAD" are different claims; a sibling's working tree can be mid-edit by a live peer session, and two reads minutes apart can see different content at the same path. Run `git -C <sibling-repo> show HEAD:<path> | grep` (what's actually landed) and `git -C <sibling-repo> status --short <path>` (whether it's dirty right now) before reporting cross-repo state, and say "committed in `<sha>`" or "uncommitted working tree" explicitly — never just "verified on disk." A reported-cleared gate can in fact be an uncommitted diff that changes between two reads as a live peer session edits it — the gap between "on disk" and "at HEAD" is exactly what a plain file read cannot see.
...
## A comment asserting a peer repo's state is a claim, not evidence — and it can stay false for months

A docstring or comment that justifies its own module by asserting what a **peer repo** does — "a peer change removes the call from X; this is the replacement, not an addition alongside it" — describes an intent that may never have landed. Nothing recomputes it, no test covers it, and it reads as settled fact to every subsequent reader, including a reviewer or an oracle built on that premise. When the assertion is false, both surfaces run, and the resulting bug looks like a race in whichever one you happen to be debugging.

Prefer a verifiable form: name the commit that made it true, or state the sequence ("removed on `<date>`; until then both fired") rather than asserting only the end state. An assertion with a SHA in it fails loudly on inspection; an assertion in the present tense fails silently, forever.

The generalization is broader than cross-repo: treat any comment asserting a fact about code you do not own — a peer repo, a sibling module, a registration in a config file — as a hypothesis to check against live disk before relying on it. The check is usually one grep, and it is worth running even when the claim turns out true, because you cannot tell the true claims from the false ones by reading them. A docstring can assert a peer engine ceremony stopped archiving a session's bookkeeping dir when the removal never actually landed — both paths still fire, and a live session's bookkeeping dir gets moved out from under it, resetting every fire-once sentinel that depended on it. Correct such a docstring by recording the sequence with a commit reference instead of asserting the end state.

## Inbound ask adjudication — own the design call, don't bounce a multi-shape choice back to the PM

A multi-shape inbound memo ask ("which of these fits best?") is engineering, not product. Synthesize a comprehensive shape grounded in this repo's own substrate — settled doctrine plus the current consumer set — and own the disposition, or decline with rationale if the sender's framing is wrong. Reserve PM-relay for genuine product/scope-fork decisions, not for laundering an EM-altitude design call through the PM because the memo happened to phrase it as a question. *(Source: universal.)*
...
## A sent memo can be answered reply-in-place in the RECEIVER's copy — check their archive, not just your inbox

Whether a sent memo was answered cannot be determined from your own `cross-repo/inbox/` alone. A receiver may action a memo *in place*: stamp `status: actioned` with an `actioned_note` pointing at an appended `## EM Response` section in **their own copy**, filed to **their** `cross-repo/archive/`. A reply-in-place never reaches the sender's inbox, so an inbox-only check on the sender's side reports a false negative — and can push toward re-sending or chasing a memo that was already answered.

**Rule:** to determine whether a sent memo was answered, check the receiver's copy at `<receiver-repo>/cross-repo/{inbox,archive}/<memo-basename>` and read **both** its frontmatter (`status`/`decision`/`decision_note`/`actioned_note`/`realized_by`) and its body for an appended response section — `status: actioned` with an `actioned_note` pointing at the body is a reply, not a silent close.

**Grep caution:** the literal phrase "see ## EM Response in body" can appear verbatim in the *frontmatter* (as the `actioned_note` value itself), so a naive `sed`/`grep` range anchored on `## EM Response` matches the frontmatter line and prints your own memo back at you. Anchor on `^## ` instead to reach the actual appended section. This is the sender-side counterpart to not re-nagging the PM about an already-sent memo — check the receiver's copy before reporting "no reply."
...
## A migration's notify list and its migrate list are complements, not the same set

A fleet migration usually splits repos into two groups: the ones whose records get physically migrated, and the ones deliberately left alone. The natural reflex when writing the follow-up notification is to send it to the repos the migration touched — they're the ones on the campaign's mind, and they're the ones whose diffs prove the work happened. That reflex sends the notice to exactly the population that no longer needs it.

The repo that needs the notice is the one holding **unmigrated** records against **migrated** machinery — it is the only place where the change is still latent, and it has no local diff to remind anyone. So the correct rule is: **notify = fleet minus migrated, or notify everyone.** Notify ⊆ migrated is always wrong, even though it always looks tidy.

**Second-order tell, worth catching in review:** an alias or compatibility rule declared "permanent" to make a half-migrated fleet survivable is a promise that must hold on *every* surface reading the aliased field — and it breaks first on whichever surface was written generically rather than domain-aware, since generic validators are where a domain-specific tolerance rule goes to die. A "permanent alias" ruling is a claim to enumerate the readers against, not a property the codebase acquires by ratification. Corollary for error text: a gate that rejects a legacy value should name the migration, not only the allowed set — an error listing valid values without the alias map reads as a wall when the fix is a one-line rename.

*Case:* a vocabulary migration's notification can go to only a subset of the repos actually touched by the migrated set, skipping a repo that then hits a hard claim-gate failure on a pre-migration value it was never told about — a sweep of every sibling can turn up many more unclaimable instances across every unnotified repo. The typical root defect: one kind-reading surface at the gate never canonicalises the legacy value, while the deny path and the cross-field rules both do — falsifying the "permanent alias" ruling precisely at the gate that mattered.
...
**Worked example.** A memo follow-up (stripping stale `[env]` from a tracked machine-local file) can be treated as blocked until a sibling's dependency ships on *their* `origin/main`, with an `awaiting_gate` handoff authored to track it — both the gate and the handoff are wrong under a coordinated simultaneous-publish model: nobody needs to land their change on `origin/main` first, since everyone publishes together at the same time. The sender's sequencing framing assumes single-repo-at-a-time cutover; under coordinated simultaneous publish, the recommended wait has no benefit.
