...
- ~~**BUT plugin-declared hooks do NOT fire under `--plugin-dir`, even on a fresh boot** (bug
  ...
  *only* delivery path in play on this machine: `~/.claude/settings.json` carries **no `hooks` key
  at all**, so settings.json cannot be delivering them. A registration added to `hooks.json` alone
  fired in six independent sessions. See § Hook REGISTRATION staleness for the measurement. The
  2026-07-04 FAIL is a correct record of what that build did; it is not a standing constraint.
...
## Disk-truth (this machine, ground-truth verified)
...
- Registry schema: `git` source → `installLocation` = a clone under
  ...
  **directly** (coordinator-claude, example-game-workbench-repo, example-custom-plugins all point at their source dir).
...
| | Guard **LOGIC** (the script body) | Hook **REGISTRATION** (the `hooks.json` entry) |
|---|---|---|
| Read when | every tool call — fresh `python3` process | **session start**, then cached in-memory |
| Reaches a running session? | ✅ immediately, no action | ❌ **no** — until `/reload-plugins` or a restart |
| Refresh path needed? | none, by construction | `/reload-plugins`, per session |
...
refutation, and it is a different mechanism** — a reader who collapses the two will resurrect a
refuted claim.

### The measurement

Method: registered a temporary `PreToolUse` matcher on `Read` in `coordinator/hooks/hooks.json`
(nothing else touched) pointing at a script that appends `session_id` to a log on every firing,
then compared sessions that booted before and after the edit. Edit landed ~14:45 local.
...
1. **Boot-read confirmed.** Every session that booted after the edit picked the registration up
   with no action at all.
2. **No mid-session re-read.** The editing session never saw its own change — not on the next tool
   call, not 21 hours later.
3. **`/reload-plugins` reloads registrations.** The harness reported `53 hooks` reloaded and the
   very next `Read` fired the probe. This settles the mechanism table above: no `may need restart`
   hedge applies.

**Removal is symmetric, and is the trap.** Reverting `hooks.json` on disk did **not** un-register
the probe in the reloaded session — it kept firing from the in-memory roster. A registration you
*delete* stays live in every running session exactly as long as one you *add* stays dead.

### Disposition — documentation and discoverability, not a propagation mechanism

Churn is the input that decides this, and it is high: `hooks.json` carries **46 registrations**;
of 62 commits touching the file, **51 changed the registration set — 50 of those in July 2026
alone** (~1.7 registration-changing commits per working day). The "edited twice a year, just
document it" disposition the spinoff offered as a legitimate outcome is **not** what the evidence
supports.

**Why not a detection surface (outcome 2), yet.** It was the leading candidate until the reload
answer came back positive: `/reload-plugins` works, so there is no *unfixable* divergence to
...
**Why not a pinned version-dependent observation (outcome 3).** The behaviour was not flaky. Seven
sessions, two calendar days, one build — boot-read and reload both reproduced every time, with no
ambiguous run. Recording it as "unreliable, re-probe each time" would be less true than the
statement now in the mechanism table.

**The residual worth knowing, because documentation is what discharges it.** `/reload-plugins`
heals *only the session that runs it*. Five sessions were live on this machine during the probe
window; each would have needed its own reload. So the operator-facing rule is not "reload after
editing `hooks.json`" but:

> **After changing a registration in `coordinator/hooks/hooks.json`, every live session on the
> machine keeps its booted roster until it individually runs `/reload-plugins` or restarts —
> including sessions belonging to other repos. Your own reload does not fix theirs.**

## Hook-delivery — SOLVED via settings.json external-abs-path (runtime-proven)

> **Re-probe first: the premise below does not hold on every host.** Everything in
> this section is a correct record of what was measured on 2026-07-04, and the settings.json
> external-abs-path mechanism still works. The *problem* it works around is host-dependent.
> Plugin-declared hooks in `coordinator/hooks/hooks.json` were observed firing normally via
> `--plugin-dir` on a POSIX host on 2026-07-28, on a machine whose `~/.claude/settings.json`
> carried an **empty** hooks block — zero registered commands — so settings.json cannot have been
> the delivery path. `pickup-autofire.py`, registered nowhere but the plugin's own `hooks.json`,
> fired; so did the Bash `cd`-prefix guard from the same file. Either #38699 was fixed upstream or
> it never covered this path.
>
> Treat "#38699 makes plugin hooks dead" as a **dated observation to re-probe**, not a standing
> constraint — it has already been cited as settled fact in a hooks.json comment and in a decision
> to build a second mechanism. The probe is cheap: read the live `settings.json` hooks block to see
> which delivery path is actually in play, and grep the session transcript for the hook's own
> output.
>
> Name the delivery mechanism in any finding about a dead hook — `--plugin-dir`, directory-source
> marketplace, or settings.json. A sibling repo independently measured a genuinely dead plugin hook
> (0 of 14 transcripts) and generalized it to "plugin hooks.json never delivers"; theirs loads via
> a directory-source marketplace, a different mechanism, and the generalization does not survive
> the counter-case above. A claim that does not name the mechanism cannot be confirmed or refuted
> by anyone else.

The `--plugin-dir` FAIL leaves plugin-declared hooks dead (#38699) — **as measured on 2026-07-04;
see the re-probe note above before relying on this.** This is the workaround, and it
...
> resolved cold. The real defense-in-depth floor for bootstrap-critical resolution is a rung with **zero
> executable dependency**, not merely an absolute-path invocation of one — an executable rung carries its
...
cold (fresh terminal, scrubbed PATH), not only the library in isolation.

## Dev-vs-OSS resolution — a committed sentinel, not coexistence-fail-loud

A resolver that must choose between a working-dev clone (this repo) and an installed OSS package can
legitimately find BOTH present on the same machine — that is the normal developer state, not an anomaly.
A naive both-present → fail-loud therefore regresses every launch on exactly the machines that hold both.
**Disambiguate with a committed repo-root sentinel** — `.coordinator-dev-repo` — as the authoritative dev
signal, placed OUTSIDE the publish `SOURCE_DIR` so it is structurally absent from OSS byte-copies and
cannot leak into a published package. Reserve fail-loud for the genuinely-ambiguous case: an unmarked
candidate coexisting with the OSS install.
