...
*Post-ship dirty scratch files surface as untracked noise once the real outputs (archived specs, deleted scaffolds, wiki updates) have already shipped. Tracking scratch then deleting it one commit later is obvious post-hoc noise. PM call: scratch artifacts are not useful post-ship.*
...
This wiki governs *skill-emitted working-notes scratch* — a skill's own scratch dir, self-cleaned by the emitting skill at the end of its own run (Pattern A/B above). It does NOT govern *provisioned subagent sidecars* under `state/subagent-share/` (review-findings, staff-eng-review, assessment, run-report sidecars — the plan-pipeline lens sidecars live at `.coordinator-local/plan-sidecars/` and are UNREAPED BY DESIGN) — those are a cross-session artifact type with their own typed, cadence-wired reaper (`bin/reap-stale-subagent-sidecars.py`, claude-klabauter-resident), gated on session liveness AND/OR an age floor rather than same-skill self-clean. See `state-placement-law.md § Delete-by-Convention Reap Doctrine` for that rule — it generalizes this wiki's core principle ("content already folded into a durable artifact is noise once shipped") to the provisioned-sidecar case.

## Cross-references

- [`cleanup-sweep-hazards.md`](../bug-blitz-residue/cleanup-sweep-hazards.md) — sibling failure modes for cleanup operations.
- [`writing-skills.md`](../skills-corpus/writing-skills.md) — when writing a new skill that emits scratch, name the cleanup phase explicitly.
- [`scoped-safety-commits.md`](../concurrent-em-git-operations/scoped-safety-commits.md) — explicit-path commits keep scratch out of unrelated commits even when it does end up tracked.
- [`state-placement-law.md § Delete-by-Convention Reap Doctrine`](./state-placement-law.md) — the parallel reap rule for provisioned `state/subagent-share/` sidecars.
