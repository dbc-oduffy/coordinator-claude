...
> the upstream doctrine meta-repo) gets a working self-improvement loop on their own installed copy — entirely within
> their own repo, with zero dependency on that meta-repo's central machinery — and why that loop is deliberately
...
`state/`; (b) that gives them a genuine self-evolution loop; (c) the upstream improvement queue is
fleet-private, does not ship to them, is not readable by them, and their local lessons will never reach
upstream. The last point is a PM-ratified accepted asymmetry, handled by periodic release discipline — **not** a feedback channel.
...
touches `~/.claude/state`; nothing depends on the upstream meta-repo or its engine repo being present.
...
- **Local doctrine edits** — "helping the consumer evolve their own setup" is not a separate mechanism:
  ...
  The same loop that evolves upstream doctrine evolves theirs — just pointed at their disk.

The consumer runs `/coordinator:learn-lessons` from their own repo; local mode is auto-detected; no
`--central` gate, no PM cross-repo promotion, no engine repo. That is the whole loop, and it is already built.
...
`~/.claude` and never the engine repo. This is enforced by a single discriminator repeated across the three seams:
**the `~/.claude`/engine-repo routing branch fires only when the current git root IS the meta-repo.** A
...
|------|-------------------------------------------------|-------------------------------|
| `coordinator-state-root.py` (no `--central`, Rule 5) | meta-repo → engine state | **`$GIT_ROOT/state`** |
| `coordinator-queue-append._output_path` | `_same_path(git_root, home)` → engine state | **`git_root/<output_dir>`** |
| `coordinator-lesson-add._lessons_dir` (dedup scan)† | `_same_path(root, home)` → engine state | **`root/state/lessons`** |
...
. Rows 1 and 2 are genuine independent guards.
...
engine-repo write target on the consumer-local capture→route path is reachable without the `_same_path(git_root, home)`
...
redirect to the engine repo."

## The fleet-private boundary (stated, not implied)

The upstream central improvement machinery is **fleet-private**. It does not ship to consumers, is not readable
...
- **`state/lessons-outbox/` and the central improvement queue
  ...
  consumer's `state/lessons/` is never visible to an upstream drain unless the consumer's disk is literally a
  machine that upstream operator already scans.
- **There is no phone-home.** `coordinator-lesson-promote`'s only write target is a *local*
  ...
  lesson path — in the bash CLI or its engine-native port. For a consumer, an outbox entry (if they ever
...
- **The dogfooding-blindness gap is accepted, by design.** Publishing `coordinator-claude` downstream
  severs the loop: a consumer's lessons accumulate locally but never reach the upstream doctrine-evolution cycle,
  and upstream doctrine improvements reach them only on the next marketplace/percolate release. This asymmetry
  is a **PM-ratified accepted cost** (decision P-B), handled by **release discipline** — the upstream operator periodically
  cuts a fresh coordinator build and runs as a consumer — **not** by any feedback/telemetry/upload channel.
  A back-channel from consumer lessons to upstream was explicitly considered and **dropped** (option (d) in the
...
**own** durable data — data belonging to the consumer's installed copy, not to upstream or to any
...
settings-home mechanism the upstream meta-repo itself uses for machine-local config, now extended to give a consumer
install an equivalent per-repo durable home outside their working tree. Neither path is
`~/.claude` or the engine repo; both stay entirely on the consumer's own machine.

## Negative-spec

- This is not the **engine-invoked delivery seam** — that is a separate, DEFERred
  deliverable gated on engine maturity. Nothing here percolates `cc_invoke.py`,
  `coordinator-state-root.py`'s engine-routing branches, or `_cc_route` into the OSS copy.
- The fleet-private central queue is **not** to be shipped, exposed, or wired to consumers in any form.
- No feedback / telemetry / upload channel from consumer lessons back to upstream. Dogfooding-blindness is the
...
- `docs/wiki/hook-best-practices/state-placement-law.md` § Taxonomy — the SSOT for per-repo vs central/fleet-private state.
- `docs/wiki/lesson-triage/learn-lessons-routing.md` — change-kind enum and local-mode auto-apply bounds.
