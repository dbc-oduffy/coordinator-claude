...
type: doctrine
related:
  - global-doctrine/CLAUDE.md § Implementation Standards — Extensions
  - docs/wiki/test-design-discipline.md
  - docs/wiki/em-operating-model/verification-before-completion.md
---
...
## Two loops: inner red-green, outer plan review
...
- **Outer loop — plan review:** the plan declares its acceptance criteria; the Staff Engineer (or any named reviewer) reads the AC section as part of plan review, applying a test-scrutiny lens to assess whether the stated criteria are specific and testable. There is no automated enforcement at the merge boundary — the outer loop is a design-time lens, not a runtime gate.

The two loops reinforce each other: clean inner-loop work produces the evidence the reviewer examines at the outer loop. Any inner-loop discipline lost to sunk-cost rationalization (a skipped test, a weakened assertion) will surface at `/workstream-complete` — `code-reviewer` runs on the diff and flags weak tests, dead code, and missing coverage before the work lands. **Test-first applies at both altitudes** — acceptance criteria are specified before implementation at the outer loop, and individual tests are written before their code at the inner loop.
...
> The shorthand stays "Iron Law" because the substance is iron-clad — production code follows a failing test. The framing below leads with the better path rather than an imperative; `code-reviewer` on the diff at `/workstream-complete` and `/handoff` is the structural backstop — it flags weak tests, dead code, and missing coverage before the diff lands.
...
The cleanest path is: close the reference, write the failing test against nothing, watch it fail, write minimal code to pass. The structural backstop for any inner-loop discipline lost here is `code-reviewer` on the diff at `/workstream-complete` and `/handoff` — skipped or weakened tests surface at that review before the work is declared done.
...
When one of these fires, the cleanest path is to re-derive from the test: write the failing test against an empty implementation, watch it fail, write minimal code to pass. The re-derivation is usually less work than debugging from the testing-after artifact later, and it puts the design pressure back where it earns its keep. If inner-loop discipline slips, `code-reviewer` on the diff at `/workstream-complete` and `/handoff` is the backstop — it flags weak tests and missing coverage before the work lands.
...
The clean signature of TDD work: every production change has a test that failed first, the failure was watched, the minimal code passed it. When the inner loop runs cleanly, the diff-review at `/workstream-complete` is a confirmation, not a correction — the two altitudes are designed to reinforce each other.
