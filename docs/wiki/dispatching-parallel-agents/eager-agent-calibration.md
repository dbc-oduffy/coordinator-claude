...
This wiki is the doctrine reference for three coordinated surfaces that together redirect executor agents toward portable, multi-machine code. This wiki is the stable summary for future reference and RAG retrieval. It does not duplicate plan content — it names the surfaces, explains their binding logic, and records the framing that prevents doctrine confusion.
