...
- `docs/wiki/em-operating-model/workflow-orchestration.md` § The Workflow Script Is the Wave-Map — the wave-map-to-
  Workflow mapping table that `dispatch_feed`'s field set makes into a typed artifact.
- `docs/wiki/schema-and-validation-contracts/schema-version-gate.md` — the additive-bump discipline this schema's 1.0.0 → 1.1.0
  move follows.
...
- `coordinator/docs/wiki/skills-corpus/computed-engine-model.md` § The subagent-sidecar convention — the
  generalization this page's three-role table is now a specialization of.
- `coordinator/docs/wiki/planning/plan-completeness-ledger.md` — the fourth, per-plan artifact that reads
  this page's run-report sidecars but is not a fourth role on them (see § Not a fourth role
  above).
- `coordinator/docs/wiki/claude-md-surfaces/invisible-doctrine.md` — the discharge-test framing this page's N-role
...
`coordinator/docs/wiki/skills-corpus/computed-engine-model.md` § The subagent-sidecar convention names the
