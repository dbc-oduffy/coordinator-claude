...
This page defines the per-entry YAML frontmatter schema for `state/debt-backlog/*.yaml`, the structured directory form that replaced the legacy prose-form debt-backlog queue.
...
repo — the **parked tier** of the Queue Terminus Doctrine (`docs/wiki/ceremony-calibration/queue-terminus-doctrine.md`).
...
chunk, `/bug-blitz`) disposes an item as "park, with a named reason" under that ceremony's
existing PM gate, or when the EM writes one directly via `coordinator-queue-append --schema
...
`docs/wiki/schema-and-validation-contracts/backlog-prune-discipline.md § Age-Ping — Parked-Tier Aging Discipline`. Closure moves
...
**`initiative` — the triage-theme grouping key (schema field, not in the base-set table above).**
`debt-backlog.schema.json:74` carries a nullable `initiative` FK to
`state/initiatives/<id>.yaml`, mirrored on `improvement-queue.schema.json:71` and
`bug-backlog.schema.json:78`. The queue-triage terminus
(`docs/wiki/ceremony-calibration/queue-terminus-doctrine.md`) reuses this same FK as its triage-theme grouping key: a
value is written ONLY when a title-keyword cluster graduates to a themed baton — never as a bulk
pass, and never over the ~340 `queue_scope: central` entries. The same value is written on the
resulting handoff baton, giving bidirectional row↔baton traceability. See
`docs/wiki/em-operating-model/initiative-govern-discipline.md § Queue-triage themes vs true initiatives` for what
distinguishes a triage-theme value from a true initiative.
...
| `created_by_agent` | string | Persona or agent identity string (e.g. `"staff-eng"`, `"code-reviewer"`, `"em"`). **DISTINCT from `created_by_session`** — a single session can involve multiple agent personas; do not collapse agent identity into session identity. |
...
|---|---|---|
| `open` | **Legacy value.** Some pre-rescope entries on disk carry `status: open` with no structured park reason and are not retro-migrated to `deferred` (Anti-scope: "Retro-triage of the existing holding-tier corpus" is out of scope — the age-ping mechanically re-surfaces the whole corpus on the next triage run instead). Not written for new parked entries — see `deferred` below. | LIVE |
...
| `deferred` | **The modal status for a deliberately parked entry.** Every item that lands here via an explicit park decision (Queue Terminus Doctrine outcome class 4 — `docs/wiki/ceremony-calibration/queue-terminus-doctrine.md`) is written `status: deferred` carrying a **mandatory** `why_blocked` field naming the park reason — never `open`. `open` would make a parked entry indistinguishable from an untriaged queue row; `deferred` + mandatory `why_blocked` is what preserves the named-reason discipline the terminus doctrine's outcome class 4 requires (no silent retention, no default sink). **Carve-out — a ceremony park only:** this parked-tier `deferred` case is EXEMPT from the separate PM authorization the base queue-family enum otherwise requires for `deferred`, because `/debt-triage` authorizes the whole four-outcome disposition at its Step 5 gate and Step 6b class 4 only performs the write (DEC-2/DEC-7 of the terminus plan hang the disposition off that existing gate rather than adding a second one). **No ceremony parks a bug row** — `/bug-blitz`'s fourth outcome is close (already-fixed/file-removed/wontfix), never park — so a `deferred` bug row is hand-authored, carries the base PM authorization in full, and gets no exemption from it. `why_blocked` is mandatory either way; the exemption is about *who signs off*, not about whether a reason is recorded. | BLOCKED |
...
body: |
  The overlap pass in fan-out-dispatch.py verifies that a pinned interface file exists
...
## Aging — parked-tier age-ping and draw-on-demand

The parked tier is age-pinged at every queue triage run and drawn from on demand when the
pickable baton inbox is low (Queue Terminus Doctrine, `docs/wiki/ceremony-calibration/queue-terminus-doctrine.md`).
That contract is queue-family-generic — it applies identically to `state/debt-backlog/` and
`state/bug-backlog/` — and lives once in
`docs/wiki/schema-and-validation-contracts/backlog-prune-discipline.md § Age-Ping — Parked-Tier Aging Discipline`, not here.
This wiki owns only the debt-backlog-specific shape of the parked state (`deferred` +
mandatory `why_blocked`, § Status enum above); read the prune-discipline wiki for the
aging/draw-on-demand mechanics themselves.
