...
This page documents the pluggy seam that splits chunk-metadata ownership between the host's Layer-1 validation algorithm and addon-owned per-source-type extras vocabulary.
...
`sub_index`, `sub_total`, `parent_chunk_id`, `chunker_id`, etc.; `chunker_id` is
injected via `indexer/chunk_utils.make_chunk()`), `_compute_drift` (the per-record
...
`tasks/w2a-1i-revised/the Staff Engineer-pluggy-seam-design-2026-05-16.md §5`.
...
(`blueprint`, `bp_`, `unreal`, `example-game-repo`, `uproject`, `umap`, `uasset`). It is
...
`project-rag-ue-addon` repo's `docs/reference/chunk-landing-policy.md` previously served
...
- **Canonical authority for canonical-eight requirements:** `core/chunk_schema.py`
  (host). The landing-policy document is not authoritative for which fields are
