...
- **Provenance is a LINK, not a self-declaration.** `derived_from` points at the sizing object this
  ...
  **`derived_from` takes the bare path or reference and nothing else** — its pattern is anchored, so
  a trailing parenthetical saying which size the review finalised, or why the scout's file was not
  used, fails the write outright. That rationale is worth recording; record it as a YAML comment on
  the line above, where the field's own grammar cannot reject it.
...
**An evidence command is plan-scoped by construction.** Write it over the plan's own commits
(`git diff --stat <first>..<last> -- <paths>`), never over the worktree: a bare
`git diff --stat -- <paths>` run while a sibling plan is in flight reports that sibling's changes
and reads as a brightline violation the plan did not commit. The author cannot know which siblings
a concurrent wave will be running, so the scope belongs in the command.
...
> See `coordinator/docs/wiki/dispatching-parallel-agents/pre-dispatch-verification.md` for the pre-dispatch confidence checklist.
...
`coordinator/docs/wiki/planning/plan-execute-session-split.md` for
...
`docs/wiki/dispatching-parallel-agents/delegate-execution.md`, code review via `/review-code` between tasks, fast iteration — the
...
step 3. See `coordinator/docs/wiki/planning/plan-execute-session-split.md` for the full rule.
