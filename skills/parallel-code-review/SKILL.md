...
The narrowed code-semantics scope is `patrik_scope` from the Step 7 prelude (`workweek-trail-scope.sh` output: `the Staff Engineer` SHA set + `patrik_seam_files` path set). It is NOT the whole week — it is unreviewed-since-workstream-complete commits PLUS cross-segment seam files. The 3 specialist workers see the full diff; only the code-semantics lens is narrowed and chunked.
