...
**Grouping: prefer your own judgment over the mechanical default.** Bare invocation groups mechanically (by directory, short bounded subject, full path list in the body) — right for an unattended trigger. After deliberate, describable work, author real per-group messages via `--groups-json <file>` (a JSON list of `{"paths": [...], "message": "..."}`) or `--message "<subject>"` for one well-described group. Either way, any path you name that ISN'T in the computed safe pathspec is silently dropped — the boundary is computed, never caller-widened.
