...
- **Commit shape:** default scoped commit per `snippets/scoped-commit-route.md` — the agree-case vs. private-index form is selected for you from observed state; never `git add -A`/`.`. → `coordinator/docs/wiki/scoped-safety-commits.md § The trailing pathspec is a proxy for scope, valid only while index and worktree agree`.
