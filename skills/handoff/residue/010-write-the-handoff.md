...
**`distill_fate:` — resolved at authorship, never deferred to a later `/distill` pass.** The harvest-vs-delete call is cheapest now, while you hold full context. No op writes it yet (`d2` `lint-frontmatter` validates against schema, it does not set values) — queued at `state/improvement-queue/2026-07-25-no-op-stamps-handoff-distill-fate-at-aut-092cf9e386ce.yaml`. Same enum as `cross-repo-memo.schema.json`: `ephemeral` (routine checkpoint, the common case), `commitment` (this handoff IS the open loop until a gate closes), `ratification` (a settled decision with no other durable home — prefer promoting to `docs/decisions/`/`docs/wiki/`, then stamp `ephemeral`).
