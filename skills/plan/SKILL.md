...
- _Cross-repo work whose shared contract is itself the unknown, even when a handoff says "ready to execute" / "XS"?_ (negotiated co-design — the contract/hookspec/shared fixture takes round-trips to converge, not a single well-understood ask the sibling implements on their own surface)
  ...
  - **A multi-wave plan that sends a memo carries a wave-boundary re-read of it.** The memo is an output of the plan, and a later wave of the *same plan* can contradict it — a wave-1 promise of "C5 ships only the report-dict half, zero contract movement" survived intact until wave 3 moved the contract, leaving the sibling holding a promise this tree had stopped keeping, with no surface on either side able to notice. Write the re-read into the spine, not into anyone's memory: at each wave close, enumerate the memos this plan has sent and ask whether the wave just landed contradicts any of them; where it does, the correction memo is part of that wave's deliverable. Prefer scope claims stated at their ref (*"as of C5 as planned"*) over unqualified promises about what the plan will not do — the promise is written before the waves that could break it exist. Tripwire: `A-MULTI-WAVE-PLAN-NEVER-RE-READS-ITS-OWN-SENT-MEMOS`.
