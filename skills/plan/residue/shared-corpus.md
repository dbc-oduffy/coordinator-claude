...
- _Scope-path staleness re-check — at B.0 AND again immediately before the Exit's write-time commit?_
  ...
  → **Re-run at commit time, not only at B.0.** Composition on a large plan takes long enough for a peer to land the work mid-draft. **Evidence:** an architecture-tier plan was committed seventeen minutes after a peer shipped a substantial part of what it still described as open — into the plan's own primary scope file, which had grown by over a hundred lines since the body's citations were taken. The B.0 pass was clean when it ran; nothing re-checked before the commit.
  ...
  → **Cite a peer repo's SHA as `<repo>@<sha>`, never bare.** Read `gates.substrate.peer_sha_lint` (`bad_citations[]`) — observed 2026-08-14: it also flags in-repo local SHAs/session ids, so `bad_citations` runs near-100% false-positive on a single-repo plan; treat a hit as a prompt to inspect, not an automatic defect.
...
- _Verified-scope collapse — does Branch B's own verification show the ask is materially smaller than what was sized?_
  ...
  → **Termination.** Fires at most once per pass — no reachable outcome re-enters this row's home, so plan→sizing→plan ping-pong is structurally impossible. Branch A's trampoline adds no cycle: it terminates on the conform detent, and the two edges have disjoint re-entry points.
  → **No-sizing-object case: unreachable twice over.** The express lane never arrives at `plan` (Branch A), and the wall refuses Branch B without an object on disk — while this row evaluates only after Branch B is green, i.e. after an object was guaranteed present.
...
- _A "soon = now" deferral candidate?_ (an item deferred because the EM thinks it lower-priority than the headline work)
  → Either ship it in this plan or get explicit PM disposition — and disposition means **both** the case against shipping now (`case_against`) **and** a non-vacuous `disposition_detail`, plus a recommendation stating your confidence. A one-sided argument does not satisfy this row. No silent deferrals. _Full both-sides form: `coordinator/docs/wiki/writing-plans.md`._
...
- _A single chunk would hand ONE executor multiple independent deliverables?_ (N modules or disjoint files whose **write** targets do not overlap — check it as a pairwise intersection over each deliverable's own write-target file list; an empty intersection between any two is the fan-out signal)
  → **Decompose into N chunks at plan-write time — fan-out chunking is a plan-author obligation, not an execution-time afterthought.** Disjoint **write** targets make a chunk a fan-out candidate. Only two justifications keep them together: (a) genuine write-overlap — and even then it is a *sequence* of small dispatches, not one fat executor — or (b) an unpinnable shared interface. **A shared read-only source and a pinned import contract are NOT serialization reasons** — both are reads, and only WRITE-overlap gates parallelism; a plan collapsing N disjoint-write deliverables under "file-overlap" has mistaken read-overlap for write-overlap. Size each chunk to ~5–10 min on one coherent surface, 15 min hard ceiling. **`writes:` is REQUIRED on every non-deferred row** (`deferred: true` rows are exempt — harvest candidates, never dispatch candidates): it is the pairwise-intersection input above, written down instead of discarded. **`depends_on:` (array of `{chunk, gate_kind}`) is required wherever the author imposes a gate the file-write graph cannot compute** — never for write-overlap, which the wave-builder derives on its own; consult `plan-tasks.schema.json`'s `gate_kind` NEGATIVE SPEC before writing one, three of the six discriminator gate kinds are deliberately unwritable there. These arrays are what make wave-map derivation a pure function of the spine rather than NLP over prose bodies — an undeclared spine is one `dispatch.emit` cannot fire on at all. _Late-correction surface: [`skills/execute-plan/SKILL.md`](../execute-plan/SKILL.md) Phase 1.6 wave-map authoring; the ledger schema lives only there._
...
- _Plan contains a chunk authoring a handoff, spinoff, or workstream-complete artifact?_ — grep the drafted chunk headings/bodies for authoring verbs against those three artifact names; any hit fires this row.
  → **Reject the chunk.** These are PM-gated session-continuity artifacts, not plan deliverables. "Chunk N: write a spinoff" launders the PM gate through plan approval — by execution time it reads as a checklist item and the spinoff's Step 0 gate never fires. Two shapes this prevents: (a) a terminal "author handoff" chunk as wrap-up ceremony where commit-and-stop or `/workday-complete` is the artifact; (b) a terminal "author spinoff to <other-EM>" chunk as cross-repo messaging where the primitive is PM-as-relay or `cross-repo-memo`. Genuine cross-EM coordination makes the chunk "surface cross-repo brief to PM".
...
Plan review altitude is binary — there is no Sonnet-tier plan reviewer, and on the `spec-dispatch`
lane review is skipped entirely, so its compensating control lives here, on both lanes: for
`spec-dispatch` the compensating control is not `/workstream-complete` or `/handoff` but the light
terminal's own mandatory scoped `code-reviewer` pass (§ `spec-dispatch` row), binding on every
exit.
