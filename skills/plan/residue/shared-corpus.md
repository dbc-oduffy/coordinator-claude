...
- _Plan's mechanism rests on a single external-tool BEHAVIORAL fact (not a signature)?_
  → **Route it through a `docs-checker`/citation pass even when it looks self-evident to the author** — e.g. a matcher's path-relativity assumption, a cache's invalidation trigger, an API's ordering guarantee. "Self-evident" is exactly the state that otherwise skips verification; a wrong assumption here invalidates the whole mechanism silently, at execution rather than at review. Cite the confirming source in the plan body.
...
- _Plan scopes an edit to a function the production path may not call?_
  → **STEP-0 caller-grep.** `git grep '<F>('` (or the language equivalent) to confirm the production path reaches the proposed edit locus before scoping a chunk to it. A chunk scoped to "inject in function F" that targets an emitter the production path never calls is a silent no-op, caught only at review or integration if skipped here.
- _Plan adds a transitional default (`X=None`) meaning to cut param `X` later, at a call site invoked positionally?_
  → **Verify the call shape (positional vs keyword) before scoping the cut — not after.** Grep each call site (`git grep '<callee>('`) for its literal arg list. If any live call is positional and `X` sits before a trailing arg, cutting `X` shifts that trailing arg into `X`'s vacated slot — silent, review-invisible corruption, not a loud break. **On a positional hit:** the chunk that removes `X` MUST also pin a keyword-rebind of the surviving trailing arg(s) at that call site, in the same edit — never deferred to "the executor will notice."
...
- _A "soon = now" deferral candidate?_ (an item deferred because the EM thinks it lower-priority than the headline work)
  → Either ship it in this plan or get explicit PM disposition — and disposition means **both** the case against shipping now (`case_against`) **and** a non-vacuous `disposition_detail`, plus a recommendation stating your confidence. No silent deferrals. _Full both-sides form: `coordinator/docs/wiki/writing-plans.md`._
