...
- **`wave: <N>` is a concurrency-gate primitive, NOT a sprint synonym.** Two distinct shapes:
  ...
  Do NOT use `wave:` for time-boxing or unit-of-effort grouping; `sprint:` is for that. Waves of one stub each are the tell that a sprint sequence has been mislabelled as parallelism.
