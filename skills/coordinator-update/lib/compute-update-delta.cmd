@echo off
setlocal
REM Windows launcher for compute-update-delta.py — python-direct (NO bash re-exec).
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "compute-update-delta.py"
...
echo [compute-update-delta] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [compute-update-delta] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127

:run_baked
"%_py%" "%~dp0compute-update-delta.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0compute-update-delta.py" %*
exit /b %ERRORLEVEL%
