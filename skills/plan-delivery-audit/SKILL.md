...
- `*.coverage-check.md`
- `*.docs-check.md`
- `*.review-the Staff Engineer.md`
- `*.review-sid.md`
- `*.review-the Data Science Reviewer.md`
...
Does the code at HEAD actually deliver what the plan claims? Two sub-checks, both required:

**(a) Cited-artifact existence.** For each file, path, symbol, or structural element the plan
claims it created or changed (read from the plan's scope section, chunk list, and AC criteria
prose), confirm it exists at HEAD in the claimed shape:

- Named files / paths → `ls <path>` or `test -f <path>` on disk
- Named symbols or function signatures → `grep -n '<symbol>' <file>` on disk
- Named schema fields, frontmatter keys, config entries → grep the owning file

**Any claimed artifact absent or wrong-shaped at HEAD = PARTIAL**, regardless of what Oracle 1
asserts. If the plan names no concrete artifacts to verify (purely narrative ACs with no
file/path/symbol claims), note that Oracle 2 is unverifiable and treat the plan conservatively.

**(b) Project test suite.** Run the project's configured fast-test/validate command at HEAD
(the `coordinator:validate` skill / `fast_test_cmd` in the project config). A failing test
suite is independent falsifying evidence even when artifact checks pass.

**Dispatch shape:** For audits covering ≥4 plans, dispatch parallel read-only Sonnet scouts
(one per plan). Each scout reads the plan's scope/chunks/AC prose, extracts the concrete
artifact claims, runs existence/grep checks for each, and reports PRESENT / ABSENT / UNVERIFIABLE
per artifact. EM runs the project test suite once (not per-plan) and applies the result to all
implemented plans in the batch. Do NOT modify files, commit, or push during Oracle 2 runs.
...
|------------------|-----------------|-------------------|--------|
| `status: implemented` (or `shipped`) | Cited artifacts exist at HEAD AND project tests green | ≥1 trail record covers delivery commits | **DELIVERED+REVIEWED** |
| `status: implemented` (or `shipped`) | Cited artifacts exist at HEAD AND project tests green | No trail record covers delivery commits | **DELIVERED-UNREVIEWED** |
| `status: implemented` (or `shipped`) | ≥1 cited artifact absent/wrong at HEAD, OR project tests red, OR no cited artifacts to verify (claim unverifiable) | (any) | **PARTIAL** |
...
- A plan with `status: implemented` but unverifiable Oracle 2 (no concrete artifact claims to
  check, and no project test suite configured) goes into **PARTIAL**, not DELIVERED.
  Self-assertion without any independently checkable evidence is not delivery.
...
|------|-----------------|-----------------|-------------------|--------|
| `docs/plans/YYYY-MM-DD-name.md` | `status: X`; artifact count | N/M artifacts present; missing if any; tests green/red | trail record `A..B` covers commits / UNCOVERED | **BUCKET** |
```
...
4. **Method notes** — flag any plan with unverifiable Oracle 2 (no concrete artifact claims
   to check) or missing delivery commits.
...
the key entirely. The session/nonce key exists so two plan-delivery audits run the same day
do not silently clobber each other's output. Create the `state/audits/` directory if absent.
...
**On DELIVERED-UNREVIEWED and PARTIAL buckets:** the worked example's 4 plans landed cleanly in DELIVERED+REVIEWED / IN-FLIGHT / ABANDONED in their resolved state — but the 05-24 row is annotated above with what it would have been under a live-only Oracle 3 (`DELIVERED-UNREVIEWED`). That's the falsifying demonstration of why archive-aware reading is load-bearing: the same plan, audited the same day, resolves to a different bucket depending on whether Oracle 3 globs the archive. PARTIAL is exercised when Oracle 1 claims `implemented` but Oracle 2 finds a claimed artifact absent/wrong at HEAD or the project test suite red — none of the 4 audited plans hit that path; the bucket is reached via the decision tree's Oracle 2 branch when artifact checks or project tests fail.

**Decision tree walk for each row:**

1. **patch-and-send-back (05-24):** Oracle 1 = `implemented` → proceed to Oracle 2. Oracle 2 = cited artifacts verified at HEAD: 9/10 claims pass (grep/file checks); AC3 is a URL liveness check — inherently unverifiable by grep/ls → "all verifiable artifact claims confirmed". Proceed to Oracle 3. Oracle 3 = archived trail record covers all delivery commits via range-membership → COVERED. Bucket: **DELIVERED+REVIEWED**. ✓

2. **game-dev-ownership (05-26):** Oracle 1 = `implemented` → proceed to Oracle 2. Oracle 2 = cited artifacts verified at HEAD: 12/13 claims pass (grep/file checks); AC7 is an external coordinator dependency (landed externally) → "all verifiable artifact claims confirmed". Proceed to Oracle 3. Oracle 3 = live trail record covers all 9 delivery commits → COVERED. Bucket: **DELIVERED+REVIEWED**. ✓
...
2. Filter out sidecars (any filename containing `.prior-art-check`, `.coverage-check`,
   `.docs-check`, `.review-the Staff Engineer`, `.review-`, or `.check.`).
...
> "Read `<plan-path>` and extract every concrete artifact claim (files, paths, symbols, schema
> fields, config entries) from its scope section, chunk list, and AC criteria prose. Verify each
...
`state/audits/YYYY-MM-DD-<SID_SHORT>-plan-delivery-audit.md` (your session's audit — see
"Output format" above for how `SID_SHORT` is resolved). Surface any DELIVERED-UNREVIEWED plans
