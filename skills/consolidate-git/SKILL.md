...
1. **Choose absorption strategy (inline override required on each git op that switches branches):**
   - **Cherry-pick** (default for 1-3 commits):
     ```bash
     # Inline override required: cherry-pick involves switching from the active work branch;
     # COORDINATOR_OVERRIDE_BRANCH=1 is still required even after block-off-daily-branch.sh retirement.
...
### Step 6: Post-Absorb Re-Verify Shared Infra (example-repo T1.7)
