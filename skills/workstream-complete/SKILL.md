...
- **Terminal-baton drain — MANDATORY, and the last thing this ceremony does.** `apply`'s
  `handoff_dispositions` payload (§ Apply) already stamps `deployment_state` + `shipped_in` for
  every emissible baton this close disposes — do not hand-stamp those. Hand-stamp only the two
  non-emissible states § Apply carves out (a baton whose deliverable is carried solely by the
  close's own commit, and a baton already archived by `/handoff`'s supersession route): double-writing
  `deployment_state`/`shipped_in` against a baton the payload leg already stamped is the failure
  this narrowing exists to prevent. Bind this strictly after `apply`'s own disposal read — once
  every payload-stamped baton and every hand-stamped carve-out baton alike carries a terminal
  `deployment_state` — then run `sweep-terminal-handoffs`. This is not a tidy-up you may defer to the next ceremony: a record stamped archivable and left in `state/handoffs/` is an unfinished close, because every surface that counts open batons reads it as *unstarted* for however long the gap runs. **The close condition is checkable and you check it: `state/handoffs/` holds no record carrying a terminal `deployment_state` (`shipped`/`continued`/`closed`, plus legacy `abandoned` — `HANDOFF_TERMINAL_DEPLOYMENT`; `declined`/`superseded` are sizing-object and retired-handoff `status` values, never `deployment_state`) when this ceremony reports complete.** The drain is idempotent and a sub-second no-op when there is nothing to move, so there is no cost that justifies skipping it, and it drains other sessions' terminal residue too — that is a feature, not overreach. No directive performs this today; nothing downstream catches the miss. `[[terminal-batons-are-swept-at-close-not-left-to-the-next-ceremony]]`
...
**`handoff_dispositions` is another non-JP key**, `{"<handoff-basename>.md": {...}, ...}` — one entry per baton this close disposes, each value an **object**, never a bare string (`"shipped"` alone is dropped, not coerced). It carries the whole four-value vocabulary the two consumer resolvers select on, not just `shipped`: `shipped` (with `shipped_in` — see the `shipped_in` rule below), `closed` and `abandoned` (each with `closed_reason` drawn from the closed vocabulary `cancelled`, `displaced`, `stale` — read from the same entry, **never defaulted and never fabricated**; anything outside that set, including a well-written sentence, is refused rather than written), and `continued` (neither field).

**The `shipped_in` rule.** The deliverable path set is declared, not guessed: a baton's deliverable
paths are the union of `writes:` across the spine rows of the plan whose frontmatter
`deliverable_id` matches that baton — resolvable by grep, the only declared source (no
`deliverable_paths` key exists on any surface, and `deliverable_id` is plan-level, never per-row).
A baton with no such plan has no declared deliverable and therefore no `shipped` arm. `shipped_in`
is the sha of the commit that carried that baton's deliverable; when the close spans several
commits, it is the last commit in this session's own owned-commit list that touched that baton's
deliverable paths — the commit after which that baton's acceptance criteria hold. Resolve it with
this session's own owned-commit enumeration, sliced `<sha>~1..<sha>` per commit exactly as
`sha_range` is sliced above (never an `oldest..newest` range, which attributes peer commits on a
shared branch) — but filtered by whether the commit touched that baton's deliverable paths, never
by a code-only `.md`/`.yaml`/`.yml` filter: that filter discards exactly the doc-only commits this
repo's batons usually ship in. One sha per baton, full-length as `git rev-parse` prints it — never
a range, never a PR ref, never a list. It is never the close's own commit-tail sha, and the engine
never derives or defaults one. **No fallback arm**: if no commit this session owns touched that
baton's deliverable, the disposition is not `shipped` — that absence routes the entry to `closed`
(with a `closed_reason`) or `continued`, never a placeholder, a nearest sha, or the close's own.

**An entry is silently dropped unless four preconditions hold**: the entry is an object (never a bare string), this session holds the claim on that baton, the record is still active under `state/handoffs/` at `apply` invocation (not yet archived), and the whole `handoff_dispositions` leg only runs when `decisions` also carries a `subject` — no `subject`, no disposal, of any baton. Miss one and `apply` drops that entry with no error.

**Two reachable states get no entry at all — deliberate omission, not the drop above.** (1) The deliverable is carried only by the close's own commit: no `shipped_in` sha exists yet, no `closed_reason` is honest, `continued` is refused. (2) The record has already been archived by `/handoff`'s supersession route, whose `continued` precondition and this leg's on-disk precondition are near-mutually-exclusive. Both omit the baton from the payload and hand the terminal stamp to the residual hand route (§ Genuine EM actions) instead. **This is a two-case carve-out, not the memo's rule** — the memo told DoE to omit every non-`shipped` baton; this omits exactly these two states and nothing else. A `closed`/`abandoned`/`continued` baton outside those two is emitted, always.

**The payload implies a ceremony order — transition, then dispose, then archive — and it is not optional.** `continued` is an assertion the engine refuses unless `deployment_state` already reads `continued`, so any transition this close makes must land before the disposal that asserts it. And any disposition at all is refused once the record has left `state/handoffs/` — so disposal must land before archival, never after. A successful disposal releases this session's claim on that baton; `closed` and `abandoned` alike land the record at `deployment_state: closed`.

**`apply` commits — on the documented route, `_run_close_commit_tail` is not optional.** It runs
unconditionally, immediately after `_execute_directives` returns, gated only on `decisions`
carrying a `subject` (and `sid`) — the same `subject` the `handoff_dispositions` leg already
requires (§ above: no `subject`, no disposal, of any baton), so a close that disposes any baton at
all has already met this gate. When it fires, it folds the `handoff_dispositions` ship-stamps into
that same commit's `stage_paths` **before** committing — the stamps land inside `apply`'s own
commit, not a separately-committed write. Treating `apply`'s exit as leaving nothing landed is
exactly the green-looking-ceremony-with-no-record failure this passage used to invite. Your own
scoped commit is a **second** commit, landing after the one the stamps were folded into — it never
carries the stamps itself, and it is still yours to make, in this order:

1. **Scoped commit** of the completion entry, Session Ledger row, and your own files, per
   `snippets/scoped-commit-route.md` — this lands after, and separately from, `apply`'s own close
   commit above.
