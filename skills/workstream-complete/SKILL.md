...
**A dispatched reviewer that returns nothing has NOT necessarily failed — recover its report before recording anything.** The subagent writes its full final text to `~/.claude/projects/<slug>/<sid>/subagents/agent-<name>-<hash>.jsonl` whether or not the harness delivers it to you; an agent going idle with no visible result is a DELIVERY failure far more often than an execution one. Read the last assistant `text` block of that file before falling back to `em-verified`, and check `stop_reason` while you are there — `end_turn` means it finished cleanly and you are simply not seeing the result. Recording `em-verified` over an undelivered verdict is not a conservative default: it silently overwrites a real reviewer's finding with your own, and a `blocked` lost this way reads as a clean close. Tripwire: `AN-IDLE-SUBAGENT-HAS-NOT-NECESSARILY-FAILED`.
