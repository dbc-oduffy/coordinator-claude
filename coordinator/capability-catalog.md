...
**Feature-dev agents** (custom plugin — EM-routable for structured feature implementation workflows; dispatch via `subagent_type: feature-dev:<agent-name>`):
- **feature-dev-code-architect** — designs feature architectures by analyzing existing codebase patterns and conventions, then produces a comprehensive implementation blueprint persisted to disk (markdown/JSON/YAML). Dispatch when planning a new feature that requires deep codebase pattern analysis before implementation begins.
- **feature-dev-code-reviewer** — reviews code for bugs, logic errors, security vulnerabilities, code quality issues, and adherence to project conventions using confidence-based filtering to surface only high-priority issues. Dispatch after implementation to review unstaged changes or a specified file set.
