...
2. Dispatch a Sonnet reviewer on the cumulative diff via `coordinator:review-code` (default: `coordinator:code-reviewer` (Sonnet, locked); escalate to the Staff Engineer+workers when the diff includes a merge boundary or risky surface per `docs/wiki/workstream-complete-review.md`).
3. Persist the review record to `state/review-trail/<timestamp>-mise-<run-id>.json`.
