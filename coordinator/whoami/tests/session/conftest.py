...
def fake_git_root(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    Creates:
      ...
      state/            — for orientation_cache.md (F-C3-03 fix)
...
def fake_orientation_cache(fake_git_root: Path) -> Path:
    """Write a state/orientation_cache.md with a known git_head_at_generation."""
    cache = fake_git_root / "state" / "orientation_cache.md"
    cache.write_text(
