...
- _Concurrent-session pre-flight done?_ (run BEFORE scouts or reviewer dispatch — not at Branch C)
  → `ls docs/plans/<today's-date>*` AND `git log --since=today --oneline -- docs/plans/`. If another session has already authored a plan for the same scope today, surface the collision to the PM before proceeding. Two concurrent EMs independently planning the same work is the failure mode; the concurrent-session check fires here, before any scout or reviewer has spent tokens, not at the cross-plan conflict scan in Branch C (which runs after drafting). _See also `docs/wiki/writing-plans.md` § pre-plan substrate check must include today-dated plans and recent git log._
...
- _Each chunk has an identified test surface? AND each gate-bound test-AC has a named producer chunk?_
  → For each chunk, name the test (or explicitly document why no test). **Fire this BOTH directions:** chunk→test (every chunk names its test) AND test→chunk (every gate-bound AC of the form `pytest:test_X.py::Y` / `bash:tests/Z.sh` has a wave chunk whose write-files include that test FILE — not just the test path). A gate-bound test-AC with no producer chunk is a guaranteed `/workstream-complete` Step 3.8 hard-block: the oracle reads red because the test was never authored. The implementation chunk does not author its tests unless the brief says so explicitly. _See `docs/wiki/test-design-discipline.md` and `docs/wiki/writing-plans.md` § Bite-Sized Task Granularity. (Empirical: 2026-06-15 native-mcp-transport plan named 11/13 ACs as typed-prefix tests but only 1 chunk authored a test → 1/13 green at done-time.)_
