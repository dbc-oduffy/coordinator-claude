...
**Tracker shortcut.** A pre-rendered snapshot of the handoff queue is available at `state/handoff-tracker.md` (written by `/workstream-complete` and `/handoff`). Useful for a fast orientation glance; ad-hoc refresh: `node ~/.claude/plugins/coordinator/bin/render-handoff-tracker.js`. Always verify actionable items via the live queries above before acting — the tracker reflects state at the last session exit, not right now.
