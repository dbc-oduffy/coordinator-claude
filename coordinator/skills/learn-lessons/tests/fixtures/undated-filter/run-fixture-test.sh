...
for expected_id in sample-L1 sample-L8; do
...
if grep -q "id: sample-L15$" "$TMP"; then
    echo "BAD    sample-L15 selected — negative case (a) failed (undated+project must NOT pass)"
    fail=1
else
    echo "OK     sample-L15 rejected (undated+project — polarity correct)"
fi

# Negative case (b): dated+universal NOT selected.
if grep -q "id: sample-L22$" "$TMP"; then
    echo "BAD    sample-L22 selected — negative case (b) failed (dated+universal must NOT pass)"
    fail=1
else
    echo "OK     sample-L22 rejected (dated+universal — polarity correct)"
fi

# Negative case (c): dated+project NOT selected (neither-axis sanity).
if grep -q "id: sample-L29$" "$TMP"; then
    echo "BAD    sample-L29 selected — neither-axis case failed"
    fail=1
else
    echo "OK     sample-L29 rejected (dated+project — sanity)"
fi
