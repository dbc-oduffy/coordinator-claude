...
- **HARD RULE — background by default.** Pass `run_in_background: true` on any Agent dispatch >2 min (most). PreToolUse hook DENIES foreground; escape hatch `COORDINATOR_AGENT_FOREGROUND_OK=1`. → `dispatching-parallel-agents.md` § Background by Default.
...
**Cross-platform shell — bash ≥ 4 + BSD coreutils (P0 per DR-148), Linux likely-untested, Windows Git-Bash.** Every `.sh`/`bin/*`/hook must run on bash ≥ 4 with BSD coreutils. Stock macOS `/bin/bash` is 3.2 and is explicitly unsupported as an execution target (DR-148 2026-06-01) — `coordinator:install` installs brew bash and surfaces the `brew install bash` + PATH-first remediation. Scripts that use bash-4 features (`declare -A`, `local -n`, `mapfile`, `${v^^}`) ship a `BASH_VERSINFO<4` fail-loud guard with the brew remediation, not a 3.2 port-down. The guard MUST be reachable on 3.2 (a broken boot-path hook = Coordinator can't boot to fix itself), so script *syntax* must parse on 3.2 even when the script can't run there — heredocs inside `"$(...)"` are the canonical parse-trap. GNU-isms (`sed -i`, `date -d`/`%N`, `grep -P`, `realpath`/`readlink -f`) still need a `command -v` guard or BSD-portable rewrite — the coreutils axis is decided separately from the bash-version axis. Enforced in-prompt by the `code-reviewer` portability lens, not docs alone. → `cross-platform-shell-portability.md`; DR-148; DR-061.
...
- **Cruft-sweep cadence floor (Layer 1, non-negotiable).** Filesystem hygiene is distinct from `/distill` (knowledge) and `/update-docs` Phase 8b (artifacts). Boundless accumulation under normal use demands a non-agentic floor — `bin/cruft-sweep.sh` is Layer 1 (mechanical age + name + fingerprint), `/cruft-sweep` is Layer 2 (judgment + registry-diff). → `docs/wiki/cruft-sweep-cadence.md`.
