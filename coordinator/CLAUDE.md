...
- **HARD RULE — background by default.** `run_in_background: true` on any Agent dispatch >2 min. PreToolUse hook denies foreground; escape: `COORDINATOR_AGENT_FOREGROUND_OK=1`. → `dispatching-parallel-agents.md`.
...
- **EM default is plan-and-dispatch, not type code.** Handoff is context for planning, not a trigger to code. Implement directly only when a plan exists *and* dispatch is more expensive than typing.
...
- **Grep, don't invent — grep is authoritative over the spec.** Paths, framework names, helper APIs, constants, schema fields, frontmatter keys, env vars: confirm by grep from the asserting test/contract. Paginated grep truncates — use `head_limit:0`. Duplicate-detection requires body comparison, not metadata.
...
- **Frontmatter `status`: `active | consumed | superseded`** (`shipped` rejected — use `consumed` + `shipped_in:`). **`deployment_state`: `awaiting_gate | ready_to_fire | in_flight | shipped | abandoned`** — only `ready_to_fire` surfaces in start ceremonies. `/pickup` flips to `in_flight` in place at `state/handoffs/`; archival deferred.
...
- **After every review, dispatch the review-integrator — do not integrate manually.** EM reviews the escalation list, spot-checks the diff. Cross-session reviews converge on one canonical artifact.
...
- **Don't ask for:** routine implementation choices, internal refactors, naming/formatting, tradeoff-free reviewer fixes, whether to dispatch a reviewer, whether to commit/branch/stash.
