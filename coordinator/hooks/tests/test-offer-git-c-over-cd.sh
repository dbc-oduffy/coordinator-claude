...
assert_suggestion "variable target rewritten verbatim" \
  "cd \$HOME && git status" \
  "git -C \$HOME status"
# Multi-git chain: v2 preserves the full chain in the suggestion (first git is the
# one that gets the -C rewrite; the rest is appended verbatim).
assert_suggestion "multi-git chain rewrites first git in place, preserves rest" \
...
assert_suggestion "chain with non-git follower preserved in suggestion" \
...
assert_suggestion "cwd-binding note names the cd target" \
  "cd /repo && git log -1; head -3 README.md" \
  "no longer run with '/repo' as cwd"

# Multiple followers are all preserved.
assert_suggestion "multiple followers all preserved" \
...
assert_suggestion "follower captured verbatim from split point" \
  "cd /repo && git status; echo 'a;b'" \
  "echo 'a;b'"

# 2-segment commands (no followers) get the v1 message — no cwd-binding note.
...
if printf '%s' "$two_seg_reason" | grep -qF "no longer run with"; then
  FAIL=$((FAIL + 1)); echo "FAIL: 2-segment command should NOT carry cwd-binding note"
else
  PASS=$((PASS + 1))
fi
