---
description: "Setup or maintenance of coordinator scaffolding in repos. Default: deep first-time setup of one repo (interactive)."
allowed-tools: ["Read", "Write", "Edit", "Bash", "Glob", "Grep", "AskUserQuestion"]
argument-hint: "[--batch [--check-only] [--non-interactive]]"
---

# /coordinator:setup — Coordinator Repo Setup
...
> `--check-only and --non-interactive are only valid with --batch; for non-interactive single-repo setup, set coordinator.local.md first and re-run /coordinator:setup.`
...
1. **Precondition:** `~/.claude/working-repos.yaml` must exist (produced by `/coordinator:install` Phase 2 Step 4). If missing, exit with: `"working-repos.yaml not found. Run /coordinator:install first."`
...
```
=== /coordinator:setup --batch summary ===
  ...
  /coordinator:setup --batch complete.
...
- Discover repos (delegates to `~/.claude/working-repos.yaml` — produced by `/coordinator:install`).
