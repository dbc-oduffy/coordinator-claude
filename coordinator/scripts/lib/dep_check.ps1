...
#   Test-CoPhaseZeroShouldRun       — allowlist check; returns $true (run) or $false (skip)
#   Invoke-CoRunModePrompt          — agent-direct y/N prompt; exits 92 on agent-mode
#   Test-CoDepProbe                 — runs functional probe; returns present/missing/present-but-broken
#   Get-CoDepProbeAll               — NDJSON one line per dep
#   Invoke-CoVisitedSetCrashCleanup — Phase 0 entry crash-recovery reaper (>5 min TTL, age-only)
#   Initialize-CoVisitedSet         — creates visited-set file, stale-cleans >1h files
#   Test-CoVisitedSet               — $true if dep already visited
#   Add-CoVisitedSet                — atomic append dep to visited set
#   Invoke-CoConsentGate            — emits banner, enforces confirmation; exits 90/91/93
#
...
#  via Invoke-CoRunModePrompt BEFORE Phase 0 would run, but we note it here so
#  the allowlist is complete and greppable.)
# (--check is coord's repo-specific read-only extension per contract § Read-only flag carve-out)

function Test-CoPhaseZeroShouldRun {
    <#
    .SYNOPSIS
...
function Invoke-CoRunModePrompt {
    <#
    .SYNOPSIS
    ...
    Parity with bash _co_run_mode_prompt.

    Not called when Test-CoPhaseZeroShouldRun returns $false.
    ...
    $runMode = $env:COORDINATOR_RUN_MODE
    ...
    $reply = Read-Host 'Are you running this script as an autonomous agent rather than via /coordinator:setup? [y/N]'
    if ([string]::IsNullOrWhiteSpace($reply)) { $reply = 'N' }
    if ($reply -match '^[Yy]$') {
        [Console]::Error.WriteLine('AGENT_MANIFEST_PATH=docs/install/AGENT.md')
        [Console]::Error.WriteLine('[setup] Agent-direct invocation confirmed. Use /coordinator:setup instead.')
        exit 92
    }
}

function Test-CoDepProbe {
    <#
    .SYNOPSIS
    Run the functional probe for a single dep.
    Returns: 'present' | 'missing' | 'present-but-broken'
    Parity with bash _co_dep_probe.
    Spec backlink: docs/plans/2026-06-15-coordinator-install-chain-application-phase-b.md §7 C3
    #>
    param(
    ...
    # Resolve sibling root (parent of coordinator-claude repo or nested working repo).
    if (-not $SiblingRoot) {
    ...
    # X:\coordinator-claude is the root. In nested working-repo layout we strip the
    # "-claude" suffix and check the sub-folder.
    # P1 fix (mirror of bash fix): nested-layout probe always returned 'missing' because
    # ...\coordinator-claude doesn't exist; the real path is ...\coordinator
    # (i.e., ~/.claude/plugins/coordinator-claude/coordinator).
    ...
    switch ($ProbeKind) {
        ...
        'command_succeeds' {
            $cmd = $ProbeArgs['cmd']
            # F4: replaced Invoke-Expression with [scriptblock]::Create to eliminate arbitrary
            # command injection risk. The 'cmd' field originates from agent-install-manifest.json
            # which is the trust boundary for this surface — manifests from untrusted sources
            # could execute arbitrary code via Invoke-Expression. ScriptBlock::Create provides
            # equivalent eval semantics with tighter PowerShell parsing (cannot inject via
            # environment/argument expansion tricks that bypass Invoke-Expression).
            # NOTE: this same fix should cross-port to DR Phase B dep_check.ps1 — see plan §13.
            try {
...
function Get-CoDepProbeAll {
    ...
    # P2 fix: pwsh subprocess spawn was not parity with bash sibling and would fail silently
    # if pwsh wasn't on PATH; replaced with direct & invocation.
    ...
    foreach ($line in $lines) {
        ...
        $hint = switch ($status) {
            ...
            'present-but-broken' { "sibling present but functional probe failed; re-run /coordinator:setup" }
            default             { "" }
        }
...
# File: ~/.claude/coordinator-claude/chain-walk-<session-id>.json
...
function Invoke-CoVisitedSetCrashCleanup {
    <#
    .SYNOPSIS
    ...
    (Initialize-CoVisitedSet) uses a 1h TTL, so a retry within that window silently
    ...
    Visited-set keyspace: ~/.claude/coordinator-claude/ (Patrik Finding 4 correction).

    Complementary surfaces:
      Initialize-CoVisitedSet  — session-start janitor, 1h TTL (DO NOT MODIFY)
      _co_visited_set_crash_cleanup — bash parity (dep_check.sh)
    #>
    $coDir = Join-Path $env:USERPROFILE '.claude\coordinator-claude'
    if (-not (Test-Path $coDir -PathType Container)) { return }

    $staleThreshold = (Get-Date).AddMinutes(-5)
    Get-ChildItem -Path $coDir -Filter 'chain-walk-*.json' -ErrorAction SilentlyContinue | Where-Object {
        $_.LastWriteTime -lt $staleThreshold
    } | Remove-Item -Force -ErrorAction SilentlyContinue
}

function Initialize-CoVisitedSet {
    <#
    .SYNOPSIS
    ...
    $filePath = Join-Path $coDir "chain-walk-$SessionId.json"
    $data = [ordered]@{
...
function Test-CoVisitedSet {
    ...
    $filePath = Join-Path $env:USERPROFILE ".claude\coordinator-claude\chain-walk-$SessionId.json"
...
#
# coordinator-claude is DAG root (direct_deps: []). Override flags exist for
# schema-conformance only — coord has no upstream dependencies to check.
...
function Invoke-CoConsentGate {
    ...
    Parameters:
      ...
      -AcceptMissingDepsRisk : bool — --accept-missing-deps-risk flag
                              # F16: renamed from AcceptHallucRisk (stale name from earlier spec draft)
    ...
    param(
        ...
        [bool]$AcceptMissingDepsRisk      = $false,
        [bool]$Interactive           = $false,
        [string]$BannerPath          = ""
    )
    ...
    if ($SkipDepCheck -xor $AcceptMissingDepsRisk) {
    ...
    if (Test-Path $BannerPath) {
        ...
        # F14: fallback inline banner also uses the <!-- DEPS_LIST_PLACEHOLDER --> sentinel.
        $bannerText = @"
================================================================
WARNING — UNSAFE INSTALL REQUESTED

You are installing coordinator-claude WITHOUT one or more of
its dependency-chain prerequisites:

  <!-- DEPS_LIST_PLACEHOLDER -->

Without these, your install will run without a required dependency
that this contract version of coordinator-claude needs.
================================================================
"@
    ...
    if ($MissingHardDeps.Count -gt 0) {
        ...
        $bannerText = $bannerText -replace '<!-- DEPS_LIST_PLACEHOLDER -->', $depList
    }
