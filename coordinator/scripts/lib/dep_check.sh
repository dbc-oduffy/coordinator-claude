#!/usr/bin/env bash
# dep_check.sh — Phase 0 dependency-chain gate for coordinator setup.sh.
#
...
#   _co_phase_zero_should_run  — allowlist check; returns 0 (run) or 1 (skip)
#   _co_run_mode_prompt        — agent-direct y/N prompt (a); exits 92 on agent-mode
#   _co_dep_probe <dep-id>     — runs functional probe; echoes present/missing/present-but-broken
#   _co_dep_probe_all          — NDJSON one line per dep with {id, severity, status, sibling_path, hint}
#   _co_file_mtime_s           — portable file mtime in epoch seconds (BSD/GNU/date-r probe)
#   _co_visited_set_crash_cleanup — Phase 0 entry crash-recovery reaper (>5 min TTL, age-only)
#   _co_visited_set_init <session-id>   — creates visited-set file, stale-cleans >1h files
#   _co_visited_set_check <session-id> <dep-id>  — 0 if already visited, 1 if not
#   _co_visited_set_append <session-id> <dep-id> — atomic read-modify-write append
#   _co_consent_gate           — emits banner, runs confirmation; exits 90/91/93 per §3.3
#
# Visited-set keyspace: ~/.claude/coordinator-claude/chain-walk-<session-id>.json
...
# ---------------------------------------------------------------------------
_CO_DEP_CHECK_LOADED="${_CO_DEP_CHECK_LOADED:-0}"
if [[ "$_CO_DEP_CHECK_LOADED" == "1" ]]; then
  return 0 2>/dev/null || true
fi
_CO_DEP_CHECK_LOADED=1
...
#   --check         (coord-specific read-only extension per contract § Read-only flag carve-out)
...
#    /coordinator:setup? [y/N]"
#
# Behavior per §3.1(ii):
#   - --i-am-agent / COORDINATOR_RUN_MODE=agent → exits 92 immediately with
#     AGENT_MANIFEST_PATH on stderr (agent-direct-invocation detected).
#   - --i-am-human / COORDINATOR_RUN_MODE=human → returns 0 (proceed as human).
...
#
# Callers must check _co_phase_zero_should_run first; this function is not
# called when Phase 0 is skipped.
#
# Variables consumed (set by setup.sh argument parser):
#   I_AM_AGENT, I_AM_HUMAN, COORDINATOR_RUN_MODE, NON_INTERACTIVE
# ---------------------------------------------------------------------------
_co_run_mode_prompt() {
  local _run_mode="${COORDINATOR_RUN_MODE:-}"
  local _i_am_agent="${I_AM_AGENT:-false}"
  ...
  printf "[setup] Are you running this script as an autonomous agent rather than via /coordinator:setup? [y/N] " >&2
  local _reply=""
  ...
  if [[ "$_reply" == "y" || "$_reply" == "Y" ]]; then
    ...
    echo "" >&2
    echo "AGENT_MANIFEST_PATH=${_manifest_rel}" >&2
    echo "[setup] Agent-direct invocation confirmed. Use /coordinator:setup instead." >&2
    echo "[setup] Agent install guide: ${_manifest_rel}" >&2
    exit 92
  fi
  return 0
...
# ---------------------------------------------------------------------------
# _co_dep_probe <dep-id> [manifest-path]
#
...
#   $2 — (optional) absolute path to agent-install-manifest.json; defaults to
#        lib-dir-relative discovery. Callers in setup.sh pass ${_MANIFEST_PATH}
#        to avoid a second divergent resolution path (F1 fix).
#
# Variables consumed:
...
_co_dep_probe() {
  local _dep_id="$1"
  # F1: accept explicit manifest path to avoid double-resolve divergence.
  # Without this, _co_manifest_read_ndjson recomputes the path from lib/ dir,
  # which may differ from the REPO_ROOT-relative path setup.sh already resolved.
  local _manifest_path="${2:-}"
  ...
  local _dep_json=""
  local _python
  _python="$(_co_find_python)" || {
    # F2: drop 2>/dev/null — surface manifest-read errors rather than swallowing them.
    echo "ERROR: _co_dep_probe: no Python found; cannot probe dep '$_dep_id'" >&2
    echo "missing"
    return 0
  }

  # F2: pass manifest path explicitly; drop 2>/dev/null so manifest errors surface on stderr.
  # Previously _co_manifest_read_ndjson was called without args (lib-dir-relative resolution)
  # with 2>/dev/null, which swallowed corrupt-manifest errors as silently "missing".
  local _ndjson_out
  if ! _ndjson_out="$(_co_manifest_read_ndjson "${_manifest_path}")"; then
    echo "ERROR: _co_dep_probe: manifest unreadable or corrupt (dep '$_dep_id')" >&2
    echo "missing"
    return 0
  fi

  _dep_json="$(echo "${_ndjson_out}" | \
    "$_python" -c "
import json, sys
dep_id = sys.argv[1]
...
" "$_dep_id" 2>/dev/null)"
  ...
  # P1 fix: nested-layout probe always returned "missing" because
  ...
  case "$_probe_kind" in
    ...
    command_succeeds)
      ...
      ;;
    *)
      # F9: unknown probe kind — emit WARNING to stderr and return present-but-broken,
      # mirroring dep_check.ps1 default branch behavior. Previously returned "present",
      # which silently accepted an unknown probe as satisfied — a divergence from ps1's
      # "present-but-broken" and a potential masking of misconfigured manifests.
      echo "WARNING: unknown probe kind '$_probe_kind' for dep '$_dep_id'; treating as present-but-broken" >&2
      echo "present-but-broken"
      ;;
  esac
}
...
# ---------------------------------------------------------------------------
# _co_dep_probe_all [manifest-path]
#
...
#
# Arguments:
#   $1 — (optional) absolute path to agent-install-manifest.json.
#         Callers in setup.sh pass ${_MANIFEST_PATH} to avoid a second divergent
#         resolution path (F1 fix — same fix mirrored from _co_dep_probe).
#
# Concurrency-safe: one line per dep (per §5 constraint f).
# ---------------------------------------------------------------------------
_co_dep_probe_all() {
  # F1: accept explicit manifest path; thread it through to _co_dep_probe so both
  # functions resolve the same manifest, avoiding silent divergence when setup.sh
  # has already computed REPO_ROOT-relative path.
  ...
  while IFS= read -r _dep_line; do
    ...
    local _sibling_path="$_sibling_root/$_sibling_dir"
    # Nested-layout fallback: mirror the same strip-"-claude" logic as _co_dep_probe
    ...
    local _status
    # F1: pass manifest path through to _co_dep_probe to avoid second divergent resolve.
    _status="$(_co_dep_probe "$_id" "${_manifest_path}")"
...
" "$_id" "$_severity" "$_status" "$_sibling_path" "$_hint"
  # F1+F2: pass manifest path explicitly; drop 2>/dev/null so errors surface.
  done < <(_co_manifest_read_ndjson "${_manifest_path}")
}
...
_co_file_mtime_s() {
  ...
  # F17: `date -r FILE` is GNU-date-only. On macOS, `date -r` takes epoch-seconds (BSD).
  # If this fallback is reached on macOS and FILE is an actual filename, `date -r FILE`
  # errors (non-numeric) and we fall through to `echo 0`. Echo 0 causes the caller to
  # treat every file as "maximally stale" (age = now - 0 = very large), which is the safe
  # conservative behavior — files are reaped eagerly rather than silently retained.
  # In practice, the stat probes above cover both macOS (BSD stat -f %m) and Linux
  # (GNU stat -c %Y); this fallback is unreachable on any supported platform.
...
# (_co_visited_set_init) uses a 1h TTL, so a retry within that window silently
...
#
# Visited-set keyspace: ~/.claude/coordinator-claude/
# (per agent-install-contract.md § Visited-set protocol; the Staff Engineer Finding 4 correction)
#
# Complementary surfaces:
#   _co_visited_set_init  — session-start janitor, 1h TTL (DO NOT MODIFY)
#   Invoke-CoVisitedSetCrashCleanup — PowerShell parity (dep_check.ps1)
# ---------------------------------------------------------------------------
_co_visited_set_crash_cleanup() {
  local _co_dir="$HOME/.claude/coordinator-claude"
  [[ -d "$_co_dir" ]] || return 0  # nothing to clean

  local _now
  _now="$(date +%s)"
  local _stale_threshold=300  # 5 minutes in seconds

  for _candidate in "$_co_dir"/chain-walk-*.json; do
    [[ -f "$_candidate" ]] || continue
    local _mtime
    _mtime="$(_co_file_mtime_s "$_candidate")"
...
# ---------------------------------------------------------------------------
# _co_visited_set_init <session-id>
#
# Purpose: create ~/.claude/coordinator-claude/chain-walk-<session-id>.json
# visited-set file. Stale-cleans any existing chain-walk-*.json files older
# than 1 hour.
#
# Keyspace: ~/.claude/coordinator-claude/ per contract § Visited-set protocol.
# Schema: {"session_id": "<uuid>", "started_at": "<ISO8601>", "visited": []}
# ---------------------------------------------------------------------------
_co_visited_set_init() {
  local _session_id="$1"
  local _co_dir="$HOME/.claude/coordinator-claude"
  mkdir -p "$_co_dir"
  ...
  local _now
  _now="$(date +%s)"
  for _stale_file in "$_co_dir"/chain-walk-*.json; do
    [[ -f "$_stale_file" ]] || continue
    local _mtime
    _mtime="$(_co_file_mtime_s "$_stale_file")"
    local _age=$(( _now - _mtime ))
  ...
  local _visited_file="$_co_dir/chain-walk-${_session_id}.json"
  local _python
  _python="$(_co_find_python)" || return 1
...
# ---------------------------------------------------------------------------
# _co_visited_set_check <session-id> <dep-id>
#
...
# ---------------------------------------------------------------------------
_co_visited_set_check() {
  local _session_id="$1"
  local _dep_id="$2"
  local _visited_file="$HOME/.claude/coordinator-claude/chain-walk-${_session_id}.json"
  local _python
  _python="$(_co_find_python)" || return 1
...
# ---------------------------------------------------------------------------
# _co_visited_set_append <session-id> <dep-id>
#
...
#
# coordinator-claude is DAG root (direct_deps: []). The override flags exist
# for schema-conformance only — coord has no upstream dependencies to check.
# The flag pair fires only in synthetic-test mode or if a future contract
# version promotes a dep into coord's direct_deps.
#
# Flag pair for coordinator-claude:
#   --skip-dep-check + --accept-missing-deps-risk
# (predecessor §7.2 PM-ratified naming)
#
...
# P1 fix: was ACCEPT_HALLUCINATION_RISK (stale name from an earlier spec draft);
# setup.sh exports ACCEPT_MISSING_DEPS_RISK so the gate would always see false →
# exit 93 even when both override flags were supplied.
# ---------------------------------------------------------------------------
_co_consent_gate() {
  ...
  # Note: coord declares no direct deps (DAG root; direct_deps: []).
  # The banner-render and confirm flow below is structurally unreachable
  # for v1/v2 schema in this repo, but kept intact for parity with the
  # holodeck/project-rag template and forward-compat if coord ever declares a dep.
  # P2 note: dead code identified; comment added to make intent explicit;
  # code retained for schema-parity and forward-compatibility.

  # Collect missing hard deps for banner substitution.
  # Coord has no direct_deps; hard deps are structurally absent.
  ...
  # F2: drop 2>/dev/null — manifest errors should surface rather than be silently swallowed.
  done < <(_co_dep_probe_all)
...
================================================================"
  ...
  # F14: substitute <!-- DEPS_LIST_PLACEHOLDER --> sentinel (was fragile free-form text).
  # Both the banner file marker and this substitution key use the same sentinel string.
  local _dep_list=""
  ...
  _banner_rendered="${_banner_body//'<!-- DEPS_LIST_PLACEHOLDER -->'/$_dep_list}"
