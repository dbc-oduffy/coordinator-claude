...
# lacking both `timeout` and `gtimeout` _bounded runs the tool unbounded — the fan-out caller's
# 10 s outer cap (fan-out-dispatch.sh, the `timeout 10` around this probe) is the backstop there.
#
...
# Wall-clock-bound an external command if `timeout` is available; else run unbounded.
# macOS ships `gtimeout` (coreutils) or nothing — the fallback keeps the probe portable;
# a soft advisory tolerates the rare unbounded hang on platforms without `timeout`.
_bounded() {
    local secs="$1"; shift
