...
#   2=dispatch#  3=chunk-id  4=brief  5=write-files  6=AC-paths  7=gate-kind  8=runs  9=est-min  10=status
#
...
while IFS= read -r line; do
  ...
  # Extract gate-kind (col 7) and runs (col 8) from pipe-delimited row
  # Using awk for portability (BSD awk + GNU awk compatible)
  gate_kind="$(echo "$line" | awk -F'|' '{gsub(/^[ \t]+|[ \t]+$/, "", $7); print $7}')"
  runs_val="$(echo "$line" | awk -F'|' '{gsub(/^[ \t]+|[ \t]+$/, "", $8); print $8}')"
...
#   coordinator:staff-eng, coordinator:review-integrator, coordinator:review-integrator
#   data-science:staff-data-sci, coordinator:*-reviewer, persona names (the Staff Engineer, the Game Dev Reviewer, etc.)
#
