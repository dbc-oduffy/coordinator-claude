...
_run_subject() {
  ...
  STDOUT_OUT=""
  STDERR_OUT=""
  STDOUT_OUT=$(cd "$repo" && HEADER_FILE="$header_file" bash "$SUBJECT" 2>"$STDERR_TMP") || RC=$?
...
# Helper: read patrik array from .weekly-reviewer-scopes.json.
# Usage: _read_patrik_shas REPO_DIR  → newline-separated SHAs on stdout
_read_patrik_shas() {
  local repo="$1"
  local scope_file="$repo/state/review-trail/.weekly-reviewer-scopes.json"
...
with open(sys.argv[1]) as f:
    obj = json.load(f)
for sha in obj.get("patrik", []):
    print(sha)
PYEOF
}

# Helper: read patrik_seam_files from .weekly-reviewer-scopes.json.
_read_seam_files() {
  local repo="$1"
  local scope_file="$repo/state/review-trail/.weekly-reviewer-scopes.json"
  if [[ ! -f "$scope_file" ]]; then
...
#      unreviewed_set (they do NOT appear in patrik shas).
# ---------------------------------------------------------------------------
T2="$TMPBASE/t2"
_make_fixture "$T2"
...
if [[ "$RC" -ne 0 ]]; then
  ...
  # Both commits should be reviewed; neither should appear in patrik set.
  if echo "$patrik_shas" | grep -qF "$sha_c1" || echo "$patrik_shas" | grep -qF "$sha_c2"; then
    _fail "T2: single-obj" "reviewed commits ($sha_c1, $sha_c2) appeared in patrik shas (should be subtracted): $patrik_shas"
  else
    _pass "T2: single-obj JSON record — reviewed commits subtracted from patrik set"
  fi
fi
...
if [[ "$RC" -ne 0 ]]; then
  ...
  # The diff record's commits should be reviewed → not in patrik set.
  if echo "$patrik_shas_t3" | grep -qF "$sha_j1" || echo "$patrik_shas_t3" | grep -qF "$sha_j2"; then
    _fail "T3: JSONL" "JSONL-record commits appeared in patrik set (should be reviewed): $patrik_shas_t3"
  else
...
# This updated T6 asserts the NEW shape: "pending commits land in patrik set".
#
...
if [[ "$RC" -ne 0 ]]; then
  ...
  # reviewed and therefore DO appear in patrik set (more review, never less).
  if echo "$patrik_shas_t6" | grep -qF "$sha_p1" && echo "$patrik_shas_t6" | grep -qF "$sha_p2"; then
    _pass "T6: pending-excluded (C2 behaviour) — pending record's commits appear in patrik set (unreviewed)"
  else
    _fail "T6: pending-excluded (C2 behaviour)" "expected sha_p1 ($sha_p1) and sha_p2 ($sha_p2) in patrik set (pending excluded); got: $patrik_shas_t6"
  fi
fi
