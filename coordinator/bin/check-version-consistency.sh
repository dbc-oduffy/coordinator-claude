...
# CHANGELOG lives at the bundle root in the flat publish layout, or under the
# coordinator dist staging dir in the meta-repo source layout.
CHANGELOG=""
if [[ -f "$BUNDLE_ROOT/CHANGELOG.md" ]]; then
  ...
  fail "no CHANGELOG.md found (looked at bundle root and coordinator/dist/publish-repo-toplevel/)"
fi
