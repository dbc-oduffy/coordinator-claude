...
if command -v jq >/dev/null 2>&1; then
  ...
  STOP_HOOK_ACTIVE=$(echo "$HOOK_INPUT" | sed -En 's/.*"stop_hook_active"[[:space:]]*:[[:space:]]*(true|false).*/\1/p' | head -1 || echo false)
fi
...
# Max-age cap (parity with runtime-tripwire-em-check.sh MAX_TRACK_MINUTES):
# dispatched-agents.txt is append-only and never purges, so a row whose
# completion-log entry was lost (or schema-mismatched — e.g. the named-teammate
# id shape bug tracked at state/bug-backlog/
# 2026-06-30-runtime-tripwire-stop-watcher-teammate-c.yaml) can linger
# indefinitely. Past MAX_TRACK_MINUTES, the agent has either completed or the EM
# has already decided; arming an asyncRewake watcher for it is noise / false-wake.
# Same env-override name as em-check so they share the knob.
...
while IFS=$'\t' read -r agentId model _subagent_type dispatched_at; do
  ...
  # Max-age cap (mirrors em-check.sh L219-223): skip over-age dispatches so a
  # never-matched or schema-mismatched row cannot arm an unbounded asyncRewake.
...
# NOW already set before Step 6 selection loop; reuse here.
...
(
  ...
  if [[ -f "$DISPATCH_FILE" ]]; then
    while IFS=$'\t' read -r agentId model _subagent_type dispatched_at; do
      ...
      # Max-age cap re-check: if over-age by wake time, treat as cleared — do NOT
      # arm asyncRewake for a row that has lingered past MAX_TRACK_MINUTES.
      RECHECK_ELAPSED=$(( (RECHECK_NOW - dispatched_at) / 60 ))
      [[ "$RECHECK_ELAPSED" -ge "$MAX_TRACK_MINUTES" ]] && break
