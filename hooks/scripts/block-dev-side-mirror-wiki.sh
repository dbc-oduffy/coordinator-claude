...
PLUGIN_ROOT="${CLAUDE_PLUGIN_ROOT:-$(cat "${CLAUDE_HOME:-$HOME}/.claude/.doe-root" 2>/dev/null)/coordinator}"
...
if [ ! -d "$PLUGIN_ROOT" ]; then
  exit 0  # fail-open: cannot locate bundled wiki tree, so cannot evaluate write-direction trap
fi
