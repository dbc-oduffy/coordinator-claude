...
# outside the archive/daily-summaries/YYYY-MM-DD[-<machine>].md dated-file carve-out.
#
...
#   (3) Path IS archive/daily-summaries/YYYY-MM-DD[-<machine>].md → allow (daily-summary
#       carve-out; the /workday-complete Step 4b analyst + Step 4c Sonnet observer are
#       dispatched to write/append it — sanctioned, not a violation; the optional
#       per-machine suffix mirrors state/week-changelog/<date>-<machine>.md naming).
#       Tightly scoped to the dated file.
...
# YYYY-MM-DD.md (or the per-machine YYYY-MM-DD-<machine>.md variant — see
# docs/wiki/daily-summary-procedure.md § Sonnet Analyst Prompt Template step 6, which
# mirrors state/week-changelog/<date>-<machine>.md naming; workday-complete-backfill-scan.sh
# and workday-complete-step9-append-changelog.sh both read/write the machine-suffixed name).
# That is a sanctioned subagent write path — without this carve-out the daily wrap requires
# a manual COORDINATOR_OVERRIDE_SUBAGENT_ARCHIVE=1 on every run (the fight-the-hook
# anti-pattern).
# Tightly scoped to the dated summary file only (YYYY-MM-DD.md or YYYY-MM-DD-<machine>.md) —
# a subagent writing any OTHER file under archive/daily-summaries/ is still blocked.
...
# Machine-suffix note: `-<machine>` is optional and constrained to `[a-z0-9][a-z0-9-]*` —
# machine slugs are lowercased by `cs_compute_machine`, so this matches the slug shape
# without admitting arbitrary filenames.
# Spec backlink: commands/workday-complete.md Step 4b/4c; docs/wiki/daily-summary-procedure.md.
if [[ "$FILE_PATH_NORM" =~ (^|/)archive/daily-summaries/[0-9]{4}-[0-9]{2}-[0-9]{2}(-[a-z0-9][a-z0-9-]*)?\.md$ ]]; then
  exit 0
fi
...
REASON+="  1. archive/daily-summaries/YYYY-MM-DD.md or YYYY-MM-DD-<machine>.md"$'\n'
