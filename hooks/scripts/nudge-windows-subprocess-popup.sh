...
# --- Source advisory-flatten-call helper (soft-degrade: source failure → helpers absent →
#     example-orchestration-hub fast-path degrades silently; hook still exits 0 on all paths) ---
...
# Flag COORDINATOR_EXAMPLE_ORCHESTRATION_HUB_DENY_AUTHORITATIVE defaults OFF → local deny runs on every
# invocation exactly as before. Only when the flag is set AND example-orchestration-hub returns a valid
# PARSED permissionDecision ("allow"/"deny") is the example-orchestration-hub path consumed.
# Reuses INPUT captured above — does NOT re-read stdin.
# Partial / malformed JSON (mid-flush SIGTERM on timeout) leaves _PD empty → falls to local.
...
if declare -f advisory_flatten_params >/dev/null 2>&1 && declare -f advisory_call >/dev/null 2>&1; then
  ...
  _EXAMPLE_ORCHESTRATION_HUB="$(advisory_call "hooks.nudge_windows_subprocess_popup" "$_PARAMS" 2>/dev/null || true)"
  ...
  if [ "${COORDINATOR_EXAMPLE_ORCHESTRATION_HUB_DENY_AUTHORITATIVE:-}" = "1" ] && { [ "$_PD" = "allow" ] || [ "$_PD" = "deny" ]; }; then
    # ExampleOrchestrationHub returned a valid decision AND the DoE-local flag is set — consume it.
    if [ "$_PD" = "allow" ]; then
      exit 0
    else
      _EXAMPLE_ORCHESTRATION_HUB_REASON="$(printf '%s' "$_EXAMPLE_ORCHESTRATION_HUB" | jq -r '.hookSpecificOutput.permissionDecisionReason // empty' 2>/dev/null || true)"
      deny "${_EXAMPLE_ORCHESTRATION_HUB_REASON:-$REASON}"
    fi
  fi
  # AUTHORITATIVE unset / _PD not recognized → fall through to local deny below.
fi
# helper absent OR advisory did not consume → local deny, unchanged.
deny "$REASON"
