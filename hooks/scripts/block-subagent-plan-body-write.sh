...
# allowlist of sidecar suffixes (.prior-art-check.md, .the Staff Engineer-review.md, …). That
...
# Fail-open: deny hook must fail-open (allow) on unexpected error, never fail-closed; empty
# _cc_root is safe because the LIB_PATH consumer guards with [[ -f … ]] before sourcing.
...
# ---------------------------------------------------------------------------
# Identity resolution via shared resolver (C7 — coordinator-session.sh).
# Replaces the former inline hex-only format guard. Named-teammate agents emit
# `agent_id: a<name>-<16hex>` (not bare hex), so the old guard nulled them and
# allowed their plan-body writes unconditionally — the immutability bypass (AC10).
# Spec backlink: docs/plans/2026-06-30-loe-dispatch-undercount-teammate-shape.md § C9
# ---------------------------------------------------------------------------

# Source the identity resolver lib.
# Same source pattern as track-dispatched-agents.sh, track-touched-files.sh.
...
# Present on subagent fires only; absent on top-level EM fires.
# Shapes: bare hex (unnamed agent), a<name>-<16hex> (named teammate), or absent.
# No format guard here — the resolver discriminates all three cases.
_raw_agent_id=""
...
# first 8 chars of the subagent's own session uuid reconstruct the canonical
# EM-side id recorded under .agents/ by track-dispatched-agents.sh).
_input_session_id=""
if command -v jq &>/dev/null; then
...
# resolve_subagent_identity: bare-hex → passthrough (unnamed fast path);
#   a<name>-<16hex> + session_id → <name>@session-<first8(session_id)>;
#   unrecognised/empty → empty string.
# Empty result → fail-open allow (safe direction for a deny hook).
# Degraded fallback when lib unavailable: accept bare-hex only; named-teammate
# agents fail-open — conservative (better to miss a block than block legitimate work).
AGENT_ID=""
...
# Block when SUBAGENT_TYPE is coordinator:executor OR AMBIGUOUS (AC14 collision sentinel).
# AMBIGUOUS is written by track-dispatched-agents.sh when two dispatches share the same
# canonical id but carry different subagent_types — rather than silently resolving to the
# first row's type (which could be an executor, bypassing immutability), the writer marks
# the row AMBIGUOUS so every lookup on that id fails-closed here (unconditional block).
# All other types — including empty / lookup-failure — allow (safe fail-open direction).
if [[ "$SUBAGENT_TYPE" == "AMBIGUOUS" ]]; then
  : # Fall through to block unconditionally. The collision path has NO flight-sidecar carve-out.
...
# NOT applied on the AMBIGUOUS path (collision → unconditional fail-closed, no carve-out).
# Scoped to coordinator:executor only so it doesn't interfere with the AMBIGUOUS fall-through.
#
...
if [[ "$SUBAGENT_TYPE" == "coordinator:executor" ]] && \
   [[ "$FILE_PATH_NORM" =~ (^|/)tasks/[^/]+/flight/[^/]+\.md$ ]]; then
  exit 0
fi

# Block confirmed — either coordinator:executor (no sidecar carve-out applied)
# or AMBIGUOUS (collision sentinel, unconditional). Anything under docs/plans/ → deny.

# session_id already extracted early in the identity-resolver block (_input_session_id).
...
# break the JSON envelope. (AGENT_ID is embedded in REASON and passed to jq via
# --arg, which quote-escapes the value — injection-safe regardless of id format.)
# Review: code-reviewer — F5: replaced stale "lowercase hex" justification with the
# actual safety mechanism (jq --arg quote-escaping; AGENT_ID post-C9 can be name@session-…).
...
# Reason differs for the AMBIGUOUS (collision) path vs the coordinator:executor path.
if [[ "$SUBAGENT_TYPE" == "AMBIGUOUS" ]]; then
  ...
  REASON+="This block has NO flight-sidecar carve-out (collision → unconditional fail-closed)."$'\n\n'
  REASON+="Override (rare-use — read the hook source before invoking):"$'\n'
  REASON+="  COORDINATOR_OVERRIDE_SUBAGENT_PLAN_BODY=1"
else
  REASON="BLOCKED: coordinator:executor may not write the plan body."$'\n\n'
  REASON+="  Subagent: ${AGENT_ID} (coordinator:executor)"$'\n'
  REASON+="  Target:   ${FILE_PATH_SAFE}"$'\n\n'
  REASON+="Two cases — pick the one that matches your intent:"$'\n\n'
  REASON+="  • If you were STAMPING EXECUTION STATUS (in-progress / complete) into the"$'\n'
  REASON+="    plan body: don't. Your surface is the per-chunk flight sidecar —"$'\n'
  REASON+="      tasks/<plan-slug>/flight/<chunk-id>.md"$'\n'
  REASON+="    Derive <plan-slug> from the plan filename minus the YYYY-MM-DD- prefix and"$'\n'
  REASON+="    .md suffix; <chunk-id> from the dispatch brief's chunk field."$'\n\n'
  REASON+="  • If AMENDING THE PLAN BODY WAS YOUR DELIVERABLE (reconciling plan text,"$'\n'
  REASON+="    editing a section, applying a decision): you are the WRONG agent type for"$'\n'
  REASON+="    this work. Plan-body edits route to EM-inline / coordinator:enricher /"$'\n'
  REASON+="    coordinator:review-integrator. The dispatching EM should re-route this"$'\n'
  REASON+="    chunk to one of those — it is not coordinator:executor work."$'\n\n'
  REASON+="Override (rare-use — read the hook source before invoking):"$'\n'
  REASON+="  COORDINATOR_OVERRIDE_SUBAGENT_PLAN_BODY=1"
fi
