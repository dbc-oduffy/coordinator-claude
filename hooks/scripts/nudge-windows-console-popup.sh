...
# check_windows_popup CMD SESSION_ID
# Sourceable entry point for dispatcher consumers. All decision logic lives here.
# $1 = command string (tool_input.command, already JSON-decoded by caller)
# $2 = session_id (passed for dispatcher contract; not used in this hook's logic)
# On trigger: prints nested hookSpecificOutput advisory JSON, returns 0.
# On no-trigger / non-Windows: prints nothing, returns 0.
check_windows_popup() {
  local _cmd="$1"
  # $2 is session_id — not used in this hook's logic; present for dispatcher contract.
  ...
  case "$_uname_s" in
    ...
    *)
      ...
      if [[ -z "${WINDIR:-}" && "${OS:-}" != "Windows_NT" ]]; then
        return 0  # macOS, Linux, WSL2 — all silent (correct)
      fi
      ;;
  esac
  ...
  if [[ "$_cmd" == *'# popup-intentional-last-resort'* || "$_cmd" == *'# popup-safe-env-suppressed'* ]]; then
    return 0
  fi
  ...
  if [[ "$_cmd" == *'python-quiet.sh'* || "$_cmd" == *'python3-quiet.sh'* ]]; then
    return 0
  fi
  ...
  # (example-game-repo-em calibration, 2026-06-22-windows-fresh-install-fixes-reply memo item 4b.)
  ...
  if [[ "$_cmd" == *'spawn-hidden.sh'* ]]; then
    return 0
  fi
  if [[ "$_cmd" == *'pythonw'* ]]; then
    return 0
  fi
  if [[ "$_cmd" == *'CREATE_NO_WINDOW'* || "$_cmd" == *'creationflags'* ]]; then
    return 0
  fi
  ...
  # Matches run against SCAN (lookup-neutralized below), not raw _cmd.
  ...
  # binary — matching it is a false-positive (example-game-repo-em calibration,
  # 2026-06-22-windows-fresh-install-fixes-reply memo item 4a). Strip the lookup-keyword + its
  # single token from a scan-only copy; the real _cmd is untouched, so a compound that probes a
  ...
  if command -v sed >/dev/null 2>&1; then
    SCAN=$(printf '%s' "$_cmd" | sed -E 's/(^|[[:space:];|&`])(command[[:space:]]+-[vV]|which|type|hash)[[:space:]]+[^[:space:];|&`]+/\1 /g')
  fi
  ...
  #   - The `SCAN="$_cmd"` pre-seed + `command -v sed` guard is PROTECTIVE, not removable: with
...
Exception — pytest-xdist test gates (pytest -n N): do NOT route through python-quiet.sh (pythonw breaks xdist). Popup suppression for xdist workers is handled by the repo's conftest.py AllocConsole+hide hook. See coordinator docs/wiki/pytest-xdist-no-console-windows.md.
...
If the popup is intentional and expected (e.g. debugging on a local desktop), or suppressed by env-var means, add one of these marker comments to the calling shell line:
  ...
  local OFFER_TEXT_J="${OFFER_TEXT//\\/\\\\}"           # escape backslashes first
...
# --- Main guard: run as a standalone hook (stdin-driven) ---
# When sourced by a dispatcher, only the function above is defined; no execution here.
if [[ "${BASH_SOURCE[0]}" == "${0}" ]]; then
  ...
  # session_id — extracted for dispatcher contract; not used by this hook's logic.
  SID=""
  if [[ "$INPUT" == *'"session_id"'* ]]; then
    _stmp="${INPUT#*\"session_id\":\"}"
    SID="${_stmp%%\"*}"
  fi

  check_windows_popup "$CMD" "$SID"
  exit 0
fi
