...
# --- Resolve plugin root (needed for lib sourcing below and coordinator-daily-day.sh) ---
# shellcheck source=/dev/null
_HOOK_PLUGIN_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
...
# Source coordinator-is-meta-repo.sh + coordinator-example-orchestration-hub-root.sh to resolve the
# correct docs/plans/ target. Falls back to REPO_ROOT if seam is unresolvable (graceful
# degradation on un-migrated installs, matching AC13 / coordinator-example-orchestration-hub-root.sh § Rung 3).
#
# PLANS_GIT_ROOT tracks which repo owns the git staging operations — it is either
# REPO_ROOT (non-meta-repo or example-orchestration-hub unresolvable) or the example-orchestration-hub root (meta-repo + resolved).
#
# Spec backlink: docs/plans/2026-07-03-stop-the-rot-example-orchestration-hub-state-home-placement.md § C10 / AC7
DOCS_PLANS_DIR="$REPO_ROOT/docs/plans"
DOCS_README="$REPO_ROOT/docs/README.md"
PLANS_GIT_ROOT="$REPO_ROOT"

_IS_META_REPO_SH="${_HOOK_PLUGIN_ROOT}/lib/coordinator-is-meta-repo.sh"
_EXAMPLE_ORCHESTRATION_HUB_ROOT_SH="${_HOOK_PLUGIN_ROOT}/lib/coordinator-example-orchestration-hub-root.sh"
if [[ -f "$_IS_META_REPO_SH" ]] && [[ -f "$_EXAMPLE_ORCHESTRATION_HUB_ROOT_SH" ]]; then
  # shellcheck source=/dev/null
  source "$_IS_META_REPO_SH"
  # shellcheck source=/dev/null
  source "$_EXAMPLE_ORCHESTRATION_HUB_ROOT_SH"
  if coordinator_is_meta_repo "$REPO_ROOT" 2>/dev/null; then
    _HOOK_EXAMPLE_ORCHESTRATION_HUB_ROOT=""
    _HOOK_EXAMPLE_ORCHESTRATION_HUB_ROOT="$(coordinator_example_orchestration_hub_root 2>/dev/null || true)"
    if [[ -n "$_HOOK_EXAMPLE_ORCHESTRATION_HUB_ROOT" ]]; then
      DOCS_PLANS_DIR="${_HOOK_EXAMPLE_ORCHESTRATION_HUB_ROOT}/docs/plans"
      DOCS_README="${_HOOK_EXAMPLE_ORCHESTRATION_HUB_ROOT}/docs/README.md"
      PLANS_GIT_ROOT="$_HOOK_EXAMPLE_ORCHESTRATION_HUB_ROOT"
    fi
  fi
fi

# --- Guard: repo must have opted into the docs convention ---
# NEITHER docs/plans/ NOR docs/README.md exists at the effective target → not our kind of repo
...
if [[ -n "$H1_LINE" ]]; then
  ...
  # Review: code-reviewer (F2) — re-trim trailing hyphens after truncation; the pre-cut
  # trailing-trim runs before cut so truncation can re-introduce a trailing hyphen run.
  SLUG=$(printf '%s' "$SLUG" | sed 's/-*$//')
else
  # Timestamp fallback
...
# --- Build target path ---
# Source the local-day helper (hooks/scripts/ → ../../lib/).
# _HOOK_PLUGIN_ROOT is already resolved above (placement-law seam block).
source "${_HOOK_PLUGIN_ROOT}/lib/coordinator-daily-day.sh"
...
# Use PLANS_GIT_ROOT (not REPO_ROOT) — when plans are example-orchestration-hub-resident, the git
# staging operation belongs to the example-orchestration-hub repo, not the coordinator meta-repo.
git -C "$PLANS_GIT_ROOT" add -- "$TARGET_PATH" 2>/dev/null || true
...
if [[ -f "$DOCS_README" ]]; then
  ...
  if ! grep -qF "$TARGET_NAME" "$DOCS_README" 2>/dev/null; then
    ...
    git -C "$PLANS_GIT_ROOT" add -- "$DOCS_README" 2>/dev/null || true
  fi
fi
...
# When PLANS_GIT_ROOT != REPO_ROOT (plans in example-orchestration-hub), the commit command uses
# `git -C "$PLANS_GIT_ROOT"` so the user commits to the correct repo.
if [[ "$PLANS_GIT_ROOT" != "$REPO_ROOT" ]]; then
  # Plans are in example-orchestration-hub — user must commit to PLANS_GIT_ROOT, not the current repo.
  if [[ "$README_MODIFIED" -eq 1 ]]; then
    COMMIT_CMD="git -C \"$PLANS_GIT_ROOT\" add -- docs/plans/$TARGET_NAME docs/README.md && git -C \"$PLANS_GIT_ROOT\" commit -m \"plan: $SLUG\" -- docs/plans/$TARGET_NAME docs/README.md"
  else
    COMMIT_CMD="git -C \"$PLANS_GIT_ROOT\" add -- docs/plans/$TARGET_NAME && git -C \"$PLANS_GIT_ROOT\" commit -m \"plan: $SLUG\" -- docs/plans/$TARGET_NAME"
  fi
else
  if [[ "$README_MODIFIED" -eq 1 ]]; then
