...
# plugins (coordinator). Compares each plugin's locally installed SHA (version.txt)
# against its own latest published GitHub release tag. Emits a one-line stdout nag —
# naming that plugin's own update action — when the install is behind or differs;
# stays silent when current or source_is_live.
...
#
# Per-plugin loop: coordinator. Each entry checks its own release repo and renders
# its own update action (_action_hint_for). Adding a further plugin = one
# PLUGINS_TO_CHECK entry + one _action_hint_for case arm.
#
...
# coordinator installs are reconciled by the shipped /coordinator-update verb.
# A new plugin adds one case arm here alongside its loop entry.
_action_hint_for() {
    case "$1" in
