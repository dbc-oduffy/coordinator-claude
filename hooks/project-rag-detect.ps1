...
#   (silent)     — no marker found, or example-game-repo context detected (let example-game-repo hook handle it)
#
# Kill-switch: set COORDINATOR_HOOK_PROJECT_RAG_DETECT_DISABLED=1 to disable
#
# example-game-repo dedupe: if .example-game-repo/ directory or Saved/ExampleGameRepoProjectRag/ path is found
# walking up from cwd, this script exits silently — the example-game-repo-specific hook handles it.
#
...
# If we're in a example-game-repo UE project, let the example-game-repo-specific hook handle the banner.
$ExampleGameRepoDir    = Find-MarkerUpward -StartDir $Cwd -Marker ".example-game-repo"
$ExampleGameRepoSaved  = Find-MarkerUpward -StartDir $Cwd -Marker "Saved\ExampleGameRepoProjectRag"
if ($ExampleGameRepoDir -or $ExampleGameRepoSaved) {
    # example-game-repo context detected — exit silently, example-game-repo hook owns the banner
    exit 0
}
