<!-- canonical source for docs-checker-consumption — edit here, then run bin/verify-snippet-sync docs-checker-consumption --fix -->
...
- **AUTO-FIXED** — docs-checker corrected the claim inline. The edits are in a single git-revertible commit and listed in the changelog sidecar (`state/review-findings/{timestamp}-docs-checker-edits.md`). Review the changelog only if you spot something docs-checker shouldn't have touched (e.g., it edited a deliberate battle-story breadcrumb). Surface as a finding if so — the EM will revert from the docs-checker commit.
