...
Per-machine overrides. The `.local` convention matches `~/.claude/`'s existing practice (`CLAUDE.local.md`, `coordinator.local.md`, `settings.local.json`) — `.local` files are gitignored and hold values specific to one machine.
