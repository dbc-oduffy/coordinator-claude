...
#   - A `&&`/`;` inside a quoted argument (`git commit -m "a && b"`) would mis-split;
#     the unbalanced-quote guard below detects this and bails (allow) rather than
#     emit a truncated suggestion.
...
# Join backslash-newline continuations so a `cd X \<newline> git ...` is one chain.
NL=$'\n'
CMD="${CMD//\\$NL/ }"
