...
# ground truth.
#
# Per-project plugin gating: UE plugins (holodeck-control, holodeck-docs,
# holodeck, game-dev) are disabled globally and opt-in via per-project
...
elif [[ -f "$SETTINGS" ]] && jq -e '.enabledPlugins["holodeck-control@claude-unreal-holodeck"] == false' "$SETTINGS" >/dev/null 2>&1; then
