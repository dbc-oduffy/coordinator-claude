...
- **`/shape` precedes planning when the problem isn't converged.** Exploration signals ("what do you think?", a problem named without a deliverable, or the EM can't confidently restate it) → `coordinator:shape`: converge on the problem at PM/strategic altitude, ratify a problem-set, chain into `coordinator:plan`.
