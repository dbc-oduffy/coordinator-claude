...
#
# Run: bash ~/.claude/plugins/coordinator/bin/tests/test-self-claim-check-mcp-versions.sh
...
test_check_mcp_has_wiring() {
    local script="${HOME}/.claude/plugins/coordinator/bin/check-mcp-versions.sh"
    [[ ! -f "$script" ]] && script="${SCRIPT_DIR}/../check-mcp-versions.sh"
