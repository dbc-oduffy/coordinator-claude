...
#   6. Existing persona substitutions still work (the Staff Engineer → the Staff Engineer).
...
cat > "$FIXTURE_JSON" <<'FIXTURE'
{
  ...
  "version": "1.0.0",
  "author": {
    "name": "the Coordinator Authors"
  },
  "author_ascii": "the Coordinator Authors",
  "collab": "the Coordinator Authors",
  "pm_name": "the PM",
  "repository": "https://github.com/dbc-oduffy/coordinator-claude",
  "related": "https://github.com/dbc-oduffy/deep-research-claude"
}
FIXTURE
...
The Staff Engineer flagged a potential issue here.
FIXTURE
...
# Test 5a: post-fix does NOT contain "the PM O'Duffy"
echo "--- 5a. post-fix does not contain the PM O'Duffy"
if assert_not_contains "the PM O'Duffy removed" "the PM O'Duffy" "$fixed_content"; then
  pass "5a. 'the PM O'Duffy' removed after --fix"
else
  fail "5a. 'the PM O'Duffy' removed after --fix"
fi
...
# Test 6: persona substitution unchanged — the Staff Engineer → Staff Engineer (any case)
# Note: sentence-initial "the Staff Engineer" becomes "The Staff Engineer" (capitalized by
# the post-pass cleanup). We check for the role string case-insensitively.
echo "--- 6. persona substitution: the Staff Engineer → the Staff Engineer"
...
if echo "$persona_content" | grep -qiF "staff engineer" && \
   assert_not_contains "the Staff Engineer gone" "the Staff Engineer" "$persona_content"; then
  pass "6. The Staff Engineer → the Staff Engineer still works"
else
  fail "6. The Staff Engineer → the Staff Engineer still works"
fi
