...
#
# Run: bash ~/.claude/plugins/coordinator/bin/tests/test-coordinator-write-review-trail.sh
...
STANDARD_ARGS=(--sha-range "abc123..def456" --reviewer code-reviewer --scope session --verdict ok --diff-loc 42)
...
test_happy_path() {
  ...
  if [[ "$result" == true ]]; then
    ...
    local expected_json="{\"sha_range\":\"abc123..def456\",\"reviewer\":\"code-reviewer\",\"scope\":\"session\",\"verdict\":\"ok\",\"diff_loc\":42,\"session_id\":\"${sid}\"}"
...
test_idempotent_noop() {
  ...
  local expected_content="{\"sha_range\":\"abc123..def456\",\"reviewer\":\"code-reviewer\",\"scope\":\"session\",\"verdict\":\"ok\",\"diff_loc\":42,\"session_id\":\"${sid}\"}"
...
test_collision_detection() {
  ...
  printf '%s' '{"sha_range":"abc123..def456","reviewer":"code-reviewer","scope":"session","verdict":"ok","diff_loc":42,"session_id":"coll5678abcd"}' > "$target_path"
  ...
  local args_a=(--sha-range "abc123..def456" --reviewer code-reviewer --scope session --verdict ok --diff-loc 10)
  ...
  printf '%s' '{"sha_range":"abc123..def456","reviewer":"code-reviewer","scope":"session","verdict":"ok","diff_loc":99,"session_id":"colltest9876"}' > "$file_a"
  ...
  local args_first=(--sha-range "abc123..def456" --reviewer code-reviewer --scope session --verdict ok --diff-loc 10)
  local args_second=(--sha-range "abc123..def456" --reviewer code-reviewer --scope session --verdict ok --diff-loc 99)
  ...
  printf '%s' "{\"sha_range\":\"abc123..def456\",\"reviewer\":\"code-reviewer\",\"scope\":\"session\",\"verdict\":\"ok\",\"diff_loc\":10,\"session_id\":\"${sid4}\"}" > "$seeded_path"
  ...
  printf '%s' "{\"sha_range\":\"abc123..def456\",\"reviewer\":\"code-reviewer\",\"scope\":\"session\",\"verdict\":\"ok\",\"diff_loc\":10,\"session_id\":\"${sid4}\"}" > "$target_collision"
  ...
  if [[ $rc_c -eq 2 ]]; then
    ...
    local expected_collision_content="{\"sha_range\":\"abc123..def456\",\"reviewer\":\"code-reviewer\",\"scope\":\"session\",\"verdict\":\"ok\",\"diff_loc\":10,\"session_id\":\"${sid4}\"}"
    ...
    local expected_orig="{\"sha_range\":\"abc123..def456\",\"reviewer\":\"code-reviewer\",\"scope\":\"session\",\"verdict\":\"ok\",\"diff_loc\":10,\"session_id\":\"${sid4}\"}"
...
test_collision_detection_direct() {
  ...
  local first_content="{\"sha_range\":\"abc123..def456\",\"reviewer\":\"code-reviewer\",\"scope\":\"session\",\"verdict\":\"ok\",\"diff_loc\":42,\"session_id\":\"${sid}\"}"
  ...
  out=$(CLAUDE_SESSION_ID="$sid" bash "$HELPER" \
    --sha-range "abc123..def456" --reviewer code-reviewer --scope session \
...
test_invalid_scope() {
  ...
  rc=0
  out=$(CLAUDE_SESSION_ID="anyid" bash "$HELPER" \
    --sha-range "abc..def" --reviewer code-reviewer --scope both --verdict ok --diff-loc 1 2>&1) || rc=$?
...
test_invalid_verdict() {
  ...
  rc=0
  out=$(CLAUDE_SESSION_ID="anyid" bash "$HELPER" \
    --sha-range "abc..def" --reviewer code-reviewer --scope session --verdict unclear --diff-loc 1 2>&1) || rc=$?
...
test_invalid_diff_loc() {
  ...
  rc=0
  out=$(CLAUDE_SESSION_ID="anyid" bash "$HELPER" \
    --sha-range "abc..def" --reviewer code-reviewer --scope session --verdict ok --diff-loc notanumber 2>&1) || rc=$?
...
test_missing_required_arg() {
  ...
  # Omit --verdict
  out=$(CLAUDE_SESSION_ID="anyid" bash "$HELPER" \
    --sha-range "abc..def" --reviewer code-reviewer --scope session --diff-loc 5 2>&1) || rc=$?
