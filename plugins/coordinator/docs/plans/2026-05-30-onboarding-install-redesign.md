...
- **Test surface:** `grep:` assertions — playbook contains the three tier labels verbatim; the `/pickup tasks/handoffs/continue-onboarding-and-installation.md` string; the "fresh session" reframing; an explicit "edit your ~/.claude, not this clone" line; no "installation complete" line gated solely on a script exit.
...
**Owns:** new `templates/handoffs/continue-onboarding-and-installation.md` (the install copies it to `tasks/handoffs/`). New file — no overlap.
