...
1. **Machine-local registry** — `~/.claude/machine-local/registry.local.toml` `[repos]` keys (e.g. `project_rag`, `project_rag_ue_addon`). If a registered path exists on disk, that is the canonical source clone. `bin/machine-local get repos.<key>` is the reader.
