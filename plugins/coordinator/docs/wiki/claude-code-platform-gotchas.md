...
**Hook-interpreter flashes (node/python spawned by Claude Code for PreToolUse hooks, pwsh for SessionStart):** these are spawned by Claude Code's own harness, not by our scripts.  We cannot set `CREATE_NO_WINDOW` on them from our code.  The primary suppression lever for these is the **machine-belt: set Windows Terminal as the default terminal application** (changes `HKCU\Console\%%Startup\Delegation`) so ConPTY handles console allocations without opening a visible window.  See `docs/wiki/windows-console-flash-belt-experiment.md` for the test-first experiment.
