...
**Empirical validation 2026-05-16 (post-Patrik review).** The chunker-dispatch plan `docs/plans/2026-05-16-targeted-chunker-dispatch-doctrine.md` made it through Patrik review (NEEDS-CHANGES minor, 11 findings + 7 OQ resolutions) with **zero findings against the hookspec-shape decision** (additive metadata on the existing `project_rag_register_chunker` hookspec rather than authoring a new one). OQ-2 ("does adding a load-bearing default-valued field warrant an `ADDON_PROTOCOL_VERSION` bump?") resolved NO BUMP — defaulted-additive remains the rule for new fields that change dispatch behavior only when an opt-in addon populates them. The pattern survived contact with adversarial review.
...
1. **Host-side gating, not driver-side.** Dispatch gates (language intersection, substrate-glob matching) live in `Registry.iter()`, not in the addon driver loop. Driver-side gating creates two implementations that drift; the registry is the single source of truth. Validated by OQ-1 in the chunker-dispatch plan (resolved HOST, Patrik concurred).
