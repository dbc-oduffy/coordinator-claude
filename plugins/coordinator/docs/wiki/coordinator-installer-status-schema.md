...
Purpose: document the stable producer-side contract for `/coordinator:setup`'s status-report
...
Every row that `/coordinator:setup` currently emits, plus the three new rows introduced
