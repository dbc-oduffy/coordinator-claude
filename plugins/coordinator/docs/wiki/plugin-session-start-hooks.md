...
"command": "bash /Users/alice/.claude/plugins/myplugin/hooks/scripts/session-init.sh"
```
...
Write the hook script to resolve the interpreter at runtime, matching the pattern at `scan-addon-health.sh` lines 74-89:
...
- `scan-addon-health.sh` — Third pass (SessionStart hook-script existence probe). Run manually: `COORDINATOR_PLUGINS_ROOT=~/.claude/plugins scan-addon-health.sh --red-and-stale`
