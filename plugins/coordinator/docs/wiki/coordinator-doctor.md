...
**Script.** `bin/coordinator-doctor-sentinel.sh` (in the coordinator-claude plugin tree). Fires P-1..P-11 in batch, classifies each result, and writes a sentinel at `~/.claude/plugins/coordinator-claude/data/doctor-last-run.json` for [`scan-addon-health.sh`](addon-health-sentinel.md) to consume.
