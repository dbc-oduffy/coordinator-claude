...
Snippet-sync flow for `plan-coverage-check-consumption`: edit `snippets/plan-coverage-check-consumption.md` → run `bin/verify-plan-coverage-sync.sh --fix` → commit all touched files together. Consumer sentinel: `<!-- BEGIN plan-coverage-check-consumption (synced from snippets/plan-coverage-check-consumption.md) -->` / `<!-- END plan-coverage-check-consumption -->`. Consumers: `agents/staff-eng.md` (Patrik), `../game-dev/agents/staff-game-dev.md` (Sid), `../data-science/agents/staff-data-sci.md` (Camelia), `../web-dev/agents/senior-front-end.md` (Palí), `agents/eng-director.md` (Zolí).

## Tripwire registry

- **Patrik UE block** (`staff-eng.md`): gated on `project_type: game-dev` AND `project_subtypes` contains `unreal`; names UE workers (`bp-test-evidence-parser`, `perf-trace-classifier`, `schema-migration-auditor`).
