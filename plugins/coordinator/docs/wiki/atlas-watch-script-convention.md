...
- **Audit-pass closeout gate** (`bin/verify-arch-audit-atlas-refresh.sh`) — enforces atlas refresh AT the moment `/architecture-audit` closes (Branch A inline refresh, or Branch B `atlas-current-as-of:<date>` token). Stops a graded audit from landing against a stale baseline.
...
`bin/check-atlas-watch-drift.sh` is the aggregator. It is read at run-time by:
...
Aggregator-side interpretation rules (enforced in `bin/check-atlas-watch-drift.sh`, restated here so script authors understand the consumer):
...
## Within-atlas clock split (2026-06-08)
...
- **NOT a closeout gate.** This convention is the detection surface that runs between audits. The closeout-gate sibling — `bin/verify-arch-audit-atlas-refresh.sh` — is what enforces "you didn't close `/architecture-audit` against a stale atlas." Two surfaces, two jobs.
...
Both surfaces preserve the two-clock contract established in 2026-05-23:
...
The convention itself is the doctrine. Per-system `.watch.sh` seeds are per-repo. Holodeck's known case (atlas drift on `probe_registry_version` bumps and `_VALID_PHASES` rotation) lives as `.watch.sh` siblings in the holodeck repo's own `docs/architecture/systems/`, authored against that repo's source layout — not seeded from here. Other repos adopt the convention by:

1. Vendoring `bin/check-atlas-watch-drift.sh` (or pulling it via the coordinator publish chain).
...
- `bin/check-atlas-watch-drift.sh` — the aggregator.
- `bin/verify-arch-audit-atlas-refresh.sh` — the closeout-gate sibling that enforces refresh-at-close.
- `bin/check-arch-audit-staleness.sh` — the rotation-clock helper (canonical; unchanged by this convention).
- `coordinator-tripwires.md` § Atlas-refresh gate at /architecture-audit closeout — the tripwire registry entry for both surfaces.
- `cross-platform-shell-portability.md` and DR-061 — the portability rules that apply to `.watch.sh` scripts.
