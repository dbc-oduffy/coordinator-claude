...
3. For each `file:line` or `file:symbol` reference: `Read` the file and verify the cited symbol/constant/content is present.
   ...
   **Line-drift tolerance — this is mandatory, not optional:**
   - Match is confirmed if EITHER (a) the cited symbol/identifier appears within ±10 lines of the cited line number on disk, OR (b) the cited line content semantic-matches the plan's quoted excerpt.
