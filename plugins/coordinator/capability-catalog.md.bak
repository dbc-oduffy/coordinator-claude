...
**Patrik** — architecture + code review. Use `/review` (plan artifacts) or `/review-code` (code artifacts).
...
**Palí** (senior-front-end) — front-end review (tokens, design system, CSS). **Fru** — UX flow review (trust, clarity). Use `/review` (plan artifacts) or `/review-code` (code artifacts).

**eng-director** (Zolí) — Director of Engineering. Three modes: (1) **standalone primary reviewer** (default; dispatched directly via `/review`, `/review-code`, or `coordinator:eng-director` for cross-team / cross-repo / generic-substrate reviews — peer of Patrik in technical rigor, with DoE-altitude authority to set cross-team boundaries Patrik would hedge on); (2) **backstop reviewer** (chained after Patrik on High-effort architectural reviews); (3) **staff-session synthesizer** (spawned by `/staff-session`, blocked until debaters complete, resolves contested topics with DoE authority — organizational benefit, customer-serving, velocity-over-time — not by averaging the loudest debaters).
